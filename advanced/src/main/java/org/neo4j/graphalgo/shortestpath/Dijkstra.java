    ...
    SingleSourceSingleSinkShortestPath<CostType>
{
    ...
    protected class DijstraIterator implements Iterator<Node>
    {
        ...
        public DijstraIterator( Node startNode,
            ...
            HashMap<Node,CostType> mySeen, HashMap<Node,CostType> otherSeen,
         ...
         * @param otherSideDistances
         *            Map over distances from other side. A path is found and
         *            examined if this contains currentNode.
         */
        ...
        public Node next()
        {
            ...
            else
            {
                ...
                {
                    for ( Relationship relationship : currentNode
                        .getRelationships( costRelationType, getDirection() ) )
                    {
                        if ( limitReached() )
                        {
                            break;
                        ...
                        boolean backwardsEdge = relationship.getEndNode()
                            .equals( currentNode )
                            ^ backwards;
                        CostType newCost = costAccumulator.addCosts(
                            currentCost, costEvaluator.getCost( relationship,
                                backwardsEdge ) );
                        ...
                        if ( myDistances.containsKey( target ) )
                        {
                            ...
                            // already
                            // calculated?
                            if ( costComparator.compare( myDistances
                                .get( target ), newCost ) > 0 )
                            {
                            ...
                            // Equally good path found?
                                ...
                                && costComparator.compare( myDistances
                                    .get( target ), newCost ) == 0 )
                            {
                                // Put it in predecessors
                                List<Relationship> myPredecessors = predecessors
                                    .get( currentNode );
                                ...
                                {
                                    List<Relationship> predList = predecessors
                                        .get( target );
                                    if ( predList == null )
                                    {
            ...
            if ( calculateAllShortestPaths && oneShortestPathHasBeenFound )
            {
                ...
                allShortestPathsHasBeenFound = queue.isEmpty()
                    || costComparator.compare( mySeen.get( queue.peek() ),
                        currentCost ) > 0;
            }
            return currentNode;
        }
    ...
    public List<List<PropertyContainer>> getPaths()
    {
        ...
        {
            List<List<PropertyContainer>> paths1 = Util
                .constructAllPathsToNode( middleNode, predecessors1, true,
                    false );
            List<List<PropertyContainer>> paths2 = Util
                .constructAllPathsToNode( middleNode, predecessors2, false,
                    true );
    ...
    public List<List<Relationship>> getPathsAsRelationships()
    {
        ...
        for ( Node middleNode : foundPathsMiddleNodes )
        {
            List<List<Relationship>> paths1 = Util
                .constructAllPathsToNodeAsRelationships( middleNode,
                    predecessors1, false );
            List<List<Relationship>> paths2 = Util
                .constructAllPathsToNodeAsRelationships( middleNode,
                    predecessors2, true );
            // For all combinations...

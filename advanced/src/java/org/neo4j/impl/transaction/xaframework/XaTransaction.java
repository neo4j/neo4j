 ...
 *
 *     public boolean isReadyOnly() { return cmds.size() == 0; }
 *
 *     public void doAddCommand( XaCommand cmd ) { cmds.add( cmd ); }
 *
 ...
 * <CODE>
 * </pre>
 * Some other implementation that makes use of prepare could look something 
 * like this:
 * <pre>
 * <CODE>
 * // Example of XaTransaction implementation where commands are written to
...
public abstract class XaTransaction
{
	...
	/**
	 * When a command is added to transaction it will be passed via this 
	 * method. The <CODE>XaTransaction</CODE> needs to hold all the commands 
	 * in memory until it receives the <CODE>doCommit</CODE> or 
	 * <CODE>doRollback</CODE> call.
	 *
	 * @param command The command to be added to transaction
	 */
	protected abstract void doAddCommand( XaCommand command );
	
	/**
	 * Rollbacks the transaction, loop through all commands and invoke
	 * <CODE>rollback()</CODE>.
	 *
	 * @throws XAException If unable to rollback
	 */
	 ...
	 * 
	 * @throws XAException If unable to prepare
	 */
	 ...
	 * <CODE>execute()</CODE>.
	 *
	 * @throws XAEXception If unable to commit
	 */
	...
	/**
	 * If this transacgtion is created during a recovery scan of the logical 
	 * log method will be called to mark the transaction "recovered".
	 */
	 ...
	 *
	 * @param command The command to add to transaction
	 * @throws RuntimeException If problem writing command to logical log or
	 ...
	 *
	 * @param command The command that will be injected
	 */
	 ...
	 * {@link XaLogicalLog#getCurrentTxIdentifier} then calls 
	 * {@link #doCommit}.
	 *
	 * @throws XAException If unable to commit
	 */

 ...
 * created will be written to the {@link XaLogicalLog} once it is added to 
 * the transaction.
 * <p> 
 * Commands are memory buckets containing information on how to presist the 
 * operation in the XA resource. Any nececcery constraint checks must take 
 * place before any command has been executed. Once a command in the transaction
 * has started to execute there is no turning back, all of the following 
 * commands must execute without problems. To ensure this check the 
 * constraints via a tx synchronization hook, at prepare or upon command 
 * creation and signal the transaction before it starts to commit if something 
 * is wrong.
 * <p> 
 * Throwing exception when the command rollbacks or executes could (depending 
 * on TransactionManager implementation) bring down the whole system. Should 
 * however a <CODE>execute</CODE> or <CODE>rollback</CODE> call fail throw a 
 * proper runtime exception.
 * <p>
 * Note that a command is during normal execution only executed once but it 
 * must be able to execute again "re-writing" it self to underlying persistance 
 * store during recovery. Think of the following scenario:
 * <p>
 ...
 * while the 25 last commands will be executed for the first time. Also it 
 * would be possible for the system to crash during recovery causing commands 
 * to execute again and again and so on.
 */
public abstract class XaCommand
{
	...
	/**
	 * Default implementation of rollback that does nothing. This method is 
	 * not to undo any work done by the {@link #execute} method. Commands in 
	 * a {@link XaTransaction} are either all rolled back or all executed, 
	 * they're not linked together as usual execute/rollback methods. 
	 * <p>
	 ...
	 * any protests about this command not being able to execute should be 
	 * done before execution of any command within the transaction.
	 */
	 ...
	 * <p>
	 * Don't throw an <CODE>IOException</CODE> to imply something is wrong with 
	 * the command. An exception should only be thrown here if there is a real
	 * IO failure. If something is wrong with this command it should have been 
	 * detected when it was created.
	 * <p>
	 * Don't <CODE>force</CODE>, <CODE>position</CODE> or anything except 
	 * normal forward <CODE>write</CODE> with the file channel.
	 *
	 * @param fileChannel The channel to the {@link XaLogicalLog}
	 * @param buffer A small byte buffer that can be used to write command 
	 * data
	 ...
	 * scan of the logical log this method will be called to mark the command
	 * as a "recovered command".
	 */

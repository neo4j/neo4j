...
class TimeUtilTest
{
    ...
    void parseMillisFromString()
    {
        ...
        // Combine small units that would normally be rounded down to zero but add together to make more than a millisecond
    ...
    void nanosToStringAndBackAgain()
    {
        ...
        for ( int i = 0; i < 1000; i++ )
        {
            ...
            // org.neo4j.test.rule.RandomRule is much better but cannot be used here without creating a circular dependency.
            ...
            assertEquals( parseTimeMillis.apply( nanosToString( nanos ) ),
                    parseTimeMillis.apply( nanosToString( parseTimeMillis.apply( nanosToString( nanos ) ) * millisecondInNanos ) ) );
        }
    }
}

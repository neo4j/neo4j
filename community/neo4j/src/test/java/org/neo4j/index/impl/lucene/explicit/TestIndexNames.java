...
public class TestIndexNames
{
    ...
    public void makeSureIndexNamesCanBeRead()
    {
        ...
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().nodeIndexNames() ), name1 );
        String name2 = "my-index-2";
        graphDb.index().forNodes( name2 );
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().nodeIndexNames() ), name1, name2 );
        graphDb.index().forRelationships( name1 );
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().nodeIndexNames() ), name1, name2 );
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().relationshipIndexNames() ), name1 );
        finishTx( true );
        ...
        finishTx( true );
        beginTx();
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().nodeIndexNames() ), name2 );
        Neo4jTestCase.assertContains( Arrays.asList( graphDb.index().relationshipIndexNames() ), name1 );
        finishTx( false );
    }
}

    ...
    private Callable<Void> newWorker( final int iterationsToRun ) throws Exception
    {
        return new Callable<Void>()
        {
            ...
            private final byte[] createAndRollback = util.chunk(
                    RunMessage.run( "BEGIN" ), PullAllMessage.pullAll(),
                    RunMessage.run( "CREATE (n)" ), PullAllMessage.pullAll(),
                    RunMessage.run( "ROLLBACK" ), PullAllMessage.pullAll() );

            private final byte[] matchAll = util.chunk(
                    RunMessage.run( "MATCH (n) RETURN n" ), PullAllMessage.pullAll() );
            ...
            private void createAndRollback( TransportConnection client ) throws Exception
            {
                ...
                MatcherAssert.assertThat( client, util.eventuallyReceives(
                        MessageMatchers.msgSuccess( messageMatcher ),

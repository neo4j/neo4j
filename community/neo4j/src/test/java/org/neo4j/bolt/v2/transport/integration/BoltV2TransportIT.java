...
public class BoltV2TransportIT
{
    ...
    public void shouldNegotiateProtocolV2() throws Exception
    {
        connection.connect( address )
                ...
                .send( util.chunk( InitMessage.init( USER_AGENT, emptyMap() ) ) );

        MatcherAssert.assertThat( connection, TransportTestUtil.eventuallyReceives( new byte[]{0, 0, 0, 2} ) );
    }
    ...
    private <T extends AnyValue> void testSendingOfBoltV2Value( T value ) throws Exception
    {
        ...
        connection.send( util.chunk(
                RunMessage.run( "CREATE (n:Node {value: $value}) RETURN 42", map( new String[]{"value"}, new AnyValue[]{value}  ) ),
                PullAllMessage.pullAll() ) );

        MatcherAssert.assertThat( connection, util.eventuallyReceives(
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgRecord( StreamMatchers.eqRecord( equalTo( longValue( 42 ) ) ) ),
                MessageMatchers.msgSuccess() ) );
    }
    ...
    private <T extends AnyValue> void testReceivingOfBoltV2Value( String query, T expectedValue ) throws Exception
    {
        ...
        connection.send( util.chunk(
                RunMessage.run( query ),
                PullAllMessage.pullAll() ) );

        MatcherAssert.assertThat( connection, util.eventuallyReceives(
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgRecord( StreamMatchers.eqRecord( equalTo( expectedValue ) ) ),
                MessageMatchers.msgSuccess() ) );
    }
    ...
    private <T extends AnyValue> void testSendingAndReceivingOfBoltV2Value( T value ) throws Exception
    {
        ...
        connection.send( util.chunk(
                RunMessage.run( "RETURN $value", map(  new String[]{"value"} , new AnyValue[]{value} ) ),
                PullAllMessage.pullAll() ) );

        MatcherAssert.assertThat( connection, util.eventuallyReceives(
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgSuccess(),
                MessageMatchers.msgRecord( StreamMatchers.eqRecord( equalTo( value ) ) ),
                MessageMatchers.msgSuccess() ) );
    }

...
public class TransactionIT
{
    ...
    public void shouldNotFailWhenOutOfOrderRollbackInAutoCommitMode() throws Throwable
    {
        ...
        // Then
        MatcherAssert.assertThat( runRecorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( pullAllRecorder.nextResponse(), BoltMatchers.succeeded() );
    }
    ...
    public void shouldReceiveBookmarkOnCommitAndDiscardAll() throws Throwable
    {
        ...
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithMetadata( "bookmark", BOOKMARK_PATTERN ) );
    }
    ...
    public void shouldReadYourOwnWrites() throws Exception
    {
        ...
        Thread thread = new Thread( () -> {
            try ( BoltStateMachine machine = env.newMachine( boltChannel ) )
            {
                machine.init( USER_AGENT, emptyMap(), null );
                latch.await();
                machine.run( "MATCH (n:A) SET n.prop = 'two'", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
        ...
        try ( BoltStateMachine machine = env.newMachine( boltChannel ) )
        {
            ...
            machine.run( "BEGIN", ValueUtils.asMapValue( singletonMap( "bookmark", bookmark ) ), NullResponseHandler.nullResponseHandler() );
            machine.pullAll( recorder );
            machine.run( "MATCH (n:A) RETURN n.prop", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
            machine.pullAll( recorder );
            machine.run( "COMMIT", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
            machine.pullAll( recorder );

            MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithMetadata( "bookmark", BOOKMARK_PATTERN ) );
            MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithRecord( "two" ) );
            MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithMetadata( "bookmark", BOOKMARK_PATTERN ) );
        }
    ...
    public void shouldTerminateQueriesEvenIfUsingPeriodicCommit() throws Exception
    {
        ...
        Thread thread = new Thread( () -> {
            try ( BoltStateMachine stateMachine = env.newMachine( mock( BoltChannel.class ) ) )
            {
                ...
                {
                    latch.start();
                    stateMachine.run( query, EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
                    stateMachine.pullAll( NullResponseHandler.nullResponseHandler() );
                }
                finally
                {
                    latch.finish();
        ...
        machine[0].reset( NullResponseHandler.nullResponseHandler() );
    ...
    public void shouldInterpretEmptyStatementAsReuseLastStatementInAutocommitTransaction() throws Throwable
    {
        ...
        // When
        machine.run( "RETURN 1", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
        machine.pullAll( recorder );
        machine.run( "", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
        machine.pullAll( recorder );

        // Then
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithRecord( 1L ) );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithRecord( 1L ) );
    }
    ...
    public void shouldInterpretEmptyStatementAsReuseLastStatementInExplicitTransaction() throws Throwable
    {
        ...
        // When
        machine.run( "BEGIN", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
        machine.discardAll( NullResponseHandler.nullResponseHandler() );
        machine.run( "RETURN 1", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
        machine.pullAll( recorder );
        machine.run( "", EMPTY_MAP, NullResponseHandler.nullResponseHandler() );
    ...
    public void shouldCloseAutoCommitTransactionAndRespondToNextStatementWhenRunFails() throws Throwable
    {
        ...
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.wasIgnored() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithRecord( 2L ) );
    ...
    public void shouldCloseAutoCommitTransactionAndRespondToNextStatementWhenStreamingFails() throws Throwable
    {
        ...
        // Then
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(),
                AllOf.allOf( BoltMatchers.containsRecord( 1L ), BoltMatchers.failedWithStatus( Status.Statement.ArithmeticError ) ) );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeeded() );
        MatcherAssert.assertThat( recorder.nextResponse(), BoltMatchers.succeededWithRecord( 2L ) );
        Assert.assertEquals( recorder.responseCount(), 0 );

...
public abstract class ConstraintTestBase<G extends KernelAPIWriteTestSupport> extends KernelAPIWriteTestBase<G>
{
    ...
    public void setup()
    {
        try ( org.neo4j.graphdb.Transaction tx = KernelAPIWriteTestBase.graphDb.beginTx() )
        {
            for ( ConstraintDefinition definition : KernelAPIWriteTestBase.graphDb.schema().getConstraints() )
            {
                definition.drop();
            }
            tx.success();
    ...
    public void shouldBeAbleCheckExistenceOfConstraints() throws Exception
    {
        ...
        try ( org.neo4j.graphdb.Transaction tx = KernelAPIWriteTestBase.graphDb.beginTx() )
        {
            ...
            KernelAPIWriteTestBase.graphDb.schema().constraintFor( label( "FOO" ) ).assertPropertyIsUnique( "prop1" ).create();
            ConstraintDefinition dropped =
                    KernelAPIWriteTestBase.graphDb.schema().constraintFor( label( "FOO" ) ).assertPropertyIsUnique( "prop2" ).create();
            dropped.drop();
            tx.success();
        }
    ...
    public void shouldCheckUniquenessWhenAddingLabel() throws Exception
    {
        ...
        try ( org.neo4j.graphdb.Transaction tx = KernelAPIWriteTestBase.graphDb.beginTx() )
        {
            Node conflict = KernelAPIWriteTestBase.graphDb.createNode();
            conflict.setProperty( "prop", 1337 );
            nodeConflicting = conflict.getId();

            Node ok = KernelAPIWriteTestBase.graphDb.createNode();
            ok.setProperty( "prop", 42 );
            nodeNotConflicting = ok.getId();

            //Existing node
            Node existing = KernelAPIWriteTestBase.graphDb.createNode();
    ...
    private void addConstraints( String... labelProps )
    {
        ...
        try ( org.neo4j.graphdb.Transaction tx = KernelAPIWriteTestBase.graphDb.beginTx() )
        {
            ...
            {
                KernelAPIWriteTestBase.graphDb.schema().constraintFor( label( labelProps[i] ) ).assertPropertyIsUnique( labelProps[i + 1] )
                        .create();
            }
            tx.success();
        }

...
public class SchemaAcceptanceTest
{
    ...
    public void addingAnIndexingRuleShouldSucceed()
    {
        ...
        // THEN
        MatcherAssert.assertThat( Neo4jMatchers.getIndexes( db, label ), Neo4jMatchers.containsOnly( index ) );
    }
    ...
    {
        // WHEN
        IndexDefinition index = Neo4jMatchers.createIndex( db, label, propertyKey, secondPropertyKey );
    ...
    public void shouldThrowConstraintViolationIfAskedToIndexPropertyThatIsAlreadyIndexed()
    {
        ...
        // THEN
        MatcherAssert.assertThat( caught, IsNot.not( IsNull.nullValue() ) );
    }
    ...
    public void droppingExistingIndexRuleShouldSucceed()
    {
        ...
        // THEN
        MatcherAssert.assertThat( Neo4jMatchers.getIndexes( db, label ), Neo4jMatchers.isEmpty() );
    }
    ...
    public void droppingAnUnexistingIndexShouldGiveHelpfulExceptionInSameTransaction()
    {
        ...
        // THEN
        MatcherAssert.assertThat( "Index should have been deleted", Neo4jMatchers.getIndexes( db, label ), IsNot.not( Neo4jMatchers.contains( index ) ) );
    }
    ...
    public void awaitingAllIndexesComingOnlineWorks()
    {
        ...
        // WHEN
        IndexDefinition index = Neo4jMatchers.createIndex( db, label, propertyKey );
        Neo4jMatchers.createIndex( db, label, "other_property" );
    ...
    public void shouldPopulateIndex()
    {
        ...
        // THEN
        MatcherAssert.assertThat( Neo4jMatchers.findNodesByLabelAndProperty( label, propertyKey, "Neo", db ), Neo4jMatchers.containsOnly( node ) );
    }
    ...
    public void shouldRecreateDroppedIndex()
    {
        ...
        MatcherAssert.assertThat( Neo4jMatchers.getIndexes( db, label ), Neo4jMatchers.contains( index ) );
        MatcherAssert.assertThat( Neo4jMatchers.findNodesByLabelAndProperty( label, propertyKey, "Neo", db ), Neo4jMatchers.containsOnly( node ) );
    }
    ...
    public void shouldListAddedConstraintsByLabel()
    {
        ...
        // WHEN THEN
        MatcherAssert.assertThat( Neo4jMatchers.getConstraints( db, label ), Neo4jMatchers.containsOnly( constraint1 ) );
    }
    ...
    public void shouldListAddedConstraints()
    {
        ...
        // WHEN THEN
        MatcherAssert.assertThat( Neo4jMatchers.getConstraints( db ), Neo4jMatchers.containsOnly( constraint1, constraint2 ) );
    }
    ...
    {
        // GIVEN
        IndexDefinition indexA = Neo4jMatchers.createIndex( db, label, "a" );
        createUniquenessConstraint( label, "b" );

        // WHEN
        try ( Transaction tx = db.beginTx() )
        {
            MatcherAssert.assertThat( count( db.schema().getIndexes( label ) ), Matchers.is( 2L ) );
            IndexDefinition indexC = db.schema().indexFor( label ).on( "c" ).create();
            // THEN
            MatcherAssert.assertThat( count( db.schema().getIndexes( label ) ), Matchers.is( 3L ) );
            MatcherAssert.assertThat( db.schema().getIndexState( indexA ), Matchers.is( Schema.IndexState.ONLINE ) );
            MatcherAssert.assertThat( db.schema().getIndexState( indexC ), Matchers.is( Schema.IndexState.POPULATING ) );
        }
    }

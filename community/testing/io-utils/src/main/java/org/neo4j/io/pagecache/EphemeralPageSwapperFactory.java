...
public class EphemeralPageSwapperFactory implements PageSwapperFactory, AutoCloseable
{
    ...
    private static class EphemeralSwapper implements PageSwapper
    {
        ...
        public long read( long filePageId, long bufferAddress ) throws IOException
        {
            ...
            {
                long ptr = addr;
                UnsafeUtil.copyMemory( ptr, bufferAddress, filePageSize );
                read = filePageSize;
            }
            if ( read < filePageSize )
            {
                UnsafeUtil.setMemory( bufferAddress + read, filePageSize - read, (byte) 0 );
            }
            return read;
        }
        ...
        public long read( long startFilePageId, long[] bufferAddresses, int arrayOffset, int length ) throws IOException
        {
            ...
            {
                data += read( startFilePageId + i, bufferAddresses[arrayOffset + i] );
            }
            return data;
        }

...
public class ManualNeo4jProxyVerification {
    ...
    void successfullySendAndReceiveOneMessage() throws IOException {
        try (var proxy = TcpCrusherProxy.builder().build()) {
            ...
            var echoClient = new EchoClient(
                    proxyConfig.advertisedAddress().getHostName(),
                    proxyConfig.advertisedAddress().getPort());
    ...
    void startStoppingShouldCorrectly() throws IOException {
        try (var proxy = TcpCrusherProxy.builder().build()) {
            ...
            var echoClient1 = new EchoClient(
                    proxyConfig.advertisedAddress().getHostName(),
                    proxyConfig.advertisedAddress().getPort());
            try (var executor = Executors.newFixedThreadPool(2)) {
                ...
                var echoClient2 = new EchoClient(
                        proxyConfig.advertisedAddress().getHostName(),
                        proxyConfig.advertisedAddress().getPort());

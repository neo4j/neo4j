...
public class PropertyAndNodeIndexedCheck implements RecordCheck<NodeRecord, ConsistencyReport.NodeConsistencyReport>
{
    ...
    /*public void checkConsistency( PropertyRecordCheck.PropertyField field,
            PropertyRecord record,
            PropertyRecord referred,
            CheckerEngine<PropertyRecord, ConsistencyReport.PropertyConsistencyReport> engine)
    {
        if ( !field.noRelationship().is( field.valueFrom( record ) ) )
        {
            if ( !referred.inUse() )
            {
                field.notInUse( engine.report(), referred );
            }
            else
            {
                if ( field.otherReference( referred ) != record.getId() )
                {
                    field.noBackReference( engine.report(), referred );
                }
            }
        }
    }*/

...
class RecordLoading
{
    ...
    static long[] safeGetNodeLabels( CheckerContext context, long nodeId, long labelField, RecordReader<DynamicRecord> labelReader )
    {
        ...
        if ( safeLoadDynamicRecordChain( record -> records.add( safeClone( record ) ), labelReader, seenRecordIds,
    ...
    static <RECORD extends TokenRecord> List<NamedToken> safeLoadTokens( TokenStore<RECORD> tokenStore )
    {
              ...
              RecordReader<DynamicRecord> nameReader = new RecordReader<>( nameStore ) )
        {
            for ( long id = 0; id < highId; id++ )
            {
                ...
                {
                    String name;
                    if ( !NULL_REFERENCE.is( record.getNameId() ) && safeLoadDynamicRecordChain( r -> nameRecords.add( safeClone( r ) ),
    ...
    static <R extends AbstractBaseRecord> R safeClone( R record )
    {
        try
        {
            return (R) record.clone();
        }

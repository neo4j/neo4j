...
public class IndexAccessors implements Closeable
{
    ...
    public IndexAccessors(
            ...
            throws IOException
    {
        this( providers, neoStores, samplingConfig, tokenNameLookup, null /*we'll use a default below, if this is null*/ );
    }
        ...
        while ( true )
        {
            try
            {
                if ( indexes.hasNext() )
                {
                    ...
                    {
                        if ( InternalIndexState.ONLINE == indexProvider.getInitialState( indexDescriptor ) )
                        {

...
class TernaryEqualityTest {
    ...
    void shouldGiveTrueWhenComparingSameNumbersOfDifferentType() {
        NumberValue[] numbers = {
            byteValue((byte) 11), shortValue((short) 11), intValue(11), longValue(11), floatValue(11), doubleValue(11)
        };
    ...
    void shouldGiveFalseWhenComparingDifferentNumbersOfDifferentType() {
        NumberValue[] numbers = {
            byteValue((byte) 11), shortValue((short) 12), intValue(13), longValue(14), floatValue(15), doubleValue(16)
        };
    ...
    void shouldHandleListsWithNoValue() {
        ...
        assertTernaryEquality(
                listWithNoValue, list(intValue(1), intValue(2), intValue(3), intValue(4)), Equality.FALSE);
    ...
    void shouldHandleMaps() {
        assertTernaryEquality(map("key", list(intValue(42))), map("key", intArray(new int[] {42})), Equality.TRUE);
    ...
    private void assertTernaryEquality(AnyValue a, AnyValue b, Equality expected) {
        assertThat(a.ternaryEquals(b))
                .as(format("%s = %s should be %s", a, b, expected))
                .isEqualTo(expected);
        assertThat(b.ternaryEquals(a))
                .as(format("%s = %s shoudl be %s", b, a, expected))
                .isEqualTo(expected);
        switch (expected) {
            case TRUE:
                assertThat(comparator.ternaryCompare(a, b))
                        .as(format("%s = %s", a, b))
                        .isEqualTo(Comparison.EQUAL);
                assertThat(comparator.ternaryCompare(b, a))
                        .as(format("%s = %s", b, a))
                        .isEqualTo(Comparison.EQUAL);
                break;
            case FALSE:

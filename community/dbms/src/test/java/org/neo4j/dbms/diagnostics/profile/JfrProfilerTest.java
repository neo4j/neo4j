...
class JfrProfilerTest {
    ...
    private void assertHasValidRecording() throws IOException {
        Path[] recordingFiles = getRecordingFiles();
        assertThat(recordingFiles).hasSize(1);
        assertThat(isValidRecording(recordingFiles[0])).isTrue();
    }

    private boolean isValidRecording(Path file) throws IOException {
        try (var recordingFile = new RecordingFile(file)) {
            ...
            return numEvents > 0; // We have parsed the file and found at least one event
        }
    }

    private Path[] getRecordingFiles() throws IOException {
        return fs.listFiles(
                dir.homePath(), path -> path.getFileName().toString().endsWith(".jfr"));
    }
    ...
    private void createProfiler() throws IOException {
        ...
        profiler = new JfrProfiler(jmxDump, fs, dir.homePath(), Duration.ofMinutes(1), Clocks.nanoClock());
    }
}

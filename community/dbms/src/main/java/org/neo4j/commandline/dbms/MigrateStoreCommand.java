...
@Command(
        name = "migrate-store",
        header = "Migrate database store",
        description = "Migrate database store"
)
public class MigrateStoreCommand extends AbstractCommand
{
    @Option( names = "--database", paramLabel = "<database>", description = "Name of the database whose store to migrate.",
...
//    private String storageEngine;
                ...
                PageCache pageCache = createPageCache( fs, config, jobScheduler, pageCacheTracer ) )
        {
            ...
            try ( Closeable ignored = LockChecker.checkDatabaseLock( databaseLayout ) )
            {
                ...
                var indexProviderMap = getIndexProviderMap( fs, databaseLayout, config, logService, pageCache, jobScheduler, life,
                        currentStorageEngineFactory, pageCacheTracer, contextFactory );
    ...
    private static StaticIndexProviderMap getIndexProviderMap( FileSystemAbstraction fs, DatabaseLayout databaseLayout, Config config,
            LogService logService, PageCache pageCache, JobScheduler jobScheduler, LifeSupport life, StorageEngineFactory storageEngineFactory,
            PageCacheTracer pageCacheTracer, CursorContextFactory contextFactory )
    {
        ...
        var tokenHolders = storageEngineFactory.loadReadOnlyTokens( fs, databaseLayout, config, pageCache, true, contextFactory );
        var extensions = life.add( instantiateExtensions(
                fs, databaseLayout, config, logService, pageCache, jobScheduler,
                recoveryCleanupWorkCollector,
                // We use TOOL context because it's true, and also because it uses the 'single' operational mode, which is important.
                monitors, tokenHolders, pageCacheTracer, readOnly() ) );
        return life.add( StaticIndexProviderMapFactory.create(
                life, config, pageCache, fs, logService, monitors, readOnly(), TOOL, recoveryCleanupWorkCollector,
                databaseLayout, tokenHolders, jobScheduler, contextFactory, extensions ) );
    }

    private static DatabaseExtensions instantiateExtensions( FileSystemAbstraction fs, DatabaseLayout databaseLayout, Config config,
            ...
            PageCacheTracer pageCacheTracer, DatabaseReadOnlyChecker readOnlyChecker )
    {
        ...
        deps.satisfyDependencies( fs, config, logService, pageCache, recoveryCollector, monitors, jobScheduler, tokenHolders, pageCacheTracer,

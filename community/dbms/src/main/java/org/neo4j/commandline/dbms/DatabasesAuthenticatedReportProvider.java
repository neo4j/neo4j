...
/**
 * Authenticated classifier that captures the databases of the DBMS by running {@code SHOW DATABASES YIELD *} against the
 * running instance, writing the result as a single JSON file into the report. {@code SHOW DATABASES} is a DBMS-level
 * procedure, so it is run once against the {@code system} database rather than per database.
 */
@ServiceProvider
public class DatabasesAuthenticatedReportProvider extends DiagnosticsAuthenticatedReportProvider {
    static final String CLASSIFIER = "databases";
    private static final String QUERY = "SHOW DATABASES YIELD *";
    private static final String SYSTEM_DATABASE = "system";

    public DatabasesAuthenticatedReportProvider() {
        super(CLASSIFIER);
    }
    ...
    protected Map<String, List<DiagnosticsReportSource>> provideSources(
        ...
        try {
            ...
            return Map.of(CLASSIFIER, List.of(newFailedDiagnosticsSource(destination, message)));
        }

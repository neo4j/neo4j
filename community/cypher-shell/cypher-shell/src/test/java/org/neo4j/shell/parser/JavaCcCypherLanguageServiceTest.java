...
class JavaCcCypherLanguageServiceTest
{
    ...
    void querySuggestionsSanity()
    {
        ...
        assertSuggestions(
                "",
                "CREATE", "MATCH", "DROP", "UNWIND", "RETURN", "WITH", "LOAD",
                "ALTER", "RENAME", "SHOW", "START", "STOP"
        );
        assertSuggestions(
                "MATCH (n) ",
                "WHERE", "MATCH", "RETURN", "UNWIND", "MERGE", "DELETE", "WITH", "UNION"
        );
        assertSuggestions( "MATCH (n) WHERE", "CASE", "ANY", "ALL" );
        assertSuggestions(
                "MATCH (n) WHERE n.p",
                "IN", "ENDS", "STARTS", "CONTAINS", "IS", "RETURN"
        );
        assertSuggestions(
                "MATCH (n) WHERE n.p = 1",
                "AND", "OR", "RETURN"

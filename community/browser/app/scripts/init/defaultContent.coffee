...
.run([
  ...
  ($rootScope, Document, Folder) ->
    ...
    if Document.length is 0
      folders =
        general: Folder.create(name: "General", expanded: yes)
        system: Folder.create(name: "System", expanded: no)

      general_scripts = [
        {
          folder: folders.general.id
          content: """
  // Create a node
  ...
  RETURN DISTINCT head(labels(a)), type(r), head(labels(b)) LIMIT 100
    ...
    for doc in Document.all()
      ...
      if not Folder.get(doc.folder)
        Folder.create(id: doc.folder, expanded: no)

...
abstract class AbstractJsonlBookmarkWriter implements MessageBodyWriter<QueryResponseBookmarks> {
    private final JsonFactory factory;

    protected AbstractJsonlBookmarkWriter(View view) {
        this.factory = DefaultJsonFactory.INSTANCE.get().copy().setCodec(new QueryAPICodec(view));
    }

    @Override
    public boolean isWriteable(Class<?> aClass, Type type, Annotation[] annotations, MediaType mediaType) {
        return QueryResponseBookmarks.class.isAssignableFrom(aClass);
    }

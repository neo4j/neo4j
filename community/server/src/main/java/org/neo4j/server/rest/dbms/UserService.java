...
public class UserService
{
    ...
    public Response regenerateToken( String payload, @PathParam( "user" ) String user )
    {
        try
        {
    ...
    @PUT
    @Path("/{user}/password")
    public Response setPassword( String payload, @PathParam( "user" ) String user )
    {
        try
        {
            Map<String,Object> deserialized = input.readMap( payload );
    ...
    @PUT
    @Path("/{user}/authorization_token")
    public Response setToken( String payload, @PathParam( "user" ) String user )
    {
        try
        {
            Map<String,Object> deserialized = input.readMap( payload );
            ...
            String password = getString( deserialized, "password" );
            String newToken = getString( deserialized, "new_authorization_token" );

            if( security.authenticate( user, password ))
            {
                security.setToken( user, newToken );
                return output.ok( new AuthorizationRepresentation( security.userForName( user ) ) );
            }

...
.Quick info
***********
* Neo4j's configuration is found in _conf/neo4j.properties_.
...
The main configuration file for Neo4j is _conf/neo4j.properties_.
...
To enable HTTP logging, edit the _conf/neo4j.properties_ file to resemble the following:
...
the following property in _neo4j.properties_:
...
In the _conf/neo4j.properties_ file:

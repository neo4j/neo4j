...
To override the send-signed key and certificate, replace _neo4j.key_ and _neo4j.cert_ in the <<file-locations, _certificates_>> directory. The location of this directory can be configured by setting _dbms.directories.certificates_ in <<file-locations, _neo4j.conf_>>.

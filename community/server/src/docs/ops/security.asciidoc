...
To provide your own key and certificate put them in the <<file-locations, ​_certificates_>> directory. The files must be named _neo4j.key_​ and ​_neo4j.cert_​. The location of the directory can be configured by setting ​_dbms.directories.certificates_​ in <<file-locations, _neo4j.conf_>>.

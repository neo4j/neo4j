...
This is configured in the _conf/neo4j.conf_ file:
...
If you want the server to listen to external hosts, configure the Web server in the _conf/neo4j.conf_ by setting the property +org.neo4j.server.webserver.address=0.0.0.0+ which will cause the server to bind to all available network interfaces.
...
To provide your own key and certificate, replace the generated key and certificate, or change the _conf/neo4j.conf_ file to set the location of your certificate and key:

...
public class UsersDocIT extends ExclusiveServerTestBase
{
    ...
    public void regenerate_token() throws PropertyValueException, IOException
    {
        // Given
        startServer( true );
        String token = HTTP.POST( authURL(), HTTP.RawPayload.quotedJson( "{'user':'neo4j','password':'neo4j'}" ) )
        ...
        RESTDocsGenerator.ResponseEntity response = gen.get()
                .noGraph()
                .expectedStatus( 200 )
                .payload( quotedJson( "{'password':'neo4j'}" ) )
    ...
    public void change_password() throws PropertyValueException, IOException
    {
        // Given
        startServer( true );
        String originalToken = HTTP.POST( authURL(), HTTP.RawPayload.quotedJson( "{'user':'neo4j','password':'neo4j'}" ) )
                .get( "authorization_token" ).asText();
        ...
        RESTDocsGenerator.ResponseEntity response = gen.get()
                .noGraph()
                .expectedStatus( 200 )
                .payload( quotedJson( "{'password':'neo4j', 'new_password':'secret'}" ) )
                .put( server.baseUri().resolve( "/user/neo4j/password" ).toString() );
        ...
        assertEquals(200, HTTP.POST( authURL(), HTTP.RawPayload.quotedJson( "{'user':'neo4j','password':'secret'}" ) ).status());

        // And then the old password should not be invalid
        assertEquals(422, HTTP.POST( authURL(), HTTP.RawPayload.quotedJson( "{'user':'neo4j','password':'neo4j'}" ) ).status());
    }
    ...
    public void set_token() throws PropertyValueException, IOException
    {
        ...
        RESTDocsGenerator.ResponseEntity response = gen.get()
                .noGraph()
                .expectedStatus( 200 )
                .payload( quotedJson( "{'password':'neo4j', 'new_authorization_token':'EEB9E6883A24CEF7899CF35AD49D5944'}" ) )
                .put( server.baseUri().resolve( "/user/neo4j/authorization_token" ).toString() );
    ...
    public void shouldRateLimit() throws Exception
    {
        // Given
        startServer( true );
        assertRateLimited( "PUT", server.baseUri().resolve( "/user/neo4j/password" ), "{'password':'something that is wrong', 'new_password':'secret'}");
        assertRateLimited( "POST", server.baseUri().resolve( "/user/neo4j/authorization_token" ), "{'password':'something that is wrong'}" );
        assertRateLimited( "PUT", server.baseUri().resolve( "/user/neo4j/authorization_token" ), "{'password':'something that is wrong','new_authorization_token':'asd'}" );
    }
    ...
    public void shouldRequireAuthorization() throws Exception
    {
        // Given
        startServer( true );
        assertAuthorizationNeeded( "PUT", server.baseUri().resolve( "/user/neo4j/password" ), "{'password':'something that is wrong', 'new_password':'secret'}" );
        assertAuthorizationNeeded( "POST", server.baseUri().resolve( "/user/neo4j/authorization_token" ), "{'password':'something that is wrong'}" );
        assertAuthorizationNeeded( "PUT", server.baseUri().resolve( "/user/neo4j/authorization_token" ), "{'password':'something that is wrong','new_authorization_token':'asd'}" );
    }

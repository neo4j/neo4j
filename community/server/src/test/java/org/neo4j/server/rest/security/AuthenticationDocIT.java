...
public class AuthenticationDocIT extends ExclusiveServerTestBase
{
     ...
     * a new password. Until you do, the database will not give you any authorization token. The database will signal
     * that a new password is required when you try to authenticate.
     */
    @Test
    @Documented
     ...
     * token which you then include in requests to the API. The reply from this endpoint will also indicate if your
     * password should be changed, which will, for instance, be the case if the password is the default Neo4j password.
     */
    @Test
    @Documented
     ...
     *
     * Given that you have a valid authorization token, you can retrieve metadata about the current user from the
    ...
    public void authorization_metadata() throws PropertyValueException, IOException
    {
        // Given
        startServerWithConfiguredUser();
        String authToken = HTTP.POST( authURL(), RawPayload.quotedJson( "{'user':'neo4j','password':'neo4j'}" ) )
    ...
    private void assertAuthorizationRequired( String method, String path, Object payload, int expectedAuthorizedStatus ) throws JsonParseException
    {
        ...
        // When authorized
        String token = HTTP.POST( authURL(), RawPayload.quotedJson( "{'user':'neo4j','password':'neo4j'}" ) )
                .get( "authorization_token" ).asText();
    ...
    public void startServerWithConfiguredUser() throws IOException
    {
        startServer( true );
        // Set the password
        HTTP.Response put = HTTP.PUT( server.baseUri().resolve( "/user/neo4j/password" ).toString(),
                RawPayload.quotedJson( "{'password':'neo4j', 'new_password':'neo4j'}" ) );

...
class IntegrityValidatorTest {
    ...
    void shouldValidateUniquenessIndexes() throws Exception {
        ...
        doThrow(new ConstraintViolationException("error", new RuntimeException()))
                .when(indexes)
                .validateIndex(2L);
    ...
    void newIndexesNotAllowedForOldKernelVersions(IndexType indexType) {
        ...
        assertThatThrownBy(() -> validator.validateSchemaRule(index))
                .isInstanceOf(TransactionFailureException.class)
                .hasMessageContaining(
                        "Required kernel version for this transaction is V4_4, but actual version was V4_3_D4.");
    }
    ...
    void constraintBackedByRangeNotAllowedForOldKernelVersions() {
        ...
        ConstraintDescriptor constraint = ConstraintDescriptorFactory.uniqueForSchema(
                        SchemaDescriptors.forLabel(10, 28), IndexType.RANGE)
                .withOwnedIndexId(4)
                .withName("any name")
                .withId(5);

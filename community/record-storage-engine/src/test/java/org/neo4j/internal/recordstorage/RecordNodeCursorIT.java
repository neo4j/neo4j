...
class RecordNodeCursorIT
{
    ...
    void startNeoStores()
    {
        neoStores = new StoreFactory( DatabaseLayout.ofFlat( directory.homePath() ), Config.defaults(),
                new DefaultIdGeneratorFactory( directory.getFileSystem(), immediate() ), pageCache, directory.getFileSystem(),
                NullLogProvider.nullLogProvider(), PageCacheTracer.NULL ).openAllNeoStores( true );

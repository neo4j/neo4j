...
public class SchemaStoreMigration
{
    public interface SchemaStoreMigrator
    {
        ...
        void copyFilesInPreparationForMigration( FileSystemAbstraction fileSystem, RecordDatabaseLayout directoryLayout,
                RecordDatabaseLayout migrationLayout ) throws IOException;
    ...
    record SchemaStoreNewFamilyMigration(List<SchemaRule> rules) implements SchemaStoreMigrator
    {
        ...
        @Override
        public void copyFilesInPreparationForMigration( FileSystemAbstraction fileSystem, RecordDatabaseLayout directoryLayout,
                RecordDatabaseLayout migrationLayout ) throws IOException
        {
    ...
    static SchemaStoreMigrator getSchemaStoreMigration( RecordFormats oldFormat, RecordDatabaseLayout directoryLayout,
            CursorContext cursorContext, boolean requiresPropertyMigration, boolean systemDb, Config config,
            PageCache pageCache, FileSystemAbstraction fileSystem, CursorContextFactory contextFactory
    )
    {
        IdGeneratorFactory srcIdGeneratorFactory = new ScanOnOpenReadOnlyIdGeneratorFactory();
        StoreFactory srcFactory = createStoreFactory( directoryLayout, config, srcIdGeneratorFactory, pageCache, fileSystem, oldFormat, contextFactory );

        if ( !need50Migration( oldFormat ) )
        {
            if ( !requiresPropertyMigration )
            {
                // No schema store migration needed if we are not changing format family (or migrating properties for other reason)
                return new SchemaStoreNoMigration();
            }

            List<SchemaRule> schemaRules;
            try ( NeoStores srcStore = srcFactory.openNeoStores( false, StoreType.PROPERTY_KEY_TOKEN, StoreType.PROPERTY_KEY_TOKEN_NAME,
                    StoreType.LABEL_TOKEN, StoreType.LABEL_TOKEN_NAME, StoreType.RELATIONSHIP_TYPE_TOKEN, StoreType.RELATIONSHIP_TYPE_TOKEN_NAME,
                    StoreType.PROPERTY, StoreType.PROPERTY_STRING, StoreType.PROPERTY_ARRAY, StoreType.SCHEMA );
                  var srcCursors = new CachedStoreCursors( srcStore, cursorContext ) )
            {
                ...
                org.neo4j.internal.recordstorage.SchemaStorage schemaStorage =
                        new org.neo4j.internal.recordstorage.SchemaStorage( srcStore.getSchemaStore(), srcTokenHolders, KernelVersionRepository.LATEST );
        ...
        return getSchemaStore44Migrator( oldFormat, directoryLayout, cursorContext, requiresPropertyMigration, systemDb, config, pageCache, contextFactory,
                srcIdGeneratorFactory, srcFactory );
    }

    private static StoreFactory createStoreFactory( RecordDatabaseLayout databaseLayout, Config config, IdGeneratorFactory idGeneratorFactory,
            PageCache pageCache, FileSystemAbstraction fileSystem, RecordFormats formats, CursorContextFactory contextFactory )
    {

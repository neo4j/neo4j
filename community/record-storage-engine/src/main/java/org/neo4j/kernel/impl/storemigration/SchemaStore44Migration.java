...
public class SchemaStore44Migration
{
     ...
     */
    public record SchemaStore44Migrator(boolean shouldCreateNewSchemaStore, boolean systemDb, List<SchemaRule44.Index> nonReplacedIndexes,
                                        List<Pair<SchemaRule44.Constraint,SchemaRule44.Index>> nonReplacedConstraints, List<SchemaRule44> toDelete,
                                        List<SchemaRule> existingSchemaRulesToAdd, TokenHolders srcTokenHolders ) implements SchemaStoreMigrator
    {
        ...
        public void assertCanMigrate() throws IllegalStateException
        {
            if ( !systemDb && (!nonReplacedIndexes.isEmpty() || !nonReplacedConstraints.isEmpty()) )
            {
                ...
                nonReplacedConstraints.forEach( pair -> nonReplacedConstraintsString.add( pair.first().userDescription( srcTokenHolders ) ) );
                ...
                RecordDatabaseLayout migrationLayout ) throws IOException
        {
            ...
            if ( shouldCreateNewSchemaStore )
            {
                ...
                databaseFiles = asList( RecordDatabaseFile.PROPERTY_STORE, RecordDatabaseFile.PROPERTY_ARRAY_STORE,
            ...
            {
                databaseFiles = asList( RecordDatabaseFile.SCHEMA_STORE,
        ...
        public void migrate( SchemaRuleMigrationAccess dstAccess, TokenHolders dstTokensHolders ) throws KernelException
        {
            ...
            // Write all the schemaRules that already had ids first to make sure that any newly allocated ids will be unique
    ...
    static SchemaStore44Migration.SchemaStore44Migrator getSchemaStore44Migrator( RecordFormats oldFormat, RecordDatabaseLayout directoryLayout,
            CursorContext cursorContext, boolean shouldCreateNewSchemaStore, boolean systemDb, Config config, PageCache pageCache,
            CursorContextFactory contextFactory, IdGeneratorFactory srcIdGeneratorFactory, StoreFactory srcFactory )
    {
        try ( NeoStores srcStore = srcFactory.openNeoStores( false, StoreType.SCHEMA, StoreType.PROPERTY, StoreType.META_DATA,
              ...
              var srcCursors = new CachedStoreCursors( srcStore, cursorContext ) )
        {
            ...
            try ( var schemaStore44Reader = getSchemaStore44Reader( directoryLayout, oldFormat, srcIdGeneratorFactory, srcStore, srcTokensHolders,
                    config, pageCache, contextFactory, () -> kernelVersion ) )
            {
                return getSchemaStoreMigration44( schemaStore44Reader, srcCursors, systemDb, shouldCreateNewSchemaStore, srcTokensHolders );
            }
        }
    }

    public static SchemaStore44Migrator getSchemaStoreMigration44( SchemaStore44Reader schemaStore44Reader, StoreCursors srcCursors, boolean systemDb,
            boolean shouldCreateNewSchemaStore, TokenHolders srcTokenHolders )
    {
        ...
        for ( var schemaRule : all )
        {
            ...
            if ( schemaRule instanceof SchemaRule44.Constraint constraint )
            {
                ...
                if ( shouldCreateNewSchemaStore && (!indexBacked || constraint.indexType() == SchemaRule44.IndexType.RANGE) )
                {
                    toCreate.add( schemaRule.convertTo50rule() );
                }
            }
        ...
        //    In this case we need to rewrite this schema to give it the properties it should have. In this way we can keep the index id.
        ...
        return new SchemaStore44Migrator( shouldCreateNewSchemaStore, systemDb, nonReplacedIndexes, nonReplacedConstraints, toDelete, toCreate,
                srcTokenHolders );
    }
    ...
    private static SchemaStore44Reader getSchemaStore44Reader( RecordDatabaseLayout recordDatabaseLayout, RecordFormats formats,
            IdGeneratorFactory idGeneratorFactory, NeoStores neoStores, TokenHolders tokenHolders, Config config, PageCache pageCache,
            CursorContextFactory contextFactory, KernelVersionRepository kernelVersionRepository )
    {

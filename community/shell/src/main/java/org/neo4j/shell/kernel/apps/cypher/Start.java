...
public class Start extends ReadOnlyGraphDatabaseApp
{
    ...
    {
        final Map<String, Object> lastRow = IteratorUtil.lastOrNull( rows );
        if ( lastRow == null )
        {
            return;
        }
        for ( Map.Entry<String, Object> cell : lastRow.entrySet() )
        {
            final String name = cell.getKey();
            if ( IDENTIFIER.matcher( name ).matches() )
            {
                session.set( name, cell.getValue() );
            }
        }
    }

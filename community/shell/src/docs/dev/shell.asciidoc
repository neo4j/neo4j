...
You start at the http://api.neo4j.org/current/org/neo4j/graphdb/GraphDatabaseService.html#getReferenceNode()[reference node] and can then +cd+ your way through the graph (check your current path at any time with the +pwd+ command).
...
Also the last line of cypher results is set as shell variables as long as the column names are valid identifiers (like +n+). The combination of both allows for powerful operations.
...
use the +cypher VERSION+ prefix to select a certain cypher parser.

[TIP]
Cypher queries need to be terminated by a semicolon `;`.

Cypher commands are passed all shell variables as parameters and the special +self+ parameter for the current node or relationship.

The last result line of a cypher statement is set as shell variables, each column with a name that is an identifier is considered to be set as variable with the according name and value.
If all cypher results should be set as variables, use +collect+ and other aggregation functions to return only a single result line for a cypher statement.

* +start n = node(\{self}) match n-[:KNOWS]->friend return friend;+ will return the nodes connected to the current node. And the last one of those will be set as the shell variable +friend+.
* +start n=node(\{me}) create friend=\{props}, me-[r:KNOWS]->friend return me,r,friend;+ will create the friend and the relationship according to the variables available and return all three elements which are then set as variables.
...
As a simple way of exporting a database or a subset of it, the +dump+ command converts the graph of a Cypher result or the whole database into a set of Cypher +create+ statements.

[TIP]
Index export is not yet supported.

If +dump+ is called with the parameter +-s+, a simplified version with just a single Cypher create statement is printed. The statement is surrounded with +begin+ and +commit+ to control the transaction externally.

The more advanced version batches the statements into blocks of 20.000 created nodes and relationships each block surrounded by +begin+ and +commit+. It also optimizes the cypher statements by
using parameters (for properties and nodes) which are supplied by shell variables which in turn are set by results of previous Cypher statements.

Examples:
* +dump -s+ dumps the whole database as single cypher create statement
* +dump -s start n=node(\{self}) match p=n-[r:KNOWS*]->m return n,r,m;+ dumps the transitive friendship graph of the current node.
* +neo4j-shell -path db1 -c dump -s start n=node:node_auto_index(name=\'Mattias\') match p=n-[r:KNOWS]->m return p; | neo4j-shell -path db2+

...
Example: +schema+ (will list all indexes and constraints)

Example: +schema -l :Person -p name+ (will only list indexes or constraints on +:Person+ nodes for the property +name+)
...
whenever the graph is changed.  See <<graphdb-neo4j-schema>> for more.

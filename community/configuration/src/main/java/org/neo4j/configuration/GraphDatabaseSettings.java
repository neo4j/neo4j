...
public class GraphDatabaseSettings implements SettingsDeclaration
{
    ...
    @Description( "Minimum time interval after last rotation of the user log (_neo4j.log_) before it may be  " +
            "rotated again. Note that if dbms.logs.user.stdout_enabled is enabled this setting will be ignored." )
    ...
    @Description( "If `true`, Neo4j will abort recovery if transaction log files " +
            "are missing. Setting this to `false` will allow Neo4j to create new empty missing files for the already " +
            "existing database, but the integrity of the database might be compromised." )

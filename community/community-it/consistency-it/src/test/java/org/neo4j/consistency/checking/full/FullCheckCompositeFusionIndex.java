...
public class FullCheckCompositeFusionIndex
{
    ...
    void checkCompositeFusionIndexCorrectly() throws ConsistencyCheckIncompleteException
    {
        ...
        DatabaseManagementService managementService = new TestDatabaseManagementServiceBuilder()
                .setDatabaseRootDirectory( dir.homeDir() )
                .build();
        GraphDatabaseService neo4j = managementService.database( "neo4j" );

        try ( Transaction tx = neo4j.beginTx() )
        {
            tx.execute( "CALL db.createIndex( 'fusionIndex', ['myLabel'], ['prop1', 'prop2'], 'lucene+native-3.0' )" ).close();
            tx.commit();
        }
        try ( Transaction tx = neo4j.beginTx() )
        {
        ...
        ConsistencyCheckService.Result result = runFullConsistencyCheck( dir.homeDir().toPath() );
    ...
    {
        var config = Config.newBuilder().set( GraphDatabaseSettings.neo4j_home, path )
                .set( GraphDatabaseInternalSettings.experimental_consistency_checker, true ).build();

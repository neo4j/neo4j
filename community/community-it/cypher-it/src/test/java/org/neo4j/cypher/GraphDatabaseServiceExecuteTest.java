...
public class GraphDatabaseServiceExecuteTest {
    ...
    void shouldNotReturnInternalPointWhenInMap() {
        try (Transaction transaction = db.beginTx()) {
            // when
            Result execute =
                    transaction.execute("RETURN {p: point({longitude: 144.317718, latitude: -37.031738})} AS m");
    ...
    void shouldBeAbleToUseResultingPointFromOneQueryAsParameterToNext() {
        try (Transaction transaction = db.beginTx()) {
            ...
            Result result = transaction.execute(
                    "RETURN point.distance(point({longitude: 144.317718, latitude: -37.031738}),$previous) AS dist",
                    map("previous", point));
    ...
    void shouldBeAbleToUseExternalPointArrayAsParameterToQuery() {
        ...
        try (Transaction transaction = db.beginTx()) {
            Result result =
                    transaction.execute("RETURN point.distance($points[0],$points[1]) AS dist", map("points", points));
    ...
    public static class PointProcs {
        ...
        public Stream<PointResult> spatialPoint(
                @Name("longitude") double longitude, @Name("latitude") double latitude) {
        ...
        public Stream<GeometryResult> spatialPointGeometry(
                @Name("longitude") double longitude, @Name("latitude") double latitude) {

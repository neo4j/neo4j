...
class DataCollectorQueriesAcceptanceTest extends DataCollectorTestSupport {
  ...
  test("[retrieveAllAnonymized] should anonymize tokens and names inside index commands") {
    ...
    execute("CREATE RANGE INDEX name10 FOR (n:Label) ON (n.prop)")
    execute("CREATE RANGE INDEX name11 FOR ()-[r:TYPE]-() ON (r.prop1, r.prop2)")
    ...
    res.toList should beListWithoutOrder(
      ...
      querySection("CREATE INDEX index0 FOR ()-[var0:R0]-() ON (var0.p19, var0.p20)"),
      querySection("CREATE RANGE INDEX index0 FOR (var0:L0) ON (var0.p10)"),
      querySection("CREATE RANGE INDEX index0 FOR ()-[var0:R0]-() ON (var0.p19, var0.p20)"),

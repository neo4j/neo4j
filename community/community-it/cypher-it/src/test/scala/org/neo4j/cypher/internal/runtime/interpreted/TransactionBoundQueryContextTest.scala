...
class TransactionBoundQueryContextTest extends CypherFunSuite {
  ...
  var proxySPI: TransactionalProxyFactory = _
  ...
  override def beforeEach() {
    ...
    proxySPI = mock[TransactionalProxyFactory]
  ...
  test("should mark transaction successful if successful") {
    ...
    val tc = new Neo4jTransactionalContext(graph, bridge, outerTx, statement, mock[ExecutingQuery], proxySPI, transactionFactory)

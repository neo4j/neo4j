...
class BigStoreIT {
    ...
    private void createAndVerifyGraphStartingWithId(long startId, int requiredHeapMb) {
        ...
        Map<String, Object> properties = map(
                ...
                "test",
                "long string",
                "This is a long value, long enough",
                "array",
                bytes);
        ...
        try (Transaction transaction = db.beginTx()) {
            ...
            try (ResourceIterable<Relationship> relationships = refNode.getRelationships(Direction.OUTGOING)) {
                for (final var rel : relationships) {
                    ...
                    Node highNode = node.getSingleRelationship(OTHER_TYPE, Direction.OUTGOING)
                            .getEndNode();
    ...
    private void testHighIds(long highMark, int minus, int requiredHeapMb) {
        ...
        for (int i = 0; i < 2; i++) {
            try (Transaction transaction = db.beginTx()) {
                ...
                assertEquals(highMark, relAboveTheLine.getId());
                assertEquals(
                        asSet(asList(relBelowTheLine, relAboveTheLine)),
    ...
    private void setHighId(IdType type, long highId) {
        IdGeneratorFactory idGeneratorFactory =
                ((GraphDatabaseAPI) db).getDependencyResolver().resolveDependency(IdGeneratorFactory.class);

        ...
        NODE
                {
                    ...
                    {
                        IndexReadSession indexSession = tx.dataRead().indexReadSession( tx.schemaRead().indexGetForName( nameForType( indexType ) ) );
        ...
        RELATIONSHIP
                {
                    ...
                    long createEntity( KernelTransaction tx, Value value ) throws KernelException
                    {
                        ...
                        long relationshipId = tx.dataWrite().relationshipCreate( node1, tx.token().relationshipType( TOKEN ), node2 );
                        tx.dataWrite().relationshipSetProperty( relationshipId, tx.token().propertyKey( PROPERTY ), value );

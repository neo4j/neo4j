...
class CheckpointInfoFactoryTest
{
    ...
    void checkpointInfoOfDetachedCheckpoint42Entry() throws IOException, URISyntaxException
    {
        ...
        LogTailMetadata tailMetadata =
                new LogTailExtractor( fs, pageCache, Config.defaults(), StorageEngineFactory.defaultStorageEngine(), DatabaseTracers.EMPTY ).getTailMetadata(
                        databaseLayout, EmptyMemoryTracker.INSTANCE );
    ...
    void checkpointInfoOfDetachedCheckpoint50Entry()
    {
        ...
        var checkpointInfo =
                ofLogEntry( new LogEntryDetachedCheckpointV5_0( KernelVersion.LATEST, transactionId, logPosition, 2, storeId, "checkpoint" ),
                        position, positionAfterCheckpoint, postReaderPosition, null, null );
    ...
    private void prepareTestResources() throws IOException, URISyntaxException
    {
        ...
        FileUtils.copyFileToDirectory( Path.of( getClass().getResource( "checkpoint.0" ).toURI() ), logFilesDir );
        FileUtils.copyFileToDirectory( Path.of( getClass().getResource( "neostore.transaction.db.0" ).toURI() ), logFilesDir );
    }
}

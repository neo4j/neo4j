...
class TransactionLogInitializerTest
{
    ...
    {
        //Given
        var metaStore = mock( TransactionMetaDataStore.class );
        ...
        DatabaseLayout databaseLayout = Neo4jLayout.of( testDirectory.homeDir() ).databaseLayout( DEFAULT_DATABASE_NAME );

        //When
        var initializer = new TransactionLogInitializer( testDirectory.getFileSystem(), metaStore );
        initializer.initializeEmptyLogFile( databaseLayout, databaseLayout.getTransactionLogsDirectory() );

        //Then
        verify( metaStore ).resetLastClosedTransaction( txn.transactionId(), txn.transactionId(), CURRENT_FORMAT_LOG_HEADER_SIZE, true );
    }
}

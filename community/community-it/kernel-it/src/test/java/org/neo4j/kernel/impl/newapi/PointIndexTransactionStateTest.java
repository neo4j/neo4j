...
class PointIndexTransactionStateTest extends KernelAPIWriteTestBase<WriteTestSupport>
{
    ...
    void shouldPerformEqualitySeek( EntityOperations ops ) throws Exception
    {
        ...
        try ( KernelTransaction tx = beginTransaction() )
        {
            ...
            ops.assertEntityAndValueForSeek( expected, tx, index, point( -1, 1 ), PropertyIndexQuery.exact( prop, point( -1, 1 ) ) );
        }
    }
    ...
    private static void assertEntityAndValue( EntityOperations ops, Set<Pair<Long,Value>> expected, KernelTransaction tx, Object anotherValueFoundByQuery,
    ...
    private enum EntityOperations
    {
        ...
        NODE
                {
                    ...
                    @Override
                    void assertEntityAndValueForSeek( Set<Pair<Long,Value>> expected, KernelTransaction tx, IndexDescriptor index,
                            Object anotherValueFoundByQuery, PropertyIndexQuery query ) throws Exception
                    {
                    ...
                    @Override
                    void assertEntityAndValueForScan( Set<Pair<Long,Value>> expected, KernelTransaction tx, IndexDescriptor index,
        ...
        RELATIONSHIP
                {
                    ...
                    void assertEntityAndValueForSeek( Set<Pair<Long,Value>> expected, KernelTransaction tx, IndexDescriptor index,
                            Object anotherValueFoundByQuery, PropertyIndexQuery query ) throws Exception
                    {
                        try ( RelationshipValueIndexCursor relationships = tx.cursors().allocateRelationshipValueIndexCursor( tx.cursorContext(),
                                tx.memoryTracker() ) )
                        {
                            IndexReadSession indexSession = tx.dataRead().indexReadSession( index );
                            tx.dataRead().relationshipIndexSeek( tx.queryContext(), indexSession, relationships, unordered( true ), query );
                            assertEntityAndValue( this, expected, tx, anotherValueFoundByQuery, new RelationshipCursorAdapter( relationships ) );
                        }
                    }
         ...
         */
        abstract void assertEntityAndValueForSeek( Set<org.neo4j.internal.helpers.collection.Pair<Long,Value>> expected, KernelTransaction tx,
         ...
         * will create an entity with that value after initializing the cursor and assert that the new entity is not found.
         */
        abstract void assertEntityAndValueForScan( Set<Pair<Long,Value>> expected, KernelTransaction tx, IndexDescriptor index,
                Object anotherValueFoundByQuery ) throws Exception;
    }

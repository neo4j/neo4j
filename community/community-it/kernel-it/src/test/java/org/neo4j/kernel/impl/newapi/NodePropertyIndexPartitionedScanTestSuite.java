...
import org.neo4j.kernel.impl.newapi.PartitionedScanFactories.NodePropertyIndex;
import org.neo4j.kernel.impl.newapi.PartitionedScanFactories.PropertyKey;
import org.neo4j.values.storable.Value;

import static org.assertj.core.api.Assertions.assertThat;

class NodePropertyIndexPartitionedScanTestSuite
        extends PropertyIndexPartitionedScanTestSuite<NodeValueIndexCursor>
{
    @Override
    public final NodePropertyIndex getFactory()
    {
        return NodePropertyIndex.FACTORY;
    }

    @Nested
    class WithoutData extends PropertyIndexPartitionedScanTestSuite.WithoutData<NodeValueIndexCursor>
    {
        WithoutData()
        {
            super( NodePropertyIndexPartitionedScanTestSuite.this );
        }

        @Override
        EntityIdsMatchingScanQuery<PropertyKeyScanQuery> setupDatabase()
        {
            ...
            final var data = emptyScanQueries( labelAndPropKeyCombination );
    ...
    @Nested
    class WithData extends PropertyIndexPartitionedScanTestSuite.WithData<NodeValueIndexCursor>
    {
        ...
        EntityIdsMatchingScanQuery<PropertyKeyScanQuery> setupDatabase()
        {
            final var numberOfLabels = 1;
            final var numberOfPropKeys = 2;
            final var numberOfNodeProperties = 1 << 12;
            ...
            final var data = createData( numberOfNodeProperties, labelAndPropKeyCombination );
            createIndexes( createIndexPrototypes( labelAndPropKeyCombination ) );
            return data;
        }

        @Override
        protected EntityIdsMatchingScanQuery<PropertyKeyScanQuery> createData( int numberOfProperties,
                                                                               Pair<Integer,int[]> labelAndPropKeyCombination )
        {
            // given  a set of queries
            final var tracking = new TrackEntityIdsMatchingScanQuery();
            ...
            assertThat( tracking.get().scanQueries().size() ).as( "queries should exist" ).isGreaterThan( 0 );
    ...
    {
        return String.format( "%s[%s[%d] {%s}]", NodePropertyIndex.FACTORY.name(), Label.FACTORY.name(), tokenId,

...
class VectorIndexEntryUpdateTest {

    // TODO VECTOR: when fully adapted to vectors, consider sharing code with Fulltext?
    ...
    abstract static class VectorIndexEntryUpdateTestBase {
        ...
        final void setup() {
            ...
            Mockito.when(databaseIdRepository.getByName(DEFAULT_DATABASE_NAME))
                    .thenReturn(Optional.of(defaultDatabaseId));
            ...
            provider = new VectorIndexProviderFactory()
                    .create(
                            pageCache,
                            fs,
            ...
            final var schema = SchemaDescriptors.forLabel(123, 321);
        ...
        VectorIndexEntryUpdateTestBase(VectorSimilarityFunction similarityFunction) {
            this.indexConfig = toIndexConfigFromIndexSettingObjectMap(Map.of(
        ...
        private void populatorTest(
                Collection<ValueIndexEntryUpdate<IndexDescriptor>> updates, Iterable<Long> expectedIds)
                throws Exception {
    ...
    @Nested
    class Euclidean extends VectorIndexEntryUpdateTestBase {
        Euclidean() {
    ...
    @Nested
    class Cosine extends VectorIndexEntryUpdateTestBase {
        Cosine() {

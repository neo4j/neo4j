...
class TestMultipleFilters extends TraversalTestBase {
    ...
    void testNarrowingFilters() {
        try (Transaction transaction = beginTx()) {
            ...
            Evaluator mustNotHaveMoreThanTwoOutRels = path ->
                    Evaluation.ofIncludes(Iterables.count(path.endNode().getRelationships(Direction.OUTGOING)) <= 2);

            TraversalDescription description =
                    transaction.traversalDescription().evaluator(mustBeConnectedToK);
    ...
    void testBroadeningFilters() {
        try (Transaction transaction = beginTx()) {
            MustBeConnectedToNodeFilter mustBeConnectedToC =
                    new MustBeConnectedToNodeFilter(getNodeWithName(transaction, "c"));
            MustBeConnectedToNodeFilter mustBeConnectedToE =
                    new MustBeConnectedToNodeFilter(getNodeWithName(transaction, "e"));
            ...
            expectNodes(transaction
                    .traversalDescription()
                    .evaluator(mustBeConnectedToC)
                    .evaluator(mustBeConnectedToE)
                    .traverse(aNode));

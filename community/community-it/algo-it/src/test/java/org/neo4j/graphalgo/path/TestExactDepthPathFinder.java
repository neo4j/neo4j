...
class TestExactDepthPathFinder extends Neo4jAlgoTestCase {
    ...
    void testSingle() {
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            Path path =
                    finder.findSinglePath(graph.getNode(transaction, "SOURCE"), graph.getNode(transaction, "TARGET"));
            assertNotNull(path);
    ...
    void testAll() {
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            PathFinder<Path> finder = newFinder(context);
            assertPaths(
                    finder.findAllPaths(graph.getNode(transaction, "SOURCE"), graph.getNode(transaction, "TARGET")),
                    "SOURCE,z,9,0,TARGET",
    ...
    void shouldHandleDirectionalGraph() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            var context = new BasicEvaluationContext(transaction, graphDb);
            assertPaths(
                    new ExactDepthPathFinder(context, forDirection(Direction.OUTGOING), 3, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,b,c,g");
            assertPaths(
                    new ExactDepthPathFinder(context, forDirection(Direction.OUTGOING), 4, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,d,e,f,g");
            assertPaths(
                    new ExactDepthPathFinder(context, forDirection(Direction.OUTGOING), 5, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,h,i,j,k,g");
            transaction.commit();
        }
    ...
    void shouldHandleNondirectedGraph() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            var context = new BasicEvaluationContext(transaction, graphDb);
            assertPaths(
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 3, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,b,c,g");
            assertPaths(
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 4, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,d,e,f,g");
            assertPaths(
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 5, Integer.MAX_VALUE, false)
                            .findAllPaths(a, g),
                    "a,h,i,j,k,g");
            transaction.commit();
        }
    ...
    void shouldHandleSimpleChainEvenDepth() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            var context = new BasicEvaluationContext(transaction, graphDb);
            assertPaths(
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 2, Integer.MAX_VALUE, false)
                            .findAllPaths(a, c),
                    "a,b,c");
            assertPaths(
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 2, Integer.MAX_VALUE, false)
    ...
    void shouldHandleNeighbouringNodes() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            ExactDepthPathFinder pathFinder =
                    new ExactDepthPathFinder(context, allTypesAndDirections(), 1, Integer.MAX_VALUE, false);
    ...
    void testExactDepthPathsReturnsNoLoops() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            var context = new BasicEvaluationContext(transaction, graphDb);
            assertPaths(
                    pathsWithLength(context, PathExpanders.forType(MyRelTypes.R1), 3)
                            .findAllPaths(a, e),
                    "a,b,c,e",
                    "a,b,c,e");
            assertPaths(
                    pathsWithLength(context, PathExpanders.forType(MyRelTypes.R1), 4)
                            .findAllPaths(a, e),
                    "a,b,d,c,e");
            assertPaths(pathsWithLength(context, PathExpanders.forType(MyRelTypes.R1), 6)
                    .findAllPaths(a, e));
            transaction.commit();
        }
    }
    ...
    void testExactDepthPathsLoopsAllowed() {
        ...
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            var context = new BasicEvaluationContext(transaction, graphDb);
            assertPaths(
                    new ExactDepthPathFinder(context, forDirection(Direction.OUTGOING), 6, Integer.MAX_VALUE, true)
                            .findAllPaths(a, e),
                    "a,b,c,d,b,c,e");
            transaction.commit();
        }

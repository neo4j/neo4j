...
public abstract class SingleSourceShortestPathTest extends Neo4jAlgoTestCase {
    ...
    void testMultipleRelTypes() {
        try (Transaction transaction = graphDb.beginTx()) {
            ...
            // one shortcut
            singleSource = getSingleSourceAlgorithm(
                    graph.getNode(transaction, "a"), Direction.BOTH, MyRelTypes.R1, MyRelTypes.R2);
            assertEquals(3, (int) singleSource.getCost(graph.getNode(transaction, "e")));
            // other shortcut
            singleSource = getSingleSourceAlgorithm(
                    graph.getNode(transaction, "a"), Direction.BOTH, MyRelTypes.R1, MyRelTypes.R3);
            assertEquals(3, (int) singleSource.getCost(graph.getNode(transaction, "e")));
            // both shortcuts
            singleSource = getSingleSourceAlgorithm(
                    graph.getNode(transaction, "a"), Direction.BOTH, MyRelTypes.R1, MyRelTypes.R2, MyRelTypes.R3);

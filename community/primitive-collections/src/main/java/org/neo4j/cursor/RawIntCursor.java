 ...
 */
public interface RawIntCursor<EXCEPTION extends Exception> extends IntSupplier, AutoCloseable
{
    /**
    ...
    default void forAll( IntConsumer consumer ) throws EXCEPTION
    {
        try
        {
            while ( next() )
    ...
    default <R extends PrimitiveIntSet> R collect( R set ) throws EXCEPTION
    {
        try
        {
            while ( next() )

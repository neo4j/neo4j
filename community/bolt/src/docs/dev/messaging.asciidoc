...
- `FAILURE` if the request was malformed or if initialization cannot be performed at this time
...
- `FAILURE` if the request was malformed or if a statement may not be executed at this time
...
- `FAILURE` if no result stream is currently available
...
- `FAILURE` if no result stream is currently available or if retrieval fails
...
- `FAILURE` if there is no failure that requires acknowledgement

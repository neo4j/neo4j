...
|authToken   |An authorization token used to authenticate to the database. The token must contain either just contain the entry { "scheme" : "none" } or the keys `scheme`, `principal`, and `credentials`. Example { "scheme": "basic", "principal": "neo4j", "credentials": "secret"}". If no `scheme` is provided, it is assumed to be "none".
|=======================

...
class KeepAliveConnectorListenerTest {

    @Test
    void shouldRegisterKeepAliveConnectionListenerOnConnectionCreated() {
        ...
        new KeepAliveConnectorListener(true, 424242, NullLogProvider.getInstance()).onConnectionCreated(connection);

        var inOrder = Mockito.inOrder(connection, memoryTracker);

        inOrder.verify(connection).memoryTracker();
        inOrder.verify(memoryTracker).allocateHeap(KeepAliveConnectionListener.SHALLOW_SIZE);
        inOrder.verify(connection).registerListener(ArgumentMatchers.any(KeepAliveConnectionListener.class));

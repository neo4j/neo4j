...
public class TransportTestUtil {
    ...
    public byte[] defaultRunAutoCommitTx(String statement, MapValue params, String database) throws IOException {
        ...
        return chunk(
                new RunMessage(statement, params, meta, List.of(), null, AccessMode.WRITE, Map.of(), database),
    ...
    public byte[] defaultRunExplicitCommitTxAndCommit(String statement) throws IOException {
        return chunk(
                BoltV4Messages.begin(),
                BoltV4Messages.run(statement),
                BoltV4Messages.pullAll(),
                BoltV4Messages.commit());
    }
    ...
    @SafeVarargs
    public final <T extends TransportConnection> Consumer<T> eventuallyReceives(
    ...
    @SafeVarargs
    public final <T extends TransportConnection> Consumer<T> eventuallyReceives(
            boolean allowNoOp, Consumer<ResponseMessage>... messagesConsumers) {
        return eventuallyReceives(allowNoOp, () -> {}, messagesConsumers);
    }

    @SafeVarargs
    public final <T extends TransportConnection> Consumer<T> eventuallyReceives(
            boolean allowNoOp, Runnable noOpCallback, Consumer<ResponseMessage>... messagesConsumers) {
        return connection -> {
            try {
    ...
    @SafeVarargs
    public final <T extends TransportConnection> Consumer<T> eventuallyReceives(
            int skip, Consumer<ResponseMessage>... messagesConsumers) {
        return connection -> {
            try {
    ...
    public static Condition<TransportConnection> eventuallyReceivesSelectedProtocolVersion() {
        return eventuallyReceives(new byte[] {
            0, 0, (byte) DEFAULT_BOLT_VERSION.getMinorVersion(), (byte) DEFAULT_BOLT_VERSION.getMajorVersion()
        });
    }
    ...
    public static Condition<TransportConnection> eventuallyReceives(final byte[] expected) {
        ...
        return new Condition<>(
                ...
                "Eventually receive. Expected: " + Arrays.toString(expected) + ", actual: "
                        + Arrays.toString((byte[]) arrayReference.getValue()));
    }

    public <T extends TransportConnection> ResponseMessage receiveOneResponseMessage(T conn)
            throws IOException, InterruptedException {
        return receiveOneResponseMessage(true, () -> {}, conn);
    }

    public <T extends TransportConnection> ResponseMessage receiveOneResponseMessage(
            boolean allowNoOp, Runnable noOpCallback, T conn) throws IOException, InterruptedException {

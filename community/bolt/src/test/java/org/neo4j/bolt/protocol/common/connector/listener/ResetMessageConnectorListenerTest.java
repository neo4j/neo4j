...
public class ResetMessageConnectorListenerTest {
    @Test
    void shouldRegisterResetMessageConnectionListenerOnConnectionCreated() {
        ...
        new ResetMessageConnectorListener(NullLogProvider.getInstance()).onConnectionCreated(connection);

        var inOrder = Mockito.inOrder(connection, memoryTracker);

        inOrder.verify(connection).memoryTracker();
        inOrder.verify(memoryTracker).allocateHeap(ResetMessageConnectionListener.SHALLOW_SIZE);
        inOrder.verify(connection).registerListener(ArgumentMatchers.any(ResetMessageConnectionListener.class));

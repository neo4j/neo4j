...
public class ReadLimitConnectorListenerTest {
    @Test
    void shouldRegisterReadLimitConnectionListenerOnConnectionCreated() {
        ...
        new ReadLimitConnectorListener(10L, NullLogProvider.getInstance()).onConnectionCreated(connection);

        var inOrder = Mockito.inOrder(connection, memoryTracker);

        inOrder.verify(connection).memoryTracker();
        inOrder.verify(memoryTracker).allocateHeap(ReadLimitConnectionListener.SHALLOW_SIZE);
        inOrder.verify(connection).registerListener(ArgumentMatchers.any(ReadLimitConnectionListener.class));

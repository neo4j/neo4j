...
public class AbstractCypherAdapterStreamTest {
    ...
    void shouldIncludeNotificationsIfPresent() throws Throwable {
        ...
        MapValue msg1 = mapValues(
                ...
                "description",
                stringValue("The hinted index does not exist, please check the schema"));
        MapValue msg2 = mapValues(
                "severity",
                stringValue("WARNING"),
                "code",
                stringValue("Neo.ClientNotification.Statement.RuntimeUnsupportedWarning"),
                "title",
                stringValue("This query is not supported by the chosen runtime."),
                "description",
                stringValue(
    ...
    protected static MapValue metadataOf(AbstractCypherAdapterStream stream) throws Throwable {
        ...
        stream.handleRecords(
                new BoltResult.DiscardingRecordConsumer() {
                    @Override
    ...
    private static void assertMapEqualsWithDelta(MapValue a, MapValue b, double delta) {
        ...
        a.foreach((key, value) -> {
            ...
            if (value instanceof MapValue) {
                ...
                assertMapEqualsWithDelta((MapValue) value, (MapValue) bValue, delta);
            } else if (value instanceof DoubleValue) {
                assertThat(((DoubleValue) value).doubleValue())
                        .as("Value mismatch")
                        .isCloseTo(((DoubleValue) bValue).doubleValue(), offset(delta));
            } else {
                assertThat(value).as("Value mismatch").isEqualTo(bValue);
            }
        });
    }
    ...
    private static ExecutionPlanDescription plan(
        ...
        return plan(
                ...
                args,
                identifiers,
                new ExecutionPlanDescription.ProfilerStatistics() {
                    @Override
    ...
    private static ExecutionPlanDescription plan(
            ...
            final Map<String, Object> args,
            final List<String> identifiers,
            final ExecutionPlanDescription.ProfilerStatistics profile,
            final ExecutionPlanDescription... children) {

...
class NativeStructWriteTest {

    private Stream<NativeStructType> getStructTypes() {
        return Stream.of(NativeStructType.values());
    }

    @TestFactory
    Stream<DynamicTest> shouldWriteHeader() {
        return getStructTypes()
                .map(type -> dynamicTest(type.name(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeHeader(buf, 15, type);

                    var header = buf.readStructHeader();

                    assertThat(header.length()).isEqualTo(15);
                    assertThat(header.tag()).isEqualTo(type.getTag());
    ...
    @TestFactory
    Stream<DynamicTest> shouldWriteHeaderWithoutExplicitSize() {
        return getStructTypes()
                .map(type -> dynamicTest(type.name(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeHeader(buf, type);

                    var header = buf.readStructHeader();

                    assertThat(header.length()).isEqualTo(type.getDefaultSize());
                    assertThat(header.tag()).isEqualTo(type.getTag());
    ...
    @TestFactory
    Stream<DynamicTest> writeHeaderShouldFailWithNullPointerWhenNullIsGiven() {
        return getStructTypes()
                .map(type -> dynamicTest(type.name(), () -> {
                    var ex = assertThrows(NullPointerException.class, () -> NativeStruct.writeHeader(null, 5, type));

                    assertThat(ex).hasMessage("buf cannot be null");
                }));
    }
    ...
    Stream<DynamicTest> shouldWritePoint() {
        return Stream.of(new double[] {21, 42}, new double[] {21, 42, 84})
                .map(coords -> dynamicTest(Arrays.toString(coords), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writePoint(buf, CoordinateReferenceSystem.CARTESIAN, coords);
                    ...
                    assertThat(header.tag())
                            .isEqualTo(
                                    coords.length == 3
                                            ? NativeStructType.POINT_3D.getTag()
                                            : NativeStructType.POINT_2D.getTag());
    ...
    void shouldWritePoint2d() throws LimitExceededException, UnexpectedTypeException {
        var buf = PackstreamBuf.allocUnpooled();
        NativeStruct.writePoint2d(buf, CoordinateReferenceSystem.WGS_84, 42.5, 85);
        ...
        assertThat(header.tag()).isEqualTo(NativeStructType.POINT_2D.getTag());
    ...
    void shouldWritePoint3d() throws LimitExceededException, UnexpectedTypeException {
        var buf = PackstreamBuf.allocUnpooled();
        NativeStruct.writePoint3d(buf, CoordinateReferenceSystem.WGS_84_3D, 42.25, 84.5, 169);
        ...
        assertThat(header.tag()).isEqualTo(NativeStructType.POINT_3D.getTag());
    ...
    void shouldWriteDuration() throws LimitExceededException, UnexpectedTypeException {
        var buf = PackstreamBuf.allocUnpooled();
        NativeStruct.writeDuration(buf, 3, 2, 1, 214284);
        ...
        assertThat(header.length()).isEqualTo(4);
        assertThat(header.tag()).isEqualTo(NativeStructType.DURATION.getTag());
    ...
    Stream<DynamicTest> shouldWriteDate() {
        return Stream.of(
                ...
                .map(date -> dynamicTest(date.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeDate(buf, date);

                    var header = buf.readStructHeader();
                    var epochDay = buf.readInt();

                    assertThat(header.length()).isEqualTo(1);
                    assertThat(header.tag()).isEqualTo(NativeStructType.DATE.getTag());
    ...
    Stream<DynamicTest> shouldWriteLocalTime() {
        return Stream.of(LocalTime.of(0, 0, 0, 0), LocalTime.of(14, 37, 15, 4284), LocalTime.of(23, 59, 55, 998298))
                .map(time -> dynamicTest(time.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeLocalTime(buf, time);

                    var header = buf.readStructHeader();
                    var nanoOfDay = buf.readInt();

                    assertThat(header.length()).isEqualTo(1);
                    assertThat(header.tag()).isEqualTo(NativeStructType.LOCAL_TIME.getTag());
    ...
    Stream<DynamicTest> shouldWriteTime() {
        return Stream.of(
                ...
                .map(time -> dynamicTest(time.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeTime(buf, time);
                    ...
                    assertThat(header.tag()).isEqualTo(NativeStructType.TIME.getTag());
    ...
    Stream<DynamicTest> shouldWriteLocalDateTime() {
        return Stream.of(
                ...
                .map(dateTime -> dynamicTest(dateTime.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeLocalDateTime(buf, dateTime);
                    ...
                    assertThat(header.length()).isEqualTo(2);
                    assertThat(header.tag()).isEqualTo(NativeStructType.LOCAL_DATE_TIME.getTag());
    ...
    Stream<DynamicTest> shouldWriteDateTime() {
        return Stream.of(
                ...
                .map(dateTime -> dynamicTest(dateTime.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeDateTime(buf, dateTime);
                    ...
                    assertThat(header.length()).isEqualTo(3);
                    assertThat(header.tag()).isEqualTo(NativeStructType.DATE_TIME.getTag());
    ...
    Stream<DynamicTest> shouldWriteDateTimeZoneId() {
        return Stream.of(
                ...
                .map(dateTime -> dynamicTest(dateTime.toString(), () -> {
                    var buf = PackstreamBuf.allocUnpooled();
                    NativeStruct.writeDateTimeZoneId(buf, dateTime);
                    ...
                    assertThat(header.length()).isEqualTo(3);
                    assertThat(header.tag()).isEqualTo(NativeStructType.DATE_TIME_ZONE_ID.getTag());

                    assertThat(epochSecond).isEqualTo(dateTime.toLocalDateTime().toEpochSecond(ZoneOffset.UTC));
                    assertThat(nanoOfDay).isEqualTo(dateTime.getNano());

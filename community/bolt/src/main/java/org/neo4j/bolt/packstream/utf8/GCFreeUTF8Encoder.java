...
public class GCFreeUTF8Encoder implements UTF8Encoder {
    ...
    public ByteBuffer encode(String input) {
        try {
            ...
            throw new AssertionError(
                    "This encoder depends on java.lang.StringCoding, which failed to load or apply: " + e.getMessage(),
                    e);
        }
    }
            ...
            if (c < 0x80) {
                ...
                if (uc < 0) {
                    ...
                    throw new PackStreamException(String.format(
                            "Failure when converting to UTF-8. " + "String: %s\n%s",
                            new String(val, StandardCharsets.UTF_16), HexPrinter.hex(val)));
                } else {
    ...
    private static MethodHandle hasNegatives() {
        ...
        try {
            final Method hasNegatives = Class.forName("java.lang.StringCoding")
                    .getMethod("hasNegatives", byte[].class, int.class, int.class);
            hasNegatives.setAccessible(true);
            return lookup.unreflect(hasNegatives);
        } catch (Throwable e) {
            throw new AssertionError(
                    "This encoder depends on java.lang.StringCoding, which failed to load: " + e.getMessage(), e);
        }
    }
    ...
    private static MethodHandle getChar() {
        MethodHandles.Lookup lookup = MethodHandles.lookup();
        try {
            final Method getChar =
                    Class.forName("java.lang.StringUTF16").getDeclaredMethod("getChar", byte[].class, int.class);
            getChar.setAccessible(true);
            return lookup.unreflect(getChar);
        } catch (Throwable e) {
            throw new AssertionError(
                    "This encoder depends on java.lang.StringUTF16, which failed to load: " + e.getMessage(), e);
        }
    }

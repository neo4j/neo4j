...
public class DefaultBoltConnection implements BoltConnection {
    ...
    public void enqueue(Job job) {
        ...
        enqueueInternal(machine -> {
            channel().write(StateSignal.BEGIN_JOB_PROCESSING);
            ...
            try {
                ...
                channel().write(StateSignal.END_JOB_PROCESSING);
                channel.protector().afterRequestReceived();
            }
        });
    }
            ...
            if (hasCause(t, RejectedExecutionException.class)) {
                ...
                message = String.format(
                        "Unable to schedule bolt session '%s' for execution since there are no available threads to "
                                + "serve it at the moment. You can retry at a later time or consider increasing max thread pool "
                                + "size for bolt connector(s).",
                        id());
            } else {
                error = Error.fatalFrom(t);

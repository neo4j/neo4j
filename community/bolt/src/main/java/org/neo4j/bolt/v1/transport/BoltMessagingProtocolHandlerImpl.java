...
public class BoltMessagingProtocolHandlerImpl implements BoltMessagingProtocolHandler
{
    ...
    public BoltMessagingProtocolHandlerImpl( BoltChannel boltChannel, BoltConnection connection, Neo4jPackV1 neo4jPack,
            TransportThrottleGroup throttleGroup, LogService logging )
    {
        this.internalLog = logging.getInternalLog( getClass() );
        this.chunkedOutput = new ChunkedOutput( boltChannel.rawChannel(), DEFAULT_OUTPUT_BUFFER_SIZE, internalLog );
        this.packer = new BoltResponseMessageWriter(
                new Neo4jPackV1.Packer( chunkedOutput ), chunkedOutput );

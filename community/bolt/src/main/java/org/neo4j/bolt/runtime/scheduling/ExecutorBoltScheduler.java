...
public class ExecutorBoltScheduler extends LifecycleAdapter implements BoltScheduler {
    ...
    public ExecutorBoltScheduler(
            ...
            BoltConnector.KeepAliveRequestType keepAliveRequestType,
            Duration keepAliveSchedulingInterval) {
    ...
    @Override
    public void init() {
        threadPool = executorFactory.create(
                corePoolSize,
                maxPoolSize,
                keepAlive,
                queueSize,
    ...
    private void handleCompletion(BoltConnection connection, Boolean shouldContinueScheduling, Throwable error) {
        ...
        if (error != null) {
            log.error(
                    String.format("Unexpected error during job scheduling for session '%s'.", connection.id()), error);
    ...
    private void stopConnection(BoltConnection connection) {
        try {
            ...
            log.warn(
                    String.format("An unexpected error occurred while stopping BoltConnection [%s]", connection.id()),
                    t);
        }
    }

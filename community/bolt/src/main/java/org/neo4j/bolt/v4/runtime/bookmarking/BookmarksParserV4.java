...
public final class BookmarksParserV4 implements BookmarksParser {
    ...
    private List<Bookmark> parseBookmarks(ListValue bookmarks) throws BookmarkParsingException {
        ...
        try {
            ...
            throw BookmarkParsingException.newInvalidBookmarkError(
    ...
    private ParsedBookmark parse(String bookmarkString) throws BookmarkParsingException {
        ...
        var databaseId = databaseIdRepository
                ...
                .orElseThrow(() -> newInvalidBookmarkForUnknownDatabaseError(databaseUuid));
    ...
    private static UUID parseDatabaseId(String uuid, String bookmark) throws BookmarkParsingException {
        try {
            ...
            throw BookmarkParsingException.newInvalidSingleBookmarkError(
                    bookmark, String.format("Unable to parse database id: %s", uuid), e);
        }
    }
    ...
    private static long parseTxId(String txIdString, String bookmark) throws BookmarkParsingException {
        try {
            ...
            throw BookmarkParsingException.newInvalidSingleBookmarkError(
                    bookmark, String.format("Unable to parse transaction id: %s", txIdString), e);
        }
    }

    private static void assertSameDatabaseId(NamedDatabaseId id1, NamedDatabaseId id2, ListValue bookmarks)
            throws BookmarkParsingException {
        if (!id1.equals(id2)) {
            throw newInvalidBookmarkMixtureError(bookmarks);
        }
    }

    private static List<Bookmark> buildBookmarks(
            NamedDatabaseId systemDbId, long maxSystemDbTxId, NamedDatabaseId userDbId, long maxUserDbTxId) {
        if (maxSystemDbTxId != ABSENT_BOOKMARK_ID && maxUserDbTxId != ABSENT_BOOKMARK_ID) {
            return List.of(
                    new BookmarkWithDatabaseId(maxSystemDbTxId, systemDbId),

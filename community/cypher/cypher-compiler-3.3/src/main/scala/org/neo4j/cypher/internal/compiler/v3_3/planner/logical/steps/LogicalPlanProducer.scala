...
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.plans.{
  DeleteExpression => DeleteExpressionPlan,
  Limit => LimitPlan,
  LoadCSV => LoadCSVPlan,
  Skip => SkipPlan,
  _
...
case class LogicalPlanProducer(cardinalityModel: CardinalityModel) extends ListSupport {
  ...
  def planAggregation(
      ...
      reportedAggregation: Map[String, Expression])(implicit context: LogicalPlanningContext): LogicalPlan = {
    val solved = left.solved.updateTailOrSelf(
      _.withHorizon(
  ...
  def planDirectedRelationshipByIdSeek(
      idName: IdName,
      relIds: SeekableArgs,
      startNode: IdName,
      endNode: IdName,
      ...
      solvedPredicates: Seq[Expression] = Seq.empty)(implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planUndirectedRelationshipByIdSeek(
      idName: IdName,
      relIds: SeekableArgs,
      leftNode: IdName,
      rightNode: IdName,
  ...
  def planVarExpand(left: LogicalPlan,
    ...
    case l: VarPatternLength =>
      ...
      val solved = left.solved.amendQueryGraph(
        _.addPatternRelationship(pattern)
  ...
  def planNodeByIdSeek(idName: IdName,
  ...
  def planNodeByLabelScan(idName: IdName,
                          label: LabelName,
  ...
  def planRegularProjection(
      ...
      reported: Map[String, Expression])(implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planCountStoreNodeAggregation(query: PlannerQuery,
                                    projectedColumn: IdName,
  ...
  def planLoadCSV(inner: LogicalPlan,
                  variableName: IdName,
                  url: Expression,
                  format: CSVFormat,
                  fieldTerminator: Option[StringLiteral])(implicit context: LogicalPlanningContext): LogicalPlan = {
    val solved =
      inner.solved.updateTailOrSelf(_.withHorizon(LoadCSVProjection(variableName, url, format, fieldTerminator)))
  ...
  def planLimit(inner: LogicalPlan, count: Expression, ties: Ties = DoNotIncludeTies)(
      implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planShortestPath(inner: LogicalPlan,
                       ...
                       disallowSameNode: Boolean = true)(implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planEndpointProjection(inner: LogicalPlan,
                             start: IdName,
  ...
  def planTriadicSelection(positivePredicate: Boolean,
                           ...
                           targetId: IdName,
                           right: LogicalPlan,
                           predicate: Expression)(implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planCreateRelationship(inner: LogicalPlan, pattern: CreateRelationshipPattern)(
    ...
    CreateRelationship(inner, pattern.relName, pattern.startNode, pattern.relType, pattern.endNode, pattern.properties)(
      solved)
  }
  ...
  def planMergeCreateRelationship(inner: LogicalPlan, pattern: CreateRelationshipPattern)(
    ...
    MergeCreateRelationship(inner,
                            pattern.relName,
                            pattern.startNode,
                            pattern.relType,
                            pattern.endNode,
                            pattern.properties)(solved)
  }
  ...
  def planSetNodePropertiesFromMap(inner: LogicalPlan, pattern: SetNodePropertiesFromMapPattern)(
      implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  def planSetRelationshipPropertiesFromMap(inner: LogicalPlan, pattern: SetRelationshipPropertiesFromMapPattern)(
      implicit context: LogicalPlanningContext): LogicalPlan = {
  ...
  private def projectedDirection(pattern: PatternRelationship,
                                 from: IdName,
                                 dir: SemanticDirection): SemanticDirection = {

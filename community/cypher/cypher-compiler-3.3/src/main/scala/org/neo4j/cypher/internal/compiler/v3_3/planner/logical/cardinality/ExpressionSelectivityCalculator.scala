...
case class ExpressionSelectivityCalculator(stats: GraphStatistics, combiner: SelectivityCombiner)
    extends Expression2Selectivity {
  ...
  def apply(exp: Expression)(implicit semanticTable: SemanticTable, selections: Selections): Selectivity = exp match {
    ...
    case AsIdSeekable(seekable) =>
      (seekable.args.sizeHint.map(Cardinality(_)).getOrElse(DEFAULT_NUMBER_OF_ID_LOOKUPS) / stats
        .nodesWithLabelCardinality(None)) getOrElse Selectivity.ONE
  ...
  private def calculateSelectivityForPropertyEquality(
      ...
      propertyKey: PropertyKeyName)(implicit semanticTable: SemanticTable): Selectivity = {
  ...
  private def calculateSelectivityForSubstringSargable(
      ...
      prefix: Option[String])(implicit semanticTable: SemanticTable): Selectivity = {
    /*
     ...
     */
    val equality =
      math.BigDecimal.valueOf(calculateSelectivityForPropertyEquality(variable, None, selections, propertyKey).factor)
    val prefixLength = math.BigDecimal.valueOf(prefix match {
      case Some(n) => n.length + 1
      case None    => DEFAULT_PREFIX_LENGTH
    })
    val factor = math.BigDecimal.ONE
  ...
  private def calculateSelectivityForValueRangeSeekable(seekable: InequalityRangeSeekable, selections: Selections)(
    ...
    val selectivity = base.add(
      BigDecimalCombiner
        .andTogetherBigDecimals(

...
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.idp.expandSolverStep.planSinglePatternSide
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.idp.expandSolverStep.planSingleProjectEndpoints
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.plans._
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.steps.solveOptionalMatches.OptionalSolver
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.steps.applyOptional
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.steps.leafPlanOptions
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.steps.outerHashJoin
...
case class SingleComponentPlanner(monitor: IDPQueryGraphSolverMonitor,
    ...
    extends SingleComponentPlannerTrait {
  def planComponent(qg: QueryGraph)(implicit context: LogicalPlanningContext, kit: QueryPlannerKit): LogicalPlan = {
    ...
    val bestPlan =
      if (qg.patternRelationships.nonEmpty) {
        ...
        val (_, result) = solutions.toSingleOption.getOrElse(
          throw new AssertionError("Expected a single plan to be left in the plan table"))
  ...
  private def initTable(qg: QueryGraph, kit: QueryPlannerKit, leaves: Set[LogicalPlan])(
    ...
    for (pattern <- qg.patternRelationships)
      yield {
        ...
        val bestAccessor = kit
          .pickBest(accessPlans)
          .getOrElse(
...
object SingleComponentPlanner {
  ...
  def planSinglePattern(qg: QueryGraph, pattern: PatternRelationship, leaves: Set[LogicalPlan])(
    ...
    leaves.flatMap {
      ...
      case plan
          if plan.solved.lastQueryGraph.patternNodes.isEmpty && plan.solved.lastQueryGraph.hints.exists(
            _.isInstanceOf[RelationshipStartItem]) =>
        Set(
          context.logicalPlanProducer.planEndpointProjection(plan,
                                                             pattern.nodes._1,
                                                             startInScope = false,
                                                             pattern.nodes._2,
                                                             endInScope = false,
  ...
  def planSinglePatternCartesian(
      ...
      maybeEndPlan: Option[LogicalPlan])(implicit context: LogicalPlanningContext): Option[LogicalPlan] =
  ...
  def planSinglePatternJoins(
      ...
      startJoinNodes: Set[IdName],
      endJoinNodes: Set[IdName],
      maybeStartPlan: Option[LogicalPlan],
      maybeEndPlan: Option[LogicalPlan])(implicit context: LogicalPlanningContext): Iterable[LogicalPlan] =
    (maybeStartPlan, maybeEndPlan) match {
      case (Some(startPlan), Some(endPlan)) if qg.hints.nonEmpty && qg.size == 1 =>
        ...
        val join1b = leftExpand.map(expand =>
          context.logicalPlanProducer.planNodeHashJoin(endJoinNodes, endPlan, expand, endJoinHints))
        val join2a = rightExpand.map(expand =>
          context.logicalPlanProducer.planNodeHashJoin(startJoinNodes, startPlan, expand, startJoinHints))
        val join2b = rightExpand.map(expand =>
          context.logicalPlanProducer.planNodeHashJoin(startJoinNodes, expand, startPlan, startJoinHints))

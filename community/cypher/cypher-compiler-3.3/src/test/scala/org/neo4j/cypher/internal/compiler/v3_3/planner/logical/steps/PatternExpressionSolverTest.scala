...
class PatternExpressionSolverTest extends CypherFunSuite with LogicalPlanningTestSupport {
  test("Rewrites single pattern expression") {
    ...
    resultPlan should equal(
      RollUpApply(source, expectedInnerPlan, IdName("x"), IdName("  FRESHID0"), Set(IdName("a")))(solved))
  ...
  test("Rewrites pattern expression inside complex expression") {
    ...
    val expectedInnerPlan1 = Projection(b1, Map("  FRESHID0" -> PathExpression(pathStep1)(pos)))(solved)
    val rollUp1 =
      RollUpApply(source, expectedInnerPlan1, IdName("  FRESHID1"), IdName("  FRESHID0"), Set(IdName("a")))(solved)

    val expectedInnerPlan2 = Projection(b2, Map("  FRESHID3" -> PathExpression(pathStep2)(pos)))(solved)
    val rollUp2 =
      RollUpApply(rollUp1, expectedInnerPlan2, IdName("  FRESHID4"), IdName("  FRESHID3"), Set(IdName("a")))(solved)
  ...
  private def newPatExpr(left: String,
                         position: Int,
                         rightOffset: Int,
                         relOffset: Int,
                         dir: SemanticDirection): PatternExpression =
    newPatExpr(left, position, Left(rightOffset), Left(relOffset), dir)
  private def newPatExpr(left: String,
                         position: Int,
                         rightE: Either[Int, String],
                         relNameE: Either[Int, String],
                         dir: SemanticDirection): PatternExpression = {
    ...
    PatternExpression(
      RelationshipsPattern(RelationshipChain(
        ...
        NodePattern(right, Seq.empty, None)(rightPos)
      ) _)(DummyPosition(position)))
  }

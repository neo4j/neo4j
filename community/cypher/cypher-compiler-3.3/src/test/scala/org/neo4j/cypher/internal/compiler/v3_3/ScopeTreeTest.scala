...
class ScopeTreeTest extends CypherFunSuite {
  ...
  test(
    ...
    scopeTree should equal(
      scope()(
        scope(nodeSymbol("a", 7, 15))(),
        scope(nodeSymbol("a", 7, 15, 20), nodeSymbol("b", 38, 46))(),
  ...
  //////01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789
  test(
    "with 1 as p, count(*) as rng return p order by rng ==> { {} { with 1 as p, count(*) as rng return p } { order by rng } }") {

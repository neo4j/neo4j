...
class ProjectNamedPathsTest extends CypherFunSuite with AstRewritingTestSupport {
  ...
  private def parseReturnedExpr(queryText: String) =
    projectionInlinedAst(queryText) match {
      case Query(_,
                 SingleQuery(
                   Seq(_, Return(_, ReturnItems(_, Seq(AliasedReturnItem(expr, Variable("p")))), _, _, _, _)))) =>
        expr
    }
  ...
  test("MATCH p = (a)-[r]->(b) RETURN p") {
    ...
    val expected = PathExpression(
      NodePathStep(Variable("a") _,
                   SingleRelationshipPathStep(Variable("r") _, SemanticDirection.OUTGOING, NilPathStep))
    ) _
  ...
  test("MATCH p = (b)<-[r]->(a) RETURN p") {
    ...
    val expected = PathExpression(
      NodePathStep(Variable("b") _,
                   SingleRelationshipPathStep(Variable("r") _, SemanticDirection.INCOMING, NilPathStep))
    ) _
        ...
        Pattern(
          List(
            EveryPath(RelationshipChain(
              ...
              RelationshipPattern(Some(rId), List(), None, None, SemanticDirection.OUTGOING)(pos),
              NodePattern(Some(bId), List(), None)(pos)
            )(pos))
          ))(pos),
        List(),
        None
      ...
      With(
        ...
        ReturnItems(
          ...
          Seq(
            AliasedReturnItem(
              PathExpression(
                NodePathStep(aId, SingleRelationshipPathStep(rId, SemanticDirection.OUTGOING, NilPathStep)))(pos),
              fresh30)(pos),
  ...
  test("MATCH p = (a)-[r]->(b) WHERE length(p) > 10 RETURN 1") {
    ...
    val WHERE =
      Where(
        GreaterThan(
          FunctionInvocation(
            FunctionName("length")(pos),
            PathExpression(NodePathStep(aId, SingleRelationshipPathStep(rId, SemanticDirection.OUTGOING, NilPathStep)))(
              pos))(pos),

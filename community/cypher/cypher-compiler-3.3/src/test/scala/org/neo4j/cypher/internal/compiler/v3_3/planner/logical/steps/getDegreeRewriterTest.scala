...
class getDegreeRewriterTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("Rewrite exists( (a)-[:FOO]->() ) to GetDegree( (a)-[:FOO]->() ) > 0") {
    val incoming = Exists.asInvocation(
      PatternExpression(RelationshipsPattern(RelationshipChain(
        NodePattern(Some(varFor("a")), Seq.empty, None)(pos),
    ...
    val expected = GreaterThan(GetDegree(varFor("a"), Some(RelTypeName("FOO")(pos)), SemanticDirection.OUTGOING)(pos),
                               SignedDecimalIntegerLiteral("0")(pos))(pos)
  ...
  test("Rewrite exists( ()-[:FOO]->(a) ) to GetDegree( (a)<-[:FOO]-() ) > 0") {
    ...
    val expected = GreaterThan(GetDegree(varFor("a"), Some(RelTypeName("FOO")(pos)), SemanticDirection.INCOMING)(pos),
                               SignedDecimalIntegerLiteral("0")(pos))(pos)

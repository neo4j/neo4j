...
import org.neo4j.cypher.internal.frontend.v3_3.ast.rewriters.inlineNamedPathsInPatternComprehensions
...
class inlineNamedPathsInPatternComprehensionsTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("replaces named path in projection") {
    ...
    val input: PatternComprehension =
      PatternComprehension(Some(varFor("p")), RelationshipsPattern(element) _, None, Variable("p") _) _
    val output = input.copy(namedPath = None,
  ...
  test("replaces named path in predicate and projection") {
    ...
    val input: PatternComprehension =
      PatternComprehension(Some(varFor("p")), RelationshipsPattern(element) _, Some(varFor("p")), Variable("p") _) _
    val output = input.copy(
      namedPath = None,
      predicate = Some(PathExpression(projectNamedPaths.patternPartPathExpression(element)) _),

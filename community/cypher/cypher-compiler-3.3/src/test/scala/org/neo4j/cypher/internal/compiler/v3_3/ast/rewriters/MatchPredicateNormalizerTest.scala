...
import org.neo4j.cypher.internal.frontend.v3_3.ast.rewriters.MatchPredicateNormalization
...
class MatchPredicateNormalizerTest extends CypherFunSuite with RewriteTest {
  ...
  test("move single predicate from node to WHERE") {
    assertRewrite("MATCH (n {foo: 'bar'}) RETURN n", "MATCH (n) WHERE n.foo = 'bar' RETURN n")
  }

  test("move single predicates from rel to WHERE") {
    assertRewrite("MATCH (n)-[r:Foo {foo: 1}]->() RETURN n", "MATCH (n)-[r:Foo]->() WHERE r.foo = 1 RETURN n")
  }

  test("move multiple predicates from nodes to WHERE") {
    assertRewrite("MATCH (n {foo: 'bar', bar: 4}) RETURN n", "MATCH (n) WHERE n.foo = 'bar' AND n.bar = 4 RETURN n")
  }

  test("move multiple predicates from rels to WHERE") {
    assertRewrite("MATCH (n)-[r:Foo {foo: 1, bar: 'baz'}]->() RETURN n",
  ...
  test("move single label from nodes to WHERE") {
    assertRewrite("MATCH (n:LABEL) RETURN n", "MATCH (n) WHERE n:LABEL RETURN n")
  }

  test("move multiple labels from nodes to WHERE") {
    assertRewrite("MATCH (n:L1:L2) RETURN n", "MATCH (n) WHERE n:L1:L2 RETURN n")
  }

  test("move single label from start node to WHERE when pattern contains relationship") {
    assertRewrite("MATCH (n:Foo)-[r]->(b) RETURN n", "MATCH (n)-[r]->(b) WHERE n:Foo RETURN n")
  }

  test("move single label from end node to WHERE when pattern contains relationship") {
    assertRewrite("MATCH (n)-[r]->(b:Foo) RETURN n", "MATCH (n)-[r]->(b) WHERE b:Foo RETURN n")
  }
  ...
  test("move single property from var length relationship to the where clause") {
    assertRewrite("MATCH (n)-[r* {prop: 42}]->(b) RETURN n",
  ...
  test("rewrite outgoing pattern to getDegree call") {
    rewrite(parseForRewriting("MATCH (a) WHERE (a)-[:R]->() RETURN a.prop")) should matchPattern {
      case Query(_,
                 SingleQuery(
                   Seq(Match(_, _, _, Some(Where(GreaterThan(GetDegree(_, _, _), _)))), Return(_, _, _, _, _, _)))) =>
    }
  }

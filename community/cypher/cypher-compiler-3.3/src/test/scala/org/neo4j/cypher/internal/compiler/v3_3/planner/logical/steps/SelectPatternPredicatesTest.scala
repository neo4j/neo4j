...
class SelectPatternPredicatesTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  // MATCH (a) WHERE (a)-->()
  val patternExp = PatternExpression(
  ...
  when(factory.newCardinalityEstimator(any(), any()))
    .thenReturn((plan: PlannerQuery, _: QueryGraphSolverInput, _: SemanticTable) =>
      plan match {
  ...
  test("should introduce semi apply for unsolved exclusive pattern predicate") {
    ...
    val inner = Expand(Argument(Set(IdName("a")))(solved)(),
                       IdName("a"),
                       dir,
                       types,
                       IdName(nodeName),
  ...
  test("should introduce let semi apply and select or semi apply for multiple pattern predicates in or") {
    // Given
    val patternExp2 = PatternExpression(
    ...
    val inner = Expand(Argument(Set(IdName("a")))(solved)(),
                       ...
                       patternRel.name,
                       ExpandAll)(solved)
    val inner2 = Expand(Argument(Set(IdName("a")))(solved)(),
                        IdName("a"),
                        dir,
                        types,
                        IdName("  UNNAMED4"),
    ...
    // Then
    result should equal(
      Seq(SelectOrSemiApply(LetSemiApply(aPlan, inner, IdName("  FRESHID0"))(solved), inner2, varFor("  FRESHID0"))(
        solved)))
  }

  test("should introduce let semi apply and select or anti semi apply for multiple pattern predicates in or") {
    // Given
    val patternExp2: PatternExpression = PatternExpression(
      RelationshipsPattern(RelationshipChain(
        NodePattern(Some(varFor("a")), Seq(), None) _,
        RelationshipPattern(Some(varFor("  UNNAMED3")), types, None, None, sdir) _,
        NodePattern(Some(varFor("  UNNAMED4")), Seq(), None) _
    ...
    // Then
    result should equal(
      Seq(SelectOrAntiSemiApply(LetSemiApply(aPlan, inner, IdName("  FRESHID0"))(solved), inner2, varFor("  FRESHID0"))(
        solved)))
  }
  ...
  test("should introduce let anti semi apply and select or semi apply for multiple pattern predicates in or") {
    ...
    // Then
    result should equal(
      Seq(SelectOrSemiApply(LetAntiSemiApply(aPlan, inner, IdName("  FRESHID0"))(solved), inner2, varFor("  FRESHID0"))(
        solved)))
  }
  ...
  test("should introduce let select or semi apply and select or anti semi apply for multiple pattern predicates in or") {
    ...
    result should equal(
      Seq(
        SelectOrAntiSemiApply(LetSelectOrSemiApply(aPlan, inner, IdName("  FRESHID0"), equals)(solved),
                              inner2,
                              varFor("  FRESHID0"))(solved)))
  }
  ...
  test("should introduce let anti select or semi apply and select or semi apply for multiple pattern predicates in or") {
    ...
    result should equal(
      Seq(
        SelectOrSemiApply(LetSelectOrAntiSemiApply(aPlan, inner, IdName("  FRESHID0"), equals)(solved),
                          inner2,
                          varFor("  FRESHID0"))(solved)))
  }
}

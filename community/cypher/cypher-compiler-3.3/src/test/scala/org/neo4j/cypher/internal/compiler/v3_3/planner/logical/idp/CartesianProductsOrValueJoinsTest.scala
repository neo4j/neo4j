...
class CartesianProductsOrValueJoinsTest
  ...
  private def testThisWithAssertion(graph: QueryGraph, input: Set[PlannedComponent], assertion: LogicalPlan => Unit): Unit = {
    new given {
      qg = graph
  ...
  private def testThis(graph: QueryGraph, input: Set[PlannedComponent], expectedPlan: LogicalPlan): Unit =
    testThisWithAssertion(graph, input, (result: LogicalPlan) => result should equal(expectedPlan))
}

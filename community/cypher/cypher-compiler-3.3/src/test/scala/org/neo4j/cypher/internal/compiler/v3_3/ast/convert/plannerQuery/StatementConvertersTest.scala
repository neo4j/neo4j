...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("MATCH (n) WHERE n:A:B RETURN n") {
    ...
    query.queryGraph.selections should equal(
      Selections(Set(Predicate(Set(IdName("n")), HasLabels(nIdent, Seq(A)) _),
  ...
  test("match (a)-[r]->(b)-[r2]->(c) return a,r,b, c") {
    ...
    val predicate: Predicate =
      Predicate(Set(IdName("r"), IdName("r2")), Not(Equals(Variable("r") _, Variable("r2") _) _) _)
  ...
  test("match (a)-[r1:CONTAINS*0..1]->(b)-[r2:FRIEND*0..1]->(c) return a,b,c") {
    ...
    query.queryGraph.patternRelationships should equal(
      Set(
        PatternRelationship(IdName("r1"),
                            (IdName("a"), IdName("b")),
                            OUTGOING,
                            Seq(relType("CONTAINS")),
                            VarPatternLength(0, Some(1))),
        PatternRelationship(IdName("r2"),
                            (IdName("b"), IdName("c")),
                            OUTGOING,
                            Seq(relType("FRIEND")),
                            VarPatternLength(0, Some(1)))
  ...
  test("optional match (a)-[r]->(b) return a,b,r") {
    ...
    val optMatchQG = query.queryGraph.optionalMatches.head
    optMatchQG.patternRelationships should equal(
      Set(
        PatternRelationship(IdName("r"), (IdName("a"), IdName("b")), OUTGOING, Seq.empty, SimplePatternLength)
      ))
  ...
  test("match (a) where (a)-->() return a") {
    ...
    val exp = GreaterThan(GetDegree(varFor("a"), None, OUTGOING) _, SignedDecimalIntegerLiteral("0") _) _
  ...
  test("MATCH (a) WITH 1 as b RETURN b") {
    ...
    query.tail should equal(
      Some(RegularPlannerQuery(QueryGraph(Set.empty, Set.empty, Set(IdName("b"))),
                               RegularQueryProjection(Map("b" -> Variable("b") _)))))
  }
  ...
  test("match (n) return n limit 10") {
    ...
    query.horizon should equal(
      RegularQueryProjection(
        ...
        QueryShuffle(sortItems = Seq.empty, skip = None, limit = Some(SignedDecimalIntegerLiteral("10")(pos)))))
  }
  ...
  test("match (n) return n skip 10") {
    ...
    query.horizon should equal(
      RegularQueryProjection(
        projections = Map("n" -> varFor("n")),
        QueryShuffle(sortItems = Seq.empty, skip = Some(SignedDecimalIntegerLiteral("10")(pos)), limit = None)))
  }
  ...
  test("MATCH (a) WITH a LIMIT 1 MATCH (a)-[r]->(b) RETURN a, b") {
    ...
    query.horizon should equal(
      RegularQueryProjection(
        ...
        QueryShuffle(sortItems = Seq.empty, skip = None, limit = Some(SignedDecimalIntegerLiteral("1")(pos)))))
  ...
  test("MATCH (a:Start) WITH a.prop AS property LIMIT 1 MATCH (b) WHERE id(b) = property RETURN b") {
    val query =
      buildPlannerQuery("MATCH (a:Start) WITH a.prop AS property LIMIT 1 MATCH (b) WHERE id(b) = property RETURN b")
    query.tail should not be empty
    query.queryGraph.selections.predicates should equal(
      Set(
    ...
    tailQg.horizon should equal(RegularQueryProjection(projections = Map("b" -> varFor("b")), QueryShuffle()))
  }
  ...
  test("MATCH (a:Start) WITH a.prop AS property MATCH (b) WHERE id(b) = property RETURN b") {
    ...
    query.horizon should equal(
      RegularQueryProjection(Map("property" -> Property(Variable("a") _, PropertyKeyName("prop") _) _)))

    val secondQuery = query.tail.get
    secondQuery.queryGraph.selections.predicates should equal(
      Set(Predicate(
        Set(IdName("b"), IdName("property")),
        In(FunctionInvocation(FunctionName("id") _, Variable("b") _) _, ListLiteral(Seq(Variable("property")(pos))) _) _
      )))

    secondQuery.horizon should equal(RegularQueryProjection(Map("b" -> Variable("b") _)))
  }
  ...
  test("MATCH (a:Start) WITH a.prop AS property, count(*) AS count MATCH (b) WHERE id(b) = property RETURN b") {
    ...
    tailQg.queryGraph.patternNodes should equal(Set(IdName("b")))
    tailQg.queryGraph.patternRelationships should be(empty)
    tailQg.queryGraph.selections.predicates should equal(
      Set(
        Predicate(
          Set(IdName("b"), IdName("property")),
  ...
  test("MATCH (n:Awesome {prop: 42}) USING INDEX n:Awesome(prop) RETURN n") {
    ...
    query.queryGraph.hints should equal(
      Set[Hint](UsingIndexHint(varFor("n"), LabelName("Awesome") _, Seq(PropertyKeyName("prop")(pos))) _))
  }
  ...
  test("MATCH shortestPath((a)-[r]->(b)) RETURN r") {
    ...
    query.queryGraph.patternNodes should equal(Set(IdName("a"), IdName("b")))
    query.queryGraph.shortestPathPatterns should equal(
      Set(
        ShortestPathPattern(
          Some("  FRESHID6"),
          PatternRelationship(IdName("r"), (IdName("a"), IdName("b")), OUTGOING, Seq.empty, SimplePatternLength),
  ...
  test("Funny query from boostingRecommendations") {
    val query = buildPlannerQuery("""MATCH (origin)-[r1:KNOWS|WORKS_AT]-(c)-[r2:KNOWS|WORKS_AT]-(candidate)
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    val query = buildPlannerQuery("""MATCH (owner)
  ...
  test("UNWIND [1,2,3] AS x MATCH (n) WHERE n.prop = x RETURN n") {
    ...
    val set: Set[Predicate] =
      Set(Predicate(Set(IdName("n"), IdName("x")), In(nProp, ListLiteral(Seq(varFor("x"))) _) _))
  ...
  test("MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a2)<-[r]-(b2) WHERE a1 = a2 RETURN a1, r, b2, a2") {
    val query = buildPlannerQuery(
      "MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a2)<-[r]-(b2) WHERE a1 = a2 RETURN a1, r, b2, a2")

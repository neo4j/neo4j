...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("match (n) return n.prop order by n.prop2 DESC") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(n),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(n -> Variable(n),   FRESHID19 -> Property(Variable(n),PropertyKeyName(prop))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(n,   FRESHID19),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(  FRESHID19 -> Variable(  FRESHID19),   FRESHID35 -> Property(Variable(n),PropertyKeyName(prop2))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID19,   FRESHID35),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(  FRESHID19 -> Variable(  FRESHID19),   FRESHID35 -> Variable(  FRESHID35)),QueryShuffle(Vector(DescSortItem(Variable(  FRESHID35))),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID19,   FRESHID35),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(n.prop -> Variable(  FRESHID19)),QueryShuffle(List(),None,None)),None)))))))"""
  ...
  test("MATCH (a) WITH a WHERE TRUE RETURN a") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(a),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(a -> Variable(a),   FRESHID23 -> True()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(a,   FRESHID23),Selections(Set(Predicate(Set(  FRESHID23),Variable(  FRESHID23)))),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(a -> Variable(a)),QueryShuffle(List(),None,None)),None)))"""
    result should equal(expectation)
  }
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS collected WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(owner),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(collected -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collected),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collected -> Variable(collected),   FRESHID54 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED62)),List(),None,None,BOTH,false),NodePattern(Some(Variable(  UNNAMED64)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collected,   FRESHID54),Selections(Set(Predicate(Set(  FRESHID54),Variable(  FRESHID54)))),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))"""
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(owner),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(xyz -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, xyz),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> GreaterThan(Variable(xyz),SignedDecimalIntegerLiteral(0))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collection),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> Variable(collection),   FRESHID82 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED90)),List(),None,None,BOTH,false),NodePattern(Some(Variable(  UNNAMED92)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collection,   FRESHID82),Selections(Set(Predicate(Set(  FRESHID82),Variable(  FRESHID82)))),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))))""".stripMargin
  ...
  test("MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a1)<-[r]-(b2) RETURN a1, r, b2") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(r,(a1,b1),OUTGOING,List(),SimplePatternLength)),Set(a1, b1),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(r -> Variable(r), a1 -> Variable(a1)),QueryShuffle(List(),None,Some(SignedDecimalIntegerLiteral(1)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(r, a1),Selections(Set()),Vector(QueryGraph(Set(PatternRelationship(r,(a1,b2),INCOMING,List(),SimplePatternLength)),Set(a1, b2),Set(r, a1),Selections(Set()),Vector(),Set(),Set(),Vector())),Set(),Set(),Vector()),RegularQueryProjection(Map(a1 -> Variable(a1), r -> Variable(r), b2 -> Variable(b2)),QueryShuffle(List(),None,None)),None)))"""
  ...
  test("MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a2)<-[r]-(b2) WHERE a1 = a2 RETURN a1, r, b2, a2") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(r,(a1,b1),OUTGOING,List(),SimplePatternLength)),Set(a1, b1),Set(),Selections(Set()),Vector(),Set(),Set(),Vector()),RegularQueryProjection(Map(r -> Variable(r), a1 -> Variable(a1)),QueryShuffle(List(),None,Some(SignedDecimalIntegerLiteral(1)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(r, a1),Selections(Set()),Vector(QueryGraph(Set(PatternRelationship(r,(a2,b2),INCOMING,List(),SimplePatternLength)),Set(a2, b2),Set(r, a1),Selections(Set(Predicate(Set(a1, a2),Equals(Variable(a1),Variable(a2))))),Vector(),Set(),Set(),Vector())),Set(),Set(),Vector()),RegularQueryProjection(Map(a1 -> Variable(a1), r -> Variable(r), b2 -> Variable(b2), a2 -> Variable(a2)),QueryShuffle(List(),None,None)),None)))"""

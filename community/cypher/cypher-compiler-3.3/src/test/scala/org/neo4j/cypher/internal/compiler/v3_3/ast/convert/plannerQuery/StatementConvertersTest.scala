...
import org.neo4j.cypher.internal.compiler.v3_3.planner.{LogicalPlanningTestSupport, _}
...
import org.neo4j.cypher.internal.v3_3.logical.plans.{FieldSignature, ProcedureReadOnlyAccess, ProcedureSignature, QualifiedName}
...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("match (n) return n.prop order by n.prop2 DESC") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(n),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(n -> Variable(n),   FRESHID19 -> Property(Variable(n),PropertyKeyName(prop))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(n,   FRESHID19),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(  FRESHID19 -> Variable(  FRESHID19),   FRESHID35 -> Property(Variable(n),PropertyKeyName(prop2))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID19,   FRESHID35),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(  FRESHID19 -> Variable(  FRESHID19),   FRESHID35 -> Variable(  FRESHID35)),QueryShuffle(Vector(DescSortItem(Variable(  FRESHID35))),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID19,   FRESHID35),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(n.prop -> Variable(  FRESHID19)),QueryShuffle(List(),None,None)),None)))))))"""
  ...
  test("MATCH (a) WITH a WHERE TRUE RETURN a") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(a),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(a -> Variable(a),   FRESHID23 -> True()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(a,   FRESHID23),Selections(Set(Predicate(Set(  FRESHID23),Variable(  FRESHID23)))),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(a -> Variable(a)),QueryShuffle(List(),None,None)),None)))"""
    result should equal(expectation)
  }
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS collected WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(owner),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(collected -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collected),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collected -> Variable(collected),   FRESHID54 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED62)),List(),None,None,BOTH,false),NodePattern(Some(Variable(  UNNAMED64)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collected,   FRESHID54),Selections(Set(Predicate(Set(  FRESHID54),Variable(  FRESHID54)))),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))"""
  ...
  test("Funny query from boostingRecommendations") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(r1,(  origin@7,c),BOTH,List(RelTypeName(KNOWS), RelTypeName(WORKS_AT)),SimplePatternLength), PatternRelationship(r2,(c,  candidate@60),BOTH,List(RelTypeName(KNOWS), RelTypeName(WORKS_AT)),SimplePatternLength)),Set(  origin@7, c,   candidate@60),Set(),Selections(Set(Predicate(Set(r1, r2),Not(Equals(Variable(r1),Variable(r2)))), Predicate(Set(  origin@7,   candidate@60),Not(PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(  origin@7)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED143)),List(RelTypeName(KNOWS)),None,None,BOTH,false),NodePattern(Some(Variable(  candidate@60)),List(),None)))))), Predicate(Set(r1, r2),Equals(FunctionInvocation(Namespace(List()),FunctionName(type),false,Vector(Variable(r1))),FunctionInvocation(Namespace(List()),FunctionName(type),false,Vector(Variable(r2))))), Predicate(Set(  origin@7),In(Property(Variable(  origin@7),PropertyKeyName(name)),ListLiteral(List(StringLiteral(Clark Kent))))))),Vector(),List(),Set(),Vector()),AggregatingQueryProjection(Map(  FRESHID178 -> Property(Variable(  origin@7),PropertyKeyName(name)),   FRESHID204 -> Property(Variable(  candidate@60),PropertyKeyName(name))),Map(  FRESHID223 -> FunctionInvocation(Namespace(List()),FunctionName(SUM),false,Vector(FunctionInvocation(Namespace(List()),FunctionName(ROUND),false,Vector(Add(Property(Variable(r2),PropertyKeyName(weight)),Multiply(FunctionInvocation(Namespace(List()),FunctionName(COALESCE),false,Vector(Property(Variable(r2),PropertyKeyName(activity)), SignedDecimalIntegerLiteral(0))),SignedDecimalIntegerLiteral(2)))))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID178,   FRESHID204,   FRESHID223),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(  FRESHID178 -> Variable(  FRESHID178),   FRESHID204 -> Variable(  FRESHID204),   FRESHID223 -> Variable(  FRESHID223)),QueryShuffle(List(DescSortItem(Variable(  FRESHID223))),None,Some(SignedDecimalIntegerLiteral(10)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(  FRESHID178,   FRESHID204,   FRESHID223),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(origin -> Variable(  FRESHID178), candidate -> Variable(  FRESHID204), boost -> Variable(  FRESHID223)),QueryShuffle(List(),None,None)),None)))))"""
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(owner),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(xyz -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, xyz),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> GreaterThan(Variable(xyz),SignedDecimalIntegerLiteral(0))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collection),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> Variable(collection),   FRESHID82 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED90)),List(),None,None,BOTH,false),NodePattern(Some(Variable(  UNNAMED92)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(owner, collection,   FRESHID82),Selections(Set(Predicate(Set(  FRESHID82),Variable(  FRESHID82)))),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))))""".stripMargin
  ...
  test("MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a1)<-[r]-(b2) RETURN a1, r, b2") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(r,(a1,b1),OUTGOING,List(),SimplePatternLength)),Set(a1, b1),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(r -> Variable(r), a1 -> Variable(a1)),QueryShuffle(List(),None,Some(SignedDecimalIntegerLiteral(1)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(r, a1),Selections(Set()),Vector(QueryGraph(Set(PatternRelationship(r,(a1,b2),INCOMING,List(),SimplePatternLength)),Set(a1, b2),Set(r, a1),Selections(Set()),Vector(),List(),Set(),Vector())),List(),Set(),Vector()),RegularQueryProjection(Map(a1 -> Variable(a1), r -> Variable(r), b2 -> Variable(b2)),QueryShuffle(List(),None,None)),None)))"""
  ...
  test("MATCH (a1)-[r]->(b1) WITH r, a1 LIMIT 1 OPTIONAL MATCH (a2)<-[r]-(b2) WHERE a1 = a2 RETURN a1, r, b2, a2") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(r,(a1,b1),OUTGOING,List(),SimplePatternLength)),Set(a1, b1),Set(),Selections(Set()),Vector(),List(),Set(),Vector()),RegularQueryProjection(Map(r -> Variable(r), a1 -> Variable(a1)),QueryShuffle(List(),None,Some(SignedDecimalIntegerLiteral(1)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(r, a1),Selections(Set()),Vector(QueryGraph(Set(PatternRelationship(r,(a2,b2),INCOMING,List(),SimplePatternLength)),Set(a2, b2),Set(r, a1),Selections(Set(Predicate(Set(a1, a2),Equals(Variable(a1),Variable(a2))))),Vector(),List(),Set(),Vector())),List(),Set(),Vector()),RegularQueryProjection(Map(a1 -> Variable(a1), r -> Variable(r), b2 -> Variable(b2), a2 -> Variable(a2)),QueryShuffle(List(),None,None)),None)))"""

...
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.cardinality.ABCDCardinalityData
import org.neo4j.cypher.internal.compiler.v3_3.planner.logical.cardinality.RandomizedCardinalityModelTestSuite
import org.neo4j.cypher.internal.compiler.v3_3.spi.GraphStatistics
import org.neo4j.cypher.internal.ir.v3_3.IdName

class AssumeIndependenceQueryGraphCardinalityModelTest
    extends RandomizedCardinalityModelTestSuite
    with ABCDCardinalityData {
  ...
  test("all queries") {
    ...
    forAll(queries) { (q: String, expected: Double) =>
      forQuery(q).shouldHaveQueryGraphCardinality(expected)
    }
  }
...
// and all (:A)-[:T1]->()-[:T1]->(:B)
    ...
    forQuery("MATCH (a:A)-[r:T1*1..2]->(b:B)").shouldHaveQueryGraphCardinality(maxRelCount * totalSelectivity)
  }

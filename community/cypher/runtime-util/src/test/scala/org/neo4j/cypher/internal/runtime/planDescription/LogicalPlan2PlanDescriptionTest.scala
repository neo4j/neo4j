...
class LogicalPlan2PlanDescriptionTest extends CypherFunSuite with TableDrivenPropertyChecks {
  ...
  test("tests") {
    ...
    val lhsLP = attach(AllNodesScan(IdName("a"), Set.empty), 2.0)
    val lhsPD = PlanDescriptionImpl(id, "AllNodesScan", NoChildren, Seq(EstimatedRows(2)), Set("a"))

    val rhsLP = attach(AllNodesScan(IdName("b"), Set.empty), 2.0)
    ...
    val modeCombinations = Table(
      ...
      attach(AllNodesScan(IdName("a"), Set.empty), 1.0) ->
      ...
      , attach(AllNodesScan(IdName("b"), Set.empty), 42.0) ->
      ...
      , attach(NodeByLabelScan(IdName("node"), AstLabelName("X")(DummyPosition(0)), Set.empty), 33.0) ->
      ...
      , attach(NodeByIdSeek(IdName("node"), ManySeekableArgs(ListLiteral(Seq(SignedDecimalIntegerLiteral("1")(pos)))(pos)), Set.empty), 333.0) ->
      ...
      , attach(NodeIndexSeek(IdName("x"), LabelToken("Label", LabelId(0)), Seq(PropertyKeyToken("Prop", PropertyKeyId(0))), ManyQueryExpression(ListLiteral(Seq(StringLiteral("Andres")(pos)))(pos)), Set.empty), 23.0) ->
      ...
      , attach(NodeUniqueIndexSeek(IdName("x"), LabelToken("Lebal", LabelId(0)), Seq(PropertyKeyToken("Porp", PropertyKeyId(0))), ManyQueryExpression(ListLiteral(Seq(StringLiteral("Andres")(pos)))(pos)), Set.empty), 95.0) ->
      ...
      , attach(Expand(lhsLP, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r1"), ExpandAll), 95.0) ->
      ...
      , attach(Expand(lhsLP, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("a"), IdName("r1"), ExpandInto), 113.0) ->
      ...
      , attach(NodeHashJoin(Set(IdName("a")), lhsLP, rhsLP), 2345.0) ->

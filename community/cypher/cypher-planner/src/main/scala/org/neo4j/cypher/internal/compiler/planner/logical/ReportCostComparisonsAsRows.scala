...
class ReportCostComparisonsAsRows extends CostComparisonListener {
  ...
  override def report[X](projector: X => LogicalPlan,
    ...
    def stringTo(level: Int, plan: LogicalPlan): String = {
      ...
      val cost = context.cost.costFor(plan, context.input, context.semanticTable, context.planningAttributes.cardinalities, context.planningAttributes.providedOrders, CostModelMonitor.DEFAULT)

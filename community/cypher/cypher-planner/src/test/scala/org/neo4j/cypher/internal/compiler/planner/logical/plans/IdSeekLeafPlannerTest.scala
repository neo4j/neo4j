...
class IdSeekLeafPlannerTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("simple node by id seek with a collection of node ids") {
    ...
    when(factory.newCostModel(config, any[ExecutionModel])).thenReturn((plan: LogicalPlan, _: QueryGraphSolverInput, _: SemanticTable, _: Cardinalities, _: ProvidedOrders) => plan match {
      case _: NodeByIdSeek => Cost(1)
      case _               => Cost(Double.MaxValue)
    })
    val context = newMockedLogicalPlanningContext(planContext = newMockedPlanContext(), metrics = factory.newMetrics(statistics, evaluator, config, VolcanoModelExecution))
  ...
  test("simple undirected typed relationship by id seek with a collection of relationship ids") {
    ...
    val context = newMockedLogicalPlanningContext(planContext = newMockedPlanContext(), metrics = factory.newMetrics(statistics, evaluator, config, VolcanoModelExecution), semanticTable = semanticTable)

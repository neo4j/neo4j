  ...
  for (TestOrder(plannedOrder, cypherToken, sortOrder) <- List(ASCENDING, DESCENDING)) {
    ...
    test(
      ...
      plan should equal(planner.subPlanBuilder()
        .relationshipTypeScan("()-[r:REL]->()", plannedOrder)
        .build())
    }
    ...
    test(
      ...
      plan should equal(planner.subPlanBuilder()
        .unionRelationshipTypesScan("()-[r:REL|LER]->()", plannedOrder)
        .build())
    }
      ...
      case CompositeIndexOrderByMorePropsTestData(
        ...
        val expectedPlan = {
          ...
          planBuilderWithSorting
            ...
            .relationshipIndexOperator(
              "()-[r:REL(prop1 >= 42, prop2 <= 3, prop3 > 'a', prop4 < 'f')]->()",
              indexOrder = indexOrder,
              getValue = _ => GetValue,
              supportPartitionedScan = false
            )

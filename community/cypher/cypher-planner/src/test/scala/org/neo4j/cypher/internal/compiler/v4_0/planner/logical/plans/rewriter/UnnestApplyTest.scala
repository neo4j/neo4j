...
class UnnestApplyTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("unnesting varlength expands should work well") {
    ...
    val expand = VarExpand(arg, "a", SemanticDirection.OUTGOING, SemanticDirection.OUTGOING, Seq.empty, "b", "r", VarPatternLength(1, None), ExpandAll, varFor("tempNode"), varFor("tempEdge"), trueLiteral, trueLiteral, Seq.empty)
    ...
    // Then
    result should equal(VarExpand(lhs, "a", SemanticDirection.OUTGOING, SemanticDirection.OUTGOING, Seq.empty, "b", "r", VarPatternLength(1, None), ExpandAll, varFor("tempNode"), varFor("tempEdge"), trueLiteral, trueLiteral, Seq.empty))
  }

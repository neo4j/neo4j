...
class LeafPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 with PlanMatchHelp {
  ...
  test("should pick expands in an order that minimizes early cardinality increase") {
    val config = new given {
      cardinality = mapCardinality {
        ...
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "b") => 200.0
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "c") => 300.0
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "b", "c") => 100.0
  ...
  test("should pick expands in an order that minimizes early cardinality increase with estimates < 1.0") {
    val config = new given {
      cardinality = mapCardinality {
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a") =>  5.0
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("b") => 10.0
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("c") => 10.0
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "b") => 0.4
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "c") => 0.5
        case RegularPlannerQuery(queryGraph, _, _, _, _) if queryGraph.patternNodes == Set("a", "b", "c") => 0.1
        case _ => throw new IllegalStateException("Unexpected PlannerQuery")
      }
      knownLabels = Set("A", "B", "C")
    }

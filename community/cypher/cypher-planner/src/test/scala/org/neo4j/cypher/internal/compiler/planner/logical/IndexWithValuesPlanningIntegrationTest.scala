...
class IndexWithValuesPlanningIntegrationTest extends CypherPlannerTestSuite with LogicalPlanningIntegrationTestSupport
  ...
  test("should plan seek with GetValue when the relationship property is projected") {
    ...
    plan should equal(planner.subPlanBuilder()
      ...
      .relationshipIndexOperator("()-[r:REL(prop1 = 123)]-()", getValue = _ => GetValue)
      .build())
  }
  ...
  test(
    ...
    val plan = planner
      .plan("MATCH ()-[r:REL]-() WHERE r.prop CONTAINS 'foo' RETURN r.prop")
      .stripProduceResults
  ...
  test(s"should plan scan scan with GetValue when the relationship property is projected") {
    ...
    plan should equal(planner.subPlanBuilder()
      .projection("cacheR[r.prop1] AS `r.prop1`")
      .relationshipIndexOperator("()-[r:REL(prop1)]-()", getValue = _ => GetValue)
      .build())
  }

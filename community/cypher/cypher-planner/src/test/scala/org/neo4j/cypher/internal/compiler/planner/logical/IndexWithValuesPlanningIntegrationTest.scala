...
class IndexWithValuesPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2
        ...
        Projection(
          CacheProperties(
            Distinct(
              ...
              Map(varFor("n") -> varFor("n"))
            ),
            Set(cachedNodeProp("n", "prop1"), cachedNodeProp("n", "prop2"))
          ),
          Set(varFor("n")),
          Map(varFor("n.prop1") -> cachedNodeProp("n", "prop1"), varFor("n.prop2") -> cachedNodeProp("n", "prop2"))
        )
    ...
    Seq(
      ...
      case Seq(seek1, seek2) =>
        Projection(
          ...
          Set(varFor("n")),
          Map(cachedNodePropertyProj("n", "prop1"), varFor("n.prop2") -> cachedNodeProp("n", "prop2"))
        )
      case _ => sys.error("the impossible happened")
    }.toSeq should contain(plan._1)
  }
  ...
  test("should plan projection and index seek with GetValue when another predicate uses the property 2") {
    ...
    plan._1 should equal(
      Projection(
        ...
        Map(varFor("n.foo") -> cachedNodeProp("n", "foo"))
      )
    )
  }
  ...
  test("should plan index seek with GetValue when the property is projected after a renaming projection") {
    ...
    plan._1 should equal(
      Projection(
        ...
        Set(varFor("m"), varFor("r"), varFor("o")),
        Map(varFor("m.prop") -> cachedNodeProp("n", "prop", "m"))
      )
    )
  }
    ...
    plan._1 should equal(
      Projection(
        Projection(
          nodeIndexSeek("n:Awesome(prop = 42)", _ => GetValue),
          Set(varFor("n")),
          Map(cachedNodePropertyProj("foo", "n", "prop"), varFor("bar") -> trueLiteral)
        ),
        Set(varFor("bar")),
        Map(varFor("baz") -> varFor("bar"))
      )
    )
  }
  ...
  test("should not be fooled to use a variable when the node variable is defined twice") {
    ...
    plan._1 should equal(
      Projection(
        ...
        Set(varFor("m"), varFor("n"), varFor("r")),
  ...
  test("should plan projection and index seek with GetValue when the property is projected inside of a function") {
    ...
    plan._1 should equal(
      Projection(
        ...
        Set(varFor("n")),
        Map(varFor("toUpper(n.prop)") -> function("toUpper", cachedNodeProp("n", "prop")))
      )
    )
  }
    ...
    plan._1 should equal(
      Projection(
        Sort(
          ...
          Seq(Ascending(varFor("toUpper(n.prop)")))
        ),
        Set(varFor("n"), varFor("toUpper(n.prop)")),
  ...
  test("should plan index seek with GetValue when the property is part of an aggregating column") {
    ...
    plan._1 should equal(
      Aggregation(
        nodeIndexSeek("n:Awesome(prop = 42)", _ => GetValue),
        Map(varFor("nums") -> prop("n", "foo")),
        Map(varFor("sum(n.prop)") -> sum(cachedNodeProp("n", "prop")))
      )
    )
  }
    ...
    plan._1 should equal(
      Sort(
        Aggregation(
          ...
          Map(varFor("sum(n.foo)") -> sum(prop("n", "foo")))
        ),
        Seq(Ascending(varFor("n.prop")))
      )
    )
  }
    ...
    plan._1 should equal(
      ProcedureCall(
        ...
        ResolvedCall(
          ...
          IndexedSeq(ProcedureResultItem(None, varFor("value"))(pos))
        )(pos)
      )
    )
  }
  ...
  test(
    ...
    plan._1 should equal(
      Projection(
        ...
        Set(varFor("n")),
        Map(varFor("n.prop") -> cachedNodeProp("n", "prop"))
      )
    )
  }
  ...
  test(
    ...
    planner.plan(query) should equal(
      planner.planBuilder()
        .produceResults("`r.prop`")
        .projection(project = Seq("r.prop AS `r.prop`"), discard = Set("r", "a", "b"))
        .relationshipIndexOperator(
          "(a)-[r:REL(prop CONTAINS 'foo')]-(b)",
          indexOrder = IndexOrderNone,
          argumentIds = Set(),
  ...
  test(
    ...
    planner.plan(query) should equal(
      planner.planBuilder()
        ...
        .projection(project = Seq("r.foo AS `r.foo`"), discard = Set("r", "a", "b"))
        .relationshipIndexOperator(
          "(a)-[r:REL(prop CONTAINS 'foo')]-(b)",
          indexOrder = IndexOrderNone,
          argumentIds = Set(),
  ...
  test(
    ...
    plan should equal(planner.subPlanBuilder()
      .projection(project = Seq("r.prop AS `r.prop`"), discard = Set("count", "r", "a", "b"))
      .apply()
  ...
  test("should plan scan (relationship) with GetValue when composite existence constraint on projected property") {
    ...
    withClue("Index scan on two properties should be planned if they are only available through constraint") {
      plan should equal(planner.subPlanBuilder()
        .projection(project = Seq("cacheR[r.prop2] AS `r.prop2`"), discard = Set("r", "a", "b"))
        .relationshipIndexOperator(
  ...
  test("should plan scan (node) with GetValue when composite existence constraint on projected property") {
    ...
    withClue("Index scan on two properties should be planned if they are only available through constraint") {
      plan should equal(planner.subPlanBuilder()
        .projection(project = Seq("cacheN[a.prop2] AS `a.prop2`"), discard = Set("a"))
        .nodeIndexOperator(
          "a:A(prop1, prop2)",
          getValue = Map("prop1" -> DoNotGetValue, "prop2" -> GetValue),
          indexType = IndexType.RANGE
  ...
  test(
    ...
    plan should equal(planner.subPlanBuilder()
      .projection(project = Seq("r.prop2 AS `r.prop2`"), discard = Set("r", "a", "b"))
      .relationshipIndexOperator(
        "(a)-[r:REL(prop1, prop2)]-(b)",
        getValue = _ => DoNotGetValue,
        indexType = IndexType.RANGE
  ...
  test("should plan scan with GetValue when the property is used in avg function") {
    ...
    plan._1 should equal(
      Aggregation(
        ...
        Map.empty,
        Map(varFor("avg(n.prop)") -> avg(cachedNodeProp("n", "prop")))
      )
    )
  }
  ...
  test("should plan scan with DoNotGetValue when the property is used in avg function") {
    ...
    plan._1 should equal(
      Aggregation(
        ...
        Map.empty,
        Map(varFor("avg(n.prop)") -> avg(prop("n", "prop")))
      )
    )
  }
  ...
  test("should plan scan with DoNotGetValue when the property is used in sum function") {
    ...
    plan._1 should equal(
      Aggregation(
        ...
        Map.empty,
        Map(varFor("sum(n.prop)") -> sum(prop("n", "prop")))
      )
    )
  }
  ...
  test(
    ...
    plan shouldBe planner.subPlanBuilder()
      .projection(project = Seq("r.otherProp AS `r.otherProp`"), discard = Set("r", "a", "b"))
      .relationshipIndexOperator(
        "(a)-[r:REL(prop1 = 42, prop2 = 21)]->(b)",
        getValue = _ => DoNotGetValue,
        indexType = IndexType.RANGE
  ...
  private def cachedNodePropertyProj(node: String, property: String) =
    varFor(s"$node.$property") -> cachedNodeProp(node, property)
  ...
  private def propertyProj(node: String, property: String) =
    varFor(s"$node.$property") -> prop(node, property)

...
class LogicalPlanProducerTest extends CypherFunSuite with LogicalPlanningTestSupport2 with PlanMatchHelp{
  ...
  test("Create should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planCreate(ctx.lhs, CreatePattern(Seq(CreateNode("n", Seq(), None)), Seq()), InterestingOrder.empty, ctx.context))
  }

  test("MergeCreateNode should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planMergeCreateNode(ctx.lhs, CreateNode("n", Seq(), None), InterestingOrder.empty, ctx.context))
  }

  test("MergeCreateRelationship should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planMergeCreateRelationship(ctx.lhs, CreateRelationship("r", "n", relTypeName("R"), "m", SemanticDirection.OUTGOING, None), InterestingOrder.empty, ctx.context))
  }

  test("DeleteNode should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planDeleteNode(ctx.lhs, DeleteExpression(varFor("n"), false), InterestingOrder.empty, ctx.context))
  }

  test("DeleteRelationship should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planDeleteRelationship(ctx.lhs, DeleteExpression(varFor("r"), false), InterestingOrder.empty, ctx.context))
  }
  ...
  test("DeleteExpression should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planDeleteExpression(ctx.lhs, DeleteExpression(varFor("x"), false), InterestingOrder.empty, ctx.context))
  }
  ...
  test("SetProperty should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetProperty(ctx.lhs, SetPropertyPattern(varFor("x"), PropertyKeyName("p")(pos), literalInt(1)), InterestingOrder.empty, ctx.context))
  }

  test("SetPropertiesFromMap should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetPropertiesFromMap(ctx.lhs, SetPropertiesFromMapPattern(varFor("x"), mapOfInt("p" -> 1), false), InterestingOrder.empty, ctx.context))
  }

  test("SetNodeProperty should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetNodeProperty(ctx.lhs, SetNodePropertyPattern("x", PropertyKeyName("p")(pos), literalInt(1)), InterestingOrder.empty, ctx.context))
  }

  test("SetNodePropertiesFromMap should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetNodePropertiesFromMap(ctx.lhs, SetNodePropertiesFromMapPattern("x", mapOfInt("p" -> 1), false), InterestingOrder.empty, ctx.context))
  }

  test("SetRelationshipProperty should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetRelationshipProperty(ctx.lhs, SetRelationshipPropertyPattern("x", PropertyKeyName("p")(pos), literalInt(1)), InterestingOrder.empty, ctx.context))
  }

  test("SetRelationshipPropertiesFromMap should eliminate provided order") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSetRelationshipPropertiesFromMap(ctx.lhs, SetRelationshipPropertiesFromMapPattern("r", mapOfInt("p" -> 1), false), InterestingOrder.empty, ctx.context))
  }
  ...
  test("ProcedureCall RW should eliminate provided order") {
    val writer = ProcedureSignature(
      ...
      id = 0)
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planCallProcedure(ctx.lhs, ResolvedCall(writer, Seq(), IndexedSeq())(pos), InterestingOrder.empty, ctx.context))
  }
  ...
  test("ProcedureCall RO should retain provided order") {
    ...
    shouldRetainProvidedOrder(ctx =>
      ctx.producer.planCallProcedure(ctx.lhs, ResolvedCall(reader, Seq(), IndexedSeq())(pos), InterestingOrder.empty, ctx.context))
  }
  ...
  test("uncorrelated Subquery should eliminate provided order when rhs contains update") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planSubqueryCartesianProduct(ctx.lhs, ctx.rhsWithUpdate, ctx.context)
    )
  }

  test("uncorrelated Subquery should retain provided order when rhs contains no update") {
    shouldRetainProvidedOrder(ctx =>
      ctx.producer.planSubqueryCartesianProduct(ctx.lhs, ctx.rhsWithoutUpdate, ctx.context)
    )
  }
  ...
  test("ForeachApply should eliminate provided order when rhs contains update") {
    shouldEliminateProvidedOrder(ctx =>
      ctx.producer.planForeachApply(ctx.lhs, ctx.rhsWithUpdate, ForeachPattern("x", varFor("x"), SinglePlannerQuery.empty), ctx.context, InterestingOrder.empty, varFor("x"))
    )
  }

  test("ForeachApply should retain provided order when rhs contains no update") {
    shouldRetainProvidedOrder(ctx =>
      ctx.producer.planForeachApply(ctx.lhs, ctx.rhsWithoutUpdate, ForeachPattern("x", varFor("x"), SinglePlannerQuery.empty), ctx.context, InterestingOrder.empty, varFor("x"))
    )
  }
  ...
  test("SelectOrAntiSemiApply should retain provided order when rhs contains no update") {
    shouldRetainProvidedOrder(ctx =>
      ctx.producer.planSelectOrAntiSemiApply(ctx.lhs, ctx.rhsWithoutUpdate, varFor("x"), InterestingOrder.empty, ctx.context)
    )
  }
  ...
  test("LetSelectOrAntiSemiApply should retain provided order when rhs contains no update") {
    shouldRetainProvidedOrder(ctx =>
      ctx.producer.planLetSelectOrAntiSemiApply(ctx.lhs, ctx.rhsWithoutUpdate, "x", varFor("x"), InterestingOrder.empty, ctx.context)
    )
  }

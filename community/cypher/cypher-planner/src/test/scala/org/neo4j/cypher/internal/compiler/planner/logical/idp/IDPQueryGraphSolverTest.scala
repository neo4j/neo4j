...
class IDPQueryGraphSolverTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should plan for a join between two pattern relationships") {
    ...
    new given {
      ...
      val plan: LogicalPlan = queryGraphSolver.plan(cfg.qg, InterestingOrder.empty, ctx).result
      plan should equal(
        NodeHashJoin(Set("c"),
          Expand(
  ...
  test("should plan big star pattern") {
    ...
    new given {
      ...
      queryGraphSolver.plan(cfg.qg, InterestingOrder.empty, ctx) // should not throw
    }
  }
  ...
  test("should plan for optional single relationship pattern between two known nodes") {
    ...
    new given {
      ...
      queryGraphSolver.plan(cfg.qg, InterestingOrder.empty, ctx).result should equal(
        LeftOuterHashJoin(
          Set("a", "b"),
          CartesianProduct(
            AllNodesScan("a", Set.empty),

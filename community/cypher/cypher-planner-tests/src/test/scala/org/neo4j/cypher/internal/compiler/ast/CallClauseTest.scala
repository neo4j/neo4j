...
class CallClauseTest extends CypherPlannerTestSuite with AstConstructionTestSupport {
  ...
  test("should resolve CALL my.proc.foo") {
    ...
    val signature =
      ProcedureSignature(name, signatureInputs, signatureOutputs, None, ProcedureReadOnlyAccess, id = ID)
    ...
    unresolved.procedureName should equal(resolved.procedureName)
  ...
  test("should verify that output field names are correct during semantic checking of resolved calls") {
    ...
    result.errors.head.gqlStatusObject should equal(
      GqlHelper.getGql42001_42N50("p", java.util.List.of("x", "y"), 0, 0, 0)
    )
  }

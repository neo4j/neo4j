...
class RelationshipTypeScanLeafPlannerTest extends CypherPlannerTestSuite with LogicalPlanningTestSupport {
  ...
  test("simple outgoing directed type scan") {
    ...
    // when
    val resultPlans = relationshipTypeScanLeafPlanner(qg, InterestingOrderConfig.empty, context)
  ...
  test("outgoing directed relationship type scan with required ascending order") {
    ...
    // when
    val resultPlans = relationshipTypeScanLeafPlanner(
      qg,

...
class VarLengthPlanningIntegrationTest
  ...
  test("should plan the right quantifier on two conflicting ones - in pattern comprehension") {
    ...
    plan shouldEqual planner.subPlanBuilder()
      ...
      .|.filter("size(e) >= 2", "size(e) <= 3", "all(anon_1 IN e WHERE single(anon_2 IN e WHERE anon_1 = anon_2))")
      .|.argument("a", "b", "e")
      .expand("(a)-[e*1..2]->(b)")
      .allNodeScan("a")
      .build()
  ...
  test("should plan varExpand with relationship equality using Trail semantics") {
    ...
    planner.plan(
      ...
      planner.planBuilder()
        ...
        .nodeHashJoin("a")
        .|.filter("c:C")
        .|.expand("(a)-[anon_0:R*1..5]->(c)", pathMode = TraversalPathMode.Trail)
        .|.nodeByLabelScan("a", "A", IndexOrderNone)
        .filter("size(r) >= 1", "size(r) <= 5", "a:A")
        .expand("(b)<-[r:R*1..5]-(a)", pathMode = TraversalPathMode.Trail)
  ...
  testVersionsExcept5(
    ...
    planner.plan(
      ...
      planner.planBuilder()
        ...
        .nodeHashJoin("a")
        .|.filter("c:C")
        .|.expand("(a)-[anon_0:R*1..5]->(c)", pathMode = TraversalPathMode.Walk)
        .|.nodeByLabelScan("a", "A", IndexOrderNone)
        .filter("size(r) >= 1", "size(r) <= 5", "a:A")
        .expand("(b)<-[r:R*1..5]-(a)", pathMode = TraversalPathMode.Walk)
        .nodeByLabelScan("b", "B", IndexOrderNone)
        .build()
    )
  }
  ...
  test(
    ...
    planner.plan(
      ...
      planner.planBuilder()
        ...
        .|.expand("(a)-[r*1..5]-(b)", expandMode = ExpandInto, pathMode = TraversalPathMode.Trail)
        .|.argument("a", "b")
        .apply()
        .|.optional("a", "b")
        .|.shortestPath(

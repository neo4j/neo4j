...
class RelationshipIndexScanPlanningIntegrationTest extends CypherPlannerTestSuite
  ...
  test("should plan undirected relationship index scan with CONTAINS") {
    ...
    planner.plan("MATCH (a)-[r:REL]-(b) WHERE r.prop CONTAINS 'test' RETURN r") should equal(
      planner.planBuilder()
        .produceResults("r")
        .relationshipIndexOperator("()-[r:REL(prop CONTAINS 'test')]-()", indexType = IndexType.TEXT)
        .build()
    )
  }
  ...
  test("should plan undirected relationship index scan with ENDS WITH") {
    ...
    planner.plan("MATCH (a)-[r:REL]-(b) WHERE r.prop ENDS WITH 'test' RETURN r") should equal(
      planner.planBuilder()
        .produceResults("r")
        .relationshipIndexOperator("()-[r:REL(prop ENDS WITH 'test')]-()", indexType = IndexType.TEXT)
        .build()
    )
  }
  ...
  test(
    ...
    planner.plan(
      ...
      planner.planBuilder()
        .produceResults(column("n", "cacheN[n.prop]"), column("r", "cacheR[r.prop]"))
  ...
  test("should plan undirected relationship index scan over NodeByLabelScan using a hint") {
    ...
    planner.plan("MATCH (a:A)-[r:REL]-(b) USING INDEX r:REL(prop) WHERE r.prop IS NOT NULL RETURN r") should equal(
      planner.planBuilder()
        .produceResults(column("r", "cacheR[r.prop]"))
        .filter("a:A")
        .relationshipIndexOperator("(a)-[r:REL(prop)]-()", _ => GetValue, indexType = IndexType.RANGE)
        .build()
    )
  }

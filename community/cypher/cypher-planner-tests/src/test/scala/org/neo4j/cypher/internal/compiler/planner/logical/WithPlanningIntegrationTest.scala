...
class WithPlanningIntegrationTest extends CypherPlannerTestSuite
  ...
  test("Complex star pattern with WITH in front should not trip up joinSolver") {
    ...
    val planner = plannerBuilder()
      ...
      .setRelationshipCardinality("(:LabelB)-[:relTypeA]->()", 0)
      .setRelationshipCardinality("(:LabelSp)-[:relTypeA]->()", 0)
      .setRelationshipCardinality("()-[:relTypeA]->(:LabelA)", 0)
      ...
      .setRelationshipCardinality("()-[:relTypeB]->(:LabelB)", 0)
      .setRelationshipCardinality("(:LabelC)-[:relTypeB]->()", 0)
      .setRelationshipCardinality("()-[:relTypeB]->(:LabelSp)", 0)
      ...
      .setRelationshipCardinality("()-[:relTypeLink]->(:LabelD)", 0)
      .setRelationshipCardinality("(:LabelA)-[:relTypeLink]->()", 0)
  ...
  test("Subquery Apply with one row from both sides is distinct on any columns") {
    ...
    plan should equal(
      planner.planBuilder()
        ...
        .apply()
        .|.limit(1)
        .|.allRelationshipsScan("()-[]-(y)", "n")
        .limit(1)
        .allRelationshipsScan("(n)-[]-(x)")
        .build()
    )
  }

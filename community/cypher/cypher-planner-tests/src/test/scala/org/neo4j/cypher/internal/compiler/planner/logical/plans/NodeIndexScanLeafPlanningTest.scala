...
class NodeIndexScanLeafPlanningTest extends CypherPlannerTestSuite with LogicalPlanningTestSupport2 {
  ...
  private def nodeIndexScanLeafPlanner =
    NodeIndexLeafPlanner(Seq(nodeIndexScanPlanProvider))

  private def nodeIndexSearchStringScanLeafPlanner =
    NodeIndexLeafPlanner(Seq(nodeIndexStringSearchScanPlanProvider))

  private def allNodeScansLeafPlanner =
    NodeIndexLeafPlanner(Seq(nodeIndexScanPlanProvider, nodeIndexStringSearchScanPlanProvider))
  ...
  test("does not plan index scan when no index exist") {
    new givenConfig {
      ...
      // when
      val resultPlans =
        nodeIndexScanLeafPlanner(cfg.qg, InterestingOrderConfig.empty, ctx)
  ...
  test("index contains scan when there is an index on the property but node variable is restricted") {
    new givenConfig {
      ...
      // when
      val resultPlans =
        nodeIndexSearchStringScanLeafPlanner(
          cfg.qg,
  ...
  test("index scan solves is not null") {
    new givenConfig {
      ...
      // then
      resultPlans should beLike {
        case SetExtractor(plan @ NodeIndexScan(LogicalVariable(_), _, _, _, _, _, _)) =>
  ...
  test("plans index scans such that it solves hints") {
    ...
    new givenConfig {
      ...
      // then
      resultPlans should beLike {
        case SetExtractor(NodeIndexScan(LogicalVariable(_), _, _, _, _, _, _)) => ()
      }
  ...
  test("index scan does not solve seek hints") {
    val hint: UsingIndexHint =
      UsingIndexHint(v"n", labelOrRelTypeName("Awesome"), Seq(PropertyKeyName("prop")(pos)), SeekOnly)(pos)
  ...
  test("plans composite index scans when there is a composite index and multiple predicates") {
    new givenConfig {
      ...
      // then
      resultPlans should beLike {
        case SetExtractor(plan @ NodeIndexScan(LogicalVariable(_), _, properties, _, _, _, _)) =>
  ...
  test("does not plan index contains scan when no index exist") {
    new givenConfig {
      ...
      // when
      val resultPlans = nodeIndexSearchStringScanLeafPlanner(
        cfg.qg,
        InterestingOrderConfig.empty,
        ctx
      )
  ...
  test("plans index contains scans such that it solves hints") {
    ...
    new givenConfig {
      ...
      // then
      resultPlans should beLike {
        case SetExtractor(NodeIndexContainsScan(LogicalVariable(_), _, _, `litApa`, _, _, _)) => ()
      }
  ...
  test("plans index ends with scans such that it solves hints") {
    ...
    new givenConfig {
      ...
      // then
      resultPlans should beLike {
        case SetExtractor(p @ NodeIndexEndsWithScan(LogicalVariable(_), _, _, `litApa`, _, _, _)) =>
  ...
  test("does not plan index scans for arguments for: n.prop = <value>") {
    new givenConfig {
      ...
      // when
      val resultPlans =
        allNodeScansLeafPlanner(cfg.qg, InterestingOrderConfig.empty, ctx)
  ...
  test("does not plan index scan on argument for (n:Label) with aggregation") {
    new givenConfig {
      ...
      val resultPlans = allNodeScansLeafPlanner(
        cfg.qg,
        InterestingOrderConfig.empty,
        ctxWithAggregation
      )

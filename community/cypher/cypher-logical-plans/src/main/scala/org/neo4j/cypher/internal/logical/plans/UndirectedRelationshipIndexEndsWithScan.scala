...
case class UndirectedRelationshipIndexEndsWithScan(idName: String,
  ...
  override def copyWithoutGettingValues: UndirectedRelationshipIndexEndsWithScan =
    UndirectedRelationshipIndexEndsWithScan(idName, leftNode, rightNode, typeToken, IndexedProperty(property.propertyKeyToken, DoNotGetValue, property.entityType), valueExpr, argumentIds, indexOrder)(SameId(this.id))

  override def withMappedProperties(f: IndexedProperty => IndexedProperty): UndirectedRelationshipIndexEndsWithScan =
    UndirectedRelationshipIndexEndsWithScan(idName, leftNode, rightNode, typeToken, f(property), valueExpr, argumentIds, indexOrder)(SameId(this.id))
}

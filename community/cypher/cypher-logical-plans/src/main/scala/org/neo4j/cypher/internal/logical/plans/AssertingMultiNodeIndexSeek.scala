 ...
 */
case class AssertingMultiNodeIndexSeek(node: String, nodeIndexSeeks: Seq[IndexSeekLeafPlan])
  ...
  override def withoutArgumentIds(argsToExclude: Set[String]): MultiNodeIndexLeafPlan =
    copy(nodeIndexSeeks = nodeIndexSeeks.map(_.withoutArgumentIds(argsToExclude).asInstanceOf[IndexSeekLeafPlan]))(SameId(this.id))

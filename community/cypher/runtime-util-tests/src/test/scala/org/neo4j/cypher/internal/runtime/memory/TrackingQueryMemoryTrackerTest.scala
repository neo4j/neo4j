...
import org.neo4j.cypher.internal.runtime.memory.GrowingArrayTestValues.DEFAULT_SIZE_OF_GROWING_ARRAY
...
class TrackingQueryMemoryTrackerTest extends RuntimeUtilTestSuite {
  ...
  test("Tracks overall memory high water mark with AnyValues: Single transaction, single operator") {
    ...
    // Then
    val expected = Math.max(
      v1.estimatedHeapUsage() + v3.estimatedHeapUsage(),
      v1.estimatedHeapUsage() + v3.estimatedHeapUsage() - v2.estimatedHeapUsage() + v4.estimatedHeapUsage()
    )
  ...
  test(
    "Tracks individual memory per operator and per transaction plus overall: Multiple transactions, multiple operators"
  ) {
    // Given
    val queryMemoryTracker = new TrackingQueryMemoryTracker()
    val tx0MemoryTracker = TestMemoryTracker()

...
case class QueryPlanner(planSingleQuery: ((PlannerQuery, LogicalPlanningContext, Solveds, Cardinalities, TransactionLayers, IdGen) => LogicalPlan) = PlanSingleQuery()) extends Phase[CompilerContext, LogicalPlanState, LogicalPlanState] {
  ...
  override def process(from: LogicalPlanState, context: CompilerContext): LogicalPlanState = {
    val logicalPlanProducer = LogicalPlanProducer(context.metrics.cardinality, LogicalPlan.LOWEST_TX_LAYER, from.transactionLayers, from.solveds, from.cardinalities, context.logicalPlanIdGen)
    ...
    val (perCommit, logicalPlan) = plan(from.unionQuery, logicalPlanningContext, from.solveds, from.cardinalities, from.transactionLayers, context.logicalPlanIdGen)
  ...
  def plan(unionQuery: UnionQuery, context: LogicalPlanningContext, solveds: Solveds, cardinalities: Cardinalities, transactionLayers: TransactionLayers, idGen: IdGen): (Option[PeriodicCommit], LogicalPlan) =
    unionQuery match {
      case UnionQuery(queries, distinct, _, periodicCommitHint) =>
        val plan = planQueries(queries, distinct, context, solveds, cardinalities, transactionLayers, idGen)
  ...
  context.logicalPlanProducer.planProduceResult(in, unionQuery.returns.map(_.name), context)

  private def planQueries(queries: Seq[PlannerQuery], distinct: Boolean, context: LogicalPlanningContext, solveds: Solveds, cardinalities: Cardinalities, transactionLayers: TransactionLayers, idGen: IdGen) = {
    val logicalPlans: Seq[LogicalPlan] = queries.map(p => planSingleQuery(p, context, solveds, cardinalities, transactionLayers, idGen))

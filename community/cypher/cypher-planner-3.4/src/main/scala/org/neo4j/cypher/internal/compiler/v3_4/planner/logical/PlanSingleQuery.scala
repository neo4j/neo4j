...
case class PlanSingleQuery(planPart: (PlannerQuery, LogicalPlanningContext, Solveds, Cardinalities) => LogicalPlan = planPart,
  ...
  extends ((PlannerQuery, LogicalPlanningContext, Solveds, Cardinalities, TransactionLayers, IdGen) => LogicalPlan) {

  override def apply(in: PlannerQuery, context: LogicalPlanningContext,
                     solveds: Solveds, cardinalities: Cardinalities, transactionLayers: TransactionLayers, idGen: IdGen): LogicalPlan = {
    ...
    val finalPlan = planWithTail(completePlan, in.tail, ctx, solveds, cardinalities, Attributes(idGen, transactionLayers))

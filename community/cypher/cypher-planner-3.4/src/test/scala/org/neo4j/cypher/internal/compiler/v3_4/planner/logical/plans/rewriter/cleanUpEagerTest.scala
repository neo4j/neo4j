...
class cleanUpEagerTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("should not move eager below unwind") {
    ...
    val unwind = UnwindCollection(eager, IdName("i"), null)
  ...
  test("should move eager on top of unwind to below it") {
    val leaf = newMockedLogicalPlan()
    val unwind = UnwindCollection(leaf, IdName("i"), null)
    val eager = Eager(unwind)
    val topPlan = Projection(eager, Map.empty)

    rewrite(topPlan) should equal(Projection(UnwindCollection(Eager(leaf), IdName("i"), null), Map.empty))
  }

  test("should move eager on top of unwind to below it repeatedly") {
    val leaf = newMockedLogicalPlan()
    val unwind1 = UnwindCollection(leaf, IdName("i"), null)
    val eager1 = Eager(unwind1)
    val unwind2 = UnwindCollection(eager1, IdName("i"), null)
    val eager2 = Eager(unwind2)
    val unwind3 = UnwindCollection(eager2, IdName("i"), null)
    ...
    rewrite(topPlan) should equal(
      Projection(
        UnwindCollection(
          UnwindCollection(
            UnwindCollection(Eager(leaf), IdName("i"), null),
            IdName("i"), null),
          IdName("i"), null),
        Map.empty))
  }
  ...
  test("should move eager on top of load csv to below it") {
    ...
    val loadCSV = LoadCSV(leaf, url, IdName("a"), NoHeaders, None, false)
    val eager = Eager(loadCSV)
    val topPlan = Projection(eager, Map.empty)

    rewrite(topPlan) should equal(Projection(LoadCSV(Eager(leaf), url, IdName("a"), NoHeaders, None, false), Map.empty))
  }
  ...
  test("should not rewrite plan with eager below load csv") {
    val leaf = newMockedLogicalPlan()
    val eager = Eager(leaf)
    val loadCSV = LoadCSV(eager, StringLiteral("file:///tmp/foo.csv")(pos), IdName("a"), NoHeaders, None, false)

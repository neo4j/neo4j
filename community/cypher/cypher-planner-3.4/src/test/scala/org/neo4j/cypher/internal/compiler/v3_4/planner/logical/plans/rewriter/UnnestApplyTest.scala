...
class UnnestApplyTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("should unnest one level deeper") {
    /*
         ...
         LHS  OuterJoin
              Arg   RHS
     */
    val argPlan = Argument(Set(IdName("a")))
    ...
    rewrite(input) should equal(
      OuterHashJoin(Set(IdName("a")), lhs, rhs)
    )
  }
  ...
  test("apply on apply should be extracted nicely") {
    ...
    // Given
    val lhs = newMockedLogicalPlan("a")
    val arg1 = Argument(Set(IdName("a")))
    val arg2 = Argument(Set(IdName("a")))
    val expand = Expand(arg2, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), ExpandAll)
    ...
    // Then
    result should equal(Expand(lhs, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), ExpandAll))
  }
  ...
  test("apply on apply should be left unchanged") {
    ...
    val lhs2 = newMockedLogicalPlan("a")
    val rhs = newMockedLogicalPlan("a")
    val expand = Expand(lhs2, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), ExpandAll)
  ...
  test("unnesting varlength expands should work well") {
    ...
    // Given
    val lhs = newMockedLogicalPlan("a")
    val arg = Argument(Set(IdName("a")))
    val expand = VarExpand(arg, IdName("a"), SemanticDirection.OUTGOING, SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), VarPatternLength(1, None), ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
    ...
    // Then
    result should equal(VarExpand(lhs, IdName("a"), SemanticDirection.OUTGOING, SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), VarPatternLength(1, None), ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty))
  }
  ...
  test("apply on apply on optional should be OK") {
    ...
    result should equal(Apply(
      lhs,
      Optional(
        Expand(arg2, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), ExpandAll))
    ))
  }
  ...
  test("AntiConditionalApply on apply on optional should be OK") {
    ...
    val optional = Optional(expand)
    val apply2 = Apply(arg1, optional)
    val aca = AntiConditionalApply(lhs, apply2, Seq(IdName("a")))
  ...
  private def rewrite(p: LogicalPlan): LogicalPlan =
    fixedPoint((p: LogicalPlan) => p.endoRewrite(unnestApply(new Solveds, Attributes(idGen))))(p)
}

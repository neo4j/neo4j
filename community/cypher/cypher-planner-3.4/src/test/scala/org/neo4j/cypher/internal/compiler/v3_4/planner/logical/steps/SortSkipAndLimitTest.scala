...
class SortSkipAndLimitTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  private def queryGraphWith(skip: Option[Expression] = None,
    ...
    val solved = RegularPlannerQuery(QueryGraph.empty.addPatternNodes(IdName("n")))
    val plan = newMockedLogicalPlanWithSolved(idNames = Set(IdName("n")), solveds = solveds, solved = solved)

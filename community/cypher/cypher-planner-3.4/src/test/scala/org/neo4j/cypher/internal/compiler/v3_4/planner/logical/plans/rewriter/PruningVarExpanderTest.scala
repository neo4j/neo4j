...
class PruningVarExpanderTest extends CypherFunSuite with LogicalPlanningTestSupport with AstConstructionTestSupport {
  test("simplest possible query that can use PruningVarExpand") {
    ...
    val toId = IdName("to")
    val relId = IdName("r")
    val originalExpand = VarExpand(allNodes, fromId, dir, dir, Seq.empty, toId, relId, length, ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
  ...
  test("Double var expand with distinct result") {
    ...
    val dir = SemanticDirection.BOTH
    val length = VarPatternLength(1, Some(3))
    val originalExpand = VarExpand(allNodes, aId, dir, dir, Seq(RelTypeName("R")(pos)), bId, relRId, length, ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
    val originalExpand2 = VarExpand(originalExpand, bId, dir, dir, Seq(RelTypeName("T")(pos)), cId, relTId, length, ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
  ...
  test("optional match can be solved with PruningVarExpand") {
    ...
    */
    ...
    val originalExpand = VarExpand(argument, aId, dir, dir, Seq(RelTypeName("R")(pos)), bId, relRId, length, ExpandAll, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
  ...
  test("do not use pruning-varexpand when both sides of the var-length-relationship are already known") {
    ...
    val length = VarPatternLength(1, Some(3))
    val relId = IdName("r")
    val originalExpand = VarExpand(xJoin, fromId, dir, dir, Seq.empty, toId, relId, length, ExpandInto, IdName("tempNode"), IdName("tempEdge"), TRUE, TRUE, Seq.empty)
  ...
  test("should handle insanely long logical plans without running out of stack") {
    val leafPlan: LogicalPlan = Argument(Set(IdName("x")))
    var plan = leafPlan

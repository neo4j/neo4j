...
class TransitiveClosureTest extends CypherFunSuite with AstRewritingTestSupport with LogicalPlanConstructionTestSupport {
  ...
  private def shouldRewrite(from: String, to: String) {
    ...
    val input = LogicalPlanState(null, null, null, new TransactionLayers, new FakeSolveds, new FakeCardinalities, Some(original))
    ...
    val expectedInput = LogicalPlanState(null, null, null, new TransactionLayers, new FakeSolveds, new FakeCardinalities, Some(expected))

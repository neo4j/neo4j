...
class UnionPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("MATCH (a:A) RETURN a AS a UNION ALL MATCH (a:B) RETURN a AS a") {
    ...
    implicit val (_, logicalPlan, _, _ , _, _) = setup.getLogicalPlanFor("MATCH (a:A) RETURN a AS a UNION ALL MATCH (a:B) RETURN a AS a")
  ...
  test("MATCH (a:A) RETURN a AS a UNION MATCH (a:B) RETURN a AS a") {
    ...
    implicit val (_, logicalPlan, _, _, _, _) = setup.getLogicalPlanFor("MATCH (a:A) RETURN a AS a UNION MATCH (a:B) RETURN a AS a")

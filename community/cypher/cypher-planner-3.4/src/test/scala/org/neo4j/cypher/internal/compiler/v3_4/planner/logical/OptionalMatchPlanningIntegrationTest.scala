...
class OptionalMatchPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
    ...
    plan should equal(
      OptionalExpand(
        OptionalExpand(
          ...
          IdName("a"), SemanticDirection.OUTGOING, List(RelTypeName("R1") _), IdName("x1"), IdName("  UNNAMED29"), ExpandAll, Seq.empty),
        IdName("a"), SemanticDirection.OUTGOING, List(RelTypeName("R2") _), IdName("x2"), IdName("  UNNAMED60"), ExpandAll, Seq.empty)
    )
  }
  ...
  test("should solve optional matches with arguments and predicates") {
    ...
    val allNodesN: LogicalPlan = NodeByLabelScan(IdName("n"), LabelName("X") _, Set.empty)
  ...
  test(
    ...
    val (_, plan, _, _, _, cardinalities) = lom.getLogicalPlanFor(query)
    plan.treeExists {

...
class CheckForUnresolvedTokensTest extends CypherFunSuite with AstRewritingTestSupport {
  ...
  private def checkForTokens(ast: Query, semanticTable: SemanticTable): Set[InternalNotification] = {
    ...
    val compilationState = LogicalPlanState(queryText = "apa", startPosition = None, plannerName = IDPPlannerName, new TransactionLayers, maybeStatement = Some(ast), maybeSemanticTable = Some(semanticTable))

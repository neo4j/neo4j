...
class CheckForUnresolvedTokensTest extends CypherFunSuite with AstRewritingTestSupport with LogicalPlanConstructionTestSupport {
  ...
  private def checkForTokens(ast: Query, semanticTable: SemanticTable): Set[InternalNotification] = {
    ...
    val compilationState = LogicalPlanState(queryText = "apa", startPosition = None, plannerName = IDPPlannerName, new TransactionLayers, new FakeSolveds, new FakeCardinalities, maybeStatement = Some(ast), maybeSemanticTable = Some(semanticTable))

...
class IDPQueryGraphSolverTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should plan for a single node pattern") {
    ...
    new given {
      ...
      val plan = queryGraphSolver.plan(cfg.qg, ctx)
      plan should equal(
  ...
  test("should plan for a join between two pattern relationships") {
    ...
    new given {
      ...
      val plan: LogicalPlan = queryGraphSolver.plan(cfg.qg, ctx)
      plan should equal(
        NodeHashJoin(Set("c"),
          Expand(
  ...
  test("should plan big star pattern") {
    ...
    new given {
      ...
      implicit val x = ctx
      queryGraphSolver.plan(cfg.qg, ctx) // should not throw
    }
  }
  ...
  test("should plan for optional single relationship pattern between two known nodes") {
    ...
    new given {
      ...
      queryGraphSolver.plan(cfg.qg, ctx) should equal(
        OuterHashJoin(
          Set("a", "b"),
          CartesianProduct(

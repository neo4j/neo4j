...
class IDPQueryGraphSolverTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should solve self looping pattern") {
    ...
    new given {
      ...
      val plan = queryGraphSolver.plan(cfg.qg, ctx, solveds, cardinalities)
      plan should equal(
        Expand(AllNodesScan("a", Set.empty), "a", SemanticDirection.OUTGOING, Seq.empty, "a", IdName("r"), ExpandInto)
      )
  ...
  test("should plan cartesian product between 1 pattern nodes and 1 pattern relationship") {
    ...
    new given {
      ...
      val expandBtoA = Expand(AllNodesScan("b", Set.empty), "b", SemanticDirection.INCOMING, Seq.empty, "a", "r", ExpandAll)
      val allNodeScanC = AllNodesScan("c", Set.empty)
      val plan = queryGraphSolver.plan(cfg.qg, ctx, solveds, cardinalities)
      plan should equal(
        ValueHashJoin(
  ...
  test("should plan for optional single relationship pattern") {
    ...
    new given {
      ...
      val expandBtoA = Expand(AllNodesScan("b", Set.empty), "b", SemanticDirection.INCOMING, Seq.empty, "a", "r")
      val plan = queryGraphSolver.plan(cfg.qg, ctx, solveds, cardinalities)
      plan should equal(
        Apply(
          allNodeScanA,

...
class SingleComponentPlannerTest extends CypherFunSuite with LogicalPlanningTestSupport with AstConstructionTestSupport {
  test("plans expands for queries with single pattern rel") {
    ...
    val (context, solveds, cardinalities, _) = newMockedLogicalPlanningContext(planContext = mock[PlanContext])
    ...
    // then
    val plan1 = Expand(aPlan, IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r1"), ExpandAll)
    val plan2 = Expand(bPlan, IdName("b"), SemanticDirection.INCOMING, Seq.empty, IdName("a"), IdName("r1"), ExpandAll)
    val plan3 = Expand(CartesianProduct(aPlan, bPlan), IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r1"), ExpandInto)
  ...
  test("plans hashjoins and cartesian product for queries with single pattern rel and multiple index hints") {
    ...
    val plan5 = Expand(CartesianProduct(aPlan, bPlan), IdName("a"), SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r1"), ExpandInto)

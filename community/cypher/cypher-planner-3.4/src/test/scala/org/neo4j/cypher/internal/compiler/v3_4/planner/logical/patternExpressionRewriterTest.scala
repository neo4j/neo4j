...
class patternExpressionRewriterTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("Rewrites pattern expressions") {
    ...
    val (context, solveds, cardinalities, _) = newMockedLogicalPlanningContext(newMockedPlanContext, strategy = strategy)
  ...
  private val dummyPlan = AllNodesScan(IdName("a"), Set.empty)
  ...
  private def createStrategy: QueryGraphSolver = {
    ...
    when(strategy.planPatternExpression(any[Set[IdName]], any[PatternExpression], any[LogicalPlanningContext], any[Solveds], any[Cardinalities])).thenAnswer(

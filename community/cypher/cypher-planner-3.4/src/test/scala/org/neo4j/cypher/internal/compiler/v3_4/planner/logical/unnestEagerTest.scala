...
class unnestEagerTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("should unnest create node from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(CreateNode(Apply(lhs, rhs), "a", Seq.empty, None))
  }
  ...
  test("should unnest create relationship from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(CreateRelationship(Apply(lhs, rhs), "a", "b", RelTypeName("R")(pos), "c", None))
  }
  ...
  test("should unnest set node property from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(SetNodeProperty(Apply(lhs, rhs), "a", PropertyKeyName("prop")(pos), null))
  }
  ...
  test("should unnest set node property from map from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(SetNodePropertiesFromMap(Apply(lhs, rhs), "a", null, removeOtherProps = false))
  }
  ...
  test("should unnest set labels from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(SetLabels(Apply(lhs, rhs), "a", Seq.empty))
  }
  ...
  test("should unnest remove labels from rhs of apply") {
    ...
    rewrite(input, solveds) should equal(RemoveLabels(Apply(lhs, rhs), "a", Seq.empty))
  }

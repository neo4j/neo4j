  ...
  test("Should build plans expanding from the more expensive side if that is requested by using a hint") {
    (new given {
      ...
      indexOn("Person", "name")
        ...
        Expand(
          NodeIndexSeek("b", LabelToken("Person", LabelId(0)), Seq(IndexedProperty(PropertyKeyToken("name", PropertyKeyId(0)), DoNotGetValue)), SingleQueryExpression(StringLiteral("Andres")_), Set.empty, ProvidedOrder.empty),

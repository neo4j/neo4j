...
class OuterTaskCloser {

  private val _tasks: ArrayBuffer[Unit => Unit] = new ArrayBuffer(1)
  private var closed = false

  /**
   * @param task This task will be called on close()
   */
  def addTask(task: Unit => Unit): Unit = {
    if (closed)
      throw new IllegalStateException("Already closed")
    _tasks += task
  }

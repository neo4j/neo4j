...
object Foldable {
  ...
  private def foldAcc[R](
    ...
    if (remaining.isEmpty) {
      ...
      val that = remaining.pop()
      foldAcc(remaining ++= that.reverseChildren, f(that).fold(acc)(_(acc)), f, cancellation)
    }
  }
    ...
    if (remaining.isEmpty) {
      ...
      f(that) match {
        case Some(true) =>
          true
        case _ =>
          existsAcc(remaining ++= that.reverseChildren, f, cancellation)
      }
    }
  }
  ...
  cancellation: CancellationChecker,
    ...
    if (remaining.isEmpty) {
      ...
      countAcc(remaining ++= that.reverseChildren, f, next, cancellation)
    }
  }
  ...
  private def findAcc[A: ClassTag](
    ...
    if (remaining.isEmpty) {
      ...
      that match {
        case x: A => x
        case _ => findAcc(remaining ++= that.reverseChildren, cancellation)
      }
    }
  }
  ...
  private def optionFindAcc[A: ClassTag](
    ...
    if (remaining.isEmpty) {
      ...
      that match {
        case x: A => Some(x)
        case _    => optionFindAcc(remaining ++= that.reverseChildren, cancellation)
      }
    }
  }
  ...
  private def findAcc[A: ClassTag](
    ...
    if (remaining.isEmpty) {
      ...
      that match {
        ...
        case _ => findAcc(remaining ++= that.reverseChildren, predicate, cancellation)
      }
    }
  }
}

...
class CountTest extends SemanticFunSuite {
  ...
  test("valid count expression passes semantic check") {
    val expression = CountExpression(pattern, Some(property))(pos, Set.empty, Set.empty)
  ...
  test("inner where using missing identifier reports error") {
    val expression = CountExpression(pattern, Some(failingProperty))(pos, Set.empty, Set.empty)
  ...
  test("count expression can contain a node pattern with label") {
    ...
    val expression = CountExpression(p, optionalWhereExpression = None)(pos, Set.empty, Set.empty)
  ...
  test("count expression can contain a node pattern with a where outside the node pattern") {
    val p = Pattern(Seq(EveryPath(nodePat())))(pos)
    val expression = CountExpression(p, optionalWhereExpression = Some(nodePredicate))(pos, Set.empty, Set.empty)

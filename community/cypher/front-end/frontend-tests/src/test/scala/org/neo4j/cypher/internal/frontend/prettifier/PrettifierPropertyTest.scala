...
class PrettifierPropertyTest extends CypherFunSuite
  ...
  test("Prettifier output should parse to an AST that prettifies to the same string - Cypher 5 version") {
  ...
  test("Prettifier output should parse to an AST that prettifies to the same string - Cypher 25 version") {
    // To reproduce test failures, enable the following line with the seed from the TC build
    // setScalaCheckInitialSeed(seed)

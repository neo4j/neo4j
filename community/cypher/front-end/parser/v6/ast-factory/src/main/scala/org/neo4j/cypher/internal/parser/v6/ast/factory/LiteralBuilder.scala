...
import org.neo4j.cypher.internal.parser.v6.ast.factory.LiteralBuilder.cypherStringToString
import org.neo4j.cypher.internal.util.CypherExceptionFactory
import org.neo4j.cypher.internal.util.InputPosition

trait LiteralBuilder extends Cypher6ParserListener {

  protected def exceptionFactory: CypherExceptionFactory

  override def exitLiteral(ctx: Cypher6Parser.LiteralContext): Unit = {
    ctx.ast = ctx.children.get(0) match {
      case rule: AstRuleCtx => rule.ast
      case token: TerminalNode => token.getSymbol.getType match {
          case Cypher6Parser.TRUE                         => True()(pos(ctx))
          case Cypher6Parser.FALSE                        => False()(pos(ctx))
          case Cypher6Parser.INF | Cypher6Parser.INFINITY => Infinity()(pos(ctx))
          case Cypher6Parser.NAN                          => NaN()(pos(ctx))
          case Cypher6Parser.NULL                         => Null()(pos(ctx))
        }
      case other => throw new IllegalStateException(s"Unexpected child $other")
    }
  }

  final override def exitNumberLiteral(ctx: Cypher6Parser.NumberLiteralContext): Unit = {
    ctx.ast = lastChild[TerminalNode](ctx).getSymbol.getType match {
      case Cypher6Parser.UNSIGNED_DECIMAL_INTEGER => SignedDecimalIntegerLiteral(ctx.getText)(pos(ctx))
      case Cypher6Parser.DECIMAL_DOUBLE           => DecimalDoubleLiteral(ctx.getText)(pos(ctx))
      case Cypher6Parser.UNSIGNED_HEX_INTEGER     => SignedHexIntegerLiteral(ctx.getText)(pos(ctx))
      case Cypher6Parser.UNSIGNED_OCTAL_INTEGER   => SignedOctalIntegerLiteral(ctx.getText)(pos(ctx))
    }
  }

  final override def exitSignedIntegerLiteral(
    ctx: Cypher6Parser.SignedIntegerLiteralContext
  ): Unit = {

...
trait Statement extends Parser
  ...
  def CreateUser: Rule1[MultiDatabaseAdministrationCommand] = rule("CATALOG CREATE USER") {
    ...
    group(keyword("CREATE") ~~ optional(keyword("OR REPLACE")) ~~ keyword( "USER") ~~ Parameter
      ~~ optional(keyword("IF NOT EXISTS")) ~~ keyword("SET PASSWORD") ~~ (Parameter | StringLiteral) ~~
  ...
  def AlterUser: Rule1[MultiDatabaseAdministrationCommand] = rule("CATALOG ALTER USER") {
    ...
    //
    // ALTER USER $param <anything else>
  ...
  private def Database: Rule1[GraphScope] = rule("on a database")(
    group(keyword("ON") ~~ (keyword("DATABASE") | keyword("DATABASES"))) ~~
      (group(SymbolicDatabaseNameString) ~~>> (ast.NamedGraphScope(_)) |

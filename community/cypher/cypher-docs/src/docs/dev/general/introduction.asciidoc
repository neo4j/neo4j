...
You can also break down a query into multiple query parts.
This is especially important when you are mixing writing and reading clauses in the same query.

The query language is comprised of several distinct clauses.
You can read about more of them later in the manual, but here are the important ones.

...
case class normalizeWithClauses(mkException: (String, InputPosition) => CypherException) extends Rewriter {
  ...
  private val clauseRewriter: (Clause => Seq[Clause]) = {
    ...
    case clause @ With(distinct, ri, orderBy, skip, limit, where) =>
      ...
      val requiredVariablesForOrderBy = updatedOrderBy
        .map(_.dependencies)
        .getOrElse(Set.empty) diff (introducedReturnItems.map(_.variable).toSet ++ initialReturnItems.flatMap(_.alias))

      if (orderBy == updatedOrderBy && where == updatedWhere) {
        Seq(clause.copy(returnItems = ri.copy(items = initialReturnItems)(ri.position))(clause.position))
      } else if (introducedReturnItems.isEmpty) {
        Seq(
          clause.copy(returnItems = ri.copy(items = initialReturnItems)(ri.position),
                      orderBy = updatedOrderBy,
                      where = updatedWhere)(clause.position))
      } else {
        ...
        val firstProjection = if (distinct || ri.containsAggregate || ri.includeExisting) {
          ...
          val requiredReturnItems = introducedReturnItems
            .flatMap(_.expression.dependencies)
            .toSet diff initialReturnItems
            .flatMap(_.alias)
            .toSet
        ...
        Seq(
          With(distinct = distinct,
               returnItems = ri.copy(items = firstProjection)(ri.position),
               orderBy = None,
               skip = None,
               limit = None,
               where = None)(clause.position),
          With(distinct = false,
               returnItems = ri.copy(items = secondProjection)(ri.position),
               orderBy = updatedOrderBy,
               skip = skip,
               limit = limit,
               where = updatedWhere)(clause.position),
  ...
  private def partitionReturnItems(returnItems: Seq[ReturnItem]): (Seq[ReturnItem], Seq[AliasedReturnItem]) =
    returnItems.foldLeft((Vector.empty[ReturnItem], Vector.empty[AliasedReturnItem])) {
      case ((unaliasedItems, aliasedItems), item) =>
        item match {
  ...
  private def aliasOrderByAndWhere(existingAliases: Map[Expression, Variable],
                                   orderBy: Option[OrderBy],
                                   where: Option[Where]): (Seq[AliasedReturnItem], Option[OrderBy], Option[Where]) = {
  ...
  private def aliasOrderBy(existingAliases: Map[Expression, Variable],
                           originalOrderBy: OrderBy): (Seq[AliasedReturnItem], OrderBy) = {
    val (additionalReturnItems, updatedSortItems) =
      originalOrderBy.sortItems.foldLeft((Vector.empty[AliasedReturnItem], Vector.empty[SortItem])) {
        case ((returnItems, sortItems), item) =>
  ...
  private def aliasWhere(existingAliases: Map[Expression, Variable],
                         originalWhere: Where): (Option[AliasedReturnItem], Where) = {
  ...
  private def aliasExpression(existingAliases: Map[Expression, Variable],
                              expression: Expression): (Option[AliasedReturnItem], Variable) = {

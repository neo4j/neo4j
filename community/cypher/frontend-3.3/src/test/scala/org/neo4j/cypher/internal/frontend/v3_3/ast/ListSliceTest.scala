...
class ListSliceTest extends CypherFunSuite {
  val dummyList = DummyExpression(CTList(CTNode) | CTNode | CTList(CTString))
  ...
  test("shouldReturnCollectionTypesOfExpression") {
    val slice = ListSlice(dummyList,
                          ...
                          Some(SignedDecimalIntegerLiteral("2")(DummyPosition(7))))(DummyPosition(4))
  ...
  test("shouldRaiseErrorWhenNeitherFromOrTwoSpecified") {
    ...
    result.errors should equal(
      Seq(SemanticError("The start or end (or both) is required for a collection slice", slice.position)))
  }

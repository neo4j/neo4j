...
class TypeRangeTest extends CypherFunSuite {
  ...
  test("intersection of range within list") {
    ...
    rangeFromListAny & TypeRange(CTList(CTString), CTList(CTString)) should equal(
      Some(TypeRange(CTList(CTString), CTList(CTString))))
  }
  ...
  test("leastUpperBound with sub type") {
    ...
    (rangeFromListAny leastUpperBounds rangeFromListString) should equal(
      Seq(TypeRange(CTList(CTAny), CTList(CTString)), TypeRange(CTList(CTString), None)))
  }

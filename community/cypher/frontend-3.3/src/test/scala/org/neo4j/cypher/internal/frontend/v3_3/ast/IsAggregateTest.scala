...
class IsAggregateTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("distinct id(null) an aggregate expression") {
    val expr: Expression =
      FunctionInvocation(Namespace() _, FunctionName("id") _, distinct = true, Vector(Null() _))(pos)
  ...
  test("id(null) is not an aggregate expression") {
    val expr: Expression =
      FunctionInvocation(Namespace() _, FunctionName("id") _, distinct = false, Vector(Null() _))(pos)

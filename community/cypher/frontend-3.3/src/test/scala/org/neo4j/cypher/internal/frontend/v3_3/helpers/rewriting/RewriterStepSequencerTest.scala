...
class RewriterStepSequencerTest extends CypherFunSuite {
  ...
  test("if no conditions are used, what goes in is what comes out") {
    ...
    RewriterStepSequencer.newValidating("test")(ApplyRewriter("1", dummyRewriter1), ApplyRewriter("2", dummyRewriter2)) should equal(
      RewriterContract(Seq(dummyRewriter1, dummyRewriter2), Set()))
  }
  ...
  test("Should enable conditions between rewriters and collect the post conditions at the end") {
    ...
    sequencer.childRewriters should equal(
      Seq(
        dummyRewriter1,

...
abstract class ForeachApplyTestBase[CONTEXT <: RuntimeContext](
  ...
  test("foreachApply with dependency on lhs") {
    ...
    val logicalQuery = new LogicalQueryBuilder(this)
      .produceResults("x")
      .foreachApply("i", "[1, 2, 3]")
      .|.create(createNodeWithProperties("n", Seq("L"), mapOf("prop"-> add(varFor("i"), varFor("x")))))
      .|.argument("x")
      .input(variables = Seq("x"))
      .build(readOnly = false)
  ...
  test("nested foreachApply") {
    ...
    val logicalQuery = new LogicalQueryBuilder(this)
      ...
      .|.|.create(createNodeWithProperties("n", Seq("L"), mapOf("prop"-> add(varFor("x"), add(varFor("i"), varFor("j"))))))
      .|.|.argument("x", "i")
      .|.argument("x")
      .input(variables = Seq("x"))
      .build(readOnly = false)
  ...
  test("foreach on the rhs of an apply") {
    ...
    val logicalQuery = new LogicalQueryBuilder(this)
      ...
      .|.|.create(createNodeWithProperties("n", Seq("L"), mapOf("prop"-> add(varFor("x"), varFor("i")))))
      .|.|.argument("x")
      .|.argument("x")
      .input(variables = Seq("x"))
      .build(readOnly = false)

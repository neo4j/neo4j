...
trait OrderedAntiConditionalApplyTestBase[CONTEXT <: RuntimeContext] {
  ...
  test("anti conditional apply on nonempty lhs and empty rhs, where condition(lhs) always is false - with leveraged order") {
    // given
    given {
      nodeGraph(19, "RHS")
      nodeGraph(sizeHint)
  ...
  test("should aggregate on top of anti conditional apply with expand and limit and aggregation on rhs of apply - with leveraged order") {
    // given
    val nodesPerLabel = 10
    given {
      bipartiteGraph(nodesPerLabel, "A", "B", "R")

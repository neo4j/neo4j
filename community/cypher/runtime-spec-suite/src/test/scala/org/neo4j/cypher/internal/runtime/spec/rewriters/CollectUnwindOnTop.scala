...
case class CollectUnwindOnTop(
  ...
  private val instance: Rewriter =
    if (ctx.config.hints.contains(NoEager)) noop
      ...
      Rewriter.lift {
        case pr @ ProduceResult(source, columns) if !ctx.leveragedOrders(source.id) && randomShouldApply(config) =>
          ...
          val aggregation =
            Aggregation(source, Map.empty, Map(varFor(collectedRowsName) -> collectExpr), None)(ctx.idGen)

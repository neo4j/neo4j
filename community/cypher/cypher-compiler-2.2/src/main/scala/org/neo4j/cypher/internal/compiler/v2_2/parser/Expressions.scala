...
trait Expressions extends Parser
  ...
  private def Expression3: Rule1[ast.Expression] = rule("an expression") {
    Expression2 ~ zeroOrMore(WS ~ (
      ...
      | group(keyword("LIKE") ~~ Expression2 ~~ optional(keyword("ESCAPE") ~~ EscapeLiteral)) ~~>> (ast.Like(_ :ast.Expression, _, _))
      | group(keyword("NOT LIKE") ~~ Expression2 ~~ optional(keyword("ESCAPE") ~~ EscapeLiteral)) ~~>> (ast.NotLike(_: ast.Expression, _, _))
      | group(keyword("ILIKE") ~~ Expression2 ~~ optional(keyword("ESCAPE") ~~ EscapeLiteral)) ~~>> (ast.Like(_: ast.Expression, _, _, true))
      | group(keyword("NOT ILIKE") ~~ Expression2 ~~ optional(keyword("ESCAPE") ~~ EscapeLiteral)) ~~>> (ast.NotLike(_: ast.Expression, _, _, true))

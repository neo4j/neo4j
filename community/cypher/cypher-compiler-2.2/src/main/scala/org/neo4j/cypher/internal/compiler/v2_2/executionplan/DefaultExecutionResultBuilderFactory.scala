...
case class DefaultExecutionResultBuilderFactory(pipeInfo: PipeInfo, columns: List[String]) extends ExecutionResultBuilderFactory {
  ...
  case class ExecutionWorkflowBuilder() extends ExecutionResultBuilder {
    ...
    def build(graph: GraphDatabaseService, queryId: AnyRef, planType: ExecutionMode, params: Map[String, Any], notificationLogger: NotificationLogger): InternalExecutionResult = {
    ...
    private def createResults(state: QueryState, planType: ExecutionMode, notificationLogger: NotificationLogger): InternalExecutionResult = {
      val queryType =

...
case object astExpressionDocBuilder extends CustomDocBuilder[Any] {
  def newDocGenerator = DocGenerator {
    case expr: Expression =>
      ...
      inner => astExpressionConverter(inner)(expr).getOrElse(simpleDocBuilder.docGenerator.applyWithInner(inner)(expr))
  }
}

case class astExpressionConverter(pretty: FixedDocGenerator[Any]) extends (Expression => Option[Doc]) {

...
class PipeExecutionPlanBuilder(monitors: Monitors) {
  ...
  def build(plan: LogicalPlan)(implicit context: PipeExecutionBuilderContext, planContext: PlanContext): PipeInfo = {
    ...
    object buildPipeExpressions extends Rewriter {
      val instance = Rewriter.lift {
        case pattern: ast.PatternExpression =>
          ...
          val pipe = buildPipe(context.plan(pattern), Cardinality(1))
    ...
    def buildPipe(plan: LogicalPlan, c: Cardinality): Pipe = {
      ...
      val result: Pipe with RonjaPipe = plan match {
        case Projection(left, expressions) =>
          ProjectionNewPipe(buildPipe(left, c), Eagerly.immutableMapValues(expressions, buildExpression))()

        case ProjectEndpoints(left, rel, start, end, directed, length) =>
          ProjectEndpointsPipe(buildPipe(left, c), rel.name, start.name, end.name, directed, length.isSimple)()
        ...
        case Selection(predicates, left) =>
          FilterPipe(buildPipe(left, c), predicates.map(buildPredicate).reduce(_ ++ _))()

        case CartesianProduct(left, right) =>
          CartesianProductPipe(buildPipe(left, c), buildPipe(right, c))()
        ...
        case Expand(left, IdName(fromName), dir, projectedDir, types: Seq[RelTypeName], IdName(toName), IdName(relName), SimplePatternLength, _) =>
          ...
          if (types.exists(_.id == None))
            ExpandPipeForStringTypes(buildPipe(left, c), fromName, relName, toName, dir, types.map(_.name))()
          else {
            ExpandPipeForIntTypes(buildPipe(left, c), fromName, relName, toName, dir, types.flatMap(_.id).map(_.id))()
          }
        ...
        case Expand(left, IdName(fromName), dir, projectedDir, types, IdName(toName), IdName(relName), VarPatternLength(min, max), predicates) =>
          ...
          if (types.exists(_.id == None))
            VarLengthExpandPipeForStringTypes(buildPipe(left, c), fromName, relName, toName, dir, projectedDir, types.map(_.name), min, max, predicate)()
          else
            VarLengthExpandPipeForIntTypes(buildPipe(left, c), fromName, relName, toName, dir, projectedDir, types.flatMap(_.id).map(_.id), min, max, predicate)()
        ...
        case OptionalExpand(left, IdName(fromName), dir, types, IdName(toName), IdName(relName), SimplePatternLength, predicates) =>
          ...
          OptionalExpandPipe(buildPipe(left, c), fromName, relName, toName, dir, types.map(_.name), predicate)()

        case NodeHashJoin(nodes, left, right) =>
          NodeHashJoinPipe(nodes.map(_.name), buildPipe(left, c), buildPipe(right, c))()

        case OuterHashJoin(nodes, left, right) =>
          NodeOuterHashJoinPipe(nodes.map(_.name), buildPipe(left, c), buildPipe(right, c), (right.availableSymbols -- left.availableSymbols).map(_.name))()

        case Optional(inner) =>
          OptionalPipe(inner.availableSymbols.map(_.name), buildPipe(inner, c))()

        case Apply(outer, inner) =>
          val lhsCardinality = context.cardinality(outer, c)
          ApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality))()

        case SemiApply(outer, inner) =>
          val lhsCardinality = context.cardinality(outer, c)
          SemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), negated = false)()

        case AntiSemiApply(outer, inner) =>
          val lhsCardinality = context.cardinality(outer, c)
          SemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), negated = true)()

        case LetSemiApply(outer, inner, idName) =>
          val lhsCardinality = context.cardinality(outer, c)
          LetSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), idName.name, negated = false)()

        case LetAntiSemiApply(outer, inner, idName) =>
          val lhsCardinality = context.cardinality(outer, c)
          LetSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), idName.name, negated = true)()

        case apply@SelectOrSemiApply(outer, inner, predicate) =>
          val lhsCardinality = context.cardinality(outer, c)
          SelectOrSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), buildPredicate(predicate), negated = false)()

        case apply@SelectOrAntiSemiApply(outer, inner, predicate) =>
          val lhsCardinality = context.cardinality(outer, c)
          SelectOrSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), buildPredicate(predicate), negated = true)()

        case apply@LetSelectOrSemiApply(outer, inner, idName, predicate) =>
          val lhsCardinality = context.cardinality(outer, c)
          LetSelectOrSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), idName.name, buildPredicate(predicate), negated = false)()

        case apply@LetSelectOrAntiSemiApply(outer, inner, idName, predicate) =>
          val lhsCardinality = context.cardinality(outer, c)
          LetSelectOrSemiApplyPipe(buildPipe(outer, c), buildPipe(inner, lhsCardinality), idName.name, buildPredicate(predicate), negated = true)()

        case Sort(left, sortItems) =>
          SortPipe(buildPipe(left, c), sortItems)()

        case Skip(input, count) =>
          SkipPipe(buildPipe(input, c), buildExpression(count))()

        case Limit(input, count) =>
          LimitPipe(buildPipe(input, c), buildExpression(count))()

        case SortedLimit(input, exp, sortItems) =>
          TopPipe(buildPipe(input, c), sortItems.map(_.asCommandSortItem).toList, exp.asCommandExpression)()
        ...
        case Aggregation(Projection(input, expressions), groupingExpressions, aggregatingExpressions)
          ...
          DistinctPipe(buildPipe(input, c), groupingExpressions.mapValues(_.asCommandExpression))()
        ...
        case FindShortestPaths(input, shortestPath) =>
          ...
          new ShortestPathPipe(buildPipe(input, c), legacyShortestPath)()

        case Union(lhs, rhs) =>
          NewUnionPipe(buildPipe(lhs, c), buildPipe(rhs, c))()

        case UnwindCollection(lhs, identifier, collection) =>
          UnwindPipe(buildPipe(lhs, c), collection.asCommandExpression, identifier.name)()
      ...
      val cardinality = context.cardinality(plan, c)
      result.withEstimatedCardinality(cardinality.amount.toLong)
    }

    val topLevelPipe = buildPipe(plan, Cardinality(1))

...
class LegacyPipeBuilder(monitors: Monitors, eagernessRewriter: Pipe => Pipe = addEagernessIfNecessary)
  ...
  val PLANNER = PlannerVersion.rulePlanner
  ...
  private def buildIndexQuery(op: IndexOperation): PipeInfo = PipeInfo(new IndexOperationPipe(op), updating = true, version = VERSION, planner = PLANNER)
  ...
  private def buildConstraintQuery(op: UniqueConstraintOperation): PipeInfo = {
    ...
    PipeInfo(new ConstraintOperationPipe(op, label, propertyKey), updating = true, version = VERSION, planner = PLANNER)
  }
  ...
  def buildQuery(inputQuery: Query, context: PlanContext)(implicit pipeMonitor:PipeMonitor): PipeInfo = {
    ...
    PipeInfo(pipe, planInProgress.isUpdating, version = VERSION, planner = PLANNER)
  }

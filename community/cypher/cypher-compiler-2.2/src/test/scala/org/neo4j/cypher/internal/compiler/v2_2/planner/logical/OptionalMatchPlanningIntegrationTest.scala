...
class OptionalMatchPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should build plans containing joins") {
    (new given {
      ...
      Projection(
        OuterHashJoin(Set("b"),
          Expand(NodeByLabelScan("a", Left("X"), Set.empty)(PlannerQuery.empty), "a", Direction.OUTGOING, Direction.OUTGOING, Seq(), "b", "r1")(PlannerQuery.empty),
          Expand(NodeByLabelScan("c", Left("Y"), Set.empty)(PlannerQuery.empty), "c", Direction.INCOMING, Direction.OUTGOING, Seq(), "b", "r2")(PlannerQuery.empty)
        )(PlannerQuery.empty),
      ...
      Projection(
        OptionalExpand(
          OptionalExpand(
            ...
            IdName("a"), Direction.OUTGOING, Direction.OUTGOING, List(ast.RelTypeName("R1") _), IdName("x1"), IdName("  UNNAMED27"), ExpandAll, Seq.empty)(PlannerQuery.empty),
          IdName("a"), Direction.OUTGOING, Direction.OUTGOING, List(ast.RelTypeName("R2") _), IdName("x2"), IdName("  UNNAMED58"), ExpandAll, Seq.empty)(PlannerQuery.empty),
  ...
  test("should solve optional matches with arguments and predicates") {
    ...
    val expand = Expand(Selection(Vector(predicate), allNodesM)(s), IdName("m"), Direction.BOTH, Direction.BOTH, List(), IdName("n"), IdName("r"), ExpandAll, Vector())(s)
    plan should equal(
      Projection(

...
trait QueryGraphProducer extends MockitoSugar {
  ...
  def producePlannerQueryForPattern(query: String): (PlannerQuery, SemanticTable) = {
    ...
    val (rewrittenAst, rewrittenTable) = CostBasedPipeBuilder.rewriteStatement(firstRewriteStep, semanticState.scopeTree, semanticTable, rewriterSequencer, semanticChecker, postConditions, mock[AstRewritingMonitor])

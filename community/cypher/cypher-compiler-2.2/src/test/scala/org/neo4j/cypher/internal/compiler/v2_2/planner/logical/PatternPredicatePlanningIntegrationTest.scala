            ...
            nestedPlan should equal(
              Selection(
                ...
                Expand(
                  ...
                  "f", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("WORKS_AT")_), "  UNNAMED116", "  UNNAMED102", ExpandAll, Seq()
                )(PlannerQuery.empty)
              )(PlannerQuery.empty)
            )
        }
   ...
   planFor("MATCH (a) WHERE (a)-[:X]->() RETURN a").plan should equal(
      SemiApply(
        ...
        Expand(
          ...
          "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X")_), "  UNNAMED27", "  UNNAMED20"
        )(PlannerQuery.empty)
      )(PlannerQuery.empty)
    )
  }
  ...
  test("should build plans containing anti semi apply for a single negated pattern predicate") {
    planFor("MATCH (a) WHERE NOT (a)-[:X]->() RETURN a").plan should equal(
      AntiSemiApply(
        ...
        Expand(
          Argument(Set("a"))(PlannerQuery.empty)(),
          "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X")_), "  UNNAMED31", "  UNNAMED24"
        )(PlannerQuery.empty)
      )(PlannerQuery.empty)
    )
  }
    ...
    planFor("MATCH (a) WHERE (a)-[:X]->() AND (a)-[:Y]->() RETURN a").plan should equal(
      SemiApply(
        SemiApply(
          ...
          Expand(
            Argument(Set("a"))(PlannerQuery.empty)(),
            "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("Y")_), "  UNNAMED44", "  UNNAMED37"
          )(PlannerQuery.empty)
        )(PlannerQuery.empty),
        Expand(
          Argument(Set("a"))(PlannerQuery.empty)(),
  ...
  test("should build plans containing select or semi apply for a pattern predicate and multiple expressions") {
    planFor("MATCH (a) WHERE a.prop2 = 9 OR (a)-[:X]->() OR a.prop > 4 RETURN a").plan should equal(
      SelectOrSemiApply(
        ...
        Expand(
          Argument(Set("a"))(PlannerQuery.empty)(),
          "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X")_), "  UNNAMED42", "  UNNAMED35"
        )(PlannerQuery.empty),
        Ors(Set(
  ...
  test("should build plans containing select or anti semi apply for a single negated pattern predicate") {
    planFor("MATCH (a) WHERE a.prop = 9 OR NOT (a)-[:X]->() RETURN a").plan should equal(
      SelectOrAntiSemiApply(
        ...
        Expand(
          Argument(Set("a"))(PlannerQuery.empty)(),
          "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X")_), "  UNNAMED45", "  UNNAMED38"
        )(PlannerQuery.empty),
      ...
      Projection(
        SelectOrAntiSemiApply(
          LetSelectOrSemiApply(
            ...
            Expand(
              Argument(Set("a"))(PlannerQuery.empty)(),
              "a", Direction.OUTGOING, Direction.OUTGOING,  Seq(RelTypeName("Y") _), "  UNNAMED41", "  UNNAMED34"
            )(PlannerQuery.empty),
            "  FRESHID30",
            In(Property(Identifier("a") _, PropertyKeyName("prop") _) _, Collection(Seq(SignedDecimalIntegerLiteral("9")_))_)_
          )(PlannerQuery.empty),
          Expand(
            Argument(Set("a"))(PlannerQuery.empty)(),
            "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X") _), "  UNNAMED61", "  UNNAMED54"
          )(PlannerQuery.empty),
          ident("  FRESHID30")
        )(PlannerQuery.empty),
        Map("a" -> ident("a"))
      ...
      Projection(
        SelectOrAntiSemiApply(
          LetSemiApply(
            ...
            Expand(
              Argument(Set("a"))(PlannerQuery.empty)(),
              "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("Y") _), "  UNNAMED27", "  UNNAMED20"
            )(PlannerQuery.empty),
            "  FRESHID16"
          )(PlannerQuery.empty),
          Expand(
            Argument(Set("a"))(PlannerQuery.empty)(),
            "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X") _), "  UNNAMED47", "  UNNAMED40"
          )(PlannerQuery.empty),
          ident("  FRESHID16")
        )(PlannerQuery.empty),
        Map("a" -> ident("a"))
      ...
      Projection(
        SelectOrAntiSemiApply(
          LetAntiSemiApply(
            ...
            Expand(
              Argument(Set("a"))(PlannerQuery.empty)(),
              "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("Y") _), "  UNNAMED31", "  UNNAMED24"
            )(PlannerQuery.empty),
            "  FRESHID20"
          )(PlannerQuery.empty),
          Expand(
            Argument(Set("a"))(PlannerQuery.empty)(),
            "a", Direction.OUTGOING, Direction.OUTGOING, Seq(RelTypeName("X") _), "  UNNAMED51", "  UNNAMED44"
          )(PlannerQuery.empty),
          ident("  FRESHID20")
        )(PlannerQuery.empty),
        Map("a" -> ident("a"))

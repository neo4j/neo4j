...
class SeekTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("Seek finds Equals") {
    assertMatches(Equals(expr1, expr2)_) {
      case Seek(lhs, rhs) =>
        ...
        rhs should equal(SingleSeekRhs(expr2))
    }
  }
  ...
  test("Seek finds In") {
    assertMatches(In(expr1, expr2)_) {
      case Seek(lhs, rhs) =>
        lhs should equal(expr1)
        rhs should equal(MultiSeekRhs(expr2))
    }
  }

  test("MultiSeekRhs has size hint for collections") {
    MultiSeekRhs(Collection(Seq(expr1, expr2))_).sizeHint should equal(Some(2))
  }

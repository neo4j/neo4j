...
class ExpandTest
  ...
  test("finds expansion going both sides") {
    ...
    expand(plan, qg) should equal(CandidateList(Seq(
      planExpand(left = planA, from = aNode, Direction.OUTGOING, Direction.OUTGOING, types = Seq.empty, to = bNode, rName, SimplePatternLength, rRel),
      planExpand(left = planB, from = bNode, Direction.INCOMING, Direction.INCOMING, types = Seq.empty, to = aNode, rName, SimplePatternLength, rRel)
    )))
  }
  ...
  test("looping pattern is handled as it should") {
    ...
    expand(plan, qg) should equal(Candidates(
      ...
      planHiddenSelection(
        ...
        left = planExpand(left = aAndB, from = bNode, dir = Direction.INCOMING, Direction.INCOMING, types = Seq.empty,

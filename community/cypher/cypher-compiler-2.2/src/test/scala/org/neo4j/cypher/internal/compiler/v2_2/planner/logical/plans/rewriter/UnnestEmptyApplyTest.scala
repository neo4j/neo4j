...
class UnnestEmptyApplyTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("apply on apply should be extracted nicely") {
    ...
    val expand: LogicalPlan = Expand(arg2, IdName("a"), Direction.OUTGOING, Direction.OUTGOING, Seq.empty, IdName("b"), IdName("r"), SimplePatternLength, ExpandAll, Seq.empty)(solved)
    ...
    // Then
    result should equal(Expand(lhs, IdName("a"), Direction.OUTGOING, Direction.OUTGOING, Seq.empty, IdName("b"), IdName("r"), SimplePatternLength, ExpandAll, Seq.empty)(solved))
  }
  ...
  test("apply on apply on optional should be OK") {
    ...
    result should equal(Apply(
      lhs,
      Optional(
        Expand(arg2, IdName("a"), Direction.OUTGOING, Direction.OUTGOING, Seq.empty, IdName("b"), IdName("r"), SimplePatternLength, ExpandAll, Seq.empty)(solved))(solved)
    )(solved))
  }

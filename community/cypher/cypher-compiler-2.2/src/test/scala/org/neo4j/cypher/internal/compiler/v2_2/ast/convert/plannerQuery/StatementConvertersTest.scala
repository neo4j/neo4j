...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  def buildPlannerQuery(query: String, cleanStatement: Boolean = true): UnionQuery = {
    ...
    val (rewrittenAst: Statement, _) = CostBasedPipeBuilder.rewriteStatement(statement, semanticState.scopeTree, semanticTable, RewriterStepSequencer.newValidating _, semanticChecker, Set.empty, mock[AstRewritingMonitor])
  ...
  test("WITH [1,2,3] as xes, 2 as y UNWIND xes AS x RETURN x, y") {
    val UnionQuery(query :: Nil, _) = buildPlannerQuery("WITH [1,2,3] as xes, 2 as y UNWIND xes AS x RETURN x, y")

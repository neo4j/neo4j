...
import org.neo4j.cypher.internal.compiler.v2_2.perty.docbuilders.{simpleDocBuilder, defaultDocBuilder, DocBuilderTestSuite}

class AstTermDocBuilderTest extends DocBuilderTestSuite[Any] {

  val docBuilder = astTermDocBuilder orElse astParticleDocBuilder orElse astExpressionDocBuilder orElse simpleDocBuilder

  test("ORDER BY n") {
    val astNode: ASTNode = OrderBy(Seq(AscSortItem(ident("n"))_))_
    format(astNode) should equal("ORDER BY n")
  }

...
class ContainsNoNodesOfTypeTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  val condition: (Any => Seq[String]) = containsNoNodesOfType[ReturnAll]()
  ...
  test("Fails when finding ReturnAll()") {
    ...
    condition(ast) should equal(Seq("Expected none but found ReturnAll at position line 1, column 0"))
  }
}

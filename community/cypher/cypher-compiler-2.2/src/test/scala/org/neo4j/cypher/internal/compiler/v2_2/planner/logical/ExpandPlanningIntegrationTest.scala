    ...
    planFor("MATCH (a)-[r]->(b) RETURN r").plan should equal(
      Projection(
        Expand(
          ...
          "b", Direction.INCOMING, Direction.OUTGOING, Seq.empty, "a", "r"
        )(PlannerQuery.empty),
    ...
    result should equal(
      Projection(
        Expand(
          ...
          "a", Direction.OUTGOING, Direction.OUTGOING, Seq.empty, "a", "r", ExpandInto)(PlannerQuery.empty),
        Map("r" -> Identifier("r") _)
      )(PlannerQuery.empty)
    )
  }
    ...
    (new given {
      ...
      Projection(
        Expand(
          ...
          "a", Direction.BOTH, Direction.BOTH, Seq(RelTypeName("x")_), "start", "rel"
        )(PlannerQuery.empty),
        ...
        NodeHashJoin(
          ...
          Selection(
            ...
            Expand(
              ...
              "a", Direction.OUTGOING, Direction.OUTGOING, Seq.empty, "b", "r"
            )(PlannerQuery.empty)
          )(PlannerQuery.empty),
          NodeIndexSeek("b", LabelToken("Person", LabelId(0)), PropertyKeyToken("name", PropertyKeyId(0)), ManyQueryExpression(Collection(Seq(StringLiteral("Andres") _)) _), Set.empty)(PlannerQuery.empty)
        )(PlannerQuery.empty),

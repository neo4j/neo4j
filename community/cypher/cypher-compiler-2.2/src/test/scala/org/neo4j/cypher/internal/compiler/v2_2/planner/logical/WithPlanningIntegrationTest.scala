...
class WithPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should build plans that contain multiple WITH") {
    ...
    // oh perty where art thou!?
      "Projection(Limit(Expand(Apply(Limit(AllNodesScan(IdName(a),Set()),UnsignedDecimalIntegerLiteral(1)),Argument(Set(IdName(a)))),IdName(a),OUTGOING,OUTGOING,List(),IdName(b),IdName(r1),SimplePatternLength,Vector()),UnsignedDecimalIntegerLiteral(1)),Map(b -> Identifier(b)))")
  }
  ...
  test("should build plans for two matches separated by WITH") {
    ...
    result.toString should equal(
      "Projection(Expand(Apply(Limit(AllNodesScan(IdName(a),Set()),UnsignedDecimalIntegerLiteral(1)),Argument(Set(IdName(a)))),IdName(a),OUTGOING,OUTGOING,List(),IdName(b),IdName(r),SimplePatternLength,Vector()),Map(b -> Identifier(b)))")
  }

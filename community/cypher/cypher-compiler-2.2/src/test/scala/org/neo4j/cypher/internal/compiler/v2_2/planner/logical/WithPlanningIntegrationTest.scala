...
class WithPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should build plans that project endpoints of re-matched directed relationship arguments") {
    ...
    plan.toString should equal(
      "Projection(Apply(Limit(Expand(AllNodesScan(IdName(b),Set()),IdName(b),INCOMING,List(),IdName(a),IdName(r),ExpandAll),UnsignedDecimalIntegerLiteral(1)),ProjectEndpoints(Argument(Set(IdName(r))),IdName(r),IdName(u),IdName(v),None,true,SimplePatternLength)),Map(r -> Identifier(r)))")
  }
  ...
  test("should build plans that verify endpoints of re-matched directed relationship arguments") {
    ...
    plan.toString should equal(
      "Projection(Selection(List(Equals(Identifier(a),Identifier(a$$$_)), Equals(Identifier(b),Identifier(b$$$_))),Apply(Limit(Expand(AllNodesScan(IdName(b),Set()),IdName(b),INCOMING,List(),IdName(a),IdName(r),ExpandAll),UnsignedDecimalIntegerLiteral(1)),ProjectEndpoints(Argument(Set(IdName(a), IdName(b), IdName(r))),IdName(r),IdName(a$$$_),IdName(b$$$_),None,true,SimplePatternLength))),Map(r -> Identifier(r)))")
  }
  ...
  test("should build plans that project and verify endpoints of re-matched undirected relationship arguments") {
    ...
    plan.toString should equal(
      "Projection(Selection(List(Equals(Identifier(a),Identifier(a$$$_))),Apply(Limit(Expand(AllNodesScan(IdName(b),Set()),IdName(b),INCOMING,List(),IdName(a),IdName(r),ExpandAll),UnsignedDecimalIntegerLiteral(1)),ProjectEndpoints(Argument(Set(IdName(a), IdName(r))),IdName(r),IdName(a$$$_),IdName(b2),None,false,SimplePatternLength))),Map(r -> Identifier(r)))")
  }

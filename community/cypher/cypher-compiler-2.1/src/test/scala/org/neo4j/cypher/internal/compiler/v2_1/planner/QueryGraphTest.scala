...
class QueryGraphTest extends CypherFunSuite {
  test("returns no pattern relationships when the query graph doesn't contain any") {
    ...
    val qg = QueryGraph(Map.empty, Selections(), Set.empty, rels, Set.empty, Seq.empty)
  ...
  test("finds single pattern relationship") {
    ...
    val qg = QueryGraph(Map.empty, Selections(), Set.empty, Set(r), Set.empty, Seq.empty)
  ...
  test("finds multiple pattern relationship") {
    ...
    val qg = QueryGraph(Map.empty, Selections(), Set.empty, Set(r, r2), Set.empty, Seq.empty)

...
trait Patterns extends Parser
  ...
  private def RelationshipTypes: Rule1[Seq[ast.RelTypeToken]] = rule("relationship types") (
      ":" ~~ oneOrMore(RelTypeToken, separator = WS ~ "|" ~~ optional(":") ~ WS)
    | EMPTY ~ push(Seq())
  )
  ...
  private def MaybeNodeLabels: Rule1[Seq[ast.LabelToken]] = rule("node labels") (

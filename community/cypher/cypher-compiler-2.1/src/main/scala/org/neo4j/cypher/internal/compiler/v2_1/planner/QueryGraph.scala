  ...
  private def filterUnnamed(predicate: Predicate): Predicate = predicate match {
    ...
    case Predicate(deps, ors@Ors(exprs)) =>
      val newDeps = exprs.foldLeft(Set.empty[IdName]) { (acc, exp) =>
        exp match {
          case e: PatternExpression        => acc ++ SelectionPredicates.idNames(e).filter(x => isNamed(x.name))
          case e@Not(_: PatternExpression) => acc ++ SelectionPredicates.idNames(e).filter(x => isNamed(x.name))
          case e                           => acc ++ SelectionPredicates.idNames(e)
        }
      }

...
trait LogicalPlanningAction[+B] {
  self: LogicalPlanningFunction[Unit, B] =>

  final def apply(input: Unit)(implicit context: LogicalPlanContext): B = apply

  def apply()(implicit context: LogicalPlanContext): B
}

...
import org.neo4j.cypher.internal.compiler.v2_1.pprint.{PrettyToString, GeneratedPretty, Pretty}

case class Predicate(dependencies: Set[IdName], exp: Expression) extends GeneratedPretty with PrettyToString {
  def hasDependenciesMet(symbols: Set[IdName]): Boolean =
    (dependencies -- symbols).isEmpty
}

case class Selections(predicates: Set[Predicate] = Set.empty) extends GeneratedPretty with PrettyToString {

...
import org.neo4j.cypher.internal.compiler.v3_1.{
  ...
  InfoLogger,
  ExplainMode => ExplainModev3_1,
  NormalMode => NormalModev3_1,
  ProfileMode => ProfileModev3_1,
  _
}
...
trait Compatibility {
    ...
    val preparedSyntacticQueryForV_3_1: Try[PreparedQuerySyntax] =
      Try(
        compiler.prepareSyntacticQuery(preParsedQuery.statement,
                                       ...
                                       Some(helpers.as3_1(preParsedQuery.offset)),
                                       tracer))
    new ParsedQuery {
        ...
        exceptionHandler.runSafely {
          ...
          val (planImpl, extractedParameters) = compiler
            .planPreparedQuery(syntacticQuery,
                               notificationLogger,
                               planContext,
                               Some(as3_1(preParsedQuery.offset)),
                               as3_1(tracer))

          // Log notifications/warnings from planning
          planImpl.notifications(planContext).foreach(notificationLogger += _)

          (new ExecutionPlanWrapper(planImpl, preParsingNotifications, as3_1(preParsedQuery.offset)),
           extractedParameters)
        }
  ...
  class ExecutionPlanWrapper(inner: ExecutionPlan_v3_1,
                             preParsingNotifications: Set[org.neo4j.graphdb.Notification],
                             offSet: frontend.v3_1.InputPosition)
    ...
    private def queryContext(transactionalContext: TransactionalContextWrapperV3_3) = {
      val ctx =
        new TransactionBoundQueryContext(TransactionalContextWrapperV3_1(transactionalContext.tc))(searchMonitor)

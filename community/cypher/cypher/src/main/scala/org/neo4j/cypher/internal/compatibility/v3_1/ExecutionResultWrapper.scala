...
class ExecutionResultWrapper(val inner: InternalExecutionResult,
  ...
  override def notifications: Iterable[Notification] =
    inner.notifications.map(asKernelNotification(offset)) ++ preParsingNotification
  ...
  override def accept[E <: Exception](visitor: QueryResult.QueryResultVisitor[E]): Unit =
    inner.accept(new InternalResultVisitor[E] {
      override def visit(internalResultRow: InternalResultRow): Boolean =
...
object ExecutionResultWrapper {
  ...
  def asKernelNotification(offset: Option[frontend.v3_1.InputPosition])(
    ...
    case IndexHintUnfulfillableNotification(label, propertyKey) =>
      NotificationCode.INDEX_HINT_UNFULFILLABLE
        .notification(graphdb.InputPosition.empty, NotificationDetail.Factory.index(label, propertyKey))

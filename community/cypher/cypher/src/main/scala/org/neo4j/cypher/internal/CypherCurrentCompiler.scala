  ...
  override def compile(query: InputQuery,
    ...
    def resolveParameterForManagementCommands(logicalPlan: LogicalPlan): LogicalPlan = {
      ...
      logicalPlan match {
        ...
        case c@CreateUser(_, _, Some(paramPassword), _, _, _) =>
          CreateUser(c.userName, Some(getParamValue(paramPassword)), None, c.requirePasswordChange, c.suspended, c.allowExistingUser)(new SequentialIdGen(c.id.x + 1))
  ...
  protected class CypherExecutableQuery(logicalPlan: LogicalPlan,
    ...
    private def innerExecute(transactionalContext: TransactionalContext,
      ...
      val inner = if (innerExecutionMode == ExplainMode) {
        ...
        val runtimeResult = executionPlan.run(queryContext, doProfile, params, prePopulateResults, input, subscriber)

...
object MatchPattern {
  ...
  def apply(patterns: Seq[Pattern]): MatchPattern =
    if (patterns.isEmpty) {
...
case class MatchPattern(nodes: Seq[String], relationships: Seq[MatchRelationship]) {
  ...
  def disconnectedPatternsWithout(variables: Seq[String]): Seq[MatchPattern] =
    disconnectedPatterns.filterNot(_.containsVariableNamed(variables)).filter(_.nonEmpty)
  ...
  def disconnectedPatterns: Seq[MatchPattern] =
    if (nodes.isEmpty) {
      Seq(this)
    } else {

...
import org.neo4j.cypher.internal.frontend.v3_3.ShortestPathCommonEndNodesForbiddenException
...
case class ShortestPathExpression(shortestPathPattern: ShortestPath,
    ...
    if (shortestPathPattern.single) {
      ...
      val result = state.query
        .allShortestPath(start.id(),
                         ...
                         nodePredicates)
        .filter { p =>
          shortestPathPattern.allowZeroLength || p.length() > 0
        }
        .map(AnyValues.asPathValue)
        .toArray
  ...
  private def getEndPoint(m: Map[String, AnyValue], start: SingleNode): NodeValue =
  ...
  def symbolTableDependencies =
    shortestPathPattern.symbolTableDependencies + shortestPathPattern.left.name + shortestPathPattern.right.name
  ...
  private def addAllOrNoneRelationshipExpander(ctx: ExecutionContext,
                                               ...
                                               all: Boolean,
                                               predicate: Predicate,
                                               relName: String)(implicit state: QueryState): Expander = {
    predicate match {
  ...
  private def addPredicates(ctx: ExecutionContext, relTypeAndDirExpander: Expander)(
      implicit state: QueryState): (Expander, Seq[KernelPredicate[PropertyContainer]]) =
        ...
        case ((currentExpander, currentNodePredicates: Seq[KernelPredicate[PropertyContainer]]), predicate) =>
          predicate match {
            case NoneInList(RelationshipFunction(_), symbolName, innerPredicate)
                if doesNotDependOnFullPath(innerPredicate) =>

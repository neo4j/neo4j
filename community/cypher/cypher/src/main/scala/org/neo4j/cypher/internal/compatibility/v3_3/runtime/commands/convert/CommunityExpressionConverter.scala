...
object CommunityExpressionConverter extends ExpressionConverter {
  ...
  override def toCommandExpression(expression: ast.Expression, self: ExpressionConverters): Option[CommandExpression] = {
    val result = expression match {
      ...
      case e: ast.NotEquals =>
        predicates.Not(predicates.Equals(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs)))
      ...
      case e: ast.Subtract =>
        commandexpressions.Subtract(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs))
      case e: ast.UnarySubtract =>
        commandexpressions.Subtract(commandexpressions.Literal(0), self.toCommandExpression(e.rhs))
      case e: ast.Multiply =>
        commandexpressions.Multiply(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs))
      ...
      case e: ast.PatternExpression =>
        ...
        commands.PathExpression(legacyPatterns,
                                predicates.True(),
                                PathExtractorExpression(legacyPatterns),
                                allowIntroducingNewIdentifiers = false)
      case e: ast.PatternComprehension =>
        commands.PathExpression(e.pattern.asLegacyPatterns(self),
                                self.toCommandPredicate(e.predicate),
                                self.toCommandExpression(e.projection),
                                allowIntroducingNewIdentifiers = true)
      case e: ast.ShortestPathExpression =>
        commandexpressions.ShortestPathExpression(e.pattern.asLegacyPatterns(None, self).head)
      ...
      case e: ast.ListSlice =>
        commandexpressions.ListSlice(self.toCommandExpression(e.list),
                                     toCommandExpression(e.from, self),
                                     toCommandExpression(e.to, self))
      ...
      case e: ast.FilterExpression =>
        commandexpressions.FilterFunction(self.toCommandExpression(e.expression),
                                          e.variable.name,
                                          e.innerPredicate.map(self.toCommandPredicate).getOrElse(predicates.True()))
      case e: ast.ExtractExpression =>
        commandexpressions.ExtractFunction(self.toCommandExpression(e.expression),
                                           e.variable.name,
                                           self.toCommandExpression(e.scope.extractExpression.get))
      case e: ast.ListComprehension => listComprehension(e, self)
      case e: ast.AllIterablePredicate =>
        commands.AllInList(self.toCommandExpression(e.expression),
                           e.variable.name,
                           e.innerPredicate.map(self.toCommandPredicate).getOrElse(predicates.True()))
      case e: ast.AnyIterablePredicate =>
        commands.AnyInList(self.toCommandExpression(e.expression),
                           e.variable.name,
                           e.innerPredicate.map(self.toCommandPredicate).getOrElse(predicates.True()))
      case e: ast.NoneIterablePredicate =>
        commands.NoneInList(self.toCommandExpression(e.expression),
                            e.variable.name,
                            e.innerPredicate.map(self.toCommandPredicate).getOrElse(predicates.True()))
      case e: ast.SingleIterablePredicate =>
        commands.SingleInList(self.toCommandExpression(e.expression),
                              e.variable.name,
                              e.innerPredicate.map(self.toCommandPredicate).getOrElse(predicates.True()))
      case e: ast.ReduceExpression =>
        commandexpressions.ReduceFunction(self.toCommandExpression(e.list),
                                          e.variable.name,
                                          self.toCommandExpression(e.expression),
                                          e.accumulator.name,
                                          self.toCommandExpression(e.init))
      case e: ast.PathExpression => self.toCommandProjectedPath(e)
      case e: pipes.NestedPipeExpression =>
        commandexpressions.NestedPipeExpression(e.pipe, self.toCommandExpression(e.projection))
      case e: ast.GetDegree => getDegree(e, self)
      case e: PrefixSeekRangeWrapper =>
        commandexpressions.PrefixSeekRangeExpression(e.range.map(self.toCommandExpression))
      ...
      case e: ast.AndedPropertyInequalities =>
        predicates.AndedPropertyComparablePredicates(variable(e.variable),
                                                     toCommandProperty(e.property, self),
                                                     e.inequalities.map(e => inequalityExpression(e, self)))
      case e: DesugaredMapProjection =>
        commandexpressions.DesugaredMapProjection(e.name.name, e.includeAllProps, mapProjectionItems(e.items, self))
      case e: ResolvedFunctionInvocation =>
        val callArgumentCommands = e.callArguments
          .map(Some(_))
          .zipAll(e.fcnSignature.get.inputSignature.map(_.default.map(_.value)), None, None)
          .map {
            case (given, default) =>
              given.map(self.toCommandExpression).getOrElse(commandexpressions.Literal(default.get))
          }
  ...
  private def toCommandExpression(expression: ast.Function,
    ...
    expression match {
      ...
      case EndNode =>
        commandexpressions.RelationshipEndPoints(self.toCommandExpression(invocation.arguments.head), start = false)
      case Exists =>
        invocation.arguments.head match {
          case property: ast.Property =>
            commands.predicates
              .PropertyExists(self.toCommandExpression(property.map), PropertyKey(property.propertyKey.name))
  ...
  private def toCommandExpression(expression: Option[ast.Expression],
                                  self: ExpressionConverters): Option[CommandExpression] =
    expression.map(self.toCommandExpression)

  private def toCommandExpression(expressions: Seq[ast.Expression],
  ...
  private def inequalityExpression(original: ast.InequalityExpression,
    ...
    case e: ast.LessThanOrEqual =>
      predicates.LessThanOrEqual(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs))
    case e: ast.GreaterThan => predicates.GreaterThan(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs))
    case e: ast.GreaterThanOrEqual =>
      predicates.GreaterThanOrEqual(self.toCommandExpression(e.lhs), self.toCommandExpression(e.rhs))
  }
  ...
  private def caseExpression(e: ast.CaseExpression, self: ExpressionConverters) = e.expression match {
    case Some(innerExpression) =>
      val legacyAlternatives = e.alternatives.map { a =>
        (self.toCommandExpression(a._1), self.toCommandExpression(a._2))
      }
      commandexpressions
        .SimpleCase(self.toCommandExpression(innerExpression), legacyAlternatives, toCommandExpression(e.default, self))
    case None =>
      val predicateAlternatives = e.alternatives.map { a =>
        (self.toCommandPredicate(a._1), self.toCommandExpression(a._2))
      }
      commandexpressions.GenericCase(predicateAlternatives, toCommandExpression(e.default, self))
  }

  private def hasLabels(e: ast.HasLabels, self: ExpressionConverters) =
    e.labels.map { l =>
      predicates.HasLabel(self.toCommandExpression(e.expression),
                          commandvalues.KeyToken.Unresolved(l.name, commandvalues.TokenType.Label)): Predicate
    } reduceLeft {
      predicates.And(_, _)
    }

  private def mapItems(items: Seq[(PropertyKeyName, Expression)],
  ...
  private def listComprehension(e: ast.ListComprehension, self: ExpressionConverters): CommandExpression = {
    val filter = e.innerPredicate match {
      ...
      case Some(inner) =>
        commandexpressions
          .FilterFunction(self.toCommandExpression(e.expression), e.variable.name, self.toCommandPredicate(inner))
    }

...
import org.neo4j.graphdb.impl.notification.NotificationCode.CREATE_UNIQUE_UNAVAILABLE_FALLBACK
import org.neo4j.graphdb.impl.notification.NotificationCode.RULE_PLANNER_UNAVAILABLE_FALLBACK
import org.neo4j.graphdb.impl.notification.NotificationCode.START_DEPRECATED
import org.neo4j.graphdb.impl.notification.NotificationCode.START_UNAVAILABLE_FALLBACK
...
case class PreParsedQuery(statement: String,
                          ...
                          debugOptions: Set[String])(val offset: InputPosition) {
...
class CompilerEngineDelegator(graph: GraphDatabaseQueryService,
  ...
  private final val ILLEGAL_PLANNER_RUNTIME_COMBINATIONS: Set[(CypherPlanner, CypherRuntime)] = Set(
    (CypherPlanner.rule, CypherRuntime.compiled))
  ...
  def preParseQuery(queryText: String): PreParsedQuery = exceptionHandler.runSafely {
    ...
    val pickedPlanner =
      pick(planner, CypherPlanner, if (cypherVersion == configuredVersion) Some(configuredPlanner) else None)
    val pickedRuntime =
      pick(runtime, CypherRuntime, if (cypherVersion == configuredVersion) Some(configuredRuntime) else None)
    val pickedUpdateStrategy = pick(updateStrategy, CypherUpdateStrategy, None)

    assertValidOptions(CypherStatementWithOptions(preParsedStatement),
                       cypherVersion,
                       pickedExecutionMode,
                       pickedPlanner,
                       pickedRuntime)
    ...
    PreParsedQuery(statement,
                   ...
                   pickedPlanner,
                   pickedRuntime,
                   pickedUpdateStrategy,
                   debugOptions)(offset)
  }

  private def pick[O <: CypherOption](candidate: Option[O],
                                      companion: CypherOptionCompanion[O],
                                      configured: Option[O]): O = {
  ...
  private def assertValidOptions(statementWithOption: CypherStatementWithOptions,
    ...
    if (ILLEGAL_PLANNER_RUNTIME_COMBINATIONS((planner, runtime)))
      throw new InvalidArgumentException(
        s"Unsupported PLANNER - RUNTIME combination: ${planner.name} - ${runtime.name}")
  }
  ...
  private def getStatisticsDivergenceThreshold: Double = {
    val setting: (Config) => Double = config =>
      config.get(GraphDatabaseSettings.query_statistics_divergence_threshold).doubleValue()
  ...
  private def getMinimumTimeBeforeReplanning: Long = {
    val setting: (Config) => Long = config =>
      config.get(GraphDatabaseSettings.cypher_min_replan_interval).toMillis.longValue()

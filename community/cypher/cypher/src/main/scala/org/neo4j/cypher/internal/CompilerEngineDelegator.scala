...
class CompilerEngineDelegator(graph: GraphDatabaseQueryService,
    ...
    def planForVersion(input: Either[CypherVersion, ParsedQuery]): Either[CypherVersion, ParsedQuery] = input match {
      ...
      case Left(CypherVersion.v3_4) =>
        ...
        parserQuery.onError {
          ...
          case ex: apa.v3_4.SyntaxException if ex.getMessage.startsWith("CREATE UNIQUE") =>
            preParsingNotifications = preParsingNotifications +
              createUniqueNotification(ex, preParsedQuery)
            Left(CypherVersion.v3_1)
          case ex: apa.v3_4.SyntaxException if ex.getMessage.startsWith("START is deprecated") =>
            preParsingNotifications = preParsingNotifications +
  ...
  private def createStartUnavailableNotification(ex: apa.v3_4.SyntaxException, preParsedQuery: PreParsedQuery) = {
    val pos = convertInputPosition(ex.pos.getOrElse(preParsedQuery.offset))

    START_UNAVAILABLE_FALLBACK.notification(pos)
  }

  private def createStartDeprecatedNotification(ex: apa.v3_4.SyntaxException, preParsedQuery: PreParsedQuery) = {
    val pos = convertInputPosition(ex.pos.getOrElse(preParsedQuery.offset))
    START_DEPRECATED.notification(pos, startDeprecated(ex.getMessage))
  }

  private def createUniqueNotification(ex: apa.v3_4.SyntaxException, preParsedQuery: PreParsedQuery) = {
    val pos = convertInputPosition(ex.pos.getOrElse(preParsedQuery.offset))

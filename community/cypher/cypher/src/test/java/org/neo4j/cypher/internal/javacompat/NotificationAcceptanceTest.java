...
public class NotificationAcceptanceTest
{
    ...
    private Stream<String> allVersionStreams()
    {
//      return versionStream( "CYPHER 2.3", "CYPHER 3.1", "CYPHER 3.2", "CYPHER 3.3" );
      return Stream.of( "CYPHER 3.3" );
    }

    private Stream<String> version3Streams()
    {
//      return versionStream( "CYPHER 3.1", "CYPHER 3.2", "CYPHER 3.3" );
      return Stream.of( "CYPHER 3.3" );
    }
 /*
    @Test
...
*/
    ...
    {
        version3Streams().forEach( version ->
        {
            // pattern
    ...
    public void shouldNotNotifyUsingJoinHintWithCost() throws Exception
    {
        ...
        allVersionStreams().forEach( version ->
        {
    ...
    {
        version3Streams().forEach( version -> shouldNotNotifyInStream( version, " explain return { id: 42 } " ) );
    }
    ...
    {
        version3Streams().forEach( version -> shouldNotNotifyInStream( version,
    ...
    {
        version3Streams().forEach( version -> shouldNotifyInStream( version,
...
*/
        Stream.of( "CYPHER 3.3" ).forEach( version -> assertNotifications( version + "explain CALL changedProc() YIELD oldField RETURN oldField",
...
//        Stream.of( "CYPHER 3.2", "CYPHER 3.3" ).forEach( version ->
        Stream.of( "CYPHER 3.3" ).forEach( version ->
        {
            db().execute( "CREATE INDEX ON :Person(name)" );
            db().execute( "Call db.awaitIndexes()" );
...
*/
    ...
    private Matcher<Notification> deprecatedStartWarning = notification( "Neo.ClientNotification.Statement.FeatureDeprecationWarning",
            containsString( "START is deprecated, use: `MATCH (n)` instead." ), any( InputPosition.class ), SeverityLevel.WARNING );

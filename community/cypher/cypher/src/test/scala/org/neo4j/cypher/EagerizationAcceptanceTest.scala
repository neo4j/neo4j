...
class EagerizationAcceptanceTest
  ...
  test("should plan eagerness for detach delete on paths") {
    ...
    assertStats(result, relationshipsDeleted = 2, nodesDeleted = 2)
    assertNumberOfEagerness(query, 1)
  }
  ...
  test("should introduce eagerness between MATCH and DELETE directed path") {
    val a = createNode()
    val b = createNode()
    relate(a, b)
    relate(a, b)
  ...
  test("should introduce eagerness between MATCH, UNWIND, DELETE nodes in directed path") {
    val a = createNode()
    val b = createNode()
    relate(a, b)
    relate(a, b)
...
//    assertNumberOfEagerness(query, 1) -- not with rule planner
  ...
  test("should be eager between conflicting read and write inside FOREACH") {
    ...
    val query = "MATCH (a), (b) FOREACH(i in range(0, 1) | DELETE b) RETURN count(*)"

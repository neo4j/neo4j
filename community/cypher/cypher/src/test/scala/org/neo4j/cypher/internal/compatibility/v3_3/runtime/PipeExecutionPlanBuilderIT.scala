...
class PipeExecutionPlanBuilderIT extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  private val planBuilder = {
    val converters = new ExpressionConverters(CommunityExpressionConverter)
    new PipeExecutionPlanBuilder(Clock.systemUTC(),
                                 monitors,
  ...
  test("projection only query") {
    val logicalPlan =
      Projection(SingleRow()(solvedWithEstimation(1.0)), Map("42" -> SignedDecimalIntegerLiteral("42")(pos))) _
  ...
  test("simple relationship by id seek query with multiple values") {
    val astCollection: Expression =
      ListLiteral(
        Seq(SignedDecimalIntegerLiteral("42") _,
            ...
            SignedDecimalIntegerLiteral("43") _)) _
    ...
    pipeInfo.pipe should equal(
      DirectedRelationshipByIdSeekPipe("r",
                                       ManySeekArgs(converters.toCommandExpression(astCollection)),
                                       toNode,
                                       fromNode)())
  }
  ...
  test("simple undirected relationship by id seek query with multiple values") {
    ...
    val fromNode = "from"
    val toNode = "to"
    val logicalPlan = UndirectedRelationshipByIdSeek(IdName("r"),
                                                     ManySeekableArgs(astCollection),
                                                     IdName(fromNode),
                                                     IdName(toNode),
                                                     Set.empty) _
    ...
    pipeInfo.periodicCommit should equal(None)
    pipeInfo.pipe should equal(
      UndirectedRelationshipByIdSeekPipe("r",
                                         ManySeekArgs(converters.toCommandExpression(astCollection)),
                                         toNode,
                                         fromNode)())
  }
  ...
  test("simple expand into existing variable MATCH a-[r]->a ") {
    val logicalPlan =
      Expand(AllNodesScan("a", Set.empty)(solved), "a", SemanticDirection.INCOMING, Seq(), "a", "r", ExpandInto) _
    val pipeInfo = build(logicalPlan)

    val inner: Pipe =
      ExpandIntoPipe(AllNodesScanPipe("a")(), "a", "r", "a", SemanticDirection.INCOMING, LazyTypes.empty)()

    pipeInfo.pipe should equal(inner)
  }

  test("optional expand into existing variable MATCH a OPTIONAL MATCH a-[r]->a ") {
    val logicalPlan = OptionalExpand(AllNodesScan("a", Set.empty)(solved),
                                     "a",
    ...
    pipeInfo.pipe should equal(
      OptionalExpandIntoPipe(AllNodesScanPipe("a")(),
                             "a",
                             "r",
                             "a",
                             SemanticDirection.INCOMING,
  ...
  test("Aggregation on top of Projection => DistinctPipe with resolved expressions") {
    ...
    pipe should equal(
      DistinctPipe(
        AllNodesScanPipe("n")(),
        Map("n.prop" -> legacy.Property(legacy.Variable("n"), Resolved("prop", token, TokenType.PropertyKey))))())
  }

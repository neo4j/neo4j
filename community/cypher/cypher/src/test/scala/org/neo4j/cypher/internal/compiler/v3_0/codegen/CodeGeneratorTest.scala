...
class CodeGeneratorTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  val plan = ProduceResult(List("a", "b"),
                             OptionalExpand(
                               ...
                               Seq(HasLabels(variable("b"), Seq(LabelName("T1")(pos)))(pos)))(solved))
  ...
  test("optional expand into on top of expand all") {
    ...
    val optionalExpandInto = OptionalExpand(
      ...
      Seq.empty, IdName("b"), IdName("r2"), ExpandInto, Seq(HasLabels(variable("b"), Seq(LabelName("T2")(pos)))(pos)))(solved)
  ...
  test("optional expand into on top of expand all with a loop") {
    ...
    val expandInto = OptionalExpand(
      ...
      Seq(HasLabels(variable("b"), Seq(LabelName("T2")(pos)))(pos)))(solved)
  ...
  test("hash join on top of hash join") {
    ...
    val projection = Projection(join2, Map("a" -> variable("a")))(solved)
  ...
  when(semanticTable.isNode(variable("a"))).thenReturn(true)
  when(semanticTable.isNode(variable("b"))).thenReturn(true)
  when(semanticTable.isNode(variable("c"))).thenReturn(true)
  when(semanticTable.isNode(variable("d"))).thenReturn(true)
  when(semanticTable.isNode(variable("e"))).thenReturn(true)
  when(semanticTable.isNode(variable("f"))).thenReturn(true)
  when(semanticTable.isNode(variable("g"))).thenReturn(true)
  when(semanticTable.isNode(variable("h"))).thenReturn(true)
  when(semanticTable.isNode(variable("i"))).thenReturn(true)
  when(semanticTable.isRelationship(variable("r1"))).thenReturn(true)
  when(semanticTable.isRelationship(variable("r2"))).thenReturn(true)

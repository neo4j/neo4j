...
object ValueComparisonHelper {
  ...
  def beEquivalentTo(result: Seq[Map[String, Any]]) = new Matcher[Seq[ExecutionContext]] {
    override def apply(left: Seq[ExecutionContext]): MatchResult =
      MatchResult(
        matches = left.indices.forall(
          i =>
  ...
  def beEquivalentTo(value: Any) = new Matcher[AnyValue] {
    override def apply(left: AnyValue): MatchResult =
      MatchResult(matches = check(left, value),

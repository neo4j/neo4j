...
RETURN a.name""")
  ...
  test("MATCH foo RETURN foo.bar AS x ORDER BY x") {
    createNode("bar" -> 1)
    createNode("bar" -> 3)
    createNode("bar" -> 2)
  ...
  test("MATCH (foo:Bar) RETURN foo.bar AS x ORDER BY x LIMIT 1") {
    createNode("bar" -> 1)
    createNode("bar" -> 3)
    createNode("bar" -> 2)

    // when
    val result = executeWithNewPlanner("MATCH foo RETURN foo.bar AS x ORDER BY x DESC")

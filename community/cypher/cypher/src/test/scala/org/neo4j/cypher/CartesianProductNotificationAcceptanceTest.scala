...
import org.neo4j.cypher.internal.{CartesianProductNotification, NormalMode}
...
class CartesianProductNotificationAcceptanceTest extends CypherFunSuite with GraphDatabaseTestSupport {
  ...
  test("should warn when disconnected patterns") {
    ...
    //then
    verify(logger, times(1)).log(CartesianProductNotification(InputPosition(7, 1, 8)))
  }
  ...
  test("should warn when one disconnected pattern in otherwise connected pattern") {
    ...
    //then
    verify(logger, times(1)).log(CartesianProductNotification(InputPosition(29, 1, 30)))
  }
  ...
  private def createCompiler(notificationLogger: NotificationLogger) =
    CypherCompilerFactory.costBasedCompiler(
      graph, 128, 0.5, 1000L, Clock.SYSTEM_CLOCK, kernelMonitors, DEV_NULL, _ => notificationLogger, plannerName = CostPlannerName)

...
class NodeOuterHashJoinPipeTest extends CypherFunSuite {
  ...
  test("should support simple hash join over nodes") {
    ...
    val left = newMockedPipe("b", row("b" -> fromNodeProxy(node1)), row("b" -> fromNodeProxy(node2)))
  ...
  test("null in both sides should still not match anything") {
    ...
    val left = newMockedPipe("b", row("b" -> NO_VALUE, "a" -> intValue(20)))

    val right = newMockedPipe("b", row("b" -> NO_VALUE, "c" -> intValue(20)))

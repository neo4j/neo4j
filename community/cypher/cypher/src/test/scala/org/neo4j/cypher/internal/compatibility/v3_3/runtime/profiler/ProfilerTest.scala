...
class ProfilerTest extends CypherFunSuite {
  ...
  test("report page cache statistics for simplest case") {
    ...
    //THEN
    assertRecorded(decoratedResult,
                   "foo",
                   expectedRows = 10,
                   expectedDbHits = 20,
  ...
  private def createPlanDescription(first: (String, Pipe), tail: (String, Pipe)*): InternalPlanDescription = {
    val firstDescr: InternalPlanDescription =
      PlanDescriptionImpl(first._2.id, first._1, NoChildren, Seq.empty, Set.empty)
  ...
  test("should count stuff going through Apply multiple times") {
    ...
    val queryState = QueryStateHelper.emptyWith(query = queryContext, decorator = profiler)
    val planDescription =
      createPlanDescription("start1" -> s1, "start2" -> s2, "lhs" -> lhs, "rhs" -> rhs, "apply" -> apply)
  ...
  test("count dbhits for NestedPipes") {
    ...
    val profiler = new Profiler
    val queryState = QueryStateHelper.emptyWith(query = queryContext, decorator = profiler)
    val planDescription = createPlanDescription("start1" -> start1,
                                                "start2" -> start2,
                                                "lhs" -> testPipe,
  ...
  test("count page cache hits for NestedPipes") {
    ...
    val statisticProvider = new ConfiguredKernelStatisticProvider()
    val testPipe =
      ProfilerTestPipe(start1, "nested pipe", rows = 10, dbAccess = 2, statisticProvider, hits = 3, misses = 4)
    val innerPipe = NestedPipeExpression(testPipe, projectedPath)
    val start2 = SingleRowPipe()()
    val pipeUnderInspection = ProjectionPipe(start2, Map("x" -> innerPipe))()
  ...
  private def assertRecorded(result: InternalPlanDescription,
                             name: String,
                             expectedRows: Int,
                             expectedDbHits: Int,
...
case class ProfilerTestPipe(source: Pipe,
                            name: String,
                            rows: Int,
                            dbAccess: Int,

...
class LoadCsvPeriodicCommitObserverTest extends CypherFunSuite {
  ...
  test("should not trigger tx restart until after first batch has been processed") {
    // Given
    when(resource.getCsvIterator(Matchers.eq(url), any(), anyBoolean(), anyInt(), anyBoolean())).thenReturn(Iterator(
      Array("Row1"),
      Array("Row2")))
  ...
  test("headers should not count") {
    // given
    when(resource.getCsvIterator(Matchers.eq(url), any(), anyBoolean(), anyInt(), Matchers.eq(true))).thenReturn(Iterator(
      Array("header"),
      Array("Row1"),
      Array("Row2"),
      Array("Row3")))
  ...
  test("multiple iterators are still handled correctly only commit when the first iterator advances") {
    // Given
    when(resource.getCsvIterator(Matchers.eq(url), any(), anyBoolean(), anyInt(), anyBoolean())).

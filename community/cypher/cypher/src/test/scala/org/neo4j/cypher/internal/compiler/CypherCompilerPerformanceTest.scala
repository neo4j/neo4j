...
class CypherCompilerPerformanceTest extends GraphDatabaseFunSuite {
  ...
  val nexlp1 = "nexlp1" ->
    """match
      |  (toFrom:Entity{Id:{eId}}),
      |  segment-[:precedes|is]->startSegment,
      |  toFrom-[toFrom_rel:read|wrote]->startSegment
      |with segment
      |match segment-[:regarding_summary_phrase]->entity
      |with segment, entity match checkIgnoredWriter-[:wrote]->segment
      ...
      |order by SegmentCount desc skip 0 limit 51""".stripMargin

  val nexlp2 = "nexlp2" ->
    """match
      |  (toFrom:Entity{Id:{eId}}),
      |  toFrom-[toFrom_rel:read|wrote]->(segment:Segment),
      |  copy-[:of]->document-[documentSegment:containing]->segment,
      |  segment-[:in_thread]->(thread:Thread),
      |  (checkIgnoredWriter:Entity)-[:wrote]->segment
  ...
  test("plans are built fast enough") {
    val queries = Table("query", nexlp1, nexlp2, socnet1, socnet9, qmul1)
    plan(10)(warmup)

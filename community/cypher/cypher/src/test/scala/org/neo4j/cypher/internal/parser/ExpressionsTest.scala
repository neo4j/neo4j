...
class ExpressionsTest extends ParserTest[ast.Expression, legacy.Expression] with Expressions {
  ...
  test("simple_cases") {
    ...
    parsing("""CASE 1
           ...
           WHEN 2 THEN 'TWO'
         END""") shouldGive
      legacy.SimpleCase(legacy.Literal(1),
                        Seq((legacy.Literal(1), legacy.Literal("ONE")), (legacy.Literal(2), legacy.Literal("TWO"))),
                        None)
  ...
  test("array_indexing") {
    ...
    parsing("[1,2,3,4][1..2][2..3]") shouldGive
      legacy.ListSlice(legacy.ListSlice(collection, Some(legacy.Literal(1)), Some(legacy.Literal(2))),
                       Some(legacy.Literal(2)),
                       Some(legacy.Literal(3)))
    ...
    parsing("[[1,2]][0][6]") shouldGive
      legacy.ContainerIndex(
        legacy.ContainerIndex(legacy.ListLiteral(legacy.ListLiteral(legacy.Literal(1), legacy.Literal(2))),
                              legacy.Literal(0)),
        legacy.Literal(6))
  ...
  test("better_map_support") {
    parsing("map.key1.key2.key3") shouldGive
      legacy.Property(
        legacy.Property(legacy.Property(legacy.Variable("map"), PropertyKey("key1")), PropertyKey("key2")),
        PropertyKey("key3"))
    ...
    parsing("({ inner1: { inner2: 'Value' } }).key") shouldGive
      legacy.Property(legacy.LiteralMap(Map("inner1" -> legacy.LiteralMap(Map("inner2" -> legacy.Literal("Value"))))),
                      PropertyKey("key"))

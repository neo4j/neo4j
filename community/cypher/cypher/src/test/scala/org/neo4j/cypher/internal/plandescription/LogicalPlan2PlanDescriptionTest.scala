...
class LogicalPlan2PlanDescriptionTest extends CypherFunSuite with TableDrivenPropertyChecks {
  ...
  test("NodeByLabelScan") {
    ...
    assertGood(
      ...
      planDescription(id, "NodeByLabelScan", NoChildren, Seq(details("anon_123:X")), Set("anon_123")))
  }
  ...
  test("NodeByIdSeek") {
    ...
    assertGood(
      ...
      planDescription(id, "NodeByIdSeek", NoChildren, Seq(details("anon_11 WHERE id(anon_11) IN [1,32]")), Set("anon_11")))
  }
  ...
  test("RelationshipByIdSeek") {
    ...
    assertGood(attach(UndirectedRelationshipByIdSeek("  REL2", ManySeekableArgs(number("1")), "a", "  NODE32", Set("x")), 70.0),
      planDescription(id, "UndirectedRelationshipByIdSeek", NoChildren, Seq(details("(a)-[anon_2]-(anon_32) WHERE id(anon_2) = 1")), Set("anon_2", "a", "anon_32", "x")))
  }
  ...
  test("LoadCSV") {
    ...
    assertGood(
      ...
      planDescription(id, "LoadCSV", SingleChild(lhsPD), Seq(details("anon_2")), Set("anon_2", "a")))
  }
  ...
  test("NodeCountFromCountStore") {
    ...
    assertGood(attach(NodeCountFromCountStore("  UNNAMED123", List(Some(label("LabelName"))), Set.empty), 54.2),
      planDescription(id, "NodeCountFromCountStore", NoChildren, Seq(details("count( (:LabelName) ) AS anon_123")), Set("anon_123")))
  }
  ...
  test("RelationshipCountFromCountStore") {
    ...
    assertGood(attach(RelationshipCountFromCountStore("  UNNAMED122", None, Seq(RelTypeName("LIKES")(pos)), None, Set.empty), 54.2),
      planDescription(id, "RelationshipCountFromCountStore", NoChildren, Seq(details("count( ()-[:LIKES]->() ) AS anon_122")), Set("anon_122")))
  ...
  test("CreateUniquePropertyConstraint") {
    assertGood(attach(CreateUniquePropertyConstraint(" x", label("Label"), Seq(prop(" x", "prop")), None), 63.2),
      planDescription(id, "CreateUniquePropertyConstraint", NoChildren, Seq(details("CONSTRAINT ON (` x`:Label) ASSERT (` x`.prop) IS UNIQUE")), Set.empty))

    assertGood(attach(CreateUniquePropertyConstraint("x", label("Label"), Seq(prop("x", "prop")), Some("constraintName")), 63.2),
      planDescription(id, "CreateUniquePropertyConstraint", NoChildren, Seq(details("CONSTRAINT constraintName ON (x:Label) ASSERT (x.prop) IS UNIQUE")), Set.empty))

    assertGood(attach(CreateUniquePropertyConstraint("x", label("Label"), Seq(prop("x", "prop1"), prop("x", "prop2")), Some("constraintName")), 63.2),
      planDescription(id, "CreateUniquePropertyConstraint", NoChildren, Seq(details("CONSTRAINT constraintName ON (x:Label) ASSERT (x.prop1, x.prop2) IS UNIQUE")), Set.empty))
  }

  test("CreateNodeKeyConstraint") {
    assertGood(attach(CreateNodeKeyConstraint(" x", label("Label"), Seq(prop(" x", "prop")), None), 63.2),
      planDescription(id, "CreateNodeKeyConstraint", NoChildren, Seq(details("CONSTRAINT ON (` x`:Label) ASSERT (` x`.prop) IS NODE KEY")), Set.empty))

    assertGood(attach(CreateNodeKeyConstraint("x", label("Label"), Seq(prop("x", "prop1"), prop("x", "prop2")), Some("constraintName")), 63.2),
      planDescription(id, "CreateNodeKeyConstraint", NoChildren, Seq(details("CONSTRAINT constraintName ON (x:Label) ASSERT (x.prop1, x.prop2) IS NODE KEY")), Set.empty))
  }
  ...
  test("CreateNodePropertyExistenceConstraint") {
    assertGood(attach(CreateNodePropertyExistenceConstraint(label("Label"), prop(" x", "prop"), None), 63.2),
      planDescription(id, "CreateNodePropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT ON (` x`:Label) ASSERT exists(` x`.prop)")), Set.empty))

    assertGood(attach(CreateNodePropertyExistenceConstraint(label("Label"), prop("x","prop"), Some("constraintName")), 63.2),
      planDescription(id, "CreateNodePropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT constraintName ON (x:Label) ASSERT exists(x.prop)")), Set.empty))
  }
  ...
  test("CreateRelationshipPropertyExistenceConstraint") {
    assertGood(attach(CreateRelationshipPropertyExistenceConstraint(RelTypeName("R")(pos), prop(" x", "prop"), None), 63.2),
      planDescription(id, "CreateRelationshipPropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT ON ()-[` x`:R]-() ASSERT exists(` x`.prop)")), Set.empty))

    assertGood(attach(CreateRelationshipPropertyExistenceConstraint(RelTypeName("R")(pos), prop(" x", "prop"), Some("constraintName")), 63.2),
      planDescription(id, "CreateRelationshipPropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT constraintName ON ()-[` x`:R]-() ASSERT exists(` x`.prop)")), Set.empty))
  }
  ...
  test("DropUniquePropertyConstraint") {
    assertGood(attach(DropUniquePropertyConstraint(LabelName("Label")(pos), Seq(prop(" x", "prop"), prop(" x", "prop2"))), 63.2),
      planDescription(id, "DropUniquePropertyConstraint", NoChildren, Seq(details("CONSTRAINT ON (` x`:Label) ASSERT (` x`.prop, ` x`.prop2) IS UNIQUE")), Set.empty))
  }

  test("DropNodeKeyConstraint") {
    assertGood(attach(DropNodeKeyConstraint(LabelName("Label")(pos), Seq(prop(" x", "prop"), prop(" x", "prop2"))), 63.2),
      planDescription(id, "DropNodeKeyConstraint", NoChildren, Seq(details("CONSTRAINT ON (` x`:Label) ASSERT (` x`.prop, ` x`.prop2) IS NODE KEY")), Set.empty))
  }
  ...
  test("DropNodePropertyExistenceConstraint") {
    assertGood(attach(DropNodePropertyExistenceConstraint(LabelName("Label")(pos), prop("x", " prop")), 63.2),
      planDescription(id, "DropNodePropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT ON (x:Label) ASSERT exists(x.` prop`)")), Set.empty))
  }
  ...
  test("DropRelationshipPropertyExistenceConstraint") {
    assertGood(attach(DropRelationshipPropertyExistenceConstraint(RelTypeName("$R")(pos), prop("x", " prop")), 63.2),
      planDescription(id, "DropRelationshipPropertyExistenceConstraint", NoChildren, Seq(details("CONSTRAINT ON ()-[x:`$R`]-() ASSERT exists(x.` prop`)")), Set.empty))
  }
  ...
  test("Create") {
    ...
    assertGood(
      ...
      planDescription(id, "Create", SingleChild(lhsPD), Seq(details(Seq("(x:Label)", "(x)<-[anon_67:R]-(y)"))), Set("a", "x", "anon_67")))
  ...
  test("Expand") {
    assertGood(attach(Expand(lhsLP, "a", OUTGOING, Seq.empty, "  UNNAMED4", "r1", ExpandAll), 95.0),
      planDescription(id, "Expand(All)", SingleChild(lhsPD), Seq(details("(a)-[r1]->(anon_4)")), Set("a", "anon_4", "r1")))
    ...
    assertGood(attach(Expand(lhsLP, "a", BOTH, Seq(RelTypeName("R1")(pos), RelTypeName("R2")(pos)), "y", "  UNNAMED1", ExpandInto), 113.0),
      planDescription(id, "Expand(Into)", SingleChild(lhsPD), Seq(details("(a)-[anon_1:R1|R2]-(y)")), Set("a", "y", "anon_1")))
  }
  ...
  test("OptionalExpand") {
    ...
    assertGood(attach(OptionalExpand(lhsLP, "a", INCOMING, Seq(RelTypeName("R")(pos)), "  NODE5", "r", ExpandAll, None), 12.0),
      planDescription(id, "OptionalExpand(All)", SingleChild(lhsPD), Seq(details("(a)<-[r:R]-(anon_5)")), Set("a", "anon_5", "r")))
  ...
  test("Projection") {
    ...
    assertGood(attach(Projection(lhsLP, Map("x" -> varFor("  REL42"), "n.prop" -> prop("n", "prop"))), 2345.0),
      planDescription(id, "Projection", SingleChild(lhsPD), Seq(details(Seq("anon_42 AS x", "n.prop AS `n.prop`"))), Set("a", "x", "`n.prop`")))
  ...
  test("FindShortestPaths") {
    ...
    assertGood(attach(FindShortestPaths(lhsLP, ShortestPathPattern(None, PatternRelationship("r", ("  NODE23", "y"), SemanticDirection.BOTH, Seq.empty, VarPatternLength(2, Some(4))), single = true)(null), Seq.empty), 2345.0),
      planDescription(id, "ShortestPath", SingleChild(lhsPD), Seq(details("(anon_23)-[r*2..4]-(y)")), Set("r", "a", "anon_23", "y")))
    ...
    assertGood(attach(FindShortestPaths(lhsLP, ShortestPathPattern(Some("  UNNAMED12"), PatternRelationship("r", ("a", "y"), SemanticDirection.BOTH, Seq(RelTypeName("R")(pos)), VarPatternLength(2, Some(4))), single = true)(null), Seq(predicate)), 2345.0),
      planDescription(id, "ShortestPath", SingleChild(lhsPD), Seq(details("anon_12 = (a)-[r:R*2..4]-(y) WHERE r.prop = 32")), Set("r", "a", "y", "anon_12")))
    ...
    assertGood(attach(FindShortestPaths(lhsLP, ShortestPathPattern(None, PatternRelationship("r", ("a", "  UNNAMED2"), SemanticDirection.BOTH, Seq(RelTypeName("R")(pos)), VarPatternLength(2, None)), single = true)(null), Seq(predicate)), 2345.0),
      planDescription(id, "ShortestPath", SingleChild(lhsPD), Seq(details("(a)-[r:R*2..]-(anon_2) WHERE r.prop = 32")), Set("r", "a", "anon_2")))
  }
  ...
  test("MergeCreate") {
    ...
    assertGood(attach(MergeCreateRelationship(lhsLP, "r", "x", RelTypeName("R")(pos), "  NODE90", Some(properties)), 113.0),
      planDescription(id, "MergeCreateRelationship", SingleChild(lhsPD), Seq(details("(x)-[r:R]->(anon_90)")), Set("a", "r", "x", "anon_90")))
  }
  ...
  test("VarExpand") {
    ...
    assertGood(attach(VarExpand(lhsLP, "a", INCOMING, INCOMING, Seq(RelTypeName("LIKES")(pos), RelTypeName("LOVES")(pos)), "  UNNAMED123", "  UNNAMED99", VarPatternLength(1, Some(1)), ExpandAll), 1.0),
      planDescription(id, "VarLengthExpand(All)", SingleChild(lhsPD), Seq(details("(a)<-[anon_99:LIKES|LOVES]-(anon_123)")), Set("a", "anon_99", "anon_123")))

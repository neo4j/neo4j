...
class CypherPreParserTest extends CypherFunSuite with TableDrivenPropertyChecks {
  ...
  val queries = Table(
    ...
    ("CYPHER runtime=compiledExperimentalFeatureNotSupportedForProductionUse RETURN", PreParsedStatement("RETURN", Seq(ConfigurationOptions(None, Seq(CompiledRuntimeOption))), (1, 72, 71))),

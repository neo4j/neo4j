...
class OptionalExpandIntoPipeTest extends CypherFunSuite {
  ...
  test("should support expand between two nodes with a relationship") {
    ...
    // when
    val result = OptionalExpandIntoPipe(left, "a", "r", "b", SemanticDirection.OUTGOING, LazyTypes.empty, True())()
  ...
  test("should find null when two nodes have no shared relationships but do have some rels") {
    // given
    mockRelationships(relationship1)
    val left = newMockedPipe("a", row("a" -> startNode, "b" -> endNode2))
  ...
  test("should support optional expand from a node with relationships that do not match the predicates") {
    ...
    // when
    val result = OptionalExpandIntoPipe(left,
                                        "a",
                                        "r",
                                        "b",
  ...
  test("expand into null should return nulled row") {
    ...
    val result: List[ExecutionContext] = OptionalExpandIntoPipe(input,
                                                                ...
                                                                SemanticDirection.OUTGOING,
                                                                LazyTypes.empty,
                                                                True())().createResults(queryState).toList
  ...
  test("expand into should handle multiple relationships between the same node") {
    ...
    when(predicate.isTrue(any[ExecutionContext])(any[QueryState]))
      ...
      .thenReturn(false)
    // when
    val result = OptionalExpandIntoPipe(left, "a", "r", "b", SemanticDirection.OUTGOING, LazyTypes.empty, predicate)()
      .createResults(queryState)
      .toList

    // then
    result should equal(
      List(Map("a" -> fromNodeProxy(startNode), "b" -> fromNodeProxy(endNode1), "r" -> fromRelationshipProxy(rel1))))
  }

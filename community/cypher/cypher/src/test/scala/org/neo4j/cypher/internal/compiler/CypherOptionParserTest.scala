...
class CypherOptionParserTest extends CypherFunSuite {
  ...
  test("should parse multiple options") {
    parse("CYPHER 2.2 PLANNER COST PROFILE PATTERN") should equal(CypherQueryWithOptions("PATTERN", Seq(VersionOption("2.2"), CostPlanner, ProfileOption)))
  ...
  test("should parse version and planner/compiler") {
    parse("CYPHER 2.2 PLANNER COST RETURN") should equal(CypherQueryWithOptions("RETURN", Seq(VersionOption("2.2"), CostPlanner)))
    parse("PLANNER COST RETURN") should equal(CypherQueryWithOptions("RETURN",Seq(CostPlanner)))
    parse("CYPHER 2.2 PLANNER RULE RETURN") should equal(CypherQueryWithOptions("RETURN", Seq(VersionOption("2.2"), RulePlanner)))
    parse("PLANNER RULE RETURN") should equal(CypherQueryWithOptions("RETURN", Seq(RulePlanner)))
  }
}

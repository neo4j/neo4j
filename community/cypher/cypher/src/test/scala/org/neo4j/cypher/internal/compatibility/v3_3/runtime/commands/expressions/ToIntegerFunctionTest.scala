...
class ToIntegerFunctionTest extends CypherFunSuite {
  ...
  test("should throw an exception if the argument is an object which cannot be converted to integer") {
    val caughtException = intercept[ParameterWrongTypeException] { toInteger(true) }
  ...
  test("should fail for larger integers larger that 8 bytes") {
    val caughtException = intercept[CypherTypeException] { toInteger("10508455564958384115") }
  ...
  test("cannot handle -2^63-1") {
    val caughtException = intercept[CypherTypeException] { toInteger("-9223372036854775809") }
  ...
  test("cannot handle 2^63") {
    val caughtException = intercept[CypherTypeException] { toInteger("9223372036854775808") }

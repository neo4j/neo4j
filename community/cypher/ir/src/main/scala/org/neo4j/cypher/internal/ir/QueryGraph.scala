...
case class QueryGraph(// !!! If you change anything here, make sure to update the equals method at the bottom of this class !!!
  ...
  lazy val allQGsWithLeafInfo: Seq[QgWithLeafInfo] = {
    val patternComprehensions = this.folder.findByAllClass[PatternComprehension].toSet.map((e: PatternComprehension) => ExpressionConverters.asQueryGraph(e, e.dependencies.map(_.name), new AnonymousVariableNameGenerator))
    val patternExpressions = this.folder.findByAllClass[PatternExpression].toSet.map((e: PatternExpression) => ExpressionConverters.asQueryGraph(e, e.dependencies.map(_.name), new AnonymousVariableNameGenerator))
    val patterns = this.folder.findByAllClass[Pattern].map((e: Pattern) => ExpressionConverters.asQueryGraph(e))

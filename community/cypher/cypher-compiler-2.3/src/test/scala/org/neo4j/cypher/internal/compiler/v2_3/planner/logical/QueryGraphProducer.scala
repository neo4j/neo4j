...
trait QueryGraphProducer extends MockitoSugar {
  ...
  def producePlannerQueryForPattern(query: String): (PlannerQuery, SemanticTable) = {
    ...
    val (rewrittenAst, rewrittenTable) = CostBasedExecutablePlanBuilder.rewriteStatement(firstRewriteStep, semanticState.scopeTree, semanticTable, postConditions, mock[AstRewritingMonitor])

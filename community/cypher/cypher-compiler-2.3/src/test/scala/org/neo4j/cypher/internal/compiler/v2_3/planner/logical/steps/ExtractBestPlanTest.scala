...
class ExtractBestPlanTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  private def getGreedyPlanTable(logicalPlan: LogicalPlan)(implicit context: LogicalPlanningContext) : GreedyPlanTable = {
    greedyPlanTableWith(logicalPlan)
  }
  ...
  test("should throw when finding plan that does not solve all pattern relationships") {
    ...
    a [CantHandleQueryException] should be thrownBy {
      verifyBestPlan(getGreedyPlanTable(getSimpleLogicalPlanWithAandB())(logicalPlanContext).uniquePlan, query)
    }
  }
  ...
  test("should not throw when finding plan that does solve all pattern nodes") {
    ...
    verifyBestPlan(getGreedyPlanTable(getSimpleLogicalPlanWithAandB())(logicalPlanContext).uniquePlan, query).availableSymbols should equal(Set(IdName("a"), IdName("b")))
  }
  ...
  test("should throw when finding plan that contains unfulfillable index hint") {
    ...
    a [IndexHintException] should be thrownBy {
      verifyBestPlan(getGreedyPlanTable(getSimpleLogicalPlanWithAandB())(logicalPlanContext).uniquePlan, getQueryWithIdxHint())
    }
  }
  ...
  test("should issue warning when finding plan that contains unfulfillable index hint") {
    ...
    verifyBestPlan(getGreedyPlanTable(getSimpleLogicalPlanWithAandB())(logicalPlanContext).uniquePlan, getQueryWithIdxHint()).availableSymbols should equal(Set(IdName("a"), IdName("b")))
  ...
  test("should succeed when finding plan that contains fulfillable index hint") {
    ...
    verifyBestPlan(getGreedyPlanTable(plan)(logicalPlanContext).uniquePlan, getQueryWithIdxHint()).availableSymbols should equal(Set(IdName("a"), IdName("b")))

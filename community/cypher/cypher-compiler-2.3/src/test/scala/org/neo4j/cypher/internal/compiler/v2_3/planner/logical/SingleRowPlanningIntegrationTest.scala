...
import org.neo4j.cypher.internal.compiler.v2_3.planner.{PlannerQuery, LogicalPlanningTestSupport2}
...
class SingleRowPlanningIntegrationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  test("should build plans containing single row") {
    planFor("RETURN 42").plan should equal(
      Projection(
        SingleRow(), expressions = Map("42" -> SignedDecimalIntegerLiteral("42")_)
      )(PlannerQuery.empty)
    )
  }
}

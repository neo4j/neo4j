...
import org.neo4j.cypher.internal.compiler.v2_3.commands.NodeJoin
...
class UnsupportedHintsRemovalBuilder extends PlanBuilder {
  ...
  def canWorkWith(plan: ExecutionPlanInProgress, ctx: PlanContext)(implicit pipeMonitor: PipeMonitor): Boolean = {
    plan.query.start.exists {
      case Unsolved(item: NodeJoin) => true
      case i => false
    }
  }
  ...
  def apply(plan: ExecutionPlanInProgress, ctx: PlanContext)(implicit pipeMonitor: PipeMonitor): ExecutionPlanInProgress = {
    plan.copy(
      query = plan.query.copy(start = plan.query.start.filter {
        case Unsolved(item: NodeJoin) => false
        case _ => true
      })
    )
  }

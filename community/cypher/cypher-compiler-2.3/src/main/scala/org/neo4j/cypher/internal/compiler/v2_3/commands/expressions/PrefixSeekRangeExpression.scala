...
abstract class SeekRangeExpression extends Expression {
...
case class InterpolatedPrefixSeekRangeExpression(range: Interpolation)
  extends SeekRangeExpression {

  override def rewrite(f: (Expression) => Expression): Expression =
    f(copy(range = range.rewrite(f).asInstanceOf[Interpolation]))

  override def symbolTableDependencies: Set[String] =
    range.symbolTableDependencies
}

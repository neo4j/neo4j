...
case class DeleteNodePipe(src: Pipe, expression: Expression)(val estimatedCardinality: Option[Double] = None)
  ...
  protected def internalCreateResults(input: Iterator[ExecutionContext],
    ...
    input.map { row =>
      expression(row)(state) match {
        case n: Node =>
          if (!state.query.nodeOps.isDeleted(n)) state.query.nodeOps.delete(n)
          row
        case null => row
  ...
  def planDescriptionWithoutCardinality = src.planDescription.andThen(this.id, "DeleteNode", variables)

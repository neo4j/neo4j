...
import org.neo4j.cypher.internal.compiler.v3_0.ast.rewriters.DesugaredMapProjection
...
object ExpressionConverters {
  ...
  def toCommandExpression(expression: ast.Expression): CommandExpression = expression match {
    ...
    case e: DesugaredMapProjection => commandexpressions.MapProjection(e.name.name, e.includeAllProps, mapProjectionItems(e.items))

  ...
  override protected def internalCreateResults(input: Iterator[ExecutionContext],
    ...
    input.map { row =>
      ...
      val map = expression(row)(state) match {
        case IsMap(createMapFrom) => propertyKeyMap(state.query, createMapFrom(state.query))
      ...
      if (item != null) setProperties(state.query, operations(state.query), getId(item), map)
      row
    }
  }

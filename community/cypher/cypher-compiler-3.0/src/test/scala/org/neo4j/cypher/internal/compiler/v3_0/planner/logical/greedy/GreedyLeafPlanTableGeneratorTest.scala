...
import org.neo4j.cypher.internal.frontend.v3_0.ast.{Expression, HasLabels, LabelName}
import org.neo4j.cypher.internal.compiler.v3_0.pipes.LazyLabel
import org.neo4j.cypher.internal.compiler.v3_0.planner._
import org.neo4j.cypher.internal.compiler.v3_0.planner.logical.plans.{AllNodesScan, NodeByLabelScan, Selection}
import org.neo4j.cypher.internal.compiler.v3_0.planner.logical.{Cardinality, QueryPlannerConfiguration}
...
class GreedyLeafPlanTableGeneratorTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("pattern with two nodes - one allnodes and one labelscan") {
    new given {
      ...
      val hasLabels: Expression = HasLabels(varFor("a"), Seq(label))(pos)
  ...
  test("single node multiple labels") {
    new given {
      ...
      val hasLabels1 = HasLabels(varFor("a"), Seq(label1))(pos)
      val hasLabels2 = HasLabels(varFor("a"), Seq(label2))(pos)
      qg = QueryGraph(
        patternNodes = Set("a", "b"),

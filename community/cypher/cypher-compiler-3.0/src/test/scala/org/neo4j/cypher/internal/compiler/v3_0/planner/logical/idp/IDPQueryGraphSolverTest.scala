...
class IDPQueryGraphSolverTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("should plan for a single node pattern") {
    ...
    new given {
      ...
      val plan = queryGraphSolver.plan(cfg.qg, QueryPlannerConfiguration.default)
      plan should equal(
  ...
  test("should plan for a join between two pattern relationships") {
    ...
    new given {
      ...
      val plan: LogicalPlan = queryGraphSolver.plan(cfg.qg, QueryPlannerConfiguration.default)
      plan should equal(
        NodeHashJoin(Set("c"),
          Expand(
  ...
  test("should plan big star pattern") {
    ...
    new given {
      ...
      implicit val x = ctx
      queryGraphSolver.plan(cfg.qg, QueryPlannerConfiguration.default) // should not throw
    }
  }
  ...
  test("should plan cartesian product between 3 pattern nodes and using a single predicate between 2 pattern nodes") {
    ...
    new given {
      ...
      qg = QueryGraph(
        patternNodes = Set("a", "b", "c"),
        selections = Selections.from(Equals(varFor("b"), varFor("c"))(pos)))
    }.withLogicalPlanningContext { (cfg, ctx) =>
      implicit val x = ctx
  ...
  test("should plan for optional single relationship pattern between two known nodes") {
    ...
    new given {
      ...
      queryGraphSolver.plan(cfg.qg, QueryPlannerConfiguration.default) should equal(
        OuterHashJoin(
          Set("a", "b"),
          CartesianProduct(

...
class OrderByOnlyOnVariablesTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("happy when order by sort on variable") {
    val ast: ASTNode = Return(false, ReturnItems(false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, Some(OrderBy(Seq(AscSortItem(variable("n"))_))_), None, None)_

...
object PredicateSplitterTest extends AstConstructionTestSupport {
  ...
  private def ri(name: String): ReturnItem = AliasedReturnItem(variable(name), variable(name))_
  private def items(name: String*): ReturnItems = ReturnItems(includeExisting = false, name.toSet.toSeq.map(ri))_
  private def pathPred(num: Int): Expression = GreaterThan(ContainerIndex(variable("p"), SignedDecimalIntegerLiteral(num.toString)_)_, SignedDecimalIntegerLiteral("1")_)_
}
...
class PredicateSplitterTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("Does not change MATCH without named paths") {
    val MATCH: Clause = Match(optional = false, Pattern(Seq(EveryPath(NodePattern(Some(variable("a")), Seq.empty, None) _)))_, Seq.empty, Some(Where(True()_)_))_
    val statement: Statement = query(MATCH, RETURN_ALL)
    val lookup = (clause: Clause) => clause match {
      case MATCH => Set(variable("b"))
  ...
  test("Moves one predicate over named paths to newly introduced WITH") {
    ...
    val MATCH: Clause = Match(optional = false, Pattern(Seq(NamedPatternPart(variable("p"), EveryPath(NodePattern(Some(variable("a")), Seq.empty, None)_))_))_, Seq.empty, Some(Where(pred)_))_
    val statement: Statement = query(MATCH, RETURN_ALL)
    val lookup = (clause: Clause) => clause match {
      case MATCH => Set(variable("a"), variable("b"))
      case _ => Set.empty[Variable]
    }
    ...
    val EXPECTED_MATCH: Clause = Match(optional = false, Pattern(Seq(NamedPatternPart(variable("p"), EveryPath(NodePattern(Some(variable("a")), Seq.empty, None)_))_))_, Seq.empty, None)_
  ...
  test("Moves predicate over named paths to newly introduced WITH but keeps other predicate on MATCH") {
    val pred1: Expression = Equals(variable("a"), variable("b"))_
    ...
    val EXPECTED_MATCH: Clause = Match(optional = false, Pattern(Seq(NamedPatternPart(variable("p"), EveryPath(NodePattern(Some(variable("a")), Seq.empty, None)_))_))_, Seq.empty, Some(Where(pred1)_))_
  ...
  test("Does not move predicate over named paths to newly introduced WITH from OPTIONAL MATCH") {
    val pred: Expression = pathPred(0)
    val OPTIONAL_MATCH: Clause = Match(optional = true, Pattern(Seq(NamedPatternPart(variable("p"), EveryPath(NodePattern(Some(variable("a")), Seq.empty, None)_))_))_, Seq.empty, Some(Where(pred)_))_
    val statement: Statement = query(OPTIONAL_MATCH, RETURN_ALL)
    val lookup = (clause: Clause) => clause match {
      case OPTIONAL_MATCH => Set(variable("a"), variable("b"))
      case _ => Set.empty[Variable]
    }

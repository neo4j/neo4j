...
class SimplifyEqualityTest extends CypherFunSuite with LogicalPlanningTestSupport {
  test("should rewrite WHERE x.prop in [1] to WHERE x.prop = 1") {
    ...
    val predicate: Expression = In(Property(variable("x"), PropertyKeyName("prop")(pos))(pos), Collection(Seq(SignedDecimalIntegerLiteral("1")(pos)))(pos))(pos)
    val cleanPredicate: Expression = Equals(Property(variable("x"), PropertyKeyName("prop")(pos))(pos), SignedDecimalIntegerLiteral("1")(pos))(pos)
  ...
  test("should not rewrite WHERE x.prop in [1, 2]") {
    ...
    val orgPredicate: Expression = In(Property(variable("x"), PropertyKeyName("prop")(pos))(pos), collection)(pos)

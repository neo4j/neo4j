...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("MATCH (a) WITH 1 as b RETURN b") {
    ...
    query.tail should equal(Some(RegularPlannerQuery(QueryGraph(Set.empty, Set.empty, Set(IdName("b"))), UpdateGraph.empty, RegularQueryProjection(Map("b" -> Variable("b") _)))))
  }
  ...
  test("MATCH (a) WITH a WHERE TRUE RETURN a") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(a)),Set(),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(a -> Variable(a),   FRESHID23 -> True()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(a), IdName(  FRESHID23)),Selections(Set(Predicate(Set(IdName(  FRESHID23)),Variable(  FRESHID23)))),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(a -> Variable(a)),QueryShuffle(List(),None,None)),None)))""".stripMargin
    result should equal(expectation)
  }
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS collected WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(collected -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected)),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(owner -> Variable(owner), collected -> Variable(collected),   FRESHID54 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED62)),false,List(),None,None,BOTH),NodePattern(Some(Variable(  UNNAMED64)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected), IdName(  FRESHID54)),Selections(Set(Predicate(Set(IdName(  FRESHID54)),Variable(  FRESHID54)))),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))""".stripMargin
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    ...
    val result = query.toString
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(xyz -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(xyz)),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> GreaterThan(Variable(xyz),SignedDecimalIntegerLiteral(0))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection)),Selections(Set()),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> Variable(collection),   FRESHID82 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED90)),false,List(),None,None,BOTH),NodePattern(Some(Variable(  UNNAMED92)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection), IdName(  FRESHID82)),Selections(Set(Predicate(Set(IdName(  FRESHID82)),Variable(  FRESHID82)))),List(),Set(),Set()),UpdateGraph(List()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))))""".stripMargin

...
import org.neo4j.cypher.internal.compiler.v3_0.planner.logical.cardinality.IndependenceCombiner
import org.neo4j.cypher.internal.compiler.v3_0.planner.logical.plans.{IdName, PatternRelationship, SimplePatternLength, VarPatternLength}
import org.neo4j.cypher.internal.compiler.v3_0.planner.logical.{Cardinality, Selectivity}
import org.neo4j.cypher.internal.compiler.v3_0.planner.{LogicalPlanConstructionTestSupport, Predicate, Selections}
import org.neo4j.cypher.internal.compiler.v3_0.spi.GraphStatistics
import org.neo4j.cypher.internal.frontend.v3_0.ast.{AstConstructionTestSupport, HasLabels, LabelName}
import org.neo4j.cypher.internal.frontend.v3_0.test_helpers.CypherFunSuite
import org.neo4j.cypher.internal.frontend.v3_0.{LabelId, SemanticDirection, SemanticTable}
...
class PatternSelectivityCalculatorTest extends CypherFunSuite with LogicalPlanConstructionTestSupport with AstConstructionTestSupport {
  ...
  test("should return zero if there are no nodes with the given labels") {
    ...
    implicit val selections = Selections(Set(Predicate(Set[IdName]("a"), HasLabels(variable("a"), Seq(label))(pos))))
  ...
  test("should not produce selectivities larger than 1.0") {
    ...
    val predicates = labelNames.map(l => Predicate(Set[IdName]("a"), HasLabels(variable("a"), Seq(l))(pos))).toSet

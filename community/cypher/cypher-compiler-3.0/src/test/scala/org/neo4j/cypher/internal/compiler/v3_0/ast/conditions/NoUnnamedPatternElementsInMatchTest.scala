...
class NoUnnamedPatternElementsInMatchTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("unhappy when a node pattern is unnamed") {
    ...
    val ast: ASTNode = SingleQuery(Seq(
      Match(optional = false, Pattern(Seq(EveryPath(chain(chain(node(Some(variable("n"))), relationship(Some(variable("p"))), nodePattern), relationship(Some(variable("r"))), node(Some(variable("m")))))))_, Seq.empty, None)_,
      Return(distinct = false, ReturnItems(includeExisting = false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, None, None, None)_
    ))_
  ...
  test("unhappy when a relationship pattern is unnamed") {
    ...
    val ast: ASTNode = SingleQuery(Seq(
      Match(optional = false, Pattern(Seq(EveryPath(chain(chain(node(Some(variable("n"))), relationship(Some(variable("p"))), node(Some(variable("k")))), relationshipPattern, node(Some(variable("m")))))))_, Seq.empty, None)_,
      Return(distinct = false, ReturnItems(includeExisting = false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, None, None, None)_
    ))_
  ...
  test("unhappy when there are unnamed node and relationship patterns") {
    ...
    val relationshipPattern: RelationshipPattern = relationship(None)
    val ast: ASTNode = SingleQuery(Seq(
      Match(optional = false, Pattern(Seq(EveryPath(chain(chain(node(Some(variable("n"))), relationshipPattern, node(Some(variable("k")))), relationship(Some(variable("r"))), nodePattern))))_, Seq.empty, None)_,
      Return(distinct = false, ReturnItems(includeExisting = false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, None, None, None)_
    ))_
  ...
  test("happy when all elements in pattern are named") {
    val ast: ASTNode = SingleQuery(Seq(
      Match(optional = false, Pattern(Seq(EveryPath(chain(chain(node(Some(variable("n"))), relationship(Some(variable("p"))), node(Some(variable("k")))), relationship(Some(variable("r"))), node(Some(variable("m")))))))_, Seq.empty, None)_,
      Return(distinct = false, ReturnItems(includeExisting = false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, None, None, None)_
    ))_
  ...
  test("should leave where clause alone") {
    ...
    val ast: ASTNode = SingleQuery(Seq(
      Match(optional = false, Pattern(Seq(EveryPath(chain(chain(node(Some(variable("n"))), relationship(Some(variable("p"))), node(Some(variable("k")))), relationship(Some(variable("r"))), node(Some(variable("m")))))))_, Seq.empty, Some(where))_,
      Return(distinct = false, ReturnItems(includeExisting = false, Seq(AliasedReturnItem(variable("n"), variable("n"))_))_, None, None, None)_
    ))_

...
object StatementConverters {
  ...
  implicit class UnwindConverter(inner: ast.Unwind) {
    def addToQueryBuilder(builder: commands.QueryBuilder) = {
      val items: Seq[StartItem] = builder.startItems :+ commands.Unwind(commands.ReturnItem(inner.unwindItem.expression.asCommandExpression,inner.unwindItem.name,renamed = true))
      builder.startItems(items: _*)
    }
  }

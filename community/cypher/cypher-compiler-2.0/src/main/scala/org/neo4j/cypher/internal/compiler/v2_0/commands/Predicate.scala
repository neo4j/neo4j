...
case class HasProperty(identifier: Expression, propertyKey: KeyToken) extends Predicate {
  ...
  def rewrite(f: (Expression) => Expression) = f(HasProperty(identifier.rewrite(f), propertyKey.rewrite(f)))

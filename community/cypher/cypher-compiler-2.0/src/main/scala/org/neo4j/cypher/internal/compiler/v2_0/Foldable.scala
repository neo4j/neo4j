...
object Foldable {
  ...
  @tailrec
  private def foldAcc[R](those: mutable.ArrayStack[Any], acc: R, f: Any => Option[R => R]): R =
    if (those.isEmpty)
      acc
    else {
      val that = those.pop()
      foldAcc(those ++= that.reverseChildren, f(that).fold(acc)(_(acc)), f)
    }

  // partially tail-recursive (recursion is unavoidable for partial function matches)
  private def foldtAcc[R](those: mutable.ArrayStack[Any], acc: R, f: PartialFunction[Any, (R, R => R) => R]): R =
    if (those.isEmpty)
      acc
    else {
      val that = those.pop()
      if (f.isDefinedAt(that))
        foldtAcc(those, f(that)(acc, foldtAcc(mutable.ArrayStack() ++= that.reverseChildren, _, f)), f)
      else
        foldtAcc(those ++= that.reverseChildren, acc, f)
  }
}

...
case class TypeRange(lower: CypherType, upper: Option[CypherType]) {
  ...
  def intersect(that: TypeRange): Option[TypeRange] = (lower mergeUp that.lower).flatMap {
    newLower =>
      val newUpper = upper.fold(that.upper)(t => Some(that.upper.fold(t)(_ mergeDown t)))
  ...
  def mergeDown(other: TypeRange): Seq[TypeRange] = {
    val newLower = lower mergeDown other.lower
    (upper, other.upper) match {
      case (Some(u1), Some(u2)) =>
        Seq(TypeRange(newLower, Some(u1 mergeDown u2)))
      case (Some(u1), None)     =>
        if ((u1 isAssignableFrom other.lower) || (other.lower isAssignableFrom u1))
          Seq(TypeRange(newLower, Some(u1)))
        else
          Seq(TypeRange(newLower, Some(newLower)))
      case (None, Some(u2))     =>
        if ((u2 isAssignableFrom lower) || (lower isAssignableFrom u2))
          Seq(TypeRange(newLower, Some(u2)))
        else
          Seq(TypeRange(newLower, Some(newLower)))
      case (None, None)         =>
        if (lower == other.lower)
          Seq(TypeRange(newLower, None))
        else if (lower isAssignableFrom other.lower)
          Seq(TypeRange(newLower, Some(other.lower)), TypeRange(other.lower, None))
        else if (other.lower isAssignableFrom lower)
          Seq(TypeRange(newLower, Some(lower)), TypeRange(lower, None))
        else
          Seq(TypeRange(newLower, Some(newLower)))
    }
  }

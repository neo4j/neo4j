...
class MutableIdentityMapTest extends CypherFunSuite {
  case class Val()
  ...
  test("should store and retrieve based on object identity") {
    ...
    val map = MutableIdentityMap(x -> "x", y -> "y")
  ...
  test("should not overwrite equal key") {
    ...
    val map = MutableIdentityMap(x -> "x")
  ...
  test("should be mutable when updated") {
    val x = Val()
    val y = Val()
    val map = MutableIdentityMap(x -> "x")
    map.update(x, "x'")

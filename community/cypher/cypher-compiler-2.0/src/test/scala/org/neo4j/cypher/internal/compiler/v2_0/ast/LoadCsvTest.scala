...
class LoadCsvTest extends FunSuite {
  val literal = URLLiteral("yo mama")(DummyPosition(4))
  ...
  test("should reject URLs that are not file://, http://, https://, ftp://") {
    val literal = URLLiteral("morsecorba://sos")(DummyPosition(4))
    val loadCsv = LoadCSV(false, literal, identifier, None, None)(DummyPosition(6))
    val result = loadCsv.semanticCheck(SemanticState.clean)
    assert(result.errors === Vector(new SemanticError("Unsupported URL protocol: morsecorba", DummyPosition(4), SortedSet.empty)))
  }

  test("should accept http:// URLs") {
    val literal = URLLiteral("http://example.com/foo.csv")(DummyPosition(4))
    val loadCsv = LoadCSV(false, literal, identifier, None, None)(DummyPosition(6))
    val result = loadCsv.semanticCheck(SemanticState.clean)

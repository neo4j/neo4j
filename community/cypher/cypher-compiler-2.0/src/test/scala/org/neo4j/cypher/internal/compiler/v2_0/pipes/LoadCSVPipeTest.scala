...
class LoadCSVPipeTest extends Assertions {
  ...
  def should_handle_strings() {
    ...
    val pipe = new LoadCSVPipe(input, NoHeaders, "file://" + fileName, "foo")
  ...
  def should_handle_with_headers() {
    ...
    val pipe = new LoadCSVPipe(input, HasHeaders, "file://" + fileName, "foo")
  ...
  def should_be_able_to_handle_an_http_file() {
    ...
    val ctx = ExecutionContext.empty
    val input = new FakePipe(Iterator(ctx))
    val pipe = new LoadCSVPipe(input, NoHeaders, "https://dl.dropboxusercontent.com/u/47132/test.csv", "name")

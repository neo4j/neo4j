...
class MergeTest extends DocumentingTestBase with StatisticsChecker {
  ...
  @Test def merge_single_node_with_label() {
    testQuery(
      ...
      queryText = "cypher experimental merge (robert:Critic) return robert, labels(robert)",
  ...
  @Test def merge_single_node_with_properties() {
    testQuery(
      ...
      queryText = "cypher experimental merge (charlie {name:'Charlie Sheen', age:10}) return charlie",
  ...
  @Test def merge_single_node_with_label_and_property() {
    testQuery(
      ...
      queryText = "cypher experimental merge (michael:Person {name:'Michael Douglas'}) return michael",
  ...
  @Test def merge_node_and_set_property_on_creation() {
    testQuery(
      ...
      queryText = "cypher experimental merge (keanu:Person {name:'Keanu Reeves'}) " +
  ...
  @Test def merge_node_and_set_property_on_match() {
    testQuery(
      ...
      queryText = "cypher experimental merge (person:Person) on match person set person.found = true return person",
  ...
  @Test def merge_node_and_set_property_on_creation_or_update_prop() {
    testQuery(
      ...
      text = "Merge a node and set properties if the node needs to be created.",
      queryText =
        """cypher experimental merge (keanu:Person {name:'Keanu Reeves'})

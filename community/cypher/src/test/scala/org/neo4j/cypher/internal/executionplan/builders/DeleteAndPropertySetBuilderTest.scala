...
import org.neo4j.cypher.internal.commands.{Entity, DeleteEntityCommand}
...
class DeleteAndPropertySetBuilderTest extends BuilderTest {
  ...
  def does_not_offer_to_solve_done_queries() {
    val q = PartiallySolvedQuery().
      copy(updates = Seq(Solved(DeleteEntityCommand(Entity("x")))))
  ...
  def offers_to_solve_queries() {
    val q = PartiallySolvedQuery().
      copy(updates = Seq(Unsolved(DeleteEntityCommand(Entity("x")))))

...
abstract class DocumentingTestBase extends Assertions with DocumentationHelper {
  ...
  def labelsDescription: Map[String, List[Label]] = Map.empty
  ...
  def init() {
    ...
    db.inTx(() => {
      ...
      asNodeMap(labelsDescription) foreach { case (n: Node, labels: List[Label]) =>
          labels.foreach { (label: Label) => n.addLabel(label) }
      }
    })
  }

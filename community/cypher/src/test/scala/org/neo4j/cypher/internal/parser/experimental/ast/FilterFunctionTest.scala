...
class FilterFunctionTest extends Assertions {
  ...
  def shouldHaveCollectionTypesOfInnerExpression() {
    val filter = FilterFunction(Identifier("x", DummyToken(5,6)), dummyExpression, Some(True(DummyToken(5,6))), DummyToken(0, 10))
  ...
  def shouldRaiseSyntaxErrorIfMissingPredicate() {
    val filter = FilterFunction(Identifier("x", DummyToken(5, 6)), dummyExpression, None, DummyToken(0, 10))
    val result = filter.semanticCheck(Expression.SemanticContext.Simple)(SemanticState.clean)

...
class MutationTest extends ExecutionEngineHelper with Assertions {
  ...
  def create_node() {
    ...
    val createNode = new ExecuteUpdateCommandsPipe(start, graph, Seq(CreateNode("n", Map("name" -> Literal("Andres")), NoLabels)))
  ...
  def create_rel() {
    ...
    val createRel = CreateRelationship("r",
      RelationshipEndpoint(getNode("a", a), Map(), NoLabels, true),
      RelationshipEndpoint(getNode("b", b), Map(), NoLabels, true), "REL", Map("I" -> Literal("was here")))

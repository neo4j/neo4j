...
+WHERE+ defines the +MATCH+ patterns in more detail. The predicates are part of the pattern description, not a filter
...
pattern matching to find matching sub graphs. Because Cypher is declarative, it can change order of these operations.
+WHERE+ predicates can be evaluated before pattern matching, during pattern matching, or after finding matches.

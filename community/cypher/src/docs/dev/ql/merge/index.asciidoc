...
== Using MERGE with map parameters ==

+MERGE+ does not support map parameters like for example +CREATE+ does. To use map parameters with +MERGE+, it is
necessary to explicitly use the expected properties, like in the following example.

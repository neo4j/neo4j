...
if no matches are found, +OPTIONAL+ +MATCH+ will use nulls for missing parts of the pattern. +OPTIONAL+ +MATCH+ could be
considered the Cypher equivalent of the outer join in SQL.

Either the whole pattern is matched, or nothing is matched. Remember that +WHERE+ is part of the pattern description,
and the predicates will be considered while looking for matches, not after.

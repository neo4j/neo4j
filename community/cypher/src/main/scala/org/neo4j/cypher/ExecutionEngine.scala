...
import org.neo4j.cypher.internal.{CypherCompiler, LRUCache}
...
class ExecutionEngine(graph: GraphDatabaseService, logger: StringLogger = StringLogger.DEV_NULL) {
  ...
  private def createCorrectCompiler() =
    optGraphAs[InternalAbstractGraphDatabase]
      ...
      .andThen({
      case v:String => CypherCompiler(v)
      case _        => CypherCompiler()
    })
      .applyOrElse(graph, (_: GraphDatabaseService) => CypherCompiler() )

...
import graphdb.Node
import org.neo4j.cypher.internal.compiler.v1_9.pipes.{TraversalMatchPipe, EntityProducer}
...
import org.neo4j.cypher.internal.compiler.v1_9.symbols.{NodeType, SymbolTable}
...
class TraversalMatcherBuilder extends PlanBuilder with PatternGraphBuilder {
  ...
  private def extractExpanderStepsFromQuery(plan: ExecutionPlanInProgress): Option[LongestTrail] = {
    ...
    filterNot {
      case pred => pred.exists( exp => exp.isInstanceOf[PatternPredicate] || exp.isInstanceOf[PathExpression]  )
    }

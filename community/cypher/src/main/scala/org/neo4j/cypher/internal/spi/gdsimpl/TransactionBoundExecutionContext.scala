...
class TransactionBoundExecutionContext(graph: GraphDatabaseService, tx: Transaction, statement: Statement)
  ...
  def withAnyOpenQueryContext[T](work: (QueryContext) => T): T = {
    if (open) {
      ...
      try {
        val bridge   = graph.asInstanceOf[GraphDatabaseAPI].getDependencyResolver.resolveDependency(classOf[ThreadToStatementContextBridge])

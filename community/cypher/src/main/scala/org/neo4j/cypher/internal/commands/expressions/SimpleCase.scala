...
case class SimpleCase(expression: Expression, alternatives: Seq[(Expression, Expression)], default: Option[Expression])
  ...
  protected def calculateType(symbols: SymbolTable): CypherType = {
    val types: Seq[CypherType] =
      alternativeExpressions.map(_.evaluateType(AnyType(), symbols)) ++
        default.map(_.evaluateType(AnyType(), symbols))

    types reduce (_ mergeWith _)
  }
  ...
  (default.toSeq ++ alternativeComparison ++ alternativeExpressions :+ expression).

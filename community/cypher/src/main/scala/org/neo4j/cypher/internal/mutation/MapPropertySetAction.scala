...
case class MapPropertySetAction(element: Expression, mapExpression: Expression)
  ...
  def exec(context: ExecutionContext, state: QueryState) = {
    ...
    def deleteEntity(pc: PropertyContainer) {
      pc match {
        case n: Node         => state.query.nodeOps().delete(n)
        case r: Relationship => state.query.relationshipOps().delete(r)
      }
    }

    /*Make the map expression look like a map*/
    mapExpression(context) match {
      case null                 => deleteEntity(pc)

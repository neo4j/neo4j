...
sealed abstract class NodePattern extends PatternElement with SemanticChecking {
  ...
  def semanticCheck(ctx: SemanticContext): SemanticCheck =
    when (bare && (!labels.isEmpty || properties.isDefined)) {
...
case class NamedNodePattern(identifier: Identifier, labels: Seq[Identifier], properties: Option[Expression], bare: Boolean, token: InputToken) extends NodePattern {
...
case class AnonymousNodePattern(labels: Seq[Identifier], properties: Option[Expression], bare: Boolean, token: InputToken) extends NodePattern {

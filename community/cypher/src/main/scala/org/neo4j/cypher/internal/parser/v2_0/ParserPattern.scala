...
trait ParserPattern extends Base with Labels {
  ...
  private def nodeFromExpression = Parser {
    in =>
      (unnamed ~ expression).apply(in) match {
  ...
  private def shortestPath: Parser[List[AbstractPattern]] = unnamed ~ (ignoreCase("shortestPath") | ignoreCase("allShortestPaths")) ~ parens(patternForShortestPath) ^^ {
  ...
  private def tailWithRelData: Parser[Tail] = unnamed ~ opt("<") ~ "-" ~ "[" ~ optionalName ~ opt("?") ~ opt(":" ~> rep1sep(identity, "|")) ~ variable_length ~ props ~ "]" ~ "-" ~ opt(">") ~ node ^^ {
  ...
  private def tailWithNoRelData = unnamed ~ opt("<") ~ "-" ~ unnamed ~ "-" ~ opt(">") ~ node ^^ {

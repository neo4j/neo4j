...
The +MATCH+ clause allows you to specify the patterns Cypher will search for in the database, and
it is the primary way of getting data into the current set of bindings.
It is worth reading up more on the specification of the patterns themselves in <<introduction-pattern>>.

It is often coupled to a +WHERE+ clause which adds restrictions, or predicates, to the +MATCH+ patterns, making them more specific.
...
If it is the first, nothing will have been bound yet,
and Cypher will design a search to find the results matching the clause and any associated predicates specified in any +WHERE+ clause.
This could involve a scan of the database, a search for nodes of a certain label,
or a search of an index to find starting points for the pattern matching.
Nodes and relationships found by this search are available as _bound pattern elements_, and can be used for pattern matching of sub-graphs,
as well as used in any further +MATCH+ clauses, where Cypher will use the known elements, and from there find further unknown elements.

Cypher is declarative, and so usually the query itself does not specify the algorithm to use to perform the search.
Cypher will automatically work out the best approach to finding start nodes and matching patterns.
...
Read more about indexes in <<query-schema-index>>, and more about the specifying index hints to force Cypher to use a specific index in <<query-using>>.

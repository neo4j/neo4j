...
You can either introduce starting points by id, or by legacy index lookups.
Note that trying to use a legacy index that doesn't exist will generate an error.

[NOTE]
+START+ is deprecated and going to be removed in the future.
...
The +START+ clause is only needed when using legacy indexes as all other start points are correctly inferred by Cypher.

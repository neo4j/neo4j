...
You will always see the estimated rows of your execution plan, if the cost based compiler has been used.
These metrics were not calculated in older versions of Cypher, so if your query is being compiled by an older compiler, you will not see the estimated rows.

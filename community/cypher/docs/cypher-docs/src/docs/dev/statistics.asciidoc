...
Controls how much the above statistical information is allowed to change before an execution plan is considered stale.
If the relative change in any of statistics is larger than this threshold, the plan will thrown away and a new one will
be created. A threshold of 0.0 means _always replan_, and a value of 1.0 means _never replan_.

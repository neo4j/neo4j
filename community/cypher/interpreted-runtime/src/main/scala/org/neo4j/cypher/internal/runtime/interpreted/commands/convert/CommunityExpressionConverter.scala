...
case class CommunityExpressionConverter(
  ...
  private def caseExpression(id: Id, e: internal.expressions.CaseExpression, self: ExpressionConverters) = {
    val candidate = for {
      ...
      varName = name match {
        case variable: ExpressionVariable => variable
        case _ => throw new IllegalStateException("Case candidate name variable not allocated")
      }
    } yield varName -> self.toCommandExpression(id, expr)

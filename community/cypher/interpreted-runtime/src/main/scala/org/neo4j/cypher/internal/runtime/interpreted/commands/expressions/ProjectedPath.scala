...
object ProjectedPath {
  ...
  type Projector = (CypherRow, PathValueBuilder) => PathValueBuilder

  object nilProjector extends Projector {
    def apply(ctx: CypherRow, builder: PathValueBuilder) = builder
  }

  case class singleNodeProjector(node: String, tailProjector: Projector) extends Projector {
    def apply(ctx: CypherRow, builder: PathValueBuilder) = {
...
case class ProjectedPath(symbolTableDependencies: Set[String], projector: ProjectedPath.Projector) extends Expression {
  override def apply(ctx: CypherRow, state: QueryState): AnyValue = projector(ctx, state.clearPathValueBuilder).result()

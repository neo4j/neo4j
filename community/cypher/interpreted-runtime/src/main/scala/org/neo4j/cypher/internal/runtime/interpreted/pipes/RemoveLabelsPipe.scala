...
import org.neo4j.cypher.internal.runtime.interpreted.{CastSupport, GraphElementPropertyFunctions}
...
case class RemoveLabelsPipe(src: Pipe, variable: String, labels: Seq[LazyLabel])
  ...
  private def removeLabels(context: ExecutionContext, state: QueryState, nodeId: Long) = {
    val labelIds = labels.map(_.getId(state.query)).filter(_ != LazyLabel.UNINITIALIZED)

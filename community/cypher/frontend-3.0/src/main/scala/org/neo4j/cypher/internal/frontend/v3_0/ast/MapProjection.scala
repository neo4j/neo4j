...
case class MapProjection(name: Identifier, items: Seq[MapProjectionElement])(val position: InputPosition)
...
case class IdentifierSelector(id: Identifier)(val position: InputPosition) extends MapProjectionElement {
  override def semanticCheck(ctx: SemanticContext) = id.semanticCheck(ctx)
}

case class PropertySelector(id: Identifier)(val position: InputPosition) extends MapProjectionElement {

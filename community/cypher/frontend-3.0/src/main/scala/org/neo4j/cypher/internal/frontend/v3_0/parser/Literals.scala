...
trait Literals extends Parser
  ...
  def MapProjection: Rule1[ast.MapProjection] = rule {
    group(
      Identifier ~~ ch('{') ~~ zeroOrMore(LiteralEntry | PropertySelector | VariableSelector | AllPropertiesSelector, CommaSep) ~~ ch('}')

...
case class LockNodesPipe(src: Pipe, variablesToLock: Set[String])(val estimatedCardinality: Option[Double] = None)
  ...
  override def planDescriptionWithoutCardinality: PlanDescriptionImpl =
    src.planDescription.andThen(this.id, "Lock", variables, KeyNames(variablesToLock.toSeq))

  override def withEstimatedCardinality(estimated: Double): LockNodesPipe = copy()(Some(estimated))

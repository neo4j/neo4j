...
case object planShortestPaths {
  ...
  def apply(inner: LogicalPlan, queryGraph: QueryGraph, shortestPaths: ShortestPathPattern)
    ...
    val (safePredicates, needFallbackPredicates) = predicates.partition {
      ...
      case NoneIterablePredicate(_, f@FunctionInvocation(_, _, _, _)) if f.function contains Nodes => false
      case AllIterablePredicate(_, f@FunctionInvocation(_, _, _, _)) if f.function contains Nodes => false

...
import org.neo4j.cypher.internal.compiler.v3_1.planner.logical.cardinality.{SpecifiedAndKnown, SpecifiedButUnknown, Unspecified}
...
import org.neo4j.cypher.internal.frontend.v3_1.{SemanticDirection, LabelId, RelTypeId, SemanticTable}
...
class TripletConverterTest extends CypherFunSuite with LogicalPlanningTestSupport2 with AstConstructionTestSupport {
  ...
  test("Converts PatternRelationship to triplet and resolves all specs") {
    ...
    val qg = QueryGraph
      ...
      .addSelections(Selections(Set(Predicate(Set("a"), HasLabels(varFor("a"), Seq(LabelName("Animal")_))_))))

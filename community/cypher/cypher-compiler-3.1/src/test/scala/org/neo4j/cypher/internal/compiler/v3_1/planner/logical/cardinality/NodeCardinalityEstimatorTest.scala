...
import org.neo4j.cypher.internal.frontend.v3_1.ast.{Expression, HasLabels, LabelName}
...
import org.neo4j.cypher.internal.compiler.v3_1.planner.{LogicalPlanningTestSupport2, Predicate, QueryGraph, Selections}
...
class NodeCardinalityEstimatorTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  val hasPersonOnA: Expression = HasLabels(varFor("a"), Seq(LabelName("Person")_))_
  val hasAnimalOnA: Expression = HasLabels(varFor("a"), Seq(LabelName("Animal")_))_
  val hasPersonOnB: Expression = HasLabels(varFor("b"), Seq(LabelName("Person")_))_

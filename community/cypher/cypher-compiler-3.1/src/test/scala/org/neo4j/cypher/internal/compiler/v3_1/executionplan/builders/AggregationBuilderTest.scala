...
import org.neo4j.cypher.internal.compiler.v3_1.commands.expressions.{CountStar, Variable}
...
class AggregationBuilderTest extends BuilderTest {
  ...
  test("should_accept_if_stuff_looks_ok") {
    val q = PartiallySolvedQuery().
      ...
      returns = Seq(Unsolved(ReturnItem(Variable("n"), "n"))),
      aggregateToDo = true
    )
    ...
    val expectedQuery = q.copy(
      ...
      aggregateToDo = false,
      returns = Seq(Solved(ReturnItem(Variable("n"), "n"))),
      extracted = true
    )

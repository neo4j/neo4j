...
import org.neo4j.cypher.internal.compiler.v3_1.commands.expressions.{Property, LengthFunction, Variable}
import org.neo4j.cypher.internal.compiler.v3_1.commands.predicates.{Equals, HasLabel, PropertyExists, Not}
import org.neo4j.cypher.internal.compiler.v3_1.commands.values.{TokenType, KeyToken}
...
class RenderTreeTableTest extends CypherFunSuite with BeforeAndAfterAll {
  ...
  test("Var length expand contains information about its relations") {
    val expandPipe = VarLengthExpandPipe(pipe, "from", "rel", "to", SemanticDirection.OUTGOING,
      SemanticDirection.OUTGOING, LazyTypes.empty, 0, None, nodeInScope = false)(Some(1L))(mock[PipeMonitor])

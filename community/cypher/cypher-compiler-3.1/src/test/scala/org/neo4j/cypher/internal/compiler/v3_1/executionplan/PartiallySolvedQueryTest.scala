...
import org.neo4j.cypher.internal.compiler.v3_1.executionplan.builders.Unsolved
import org.neo4j.cypher.internal.compiler.v3_1.mutation.{CreateNode, DeleteEntityAction, MergeNodeAction, MergePatternAction}
...
class PartiallySolvedQueryTest extends CypherFunSuite {
  ...
  test("delete followed by merge node should not be compacted together") {
    ...
    psq.tail.get.updates.toList should equal(List(Unsolved(DeleteEntityAction(Variable("a"), forced = false))))

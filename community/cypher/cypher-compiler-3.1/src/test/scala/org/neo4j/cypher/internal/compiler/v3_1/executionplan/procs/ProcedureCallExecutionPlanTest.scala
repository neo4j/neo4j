...
class ProcedureCallExecutionPlanTest extends CypherFunSuite {
  ...
  when(ctx.callReadOnlyProcedure(any[QualifiedProcedureName], any[Seq[Any]], any[String])).thenAnswer(procedureResult)
  when(ctx.callReadWriteProcedure(any[QualifiedProcedureName], any[Seq[Any]], any[String])).thenAnswer(procedureResult)
}

...
public class QuickImport implements Callable<Void> {
    ...
    @CommandLine.Option(
            names = "--nodes",
            description = "Number of nodes to import, can have post-fix like 10M for 10 million etc.",
    ...
    @CommandLine.Option(
            names = "--relationships",
            description = "Number of relationships to import, can have post-fix like 10M for 10 million etc.",
            converter = ByteUnitConverter.class,
            arity = "1")
    ...
    @CommandLine.Option(
            names = "--labels",
            description = "Number of different labels in the data set. "
    ...
    @CommandLine.Option(
            ...
            description = "Number of different relationship types in the data set. "
    ...
    @CommandLine.Option(
            names = "--id-type",
            description = "The type of IDs in the input data, e.g. how the importer handles node ID mappings",
            defaultValue = "INTEGER")
    private IdType idType;

    @CommandLine.Option(
            names = "--high-io",
            description = "Whether or not to force high-IO setting, which is otherwise figured out automatically "
                    + "based on the target file system")
    private Boolean highIO;

    @CommandLine.Option(
            names = "--pagecache-memory",
            description = "Specifically configure the page cache memory, other than a reasonable default",
            converter = ByteUnitConverter.class)
    private Long pageCacheMemory;

    @CommandLine.Option(
            names = "--node-header",
            description = "Specification of what data the nodes will have, CSV-style like ':ID,:LABEL,name'")
    private String nodeHeader;

    @CommandLine.Option(
            names = "--relationship-header",
            description = "Specification of what data the relationships will have, CSV-style like "
                    + "':START_ID,:TYPE,:END_ID,data:long'")
    private String relationshipHeader;

    @CommandLine.Option(
            names = "--db-config",
            description = "File containing additional configuration to load for the import")
    private Path dbConfigPath;

    @CommandLine.Option(
            names = "--processors",
            description = "Limit of the number of processors/cpus to use in the import")
    private Integer processors;

    @CommandLine.Option(
            names = "--max-memory",
            description = "Limit of the maximum amount of off-heap memory the import is allowed to use",
            converter = MaxMemoryConverter.class)
    private Long maxMemory;

    @CommandLine.Option(
            names = "--factor-bad-node-data",
            description = "Factor (between 0..1) of nodes that have bad data in them",
            defaultValue = "0")
    private float factorBadNodeData;

    @CommandLine.Option(
            names = "--factor-bad-relationship-data",
            description = "Factor (between 0..1) of relationships that have bad data in them",
            defaultValue = "0")
    private float factorBadRelationshipData;

    @CommandLine.Option(
            names = "--to-csv",
            description = "Instead of importing the generated data, dump it as CSV file(s)")
    ...
    public Void call() throws Exception {
        ...
        Config dbConfig = dbConfigPath != null
                ? Config.newBuilder().fromFile(dbConfigPath).build()
                : Config.defaults();
        InternalLogProvider logging = NullLogProvider.getInstance();
        long pageCacheMemory = this.pageCacheMemory != null
                ? this.pageCacheMemory
                : org.neo4j.internal.batchimport.Configuration.MAX_PAGE_CACHE_MEMORY;
        org.neo4j.internal.batchimport.Configuration importConfig =
                new org.neo4j.internal.batchimport.Configuration.Overridden(defaultConfiguration()) {
                    @Override
    ...
    private static Header parseRelationshipHeader(
            String definition, IdType idType, Extractors extractors, Groups groups) {
        if (definition == null) {
            return DataGeneratorInput.bareboneRelationshipHeader(idType, extractors);
        }

        Configuration config = Configuration.COMMAS;
        return DataFactories.defaultFormatRelationshipFileHeader()
    ...
    private static CharSeeker seeker(String definition, org.neo4j.csv.reader.Configuration config) {
        return CharSeekers.charSeeker(
                Readables.wrap(definition),
                config.toBuilder().withBufferSize(10_000).build(),
                false);
    }
}

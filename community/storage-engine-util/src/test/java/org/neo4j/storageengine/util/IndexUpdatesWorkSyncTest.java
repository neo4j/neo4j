...
class IndexUpdatesWorkSyncTest
{
    ...
    {
        // given
        IndexDescriptor index = IndexPrototype.forSchema( SchemaDescriptors.forLabel( 1, 2 ) ).withName( "index" ).materialise( 1L );
        int threads = 4;
    ...
    private Set<IndexEntryUpdate<IndexDescriptor>> queueUpdatesInParallel( IndexDescriptor index, int threads, IndexUpdatesWorkSync workSync )
    {
        Set<IndexEntryUpdate<IndexDescriptor>> sentUpdates = Collections.newSetFromMap( new ConcurrentHashMap<>() );
        Race race = new Race();
        race.addContestants( threads, i -> throwing( () ->
        {

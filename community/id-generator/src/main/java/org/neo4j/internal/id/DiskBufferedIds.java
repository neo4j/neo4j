...
class DiskBufferedIds implements BufferedIds
{
    ...
    {
        return new PhysicalFlushableChannel( fs.open( segmentName( segmentId ), Set.of( CREATE, TRUNCATE_EXISTING, WRITE ) ),
    ...
    {
        return new ReadAheadChannel<>( fs.open( segmentName( segmentId ), Set.of( READ ) ),
    ...
    public void read( BufferedIdVisitor visitor ) throws IOException
    {
        while ( hasMoreToRead() )
        {
            ...
            if ( !visitor.startChunk( new IdController.TransactionSnapshot( userTransactionIds, timeMillis, lastCommittedTxId ) ) )
            {
                // Snapshot still open
                break;
            }
    ...
    private <CHANNEL extends Closeable> Position<CHANNEL> checkRotate( Position<CHANNEL> position, long offset,
            ThrowingIntFunction<CHANNEL,IOException> segmentFactory ) throws IOException
    {
    ...
    private boolean hasMoreToRead()
    {
        ...
        Preconditions.checkState( positionComparison <= 0, "readPosition:" + readPosition + " writePosition:" + writePosition );
    ...
    private void clearExistingSegments() throws IOException
    {
        ...
        try
        {
            existing = fs.listFiles( basePath.getParent(),
                    entry -> !fs.isDirectory( entry ) && entry.getFileName().toString().startsWith( basePath.getFileName().toString() + "." ) );
        }

...
import org.neo4j.performance.domain.filter.RunResultFilter;
...
public class RunResultSet
{
    ...
    public Pair<BenchmarkResult.Metric, RunResult> getHighestValueOf( String caseName, String metricName)
    {
        BenchmarkResult.Metric highest = null;
        RunResult bestRun = null;

        for(RunResult result : results)
        {
            BenchmarkResult.Metric current = result.getMetric(caseName, metricName);

...
/**
 * The PersistenceLayerMonitor is responsible for monitoring the 
 * persistence layer for changes that should be reflected in persistent state.
 ...
 *		proactive events) for persistence layer state changes. It returns
 ...
 *		invoked whenever an object has changed state in the persistence layer.
 ...
 *		are used only to register/unregister the PersistenceLayerMonitor on events
 ...
 * </UL>
 */
class PersistenceLayerMonitor implements ReActiveEventListener, 
	ProActiveEventListener
{
	Logger log = Logger.getLogger( PersistenceLayerMonitor.class.getName() );
	
//	private static final PersistenceLayerMonitor monitor = 
//		new PersistenceLayerMonitor();

	private final EventManager eventManager;
	private final ResourceBroker broker;
	private final PersistenceSourceDispatcher dispatcher;
	
	PersistenceLayerMonitor( EventManager eventManager, ResourceBroker broker )
	{
		this.eventManager = eventManager;
		this.broker = broker;
		this.dispatcher = broker.getDispatcher();
	}

//	static PersistenceLayerMonitor getMonitor()
//	{
//		return monitor;
//	}
	
	void registerPersistenceSource( PersistenceSource source )
	{
		dispatcher.persistenceSourceAdded( source );
	}
	
	void removePersistenceSource( PersistenceSource source )
	{
		dispatcher.persistenceSourceRemoved( source );
	}
	...
	public boolean proActiveEventReceived( Event event, EventData data )
	{
		...
		{
			dispatcher.persistenceSourceAdded( (PersistenceSource) data.getData() );
			return true;
		}*/
		try
		{
	...
	// Registers the persistence layer monitor as a proactive event listener
		...
		throws EventListenerNotRegisteredException
	{
		try
		{
			eventManager.registerProActiveEventListener( this, event );
		}
	...
	// Registers the persistence layer monitor as a reactive event listener
	// for events of type 'event'.
		...
		throws EventListenerNotRegisteredException
	{
		try
		{
			eventManager.registerReActiveEventListener( this, event );
		}
		catch ( EventListenerAlreadyRegisteredException elare )
		{
			// This is no big deal, we've just forgotten to clean up after
	...
	// Unregisters the persistence layer monitor as a proactive event listener
	// for events of type 'event.'
	private void unregisterProActiveEvent( Event event )
	{
		try
		{
			eventManager.unregisterProActiveEventListener( this, event );
		}
	...
	// Unregisters the persistence layer monitor as a reactive event listener
	// for events of type 'event.'
	private void unregisterReActiveEvent( Event event )
	{
		try
		{
			eventManager.unregisterReActiveEventListener( this, event );
		}
		catch ( EventListenerNotRegisteredException elnre )
		{
			elnre.printStackTrace();

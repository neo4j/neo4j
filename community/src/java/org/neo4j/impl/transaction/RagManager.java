 ...
 * When a {@link RWLock} cannot give the lock to a transaction the tx has
 * to wait and that may lead to a deadlock. So before the tx is put into
 * wait mode the {@link RagManager#checkWaitOn} method is invoked to check
 * if a wait of this transaction will lead to a deadlock. 
 * <p>
 * The <CODE>checkWaitOn</CODE> throws a {@link DeadlockDetectedException} if 
 * a deadlock would occur when the transaction would wait for the resource. 
 ...
 * to the tx (R->T) and when a transaction waits for a resource a relationship 
 * is added from the tx to the resource (T->R). The only thing we need to 
 * do to see if a deadlock occurs when some transaction waits for a resource 
 * is to traverse node nodespace starting on the resource and see if we can get 
 * back to the tx ( T1 wants to wait on R1 and R1->T2->R2->T3->R8->T1 <==> 
 * deadlock!).
 */
class RagManager
{
	...
	synchronized void dumpStack()
	{
		...
		{
			Transaction tx = (Transaction) transactions.next();

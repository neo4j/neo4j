...
public class TestNeoStore extends AbstractNeoTestCase
{
	...
	private static class MyPropertyIndex 
		extends org.neo4j.impl.core.PropertyIndex
	{
		private static Map<String,PropertyIndex> stringToIndex = 
			new HashMap<String,PropertyIndex>();
		private static Map<Integer,PropertyIndex> intToIndex = 
		...
		public static Iterable<PropertyIndex> index( String key )
        {
			...
			{
				return Arrays.asList( new PropertyIndex[] { 
					stringToIndex.get( key ) } ); 
			}
	...
	private void initializeStores() throws IOException
	{
		try
		{
			EventManager eventManager = 
				getEmbeddedNeo().getConfig().getEventModule().getEventManager();
			LockManager lockManager = 
				getEmbeddedNeo().getConfig().getLockManager();
			LockReleaser lockReleaser = 
				getEmbeddedNeo().getConfig().getLockReleaser();
			ds = new NeoStoreXaDataSource( "neo", 
				"nioneo_logical.log", lockManager, lockReleaser, eventManager );
		}
	...
	{
		Iterator<PropertyIndex> itr = MyPropertyIndex.index( 
			key ).iterator();
		if ( !itr.hasNext() )
		{
	...
	private Object getValue( PropertyRecord propertyRecord ) throws IOException
	{
		...
		{
			return pStore.getStringFor( propertyRecord, null );
		}
		...
		{
			return pStore.getArrayFor( propertyRecord, null );
		}

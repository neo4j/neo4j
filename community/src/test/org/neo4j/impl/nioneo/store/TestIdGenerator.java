...
public class TestIdGenerator extends TestCase
{
	...
	public void testCreateIdGenerator()
	{
		...
		try
		{
			IdGenerator idGenerator = 
				new IdGenerator( "testIdGenerator.id", 1008 );
			try
			{
				IdGenerator.createGenerator( "testIdGenerator.id" );
			...
			FileChannel fileChannel = 
				new FileInputStream( "testIdGenerator.id" ).getChannel();
	...
	public void testClose()
	{
		try
		{
			IdGenerator.createGenerator( "testIdGenerator.id" );
			IdGenerator idGenerator = 
				new IdGenerator( "testIdGenerator.id", 2 );
			idGenerator.close();
			try
			{
				idGenerator.nextId();
	...
	public void testOddAndEvenWorstCase()
	{
		...
		try
		{
			IdGenerator.createGenerator( "testIdGenerator.id" );
			IdGenerator idGenerator = 
				new IdGenerator( "testIdGenerator.id", 128 );
			for ( int i = 0; i < capacity; i++ )
			{
				idGenerator.nextId();
			}
			java.util.Map<Integer,Object> freedIds = 
			...
			for ( int i = 0; i < capacity - 1; i+=2 )
			{
				...
				{
					throw new RuntimeException( "Id=" + id + " prevId=" + 
						oldId + " list.size()=" + freedIds.size() );
				}
				oldId = id;
			}
	...
	public void testRandomTest() throws IOException
	{
		...
		IdGenerator.createGenerator( "testIdGenerator.id" );
		IdGenerator idGenerator = 
			new IdGenerator( "testIdGenerator.id", grabSize );

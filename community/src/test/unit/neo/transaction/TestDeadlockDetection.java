...
public class TestDeadlockDetection extends TestCase
{
	...
	{
		private static LockManager lm = LockManager.getManager();
	...
	public void testDeadlockDetection()
	{
		...
		try
		{
			...
			t1.getReadLock( r1 ); t1.waitForCompletionOfTask();
			t1.getReadLock( r4 ); t1.waitForCompletionOfTask();
			t2.getReadLock( r2 ); t2.waitForCompletionOfTask();
			t2.getReadLock( r3 ); t2.waitForCompletionOfTask();
			t3.getReadLock( r3 ); t3.waitForCompletionOfTask();
			t3.getWriteLock( r1 );
			// t3-r1-t1
			t2.getWriteLock( r4 );
			sleepSome();
			// t2-r4-t1
			t1.getWriteLock( r2 ); t1.waitForCompletionOfTask();
			...
			// dead lock
			sleepSome();
			t2.getWriteLock( r2 ); t2.waitForCompletionOfTask();
			...
			t2.getReadLock( r4 ); t2.waitForCompletionOfTask();
			t1.releaseWriteLock( r2 ); t1.waitForCompletionOfTask();
			t1.getReadLock( r2 ); t1.waitForCompletionOfTask();
			t1.releaseReadLock( r1 );
			t3.waitForCompletionOfTask(); // give r1 to t3
			t3.getReadLock( r2 ); t3.waitForCompletionOfTask();
			t3.releaseWriteLock( r1 ); 
			t1.getReadLock( r1 ); t1.waitForCompletionOfTask(); // give r1->t1
			t1.getWriteLock( r4 );
			t3.getWriteLock( r1 );
			t4.getReadLock( r2 ); t4.waitForCompletionOfTask();
			// deadlock
			t2.getWriteLock( r2 ); 
			...
			// resolve 
			t2.releaseReadLock( r4 ); t1.waitForCompletionOfTask();
			t1.releaseWriteLock( r4 ); t1.waitForCompletionOfTask();
			t1.releaseReadLock( r1 ); t1.waitForCompletionOfTask();
			t2.getReadLock( r4 );
			t3.waitForCompletionOfTask(); // give r1 to t3
			t3.releaseWriteLock( r1 );
			t1.getReadLock( r1 ); t1.waitForCompletionOfTask(); // give r1 to t1
			t1.getWriteLock( r4 );
			t3.releaseReadLock( r2 ); t3.waitForCompletionOfTask();
			t3.getWriteLock( r1 );
			// cleanup
			...
			t1.releaseReadLock( r1 );
			t3.waitForCompletionOfTask(); // give r1 to t3
			t3.releaseWriteLock( r1 ); t3.waitForCompletionOfTask();
			t1.releaseReadLock( r2 );
			t4.releaseReadLock( r2 );
			t2.releaseReadLock( r3 );
			t3.releaseReadLock( r3 );
			t1.waitForCompletionOfTask(); t2.waitForCompletionOfTask();
			t3.waitForCompletionOfTask(); t4.waitForCompletionOfTask();
			// -- special case...
			t1.getReadLock( r1 ); t1.waitForCompletionOfTask();
			t2.getReadLock( r1 ); t2.waitForCompletionOfTask();
			t1.getWriteLock( r1 ); // t1->r1-t1&t2
			sleepSome();
			t2.getWriteLock( r1 ); t2.waitForCompletionOfTask();
			assertTrue( t2.isLastGetLockDeadLock() );
			// t2->r1->t1->r1->t2
			t2.releaseReadLock( r1 );
			t1.waitForCompletionOfTask();
			t1.releaseReadLock( r1 ); t1.waitForCompletionOfTask();
			t1.releaseWriteLock( r1 ); t1.waitForCompletionOfTask();
		}
		catch ( Exception e )
		{
		...
		{
			t1.quit();t2.quit();t3.quit();t4.quit();
		}
	}
	...
	public static class StressThread extends Thread
	{
		...
		public void run()
		{
			try
			{
				...
				java.util.Stack<Object> lockStack = 
					new java.util.Stack<Object>();
				java.util.Stack<ResourceObject> resourceStack = 
					new java.util.Stack<ResourceObject>();
				try
				{

...
This chapter focuses on how to use the Manual Indexes and Autoindexes.
...
Legacy Indexing operations are part of the http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/index/package-summary.html[Neo4j index API].
...
| type | +exact+, +fulltext+ |
  +exact+ is the default and uses a Lucene http://lucene.apache.org/core/old_versioned_docs/versions/{lucene-version}/api/core/org/apache/lucene/analysis/KeywordAnalyzer.html[keyword analyzer].
...
| analyzer | the full class name of an http://lucene.apache.org/core/old_versioned_docs/versions/{lucene-version}/api/core/org/apache/lucene/analysis/Analyzer.html[Analyzer] | Overrides the +type+ so that a custom analyzer can be used.
...
Note that the http://lucene.apache.org/core/old_versioned_docs/versions/{lucene-version}/api/core/org/apache/lucene/search/TermQuery.html[TermQuery] is basically the same thing as using the +get+ method on the index.

...
To configure this in the _conf/neo4j.properties_ file, use the configuration keys +node_auto_indexing+ and
+relationship_auto_indexing+. For embedded mode, use the configuration options
+GraphDatabaseSettings.node_auto_indexing+ and +GraphDatabaseSettings.relationship_auto_indexing+.
...
link:javadocs/org/neo4j/graphdb/index/Index.html[+Index+] with exactly the same functionality.

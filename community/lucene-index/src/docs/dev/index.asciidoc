...
. Separate index engines can be used, with {lucene-base-uri}/index.html[Apache Lucene] being the default backend included with Neo4j.
...
Indexing operations are part of the link:javadocs/org/neo4j/graphdb/index/package-summary.html[Neo4j index API].

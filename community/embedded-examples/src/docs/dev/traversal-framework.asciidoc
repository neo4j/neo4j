...
* _Expanders_ -- define what to traverse, typically in terms of relationship direction and type.
...
Sets the rules for how positions can be revisited during a traversal as stated in link:javadocs/org/neo4j/kernel/Uniqueness.html[+Uniqueness+].
Default if not set is link:javadocs/org/neo4j/kernel/Uniqueness.html#NODE_GLOBAL[+NODE_GLOBAL+].
...
The same result can be achieved by calling the link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#order[+order+] method with ordering policies from the link:javadocs/org/neo4j/kernel/Traversal.html#preorderDepthFirst[+Traversal+] link:javadocs/org/neo4j/kernel/Traversal.html#preorderBreadthFirst[+factory+], or to write your own +BranchSelector+/+BranchOrderingPolicy+ and pass in.
...
A factory for creating BranchSelectors to decide in what order branches are returned (where a branch's position is represented as a link:javadocs/org/neo4j/graphdb/Path.html[+Path+] from the start node to the current node). Common policies are link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#depthFirst()[+depth-first+] and link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#breadthFirst()[+breadth-first+] and that's why there are convenience methods for those.
...
A more generic version of relationships where a +RelationshipExpander+ is injected, defining all relationships to be traversed for any given node. By default (and when using relationships) a link:javadocs/org/neo4j/kernel/Traversal.html#emptyExpander[default expander] is used, where any particular order of relationships isn't guaranteed.
There's another implementation which guarantees that relationships are traversed in link:javadocs/org/neo4j/kernel/OrderedByTypeExpander.html[order of relationship type], where types are iterated in the order they were added.

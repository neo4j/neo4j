...
Sets the rules for how positions can be revisited during a traversal as stated in http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/Uniqueness.html[+Uniqueness+].
Default if not set is http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/Uniqueness.html#NODE_GLOBAL[+NODE_GLOBAL+].
...
The same result can be achieved by calling the http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/TraversalDescription.html#order[+order+] method with ordering policies from http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/BranchOrderingPolicies.html[+BranchOrderingPolicies+], or to write your own +BranchSelector+/+BranchOrderingPolicy+ and pass in.
...
A factory for creating ++BranchSelector++s to decide in what order branches are returned (where a branch's position is represented as a http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/graphdb/Path.html[+Path+] from the start node to the current node).
Common policies are http://components.neo4j.org/neo4j-kernel/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/TraversalDescription.html#depthFirst()[+depth-first+] and http://components.neo4j.org/neo4j-kernel/{neo4j-version}/apidocs/org/neo4j/graphdb/traversal/TraversalDescription.html#breadthFirst()[+breadth-first+] and that's why there are convenience methods for those.
...
By default (and when using relationships) a http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/kernel/Traversal.html#emptyExpander[default expander] is used, where any particular order of relationships isn't guaranteed.
There's another implementation which guarantees that relationships are traversed in http://components.neo4j.org/neo4j/{neo4j-version}/apidocs/org/neo4j/kernel/OrderedByTypeExpander.html[order of relationship type], where types are iterated in the order they were added.

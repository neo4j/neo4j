...
public class AncestorsUtil
{
    ...
    private static LinkedList<Node> getAncestorsPlusSelf( Node node, PathExpander expander )
    {
        ...
        ancestors.add( node );
        Iterator<Relationship> relIterator = expander.expand( new SingleNodePath( node ), null ).iterator();
        while ( relIterator.hasNext() )
        {
            Relationship rel = relIterator.next();
            node = rel.getOtherNode( node );
            ancestors.add( node );
            relIterator = expander.expand( new SingleNodePath( node ), null ).iterator();
        }
        return ancestors;
    }
    ...
    private static void lookForCommonAncestor( LinkedList<Node> commonAncestors, Node currentNode, PathExpander expander )
    {
        while ( currentNode != null )
        {
            ...
            Iterator<Relationship> relIt = expander.expand( new SingleNodePath( currentNode ), null ).iterator();
            if ( relIt.hasNext() )
            {

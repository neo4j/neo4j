...
class GeometryType extends Type {
    ...
    String toString(GenericKey<?> state) {
        ...
        return format(
                "Geometry[tableId:%d, code:%d, rawValue:%d, value:%s",
                state.long1, state.long2, state.long0, asValueString);
    }
    ...
    static int compare(
            ...
            long this_long2,
            long this_long3,
            long[] this_long1Array,
            int this_coordinates_offset,
            long that_long0,
            long that_long1,
            long that_long2,
            long that_long3,
            long[] that_long1Array,
        ...
        for (int i = 0; i < dimensions; i++) {
            ...
            int coordinateComparison = Long.compare(
                    this_long1Array[this_coordinates_offset + i], that_long1Array[that_coordinates_offset + i]);
    ...
    static void assertHasCoordinates(GenericKey<?> state) {
        if (!hasCoordinates(state)) {
            throw new IllegalStateException(
                    "This geometry key doesn't have coordinates and can therefore neither be persisted nor generate point value.");
        }
    }

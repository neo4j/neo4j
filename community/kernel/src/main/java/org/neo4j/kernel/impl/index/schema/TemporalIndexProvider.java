...
public class TemporalIndexProvider extends NativeIndexProvider<TemporalSchemaKey,NativeSchemaValue>
{
    ...
    @Override
    Layout<TemporalSchemaKey,NativeSchemaValue> layout( SchemaIndexDescriptor descriptor )
    {
        return new TemporalLayout();
    }

    @Override
    protected IndexPopulator newIndexPopulator( File storeFile, Layout<TemporalSchemaKey,NativeSchemaValue> layout, SchemaIndexDescriptor descriptor,
            long indexId, IndexSamplingConfig samplingConfig )
    {
        return new TemporalIndexPopulator( pageCache, fs, storeFile, layout, monitor, descriptor, indexId, samplingConfig );
    }

    @Override
    protected IndexAccessor newIndexAccessor( File storeFile, Layout<TemporalSchemaKey,NativeSchemaValue> layout, SchemaIndexDescriptor descriptor,
            long indexId, IndexSamplingConfig samplingConfig ) throws IOException
    {
        return new TemporalIndexAccessor( pageCache, fs, storeFile, layout, recoveryCleanupWorkCollector, monitor, descriptor, indexId, samplingConfig );
    }

...
/**
 * Migrates a neo4j database from one version to the next. Instantiated with a {@link Legacy19Store}
 * representing the old version and a {@link org.neo4j.kernel.impl.nioneo.store.NeoStore} representing the new version.
 *
 * Since only one store migration is supported at any given version (migration from the previous store version)
 * the migration code is specific for the current upgrade and changes with each store format version.
 */
public class StoreMigrator19
{
    private final MigrationProgressMonitor progressMonitor;

    public StoreMigrator19( MigrationProgressMonitor progressMonitor )
    {
    ...
    {
        private final Legacy19Store legacyStore;
        private final NeoStore neoStore;
        private final long totalEntities;
        private int percentComplete;

        public Migration( Legacy19Store legacyStore, NeoStore neoStore )
        {

...
public class SpatialIndexProvider extends NativeIndexProvider<SpatialSchemaKey,NativeSchemaValue>
{
    ...
    @Override
    Layout<SpatialSchemaKey,NativeSchemaValue> layout( SchemaIndexDescriptor descriptor )
    {
        return new SpatialLayout( settingsFactory );
    }

    @Override
    protected IndexPopulator newIndexPopulator( File storeFile, Layout<SpatialSchemaKey,NativeSchemaValue> layout, SchemaIndexDescriptor descriptor,
            long indexId, IndexSamplingConfig samplingConfig )
    {
        return new SpatialIndexPopulator( pageCache, fs, storeFile, layout, monitor, descriptor, indexId, samplingConfig, configuration );
    }

    @Override
    protected IndexAccessor newIndexAccessor( File storeFile, Layout<SpatialSchemaKey,NativeSchemaValue> layout, SchemaIndexDescriptor descriptor, long indexId,
            IndexSamplingConfig samplingConfig ) throws IOException
    {
        return new SpatialIndexAccessor( pageCache, fs, storeFile, layout, recoveryCleanupWorkCollector, monitor, descriptor, indexId, samplingConfig,
                configuration );
    }

...
class LegacyLogEntryWriter
{
    ...
    LegacyLogEntryWriter( FileSystemAbstraction fs, Logging logging )
    {
        this( fs, logging, defaultLogEntryWriterFactory );
    }

    LegacyLogEntryWriter( FileSystemAbstraction fs, Logging logging, Function<WritableLogChannel,
            LogEntryWriter> factory )
    {
        this.fs = fs;
    ...
    public WritableLogChannel openWritableChannel( File file ) throws IOException
    {
        ...
        final PhysicalLogVersionedStoreChannel channel =
                new PhysicalLogVersionedStoreChannel( storeChannel, version, CURRENT_LOG_VERSION );
        return new PhysicalWritableLogChannel( channel );
    }

    public void writeLogHeader( WritableLogChannel channel, LogHeader header ) throws IOException
    {
        LogHeaderWriter.writeLogHeader( channel, header.logVersion, header.lastCommittedTxId );
    }

    public void writeAllLogEntries( WritableLogChannel channel, IOCursor<LogEntry> cursor ) throws IOException
    {
        final LogEntryWriter writer = factory.apply( channel );
        ...
        while ( cursor.next() )
        {
            ...
            {
                perhapsOrphanCommands( commands );
    ...
    private void perhapsOrphanCommands( List<Command> commands )
    {
        if ( !commands.isEmpty() )
        {
            logger.error( "Commands found without a commit entry" );
        }
    }
}

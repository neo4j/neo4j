 ...
 * </p>
 */
public class WorkSync<Material, Work extends WorkSync.CombinableWork<Material,Work>>
{
    private final Material material;
    private final AtomicReference<WorkUnit<Material,Work>> stack;
    private final WorkUnit<Material,Work> stackEnd;
    ...
    public void apply( Work work )
    {
        ...
        WorkUnit<Material,Work> unit = new WorkUnit<>( work );
    ...
    {
        WorkUnit<Material,Work> batch = reverse( stack.getAndSet( stackEnd ) );
        Work combinedWork = combine( batch );
    ...
    private WorkUnit<Material,Work> reverse( WorkUnit<Material,Work> batch )
    {
        WorkUnit<Material,Work> result = stackEnd;
        while ( batch != stackEnd )
        {
            WorkUnit<Material,Work> tmp = batch.next;
            while ( tmp == null )
            {
    ...
    private Work combine( WorkUnit<Material,Work> batch )
    {
        Work result = null;
        while ( batch != stackEnd )
        {
    ...
    private void markAsDone( WorkUnit<Material,Work> batch )
    {
        while ( batch != stackEnd )
        {
            batch.done = true;
    ...
    private static class WorkUnit<Material, Work extends CombinableWork<Material,Work>>
    {
        final Work work;
        volatile WorkUnit<Material,Work> next;
        volatile boolean done;

        private WorkUnit( Work work )
        {
            this.work = work;
        }
    }

    public static interface CombinableWork<Material, Work extends CombinableWork<Material,Work>>
    {
        Work combine( Work work );

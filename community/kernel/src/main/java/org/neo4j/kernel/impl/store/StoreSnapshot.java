 ...
 * occurring after the snapshot's latest (i.e. via {@link Recovery}).
 *
 ...
 *
 * In an attempt to ensure this, the {@link StoreSnapshotProvider} claims a lock on the {@link CheckPointer} when creating a
 * StoreSnapshot. This lock is released when calling {@link #close()}. The stream of unrecoverable files should be exhausted,
 * and the snapshot closed, as fast as possible.
 */
public final class StoreSnapshot implements AutoCloseable
{
    ...
    StoreSnapshot( Stream<StoreResource> unrecoverableFiles, Path[] recoverableFiles,

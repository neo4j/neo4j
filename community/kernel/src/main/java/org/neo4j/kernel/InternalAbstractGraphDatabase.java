...
import org.neo4j.io.pagecache.monitoring.jfr.JfrPageCacheMonitorFactory;
        ...
        implements GraphDatabaseService, GraphDatabaseAPI, SchemaWriteGuard
{
    ...
    {
        String monitorType = config.get( Configuration.pagecache_monitor );
        try
        {
            switch ( monitorType )
            {
                case "jfr": return JfrPageCacheMonitorFactory.createJfrPageCacheMonitor();
            }
        }
        catch ( Throwable throwable )
        {
            msgLog.warn( "Unable to create page cache monitor of type " + monitorType, throwable );
        }
        return new CountingPageCacheMonitor();
    }
    ...
    public static class Configuration
    {
        ...
        public static final Setting<String> pagecache_monitor =
                setting( "pagecache_monitor", Settings.STRING, "" );
    }

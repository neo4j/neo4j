        ...
        LockOperations
{
            ...
            throws ConstraintValidationKernelException, EntityNotFoundException
    {
        ...
        state.locks().implicit().acquireShared( ResourceTypes.SCHEMA, schemaResource() );

        state.locks().implicit().acquireExclusive( ResourceTypes.NODE, nodeId );
        state.assertOpen();
    ...
    {
        state.locks().implicit().acquireExclusive( ResourceTypes.SCHEMA, schemaResource() );
        state.assertOpen();
    ...
    public void relationshipDelete( final KernelStatement state, long relationshipId ) throws EntityNotFoundException
    {
        ...
        state.locks().implicit().acquireExclusive(ResourceTypes.RELATIONSHIP, relationshipId);
        state.assertOpen();
    ...
    private void lockRelationshipNodes( KernelStatement state, long startNodeId, long endNodeId )
    {
        ...
        state.locks().implicit().acquireExclusive( ResourceTypes.NODE, min( startNodeId, endNodeId ) );
        if ( startNodeId != endNodeId )
        {
            state.locks().implicit().acquireExclusive( ResourceTypes.NODE, max( startNodeId, endNodeId ) );
        }
    }
            ...
            throws ProcedureException, ProcedureConstraintViolation
    {
        ...
        // operation.
        state.locks().implicit().acquireExclusive( PROCEDURE, procedureResourceId( signature.name() ) );
    ...
    {
        state.locks().implicit().acquireExclusive( SCHEMA, schemaResource() );
        state.locks().implicit().acquireExclusive( PROCEDURE, procedureResourceId( name ) );
    ...
    {
        state.locks().implicit().acquireExclusive( ResourceTypes.GRAPH_PROPS, ResourceTypes.graphPropertyResource() );
        state.assertOpen();
    ...
    {
        state.locks().explicit().acquireExclusive( resourceType, resourceId );
        state.assertOpen();
    }
    ...
    {
        state.locks().explicit().acquireShared( resourceType, resourceId );
        state.assertOpen();
    }
    ...
    {
        state.locks().explicit().releaseExclusive( type, resourceId );
        state.assertOpen();
    }
    ...
    {
        state.locks().explicit().releaseShared( type, resourceId );
        state.assertOpen();
    }

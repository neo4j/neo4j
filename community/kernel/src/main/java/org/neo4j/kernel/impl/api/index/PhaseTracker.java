...
public class PhaseTracker
{
    ...
    private static final int PERIOD_INTERVAL = FeatureToggles.getInteger( PhaseTracker.class, "period_interval", 600 );
    ...
    PhaseTracker( Log log )
    {
        this( PERIOD_INTERVAL, log );
    }

    PhaseTracker( long periodIntervalInSeconds, Log log )
    {
    ...
    public void enterPhase( Phase phase )
    {
        ...
        if ( phase != currentPhase )
        {
            ...
            {
                // Report period
                reportPeriod( secondsSinceLastPeriodReport );
                lastPeriodReport = now;
            }
        }
    }
    ...
    void reportMain( String title )
    {
        log.debug( MESSAGE_PREFIX + mainReportString( title ) );
    }

    private void reportPeriod( long secondsSinceLastPeriodReport )
    {
    ...
    public EnumMap<Phase,Logger> times()
    {
        return times;
    }

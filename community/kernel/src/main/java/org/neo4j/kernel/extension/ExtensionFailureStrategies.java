...
public class ExtensionFailureStrategies {
    ...
    private static FailedToBuildExtensionException wrap(
            ExtensionFactory extensionFactory, UnsatisfiedDependencyException e) {
        return new FailedToBuildExtensionException(
                "Failed to build kernel extension " + extensionFactory + " due to a missing dependency: "
                        + e.getMessage(),
                e);
    }
    ...
    private static FailedToBuildExtensionException wrap(ExtensionFactory extensionFactory, Throwable e) {
        ...
        if (e instanceof LinkageError || e instanceof ReflectiveOperationException) {
            if (e instanceof LinkageError) {
                ...
                message.append(
                        " because it a reflective access to a class, method, or field, that is not in the class path: ");
            }
            ...
            message.append(" because of an unanticipated error: '")
                    .append(e.getMessage())
                    .append("'.");
        }

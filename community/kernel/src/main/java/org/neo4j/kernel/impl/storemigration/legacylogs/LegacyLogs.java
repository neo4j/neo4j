...
public class LegacyLogs
{
    ...
    public LegacyLogs( FileSystemAbstraction fs, Logging logging )
    {
        this( fs, new LegacyLogEntryReader( fs ), new LegacyLogEntryWriter( fs, logging ) );
    }
    ...
    public void migrateLogs( File storeDir, File migrationDir ) throws IOException
    {
        ...
        for ( File file : logFiles )
        {
            ...
            try ( IOCursor<LogEntry> cursor = pair.other();
                  WritableLogChannel channel = writer.openWritableChannel( new File( migrationDir, file.getName() ) ) )
            {

...
Here's an example of what such a retry block might look like when taking all these things into consideration:
[snippet,java]
----
...
If you don't want to write the code in the previous section yourself, there is a class called `org.neo4j.helpers.TransactionTemplate` that will help you achieve the same thing, and more. Below is an example of how to create, customize, and use this template for retries in transactions.
[snippet,java]
----
component=neo4j-kernel
source=examples/DeadlockDocTest.java

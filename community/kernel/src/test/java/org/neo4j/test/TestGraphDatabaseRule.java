...
public class TestGraphDatabaseRule extends ExternalResource implements Supplier<TestGraphDatabase>
{
    ...
    {
        final TargetDirectory testDirectory = TargetDirectory.forTest( testClass );
        return new TestGraphDatabaseRule( new Supplier<Pair<TestGraphDatabase,Consumer<Boolean>>>()
        {
            ...
            public Pair<TestGraphDatabase,Consumer<Boolean>> get()
            {
                ...
                cleanup();
                TestGraphDatabase.Builder builder = CommunityTestGraphDatabase.build();
                configuration.accept( builder );
                final TestGraphDatabase graphDB = builder.open( storeDir );
                Consumer<Boolean> finalCleanup = new Consumer<Boolean>()
                {
                    @Override
    ...
    {
        return ephemeral( new Consumer<TestGraphDatabase.EphemeralBuilder>()
        {
            @Override
    ...
    public static TestGraphDatabaseRule ephemeral( final Consumer<TestGraphDatabase.EphemeralBuilder> configuration )
    {
        return new TestGraphDatabaseRule( new Supplier<Pair<TestGraphDatabase,Consumer<Boolean>>>()
        {
            ...
            public Pair<TestGraphDatabase,Consumer<Boolean>> get()
            {
                ...
                TestGraphDatabase.EphemeralBuilder builder = CommunityTestGraphDatabase.buildEphemeral().withFileSystem( fs );
                configuration.accept( builder );
                final TestGraphDatabase graphDatabase = builder.open();
                Consumer<Boolean> cleanup = new Consumer<Boolean>()
                {
                    @Override
                    public void accept( Boolean success )
                    {
                ...
                return Pair.of( graphDatabase, cleanup );
            }
        } );
    }

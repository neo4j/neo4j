...
//        rStore = xaCon.getRelationshipConsumer();
    ...
    public void testCreateNeoStore() throws Exception
    {
        ...
        for ( int i = 0; i < 3; i++ )
        {
            ...
            for ( RelationshipRecord rel : 
                xaCon.getWriteTransaction().getMoreRelationships( nodeIds[i], pos ) )
            {
        ...
        long rel1, long rel2, int relType1, int relType2 ) throws IOException
    {
        ...
        while ( true )
        {
            Iterable<RelationshipRecord> relData = xaCon.getWriteTransaction().getMoreRelationships( node, pos );
        ...
        long firstNode, long secondNode, int relType ) throws IOException
    {
        ...
        Iterator<RelationshipRecord> first = 
            xaCon.getWriteTransaction().getMoreRelationships( firstNode, firstPos ).iterator();
        first.next();
        ...
        Iterator<RelationshipRecord> second = 
            xaCon.getWriteTransaction().getMoreRelationships( secondNode, secondPos ).iterator();
        second.next();
        ...
        throws IOException
    {
        ...
        Iterator<RelationshipRecord> rels = 
            xaCon.getWriteTransaction().getMoreRelationships( node, pos ).iterator();

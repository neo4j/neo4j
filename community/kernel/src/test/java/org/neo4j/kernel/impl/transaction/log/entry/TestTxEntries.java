...
public class TestTxEntries
{
    ...
    {
        final GraphDatabase db = CommunityTestGraphDatabase.buildEphemeral().withFileSystem( fs.get() ).open( storeDir );
        createSomeTransactions( db );
        EphemeralFileSystemAbstraction snapshot = fs.snapshot( shutdownDbAction( db ) );

        CommunityTestGraphDatabase.buildEphemeral().withFileSystem( snapshot ).open( storeDir ).shutdown();
    }

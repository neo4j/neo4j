...
public class AbstractDynamicStoreTest
{
    ...
    {
        assertTrue(  store.isRecordInUse( ByteBuffer.wrap( new byte[] {(byte) (inUseByte | 0x10)} ) ) );
        assertFalse( store.isRecordInUse( ByteBuffer.wrap( new byte[] {(byte) (inUseByte & ~0x10)} ) ) );
    }
    ...
    private AbstractDynamicStore newTestableDynamicStore()
    {
        return new AbstractDynamicStore( fileName, new Config(), IdType.ARRAY_BLOCK, new DefaultIdGeneratorFactory(),
                new DefaultWindowPoolFactory(), fsr.get(), StringLogger.DEV_NULL, StoreVersionMismatchHandler.ACCEPT )
        {
            @Override

...
public class LegacyLogsTest
{
    ...
    public void shouldRewriteLogFiles() throws IOException
    {
        ...
        final WritableLogChannel writableChannel = mock( WritableLogChannel.class );
        ...
        when( writer.openWritableChannel( new File( migrationDir, getLegacyLogFilename( 1 ) ) ) ).
                thenReturn( writableChannel );

        // when
        new LegacyLogs( fs, reader, writer ).migrateLogs( storeDir, migrationDir );

        // then
        verify( writer, times( 1 ) ).writeLogHeader( writableChannel, header );
        verify( writer, times( 1 ) ).writeAllLogEntries( writableChannel, cursor );
    }

...
public class LegacyLogEntryWriterTest
{
    ...
    {
        // given
        final LegacyLogEntryWriter writer = new LegacyLogEntryWriter( fs, logging );
        final File output = new File( getLegacyLogFilename( 3 ) );
        final LogHeader header = new LogHeader( CURRENT_LOG_VERSION, 1, 42l );

        // when
        try ( WritableLogChannel channel = writer.openWritableChannel( output ) )
        {
    ...
    public void shouldWriteAllTheEntryInACommitToTheFile() throws IOException
    {
        ...
        final LegacyLogEntryWriter writer = new LegacyLogEntryWriter( fs, logging, liftToFactory( logEntryWriter ) );
        ...
        writer.writeAllLogEntries( null /* ignored by the fake writer */, cursor );

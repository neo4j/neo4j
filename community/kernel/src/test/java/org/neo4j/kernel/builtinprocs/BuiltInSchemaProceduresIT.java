...
public class BuiltInSchemaProceduresIT extends KernelIntegrationTest
{
    ...
    public void testWeirdLabelName() throws Throwable
    {
        ...
        RawIterator<Object[],ProcedureException> stream =
                procedureCallOpsInNewTx().procedureCallRead( procedureName( nodesProcedureName ), new Object[0] );
...
//        printStream( stream );
    ...
    public void testRelsPropertiesRegardlessOfCreationOrder1() throws Throwable
    {
        ...
        // When
        RawIterator<Object[],ProcedureException> stream =
                procedureCallOpsInNewTx().procedureCallRead( procedureName( relsProcedureName ), new Object[0] );
...
//        printStream( stream );
    ...
    private long createNode( List<String> labels, List<String> propKeys, List<Value> propValues ) throws Throwable
    {
        ...
        DataWriteOperations dataWrite = statement.dataWriteOperations();
        TokenWriteOperations tokenWrite = statement.tokenWriteOperations();
        long nodeId = dataWrite.nodeCreate();

        for ( String labelname : labels )
        {
            int labelId = tokenWrite.labelGetOrCreateForName( labelname );
        ...
        for ( int i = 0; i < propKeys.size(); i++ )
        {
            ...
            int propKeyId = tokenWrite.propertyKeyGetOrCreateForName( propKeyName );
            dataWrite.nodeSetProperty( nodeId, propKeyId, propValue );
        }
        commit();
        return nodeId;
    }
    ...
    private void createRelationship( long startNode, String type, long endNode, List<String> propKeys, List<Value> propValues ) throws Throwable
    {
        ...
        long relId = dataWrite.relationshipCreate( typeId, startNode, endNode );
        ...
        for ( int i = 0; i < propKeys.size(); i++ )
        {
            ...
            Value propValue = propValues.get( i );
            int propKeyId = tokenWrite.propertyKeyGetOrCreateForName( propKeyName );
            dataWrite.relationshipSetProperty( relId, propKeyId, propValue );
        }
        commit();
    }

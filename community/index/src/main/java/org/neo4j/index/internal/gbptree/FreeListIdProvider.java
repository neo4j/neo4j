...
class FreeListIdProvider implements IdProvider
{
    ...
    private synchronized void flushReleaseCache( long stableGeneration, long unstableGeneration, CursorContext cursorContext ) throws IOException
    {
    ...
    public void visitFreelist( IdProviderVisitor visitor, CursorContext cursorContext ) throws IOException
    {
        ...
        try ( PageCursor cursor = pagedFile.io( 0, PagedFile.PF_SHARED_READ_LOCK, cursorContext ) )
        {
            ...
            do
            {
                ...
                int targetPos = pageId == writeMetaDataSnapshot.pageId ? writeMetaDataSnapshot.pos : freelistNode.maxEntries();
                while ( pos < targetPos )
                {
    ...
    {
        // Note: this can return write meta data for unwritten released ids. The caller is supposed to handle flushing vs. calling this method

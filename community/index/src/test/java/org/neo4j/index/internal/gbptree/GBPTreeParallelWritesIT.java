...
abstract class GBPTreeParallelWritesIT<KEY,VALUE>
{
    ...
    void start()
    {
        pageCache = PageCacheSupportExtension.getPageCache( new DefaultFileSystemAbstraction(), config().withPageSize( 256 ).withAccessChecks( true ) );
    }
    ...
    void shouldDoRandomWritesInParallel() throws IOException
    {
        ...
        try ( var index = new GBPTreeBuilder<>( pageCache, directory.file( "index" ), layout ).build() )
        {
            ...
            {
                data.forEachKeyValue( ( key, value ) -> assertThat( combined.put( key, value ) ).isNull() );
            }
            try ( var seek = allEntriesSeek( index, layout ) )
            {
                while ( seek.next() )
                {
                    ...
                    assertThat( layout.compareValue( removed.getRight(), seek.value() ) ).isZero();
                }
                ...
                if ( create )
                {
                    ...
                    synchronized ( committedIds )
                    {
                        for ( var i = 0; i < numEntries; i++ )
                        {
                            ...
                            if ( change )
                            {
                                // Remove it from created list temporarily so that no other concurrent thread will also delete/change this ID
                            ...
                            else
                            {
                                ids[i] = !committedIds.deleted.isEmpty()
                                         ? committedIds.deleted.removeAtIndex( random.nextInt( committedIds.deleted.size() ) )

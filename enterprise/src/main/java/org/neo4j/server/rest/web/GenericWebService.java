...
public abstract class GenericWebService
{
    ...
    protected Response createNodeIndex( String json, Renderer renderer )
    {
        json = dodgeStartingUnicodeMarker( json );
        PropertiesMap properties;
        try
        ...
        NodeIndexRepresentation nodeIndexRep = actions.createNodeIndex( (String) properties.getValue( "name" ) );
    ...
    protected Response getRelationshipIndexRoot( Renderer renderer )
    {
        ...
        return addHeaders( Response.ok( renderer.render( new RelationshipIndexRootRepresentation( uriInfo.getBaseUri(), database.getIndexManager() ) ), renderer.getMediaType() ) ).build();
    }

    protected Response addToIndex( IndexType indexType, String indexName, String key, String value, String json, Renderer renderer )
    {
        json = dodgeStartingUnicodeMarker( json );
        try
        {
...
//        return indexType.get( this, indexName, key, value, renderer );
    ...
    private Response getIndexedRelationships( String indexName, String key, String value, Renderer renderer )
    {
        ...
        List<IndexedRelationshipRepresentation> representations = actions.getIndexedRelationships( indexName, key, value );
        String entity = renderer.render( representations.toArray( new IndexedRelationshipRepresentation[representations.size()] ) );
        return addHeaders( Response.ok( entity, renderer.getMediaType() ) ).build();
    }
        ...
        RELATIONSHIP
                {
                    ...
                    public Response get( GenericWebService service, String indexName, String key, String value, long id, Renderer renderer )
                    {
                        if ( !service.actions.relationshipIsIndexed( indexName, key, value, id ) )
                        {
                            return Response.status( Status.NOT_FOUND ).build();
                        }
                    ...
                    public Representation add( GenericWebService service, String indexName, String key, String value, long id, Renderer renderer )
                    {
                        return service.actions.addRelationshipToIndex( indexName, key, value, id );
                    }

                    @Override
                    public boolean remove( GenericWebService service, String indexName, String key, String value, long id, Renderer renderer )
                    {
                        return service.actions.removeRelationshipFromIndex( indexName, key, value, id );
                    }
                };

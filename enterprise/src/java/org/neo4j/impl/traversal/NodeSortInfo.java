 ...
 * <p>
 * <CODE>NodeSortInfo</CODE> contains an inner class 
 * <CODE>PropertySortInfo</CODE> that can be used by a client to sort nodes 
 * depending on a node property. Here is a sample code snippet sorting nodes 
 * from a traverser:
 * <p><CODE><PRE>
 * 	// create the traverser ...
 * 	Traverser traverser = TraverserFactory.getFactory().createTraverser( .... );
 *  // sort on property "title"
 * 	Traverser sortedTraverser = traverser.sort( 
 * 		new NodeSortInfo.PropertySortInto( 
 * 			PropertyIndex.index( "title" ) ) );
 * 	// get the nodes sorted by property "title" 
 * 	while ( sortedTraverser.hasNext() )
 * 	{
 * 		Node node = sortedTraverser.nextNode();
 * 		// ...
 * 	}
 * </PRE></CODE>
 * <p>
...
public abstract class NodeSortInfo<T extends Node> implements Comparator<T>
{
	 ...
	 * or greater than the second. See {@link java.util.Comparator} for more 
	 * information.
	 * <p>
	 ...
	 * 
	 * @param node1 first argument
	 * @param node2 second argument
	 * @throws ClassCastException if argument type is non <CODE>Node</CODE>
	 */
		...
		extends NodeSortInfo<T>
	{
		 ...
		 * 
		 * @param index the node property used when sorting 
		 */
		...
		/**
		 * Creates a <CODE>PropertySortInfo</CODE> object using property 
		 * <CODE>index</CODE> for sorting. If <CODE>descending</CODE> is 
		 * set to true the order will be reversed.
		 * 
		 * @param index the node property used when sorting
		 * @param descending if true order will be reversed
		 */
		 ...
		 * information.
		 * <p>
		 * Both arguments has to be of type <CODE>Node</CODE> or a 
		 * <CODE>ClassCastException</CODE> will be thrown. If the first  
		 * argument lack the property used for sorting a negative integer will 
		 * be returned. If the second argument lacks the property a positive 
		 * integer will be returned. If both arguments lack the property zero 
		 * will be returned.
		 * <p>
		 ...
		 * be returned. If both properties aren't comparable zero will be returned.
		 *
		 * @param obj1 the first argument
		 * @param obj2 the second argument
		 * @throws ClassCastException if argument type is non Node
		 */
		public int compare( Node node1, Node node2 )
		{
			...
			if ( property1 instanceof Integer ) 
			{
				...
				{
					return ((Integer) property1).compareTo( 
						(Integer) property2 ) * direction;
				}
			...
			if ( property1 instanceof String )
			{
				...
				{
					return ( (String) property1).compareTo( 
						(String) property2 ) * direction;
				}

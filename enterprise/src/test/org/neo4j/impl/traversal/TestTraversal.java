...
public class TestTraversal extends AbstractNeoTestCase
{
	...
	private void sanityCheckTraverser
									(
										...
										Order type,
										Node startNode,
										RelationshipType traversableRel1,
										Direction direction1,
										RelationshipType traversableRel2,
		...
		throws Exception
	{
		...
		Traverser traverser = root.traverse
											(
												BREADTH_FIRST,
												StopEvaluator.END_OF_NETWORK,
												ReturnableEvaluator.ALL,
		...
		throws Exception
	{
		...
		try
		{
			...
			Traverser toTheMiddleTraverser = root.traverse
												(
													BREADTH_FIRST,
													StopEvaluator.END_OF_NETWORK,
													returnEvaluator,
...
//		throws CreateException
	...
	{
/*		Node startNode = getNeo().createNode();
		Node endNode1 = getNeo().createNode();
		Node endNode2 = getNeo().createNode();
		Relationship rel1 = startNode.createRelationshipTo( endNode1, 
			MyRelTypes.TEST );
		Relationship rel2 = startNode.createRelationshipTo( endNode2, 
			MyRelTypes.TEST );
		Traverser trav = startNode.traverse( 
			BREADTH_FIRST, StopEvaluator.END_OF_NETWORK, 
			ReturnableEvaluator.ALL, MyRelTypes.TEST, Direction.BOTH );

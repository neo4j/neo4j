...
public class TestJtaCompliance extends TestCase
{
	...
	public void setUp()
	{
		java.util.Map<String,FakeXAResource> map1 = 
			new java.util.HashMap<String,FakeXAResource>();
		map1.put( "xa_resource", new FakeXAResource( "XAResource1" ) );
		java.util.Map<String,FakeXAResource> map2 = 
			new java.util.HashMap<String,FakeXAResource>();
	 ...
	 *   calling thread with that transaction. 
	 * o Tests that after commit is invoked transaction is completed and a  
	 *   repeating call to commit/rollback results in an exception.
	 *
	 * TODO: check if commit is restricted to the thread that started
	 * the transaction, if not, do some testing.
	 */
	...
	/**
	 * o Tests that after rollback is invoked the transaction is completed and 
	 *   a repeating call to rollback/commit results in an exception.
	 *
	 * TODO: check if rollback is restricted to the thread that started
	 * the transaction, if not, do some testing.
	 */
	...
	/**
	 * o Tests that suspend temporarily suspends the transaction associated 
	 *   with the calling thread.
	 * o Tests that resume reinstate the transaction with the calling thread.
	 * o Tests that an invalid transaction passed to resume won't be 
	 *   associated with the calling thread.
	 * o Tests that XAResource.end is invoked with TMSUSPEND when transaction
	 *   is suspended.
	 * o Tests that XAResource.start is invoked with TMRESUME when transaction 
	 *   is resumed.
	 *
	 * TODO: 
	 * o Test that resume throws an exception if the transaction is 
	 *   already associated with another thread.
	 * o Test if a suspended thread may be resumed by another thread.
	 */
	...
	/**
	 * o Tests that beforeCompletion and afterCompletion are invoked.
	 * o Tests that the call is made in the same transaction context.
	 * o Tests status in before and after methods depending on commit/rollback.
	 *
	 ...
	 *
	 * TODO: Implement a FakeXAResource to check:
	 *	STATUS_COMMITTING
	 *	STATUS_PREPARED
	 *	STATUS_PREPEARING
	 *	STATUS_ROLLING_BACK

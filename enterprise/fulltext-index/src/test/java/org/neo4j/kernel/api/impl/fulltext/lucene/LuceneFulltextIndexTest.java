...
public class LuceneFulltextIndexTest extends LuceneFulltextTestSupport
{
    ...
    public void shouldDifferentiateNodesAndRelationships() throws Exception
    {
        ...
        try ( KernelTransactionImplementation tx = getKernelTransaction() )
        {
            nodesIndex = tx.schemaWrite().indexCreate( nodes, Optional.of( DESCRIPTOR.name() ), Optional.of( NODE_INDEX_NAME ) );
            relsIndex = tx.schemaWrite().indexCreate( rels, Optional.of( DESCRIPTOR.name() ), Optional.of( "rels" ) );
            tx.success();
        }
        await( nodesIndex );
        await( relsIndex );
    ...
    public void shouldPopulateIndexWithExistingNodesAndRelationships() throws Exception
    {
        ...
        try ( Transaction tx = db.beginTx() )
        {
            ...
            assertQueryFindsIds( "rels", "goodbye", firstRelID );
            assertQueryFindsIds( "rels", "different", secondRelID );
        }
    }

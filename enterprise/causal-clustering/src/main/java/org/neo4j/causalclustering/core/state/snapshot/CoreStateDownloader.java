...
public class CoreStateDownloader
{
    ...
    void downloadSnapshot( CatchupAddressProvider addressProvider, IdentityMetaData leadNode ) throws StoreCopyFailedException
    {
        try
        {
            ...
            StoreId remoteStoreId = remoteStore.getStoreId( leadNode.getAddress() );
            ...
            log.info( "Downloading snapshot from core server at %s", leadNode );
            ...
            CoreSnapshot coreSnapshot =
                    catchUpClient.makeBlockingRequest( leadNode.getAddress(), new CoreSnapshotRequest(), new CatchUpResponseAdaptor<CoreSnapshot>()
                    {
                        @Override
            ...
            else
            {
                ...
                CatchupResult catchupResult = remoteStore.tryCatchingUp( leadNode.getAddress(), localStoreId, localDatabase.storeDir(), false );

                if ( catchupResult == E_TRANSACTION_PRUNED )
                {
                    log.info( format( "Failed to pull transactions from (%s). They may have been pruned away", leadNode ) );

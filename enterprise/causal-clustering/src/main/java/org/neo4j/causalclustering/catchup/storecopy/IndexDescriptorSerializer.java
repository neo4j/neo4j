...
class IndexDescriptorSerializer
{
    ...
    static void serialize( IndexDescriptor indexDescriptor, WritableChannel channel ) throws IOException
    {
        ...
        {
            throw new IllegalArgumentException( "Not a recognized LabelSchemaDescriptor class: " + schemaDescriptor.getClass() );
        }
    }
    ...
    static IndexDescriptor deserialize( ReadableChannel channel ) throws IOException
    {
        ...
        case LABEL_SCHEMA_DESCRIPTOR:
            ...
            LabelSchemaDescriptor schema = SchemaDescriptorFactory.forLabel( labelId, propertyIds );
            if ( type == IndexDescriptor.Type.GENERAL )
            {
                return SchemaIndexDescriptorFactory.forSchema( schema );
            }
            else if ( type == IndexDescriptor.Type.UNIQUE )
            {

...
class HostnamePortAsListenAddress
{
    ...
    {
        return config.get( Settings.setting( setting.name(), rangeIgnoringSettingsFunction(), setting.getDefaultValue() ) );
    }

    private static Function<String,ListenSocketAddress> rangeIgnoringSettingsFunction()
    {
        return ((Function<String,String>) s -> s.split( "-" )[0]).andThen( Settings.LISTEN_SOCKET_ADDRESS );
    }
}

...
public class OnlineBackupCommandConfigLoaderTest
{
    ...
    @Test
    public void errorNotHandledForIncorrectAdditionalConfig() throws IOException, CommandFailed
    {
        // given a file with a bad configuration
        File additionalConf = testDirectory.file( "neo4j.conf" );
        additionalConf.createNewFile();
        appendToFile( additionalConf, "dbms.security.log_path=def\n" );

        // then
        expected.expect( InvalidSettingException.class );

        // when additional configuration is loaded
        subject.loadConfig( Optional.of( additionalConf.toPath() ) );
    }

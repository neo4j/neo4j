...
[IMPORTANT]
As of version 1.9, backups are enabled by default.
That means that the configuration parameter `online_backup_enabled` defaults to +true+ and that makes the backup service available on the default port (+6362+).

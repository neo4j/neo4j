...
public class HaBackupStrategy extends LifecycleAdapter implements BackupStrategy
{
    ...
    public PotentiallyErroneousState<BackupStageOutcome> performIncrementalBackup( File backupDestination, Config config, OptionalHostnamePort fromAddress )
    {
        ...
        try
        {
            backupProtocolService.doIncrementalBackup( resolvedAddress.getHost(), resolvedAddress.getPort(), backupDestination, ConsistencyCheck.NONE,
                    timeout, config );
            return new PotentiallyErroneousState<>( BackupStageOutcome.SUCCESS, null );
        }
        catch ( IncrementalBackupNotPossibleException e )
        {
            return new PotentiallyErroneousState<>( BackupStageOutcome.FAILURE, e ); // This means the existing backup was so old, that an incremental was not possible
        }
    ...
    public PotentiallyErroneousState<BackupStageOutcome> performFullBackup( File desiredBackupLocation, Config config, OptionalHostnamePort userProvidedAddress )
    {
        ...
        try
        {
            backupProtocolService.doFullBackup( fromAddress.getHost(), fromAddress.getPort(), desiredBackupLocation, consistencyCheck, config, timeout, forensics );
            return new PotentiallyErroneousState<>( BackupStageOutcome.SUCCESS, null );
        }

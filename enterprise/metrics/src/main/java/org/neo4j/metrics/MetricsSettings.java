...
public class MetricsSettings implements LoadableConfig
{
    ...
    public static Setting<String> metricsPrefix = setting( "metrics.prefix", STRING, "neo4j" ).build();
    ...
    @Description( "The default enablement value for all the supported metrics. Set this to `false` to turn off all " +
                  ...
                  "metrics." )
    public static Setting<Boolean> metricsEnabled = setting( "metrics.enabled", BOOLEAN, FALSE ).build();
    ...
    public static Setting<Boolean> neoEnabled = setting( "metrics.neo4j.enabled", BOOLEAN ).inherits( metricsEnabled ).build();

    @Description( "Enable reporting metrics about transactions; number of transactions started, committed, etc." )
    public static Setting<Boolean> neoTxEnabled = setting( "metrics.neo4j.tx.enabled", BOOLEAN ).inherits( neoEnabled ).build();
    ...
    public static Setting<Boolean> jvmGcEnabled =
            setting( "metrics.jvm.gc.enabled", BOOLEAN ).inherits( neoEnabled ).build();

    @Description( "Enable reporting metrics about the memory usage." )
    public static Setting<Boolean> jvmMemoryEnabled = setting( "metrics.jvm.memory.enabled", BOOLEAN ).inherits( neoEnabled ).build();

    @Description( "Enable reporting metrics about the buffer pools." )
    public static Setting<Boolean> jvmBuffersEnabled = setting( "metrics.jvm.buffers.enabled", BOOLEAN ).inherits( neoEnabled ).build();

    @Description( "Enable reporting metrics about the current number of threads running." )
    public static Setting<Boolean> jvmThreadsEnabled = setting( "metrics.jvm.threads.enabled", BOOLEAN ).inherits( neoEnabled ).build();
    ...
    public static Setting<Boolean> cypherPlanningEnabled =
            setting( "metrics.cypher.replanning.enabled", BOOLEAN ).inherits( neoEnabled ).build();

    @Description( "Enable reporting metrics about Bolt Protocol message processing." )
    public static Setting<Boolean> boltMessagesEnabled = setting( "metrics.bolt.messages.enabled", BOOLEAN ).inherits( neoEnabled ).build();

    // CSV settings
    @Description( "Set to `true` to enable exporting metrics to CSV files" )
    public static Setting<Boolean> csvEnabled = setting( "metrics.csv.enabled", BOOLEAN, FALSE ).build();
    ...
    @Description( "The reporting interval for the CSV files. That is, how often new rows with numbers are appended to " +
                  "the CSV files." )
    public static Setting<Duration> csvInterval = setting( "metrics.csv.interval", DURATION, "3s" ).build();

    // Graphite settings
    @Description( "Set to `true` to enable exporting metrics to Graphite." )
    public static Setting<Boolean> graphiteEnabled = setting( "metrics.graphite.enabled", BOOLEAN, FALSE ).build();

    @Description( "The hostname or IP address of the Graphite server" )
    public static Setting<HostnamePort> graphiteServer = setting( "metrics.graphite.server", HOSTNAME_PORT, ":2003" ).build();

    @Description( "The reporting interval for Graphite. That is, how often to send updated metrics to Graphite." )
    public static Setting<Duration> graphiteInterval = setting( "metrics.graphite.interval", DURATION, "3s" ).build();
}

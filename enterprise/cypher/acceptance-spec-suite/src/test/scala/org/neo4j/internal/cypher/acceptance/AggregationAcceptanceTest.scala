...
class AggregationAcceptanceTest extends ExecutionEngineFunSuite with LernaeanTestSupport {
  ...
  test("should aggregate using as grouping key expressions using variables in scope and nothing else") {
    ...
    val result1 = testWith(Configs.CommunityInterpreted, query1, params).toList
    val result2 = testWith(Configs.CommunityInterpreted, query2, params).toList
  ...
  test("distinct aggregation on single node") {
    ...
    relate(node1, node2)
    relate(node2, node1)
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a)--() RETURN DISTINCT a")
  ...
  test("distinct aggregation on array property") {
    ...
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a) RETURN DISTINCT a.prop")
  ...
  test("Node count from count store plan should work with labeled nodes") {
    ...
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a:Person) WITH count(a) as c RETURN c")
  ...
  test("Count should work with projected node variable") {
    ...
    val node2 = createLabeledNode("Person")
    val node3 = createNode()
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a:Person) WITH a as b WITH count(b) as c RETURN c")
    result.toList should equal(List(Map("c" -> 2L)))
  }
  ...
  test("Count should work with projected relationship variable") {
    ...
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a:Person)-[r]->() WITH r as s WITH count(s) as c RETURN c")
    result.toList should equal(List(Map("c" -> 2L)))
  }
  ...
  test("combine grouping and aggregation with sorting") {
    ...
    val r1 = relate(node1, node2)
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a)--(b) RETURN a.prop, count(a) ORDER BY a.prop")
  ...
  test("combine simple aggregation on projection with sorting") {
    val node1 = createNode()
    val node2 = createNode()
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a) WITH a as b RETURN count(b) ORDER BY count(b)")
  ...
  test("combine simple aggregation with sorting (cannot use count store)") {
    val node1 = createNode(Map("prop" -> 1))
    val node2 = createNode(Map("prop" -> 2))
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a) RETURN count(a.prop) ORDER BY count(a.prop)")
  ...
  test("combine simple aggregation with sorting (can use node count store)") {
    val node1 = createNode()
    val node2 = createNode()
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a) RETURN count(a) ORDER BY count(a)")
  ...
  test("combine simple aggregation with sorting (can use relationship count store)") {
    ...
    val node2 = createNode()
    val r1 = relate(node1, node2)
    val result = testWith(Configs.AllExceptSleipnir, "MATCH (a)-[r]-(b) RETURN count(r) ORDER BY count(r)")

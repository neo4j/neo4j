...
class ExecutionEngineTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with CreateTempFileTestSupport with CypherComparisonSupport {
  ...
  val startConf = TestConfiguration(Versions(V3_2, V3_3, Versions.Default), Planners(Planners.Rule, Planners.Default), Runtimes(Runtimes.Interpreted, Runtimes.Default)) +
  ...
  test("shouldFilterOnGreaterThan") {
    val n = createNode()
    val result = executeWith(Configs.Interpreted, "match(node) where 0<1 return node")
  ...
  test("shouldGetTwoNodes") {
    ...
    val result = executeWith(Configs.Interpreted, s"match (node) where id(node) in [${node1.getId}, ${node2.getId}] return node")
  ...
  test("shouldReturnPathLength") {
    ...
    val result = executeWith(Configs.CommunityInterpreted, "match p = (n)-->(x) where id(n) = 0 return length(p)")
  ...
  test("parameterTypeErrorShouldBeNicelyExplained") {
    ...
    executeWith(Configs.All, query, params = Map("a" -> "Andres")) should be (empty)
  }
  ...
  test("shouldBeAbleToTakeParamsFromParsedStuff") {
    ...
    val result = executeWith(Configs.Interpreted, query, params = Map("a" -> Seq[Long](0)))
...
return r""")
  ...
  test("shouldHandleCheckingThatANodeDoesNotHaveAProp") {
    ...
    val result = executeWith(Configs.Interpreted, "match (a) where id(a) = 0 and not exists(a.propertyDoesntExist) return a")
...
order by a.COL1""".format(a, b))
  ...
  test("shouldToStringArraysPrettily") {
    ...
    val string = executeWith(Configs.All,  """match (n) where id(n) = 0 return n.foo""").dumpToString()
  ...
  test("shouldHandleComparisonsWithDifferentTypes") {
    ...
    val result = executeWith(Configs.Interpreted, "match (n) where id(n) = 0 and (n.belt = 'white' OR n.belt = false) return n")
  ...
  test("shouldReturnASimplePath") {
    ...
    val conf = startConf + Configs.Cost3_2 + Configs.Procs
  ...
  test("issue 432") {
    ...
    executeWith(Configs.CommunityInterpreted, q).toList should have size 1
  }
  ...
  test("shouldSupportArrayOfArrayOfPrimitivesAsParameterForInKeyword") {
    ...
    val result = executeWith(Configs.Interpreted, "match (n) where id(n) = 0 and n.lotteryNumbers in [[42, 87], [13], [42]] return n")
  ...
  test("filtering in match should not fail") {
    ...
    val result = executeWith(Configs.Interpreted, "MATCH (n)-->(me) WHERE id(n) = 0 AND me.name IN ['Neo'] RETURN me.name")
  ...
  test("should filter nodes by single negated label") {
    ...
    // WHEN
    val result = executeWith(Configs.Interpreted, "MATCH (n) WHERE id(n) in [0, 1, 2] AND not(n:foo) RETURN n")
  ...
  test("should create index") {
    ...
    val testconfiguration = TestConfiguration(Versions(V3_1, V3_2, V3_3, Versions.Default), Planners.Default, Runtimes(ProcedureOrSchema, Runtimes.Default)) + Configs.Rule2_3
    // WHEN
  ...
  test("union ftw") {
    ...
    // WHEN
    val result = executeWith(Configs.CommunityInterpreted, "match (n) where id(n) = 0 RETURN 1 as x UNION ALL match (n) where id(n) = 0 RETURN 2 as x")
  ...
  test("union distinct") {
    ...
    // WHEN
    val result = executeWith(Configs.CommunityInterpreted, "match (n) where id(n) = 0 RETURN 1 as x UNION match (n) where id(n) = 0 RETURN 1 as x")
  ...
  test("should iterate all node id sets from start during matching") {
    ...
    // when
    val result = executeWith(Configs.Interpreted, s"MATCH (src)-[r:EDGE]-(dst) WHERE id(src) IN $nodeIds AND id(dst) IN $nodeIds RETURN r")
  ...
  test("should return null on all comparisons against null") {
    ...
    // when
    val result = executeWith(Configs.Interpreted, "return 1 > null as A, 1 < null as B, 1 <= null as C, 1 >= null as D, null <= null as E, null >= null as F")

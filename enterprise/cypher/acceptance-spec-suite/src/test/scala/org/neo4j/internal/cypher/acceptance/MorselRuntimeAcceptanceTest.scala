...
class MorselRuntimeAcceptanceTest extends ExecutionEngineFunSuite {
  ...
  test("should support count with no grouping") {
    ...
    //Then
    comparable(result) should equal(Set(Map("count(n.prop)" -> 4)))
    result.getExecutionPlanDescription.getArguments.get("runtime") should equal("MORSEL")
  }
  ...
  test("should support multiple counts with no grouping") {
    ...
    //Then
    comparable(result) should equal(Set(Map("count(n.prop)" -> 2, "count(m.prop)" -> 2)))
    result.getExecutionPlanDescription.getArguments.get("runtime") should equal("MORSEL")
  }
  ...
  test("should support count with grouping") {
    ...
    //Then
    comparable(result) should equal(Set(
  ...
  test("should support average") {
    ...
    //Then
    comparable(result) should equal(Set(Map("avg(n.prop)" -> 55.0)))
    result.getExecutionPlanDescription.getArguments.get("runtime") should equal("MORSEL")
  }
  ...
  test("should support max") {
    ...
    //Then
    comparable(result) should equal(Set(Map("max(n.prop)" -> 100)))
    result.getExecutionPlanDescription.getArguments.get("runtime") should equal("MORSEL")
  }
  ...
  test("should support min") {
    ...
    //Then
    comparable(result) should equal(Set(Map("min(n.prop)" -> 10)))
    result.getExecutionPlanDescription.getArguments.get("runtime") should equal("MORSEL")
  }
  ...
  private def comparable(result: Result): Set[Map[String, AnyRef]] = {
    import scala.collection.JavaConverters._
    val buffer = ArrayBuffer[Map[String, AnyRef]]()
    while (result.hasNext) {
      val n = result.next()
      val map = mutable.Map[String, AnyRef]()
      n.entrySet().asScala.foreach(entry => map.put(entry.getKey, entry.getValue))
      buffer.append(map)
    }
    buffer.toSet
  }

...
//  MATCH (n:X)-[r:Y]->() WITH count(r) as rcount MATCH (n)-[r:Y]->() WHERE count(r) = rcount RETURN rcount, labels(n)
  ...
  test("count store on two unlabeled nodes") {
    ...
    // Then
    compareCount(query, 0, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2)
    compareCount(query, 9, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2, executeBefore = executeBefore)
  }
  ...
  test("count store on one labeled node and one unlabeled") {
    ...
    // Then
    compareCount(query, 0, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2)
    compareCount(query, 6, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2, executeBefore = executeBefore)
  }
  ...
  test("count store on two labeled nodes") {
    ...
    // Then
    compareCount(query, 0, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2)
    compareCount(query, 4, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2, executeBefore = executeBefore)
  }
  ...
  test("count store with many nodes") {
    ...
    // Then
    compareCount(query, 0, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2)
    compareCount(query, 36, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2, executeBefore = executeBefore)
  }
  ...
  test("count store with many but odd number of nodes") {
    ...
    // Then
    compareCount(query, 0, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2)
    compareCount(query, 108, expectOtherPlanIn = expectOtherPlan + Configs.Cost3_1 + Configs.Cost3_2, executeBefore = executeBefore)
  }

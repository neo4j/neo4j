...
trait LernaeanTestSupport extends CypherTestSupport {
  ...
  protected def testWith(expectedSuccessFrom: TestConfig, query: String, params: (String, Any)*): InternalExecutionResult = {
    val firstScenario = expectedSuccessFrom.scenarios.head
    ...
    for (thisScenario <- Configs.All.scenarios if thisScenario != firstScenario) {
  ...
  object Configs {
    def CompiledSource: TestConfig = TestConfig(Set(Scenarios.CompiledSource3_2))

    def CompiledByteCode: TestConfig = TestConfig(Set(Scenarios.CompiledByteCode))

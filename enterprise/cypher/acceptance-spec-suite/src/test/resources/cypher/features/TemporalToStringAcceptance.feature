...
Feature: TemporalToStringAcceptance
  ...
  Scenario: Should serialize local date time
    ...
    When executing query:
      """
      WITH localdatetime({year:1984, month:10, day:11, hour:12, minute:31, second:14, microsecond: 645876}) as d
      RETURN toString(d) as ts, localdatetime(toString(d)) = d as b
      """
    Then the result should be, in order:
      | ts                           | b    |
      | '1984-10-11T12:31:14.645876' | true |
    And no side effects
  ...
  Scenario: Should serialize date time
    ...
    When executing query:
      """
      WITH datetime({year:1984, month:10, day:11, hour:12, minute:31, second:14, microsecond: 645876, timezone: '+01:00'}) as d
      RETURN toString(d) as ts, datetime(toString(d)) = d as b
      """
    Then the result should be, in order:
      | ts                                 | b    |
      | '1984-10-11T12:31:14.645876+01:00' | true |
    And no side effects
  ...
  Scenario: Should serialize duration
    ...
    When executing query:
      """
      UNWIND [duration({years: 12, months:5, days: 14, hours:16, minutes: 12, seconds: 70, microseconds: 1}),
    ...
    Then the result should be, in order:
      | ts                              | b    |
      | 'P12Y5M14DT16H13M10.000001S'    | true |
      | 'P12Y5M-14DT16H'                | true |
      | 'PT11M'                         | true |
      | 'PT1.999S'                      | true |
      | 'PT-1.999S'                     | true |
  ...
  Scenario: Should serialize timezones correctly
    ...
    When executing query:
      """
      WITH datetime({year: 2017, month: 8, day: 8, hour:12, minute:31, second:14, microsecond: 645876, timezone: 'Europe/Stockholm'}) as d
      RETURN toString(d) as ts
      """
    Then the result should be, in order:
      | ts                                                   |
      | '2017-08-08T12:31:14.645876+02:00[Europe/Stockholm]' |
    And no side effects

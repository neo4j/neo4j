...
case class MapperOffsets(offset: Int, aggregationMapper: AggregationMapper)
 ...
 */
class AggregationMapperOperatorNoGrouping(slots: SlotConfiguration, aggregations: Array[MapperOffsets]) extends MiddleOperator {
  ...
  override def operate(iterationState: Iteration, data: Morsel, context: QueryContext, state: QueryState): Unit = {
    ...
    while (currentRow.currentRow < data.validRows) {
      ...
      while (accCount < aggregations.length) {
        aggregations(accCount).aggregationMapper.map(currentRow, queryState)
        accCount += 1
      }
    ...
    while (i < aggregations.length) {
      ...
      data.refs(aggregation.offset) = aggregation.aggregationMapper.result
      i += 1
    }
    data.validRows = i
  }

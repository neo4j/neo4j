...
class SlotAllocationTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  test("node hash join III") {
    ...
    val rhs = NodeByLabelScan(x, LabelName("label2")(pos), Set.empty)
    val rhsE = Expand(rhs, x, SemanticDirection.INCOMING, Seq.empty, y, IdName("r2"), ExpandAll)
  ...
  test("unwind and project") {
    ...
    val leaf = Argument()
    val unwind = UnwindCollection(leaf, IdName("x"), listOf(literalInt(1), literalInt(2), literalInt(3)))
  ...
  test("should allocate RollUpApply") {
    ...
    // RollUpApply(LHS, RHS, ...)
    val rollUp =
      RollUpApply(lhsLeaf, rhsProjection, IdName("c"), IdName("x"), nullableVariables = Set(IdName("r"), IdName("y")))

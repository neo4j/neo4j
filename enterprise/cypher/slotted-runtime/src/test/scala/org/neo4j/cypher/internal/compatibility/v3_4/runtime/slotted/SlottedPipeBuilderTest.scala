...
class SlottedPipeBuilderTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  private def build(beforeRewrite: LogicalPlan): Pipe = {
    ...
    val context = PipeExecutionBuilderContext(mock[Metrics.CardinalityModel], table, IDPPlannerName, new FakeReadOnlies, new FakeCardinalities)
  ...
  test("single optional node with expand") {
    // given
    val allNodesScan = AllNodesScan(IdName("x"), Set.empty)
    val optional = Optional(allNodesScan)
    val expand = Expand(optional, IdName("x"), SemanticDirection.INCOMING, Seq.empty, IdName("z"), IdName("r"), ExpandAll)
  ...
  test("single optional node with expand into") {
    ...
    val allNodesScan = AllNodesScan(IdName("x"), Set.empty)
    val optional = Optional(allNodesScan)
    val expand = Expand(optional, IdName("x"), SemanticDirection.INCOMING, Seq.empty, IdName("x"), IdName("r"), ExpandInto)
  ...
  test("cartesian product") {
    ...
    val rhs = NodeByLabelScan(IdName("y"), LabelName("label2")(pos), Set.empty)

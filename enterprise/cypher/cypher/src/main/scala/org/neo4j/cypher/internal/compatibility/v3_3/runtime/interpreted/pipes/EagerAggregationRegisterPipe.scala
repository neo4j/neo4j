...
import org.neo4j.cypher.internal.compatibility.v3_3.runtime.pipes.{MutableMaps, Pipe, PipeWithSource, QueryState}
...
case class EagerAggregationRegisterPipe(source: Pipe, pipelineInformation: PipelineInformation, keyExpressions: Set[String], aggregations: Map[String, AggregationExpression])
  ...
  protected def internalCreateResults(input: Iterator[ExecutionContext], state: QueryState) = {
    ...
    def createResults(key: Any, aggregator: scala.Seq[AggregationFunction]): ExecutionContext = {
      ...
      //TODO: We can possibly get rid of the map all together
      val rows = PrimitiveExecutionContext(pipelineInformation)
      for ((key,value) <- newMap) {
        val offset = pipelineInformation.getReferenceOffsetFor(key)
        rows.setRefAt(offset, value)
      }
      rows
    }
    ...
    input.foreach(ctx => {
      def getOffset(key:String) = {
        pipelineInformation.getReferenceOffsetFor(key)
      }

      val groupValues: Equals = keyNamesSize match {
        case 1 => Equivalent(ctx.getRefAt(getOffset(keyNames.head)))
        case 2 => (Equivalent(ctx.getRefAt(getOffset(keyNames.head))),
          Equivalent(ctx.getRefAt(getOffset(keyNames.last))))
        case 3 => (Equivalent(ctx.getRefAt(getOffset(keyNames.head))),
          Equivalent(ctx.getRefAt(getOffset(keyNames.tail.head))),
          Equivalent(ctx.getRefAt(getOffset(keyNames.last))))
        case _ => keyNames.map( k => Equivalent(ctx.getRefAt(pipelineInformation.getReferenceOffsetFor(k))))
      }

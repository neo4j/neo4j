...
object AggregationConverter {
  ...
  def aggregateExpressionConverter(opName: String,
                                   groupingVariables: Iterable[Variable],
                                   name: String,
                                   e: ast.Expression)(implicit context: CodeGenContext): AggregateExpression = {
    ...
    e match {
      case func: ast.FunctionInvocation =>
        func.function match {

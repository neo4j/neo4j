...
import org.neo4j.cypher.internal.frontend.v3_3.{ast => frontEndAst}
...
class EnterprisePipeBuilder(fallback: PipeBuilder,
  ...
  override def build(plan: LogicalPlan): Pipe = {
    ...
    plan match {
      ...
      case NodeIndexSeek(IdName(column), label, propertyKeys, valueExpr, _) =>
        ...
        NodeIndexSeekRegisterPipe(column,
                                  label,
                                  propertyKeys,
                                  valueExpr.map(convertExpressions),
                                  indexSeekMode,
                                  pipelineInformation)(id = id)
  ...
  override def build(plan: LogicalPlan, source: Pipe): Pipe = {
    ...
    plan match {
      ...
      case CreateNode(_, idName, labels, props) =>
        CreateNodeRegisterPipe(source,
                               idName.name,
                               pipeline,
                               labels.map(LazyLabel.apply),
                               props.map(convertExpressions))(id = id)
      ...
      case _: Selection | _: Limit | _: ErrorPlan | _: Skip =>
  ...
  private def createProjectionsForResult(columns: Seq[String], pipelineInformation1: PipelineInformation) = {
    val runtimeColumns: Seq[(String, commandExpressions.Expression)] = columns map { k =>

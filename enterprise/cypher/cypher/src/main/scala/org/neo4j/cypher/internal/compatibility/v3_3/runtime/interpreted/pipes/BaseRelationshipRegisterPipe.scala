...
case class CreateRelationshipRegisterPipe(src: Pipe, RelationshipKey: String, startNode: Int, typ: LazyType, endNode: Int,
                                          pipelineInformation: PipelineInformation,
                                          properties: Option[Expression])
                                 (val id: Id = new Id)
  extends BaseRelationshipRegisterPipe(src, RelationshipKey, startNode, typ: LazyType, endNode, pipelineInformation, properties) {
  override protected def handleNull(key: String) {
    //do nothing
  }
}

case class MergeCreateRelationshipRegisterPipe(src: Pipe, RelationshipKey: String, startNode: Int, typ: LazyType, endNode: Int,
                                               pipelineInformation: PipelineInformation,
                                               properties: Option[Expression])
                                              (val id: Id = new Id)
  extends BaseRelationshipRegisterPipe(src, RelationshipKey, startNode, typ: LazyType, endNode, pipelineInformation, properties) {

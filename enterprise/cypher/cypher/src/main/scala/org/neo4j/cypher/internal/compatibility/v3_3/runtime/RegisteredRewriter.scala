...
class RegisteredRewriter(tokenContext: TokenContext) {
  ...
  def apply(in: LogicalPlan, pipelineInformation: Map[LogicalPlan, PipelineInformation])
    ...
    resultPlan
      ...
      .foreach(v => throw new InternalException(s"Failed to rewrite away $v\n$resultPlan"))
  ...
  private def createRewriterFrom(rewrites: Map[LogicalPlan, LogicalPlan]) =
  ...
  private def rewriteCreator(pipelineInformation: PipelineInformation, thisPlan: LogicalPlan): Rewriter = {
    val innerRewriter = Rewriter.lift {
      case Property(Variable(key), PropertyKeyName(propKey)) =>
        ...
        val propExpression = (slot, maybeToken) match {
          case (LongSlot(offset, _, typ, name), Some(token)) if typ == CTNode =>
            NodeProperty(offset, token, s"$name.$propKey")
          case (LongSlot(offset, _, typ, name), None) if typ == CTNode =>
            NodePropertyLate(offset, propKey, s"$name.$propKey")
          case (LongSlot(offset, _, typ, name), Some(token)) if typ == CTRelationship =>
            RelationshipProperty(offset, token, s"$name.$propKey")
          case (LongSlot(offset, _, typ, name), None) if typ == CTRelationship =>
            RelationshipPropertyLate(offset, propKey, s"$name.$propKey")
          case _ =>
            throw new CantCompileQueryException(
              s"Expressions with on object other then nodes and relationships are not yet supported")
        }
      ...
      case _: ShortestPathExpression =>
        throw new CantCompileQueryException(
          s"Expressions with shortestPath functions not yet supported in register allocation")

      case _: ScopeExpression | _: NestedPlanExpression =>
        throw new CantCompileQueryException(
          s"Expressions with inner scope are not yet supported in register allocation")
    }
  ...
  private def stopAtOtherLogicalPlans(thisPlan: LogicalPlan): (AnyRef) => Boolean = { lp =>
    lp.isInstanceOf[LogicalPlan] && lp != thisPlan
  }
}

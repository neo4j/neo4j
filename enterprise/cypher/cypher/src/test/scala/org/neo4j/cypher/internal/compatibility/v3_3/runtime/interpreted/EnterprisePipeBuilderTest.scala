...
import org.neo4j.cypher.internal.compatibility.v3_3.runtime.interpreted.expressions.NodeProperty
...
import org.neo4j.cypher.internal.frontend.v3_3.symbols.CTAny
...
class EnterprisePipeBuilderTest extends CypherFunSuite with LogicalPlanningTestSupport2 {
  ...
  private def build(beforeRewrite: LogicalPlan): Pipe = {
    ...
    val executionPlanBuilder = new PipeExecutionPlanBuilder(
      ...
      pipeBuilderFactory = EnterprisePipeBuilderFactory(pipelines))
  ...
  test("single labelscan scan") {
    ...
    // then
    pipe should equal(
      NodesByLabelScanRegisterPipe("x",
                                   LazyLabel(label),
  ...
  test("single node with expand") {
    ...
    pipe should equal(
      ExpandAllRegisterPipe(
        AllNodesScanRegisterPipe(
          "x",
          PipelineInformation(Map("x" -> xNodeSlot), numberOfLongs = 1, numberOfReferences = 0))(),
        xNodeSlot.offset,
        rRelSlot.offset,
        zNodeSlot.offset,
        SemanticDirection.INCOMING,
        LazyTypes.empty,
        PipelineInformation(Map("x" -> xNodeSlot, "r" -> rRelSlot, "z" -> zNodeSlot),
                            numberOfLongs = 3,
  ...
  test("single node with expand into") {
    ...
    pipe should equal(
      ExpandIntoRegisterPipe(
        AllNodesScanRegisterPipe(
          "x",
          PipelineInformation(Map("x" -> nodeSlot), numberOfLongs = 1, numberOfReferences = 0))(),
        nodeSlot.offset,
        relSlot.offset,
        nodeSlot.offset,
        SemanticDirection.INCOMING,
  ...
  test("single optional node with expand") {
    ...
    val expand =
      Expand(optional, IdName("x"), SemanticDirection.INCOMING, Seq.empty, IdName("z"), IdName("r"), ExpandAll)(solved)
    ...
    val expandPipeline = PipelineInformation(Map("x" -> xNodeSlot, "r" -> rRelSlot, "z" -> zNodeSlot),
                                             numberOfLongs = 3,
                                             numberOfReferences = 0)
  ...
  test("single optional node with expand into") {
    ...
    val optional     = Optional(allNodesScan)(solved)
    val expand =
      Expand(optional, IdName("x"), SemanticDirection.INCOMING, Seq.empty, IdName("x"), IdName("r"), ExpandInto)(solved)
  ...
  test("optional node") {
    ...
    // then
    val expectedPipeLineInfo = PipelineInformation(Map("x" -> LongSlot(0, nullable = true, CTNode, "x")),
                                                   numberOfLongs = 1,
                                                   numberOfReferences = 0)
    pipe should equal(
      OptionalRegisteredPipe(
  ...
  test("single node with optionalExpand ExpandInto") {
    ...
    // then
    pipe should equal(
      OptionalExpandIntoRegisterPipe(
        AllNodesScanRegisterPipe("x",
                                 PipelineInformation(Map("x" -> LongSlot(0, nullable = false, CTNode, "x")),
                                                     numberOfLongs = 1,
                                                     numberOfReferences = 0))(),
      ...
      ApplyRegisterPipe(
        NodesByLabelScanRegisterPipe("x",
                                     ...
                                     PipelineInformation(Map("x" -> LongSlot(0, nullable = false, CTNode, "x")),
                                                         numberOfLongs = 1,
                                                         numberOfReferences = 0))(),
        NodeIndexSeekRegisterPipe(
          "z",
          label,
          Seq.empty,
          SingleQueryExpression(commands.expressions.Literal(42)),
          IndexSeek,
  ...
  ignore("optional travels through aggregation used for distinct") {
    ...
    val pipelineInformation = PipelineInformation(Map("x" -> LongSlot(0, nullable = true, CTNode, "x")),
                                                  numberOfLongs = 1,
                                                  numberOfReferences = 0)
    // then
    val labelScan    = NodesByLabelScanRegisterPipe("x", LazyLabel("label"), pipelineInformation)()
  ...
  ignore("optional travels through aggregation") {
    ...
    // then
    val pipelineInfo = PipelineInformation(Map("x" -> LongSlot(0, nullable = true, CTNode, "x")),
                                           numberOfLongs = 1,
                                           numberOfReferences = 0)
  ...
  test("labelscan with projection") {
    ...
    val pipeline = PipelineInformation(
      numberOfLongs = 1,
      numberOfReferences = 1,
      slots = Map("x"             -> LongSlot(0, nullable = false, CTNode, "x"),
                  "x.propertyKey" -> RefSlot(0, nullable = true, CTAny, "x.propertyKey"))
    )
    pipe should equal(
      ProjectionRegisterPipe(
        NodesByLabelScanRegisterPipe("x", LazyLabel("label"), pipeline)(),
  ...
  test("cartesian product") {
    ...
    // then
    pipe should equal(
      CartesianProductPipe(
        NodesByLabelScanRegisterPipe(
          "x",
          LazyLabel("label1"),
        ...
        NodesByLabelScanRegisterPipe(
          ...
          PipelineInformation(numberOfLongs = 1,
                              numberOfReferences = 0,
                              slots = Map("y" -> LongSlot(0, nullable = false, CTNode, "y"))))()
      )())
  }
  ...
  test("that argument does not apply here") {
    ...
    pipe should equal(
      ApplyRegisterPipe(
        ...
        ExpandAllRegisterPipe(ArgumentRegisterPipe(lhsPipeline)(),
                              0,
                              1,
                              2,
                              SemanticDirection.INCOMING,

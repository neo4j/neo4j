...
class MutatingIntegrationTest
  ...
  test("start with a node and create a new node with the same properties") {
    ...
    val result = updateWithBothPlannersAndCompatibilityMode(
      "match (a) where id(a) = 0 with a create (b {age : a.age * 2}) return b.age")
  ...
  test("multiple deletes should not break anything") {
    ...
    val result =
      updateWithBothPlannersAndCompatibilityMode("match (a), (b) where id(a) = 0 AND id(b) IN [1, 2, 3] delete a")
    assertStats(result, nodesDeleted = 1)
  ...
  test("create multiple relationships in one query") {
    ...
    val result = executeWithCostPlannerAndInterpretedRuntimeOnly(
      "create (n) with n MATCH (x) WHERE id(x) IN [0, 1, 2] create (n)-[:REL]->(x)")
    assertStats(result, nodesCreated = 1, relationshipsCreated = 3)
  ...
  test("set a property to a collection") {
    ...
    val result = updateWithBothPlannersAndCompatibilityMode(
      "MATCH (n) with collect(n.name) as names create (m {name : names}) RETURN m.name")
    assertStats(result, propertiesWritten = 1, nodesCreated = 1)
  ...
  test("create node from map values") {
    val result = updateWithBothPlannersAndCompatibilityMode("create (n {a}) return n.age, n.name",
                                                            "a" -> Map("name" -> "Andres", "age" -> 66))
  ...
  test("create rel from map values") {
    ...
    val result = updateWithBothPlannersAndCompatibilityMode(
      "match (a), (b) where id(a) = 0 AND id(b) = 1 create (a)-[r:REL {param}]->(b) return r.name, r.age",
      "param" -> Map("name" -> "Andres", "age" -> 66))
  ...
  test("match and delete") {
    ...
    intercept[ConstraintValidationException](
      executeWithAllPlanners("match (n) where id(n) = 0 match (n)-[r:HATES]->() delete n,r"))
  }
  ...
  test("fail to create from two iterables") {
    val maps1 = List(Map("name" -> "Andres"), Map("name" -> "Michael"), Map("name" -> "Peter"))
    val maps2 = List(Map("name" -> "Andres"), Map("name" -> "Michael"), Map("name" -> "Peter"))

    try {
      updateWithBothPlannersAndCompatibilityMode("create (a {params1}), (b {params2})",
                                                 "params1" -> maps1,
                                                 "params2" -> maps2)
    } catch {
      case e: CypherTypeException => e.getCause shouldBe a[org.neo4j.cypher.internal.frontend.v3_3.CypherTypeException]
      case e: ParameterWrongTypeException =>
        e.getCause shouldBe a[org.neo4j.cypher.internal.frontend.v2_3.ParameterWrongTypeException]
    }
  }
  ...
  test("first read then write") {
    ...
    updateWithBothPlannersAndCompatibilityMode(
      "match (root) where id(root) = 0 match (root)-->(other) create (new {name:other.name}), (root)-[:REL]->(new)")

    val result = executeWithAllPlannersAndRuntimesAndCompatibilityMode(
      "match (root) where id(root) = 0 match (root)-->(other) return other.name order by other.name")
      .columnAs[String]("other.name")
      .toList
  ...
  test("string literals should not be mistaken for variables") {
    ...
    updateWithBothPlannersAndCompatibilityMode(
      "EXPLAIN create (tag1 {name:'tag2'}), (tag2 {name:'tag1'}) return [tag1,tag2] as tags")
    val result =
      executeScalar[Seq[Node]]("create (tag1 {name:'tag2'}), (tag2 {name:'tag1'}) return [tag1,tag2] as tags")
  ...
  test("failed query should not leave dangling transactions") {
    ...
    val contextBridge: ThreadToStatementContextBridge =
  ...
  test("full path in one create") {
    ...
    createNode()
    val result = updateWithBothPlannersAndCompatibilityMode(
      "match (a), (b) where id(a) = 0 AND id(b) = 1 create (a)-[:KNOWS]->()-[:LOVES]->(b)")
  ...
  test("created paths honor directions") {
    ...
    val b = createNode()
    val result = executeWithCostPlannerAndInterpretedRuntimeOnly(
      "match (a), (b) where id(a) = 0 AND id(b) = 1 create p = (a)<-[:X]-(b) return p").toList
      .head("p")
      .asInstanceOf[Path]
  ...
  test("create with parameters is not ok when variable already exists") {
    intercept[SyntaxException](
      updateWithBothPlannersAndCompatibilityMode("create a with a create (a {name:\"Foo\"})-[:BAR]->()").toList)
  }
  ...
  test("cant set properties after node is already created") {
    intercept[SyntaxException](
      updateWithBothPlannersAndCompatibilityMode("create (a)-[:test]->(b), (a {name:'a'})-[:test2]->c"))
  }

  test("can create anonymous nodes inside foreach") {
    createNode()
    val result =
      updateWithBothPlanners("match (me) where id(me) = 0 foreach (i in range(1,10) | create (me)-[:FRIEND]->())")
  ...
  test("should be able to use external variables inside foreach") {
    createNode()
    val result =
      updateWithBothPlanners("match (a), (b) where id(a) = 0 AND id(b) = 0 foreach(x in [b] | create (x)-[:FOO]->(a)) ")
  ...
  test("should be possible to remove nodes created in the same query") {
    val result = updateWithBothPlannersAndCompatibilityMode("""CREATE (a)-[:FOO]->(b)
         WITH *
         MATCH (x)-[r]-(y)

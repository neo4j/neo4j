...
public class TestGcrCacheRemoveSizeDiverge
{
    ...
    public static void startDb()
    {
        graphdb = new EmbeddedGraphDatabase(
                forTest( TestPropertyDataRace.class ).graphDbDir( true ).getAbsolutePath(), stringMap(
                        Config.CACHE_TYPE, GCResistantCacheProvider.NAME ) );
    }
    ...
    { // ...should yield a size which matches actual cache size
        ...
        final Cache<?> nodeCache = ( graphdb ).getConfig().getGraphDbModule().getNodeManager().caches().iterator().next();
        assertTrue( "We didn't get a hold of the right cache object",
                nodeCache.toString().toLowerCase().contains( "node" ) );
        ...
        Thread t1 = new Thread( "T1: Relationship loader" )
        {
            ...
            public void run()
            {
                // It will break in NodeImpl#loadInitialRelationships right
        ...
        // TODO wait for latch instead, but it seems to be a different instance
        // than the one we countDown.
        // latch.await();
        Thread.sleep( 2000 );

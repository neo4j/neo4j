...
This guide will help you understand the ways to configure and deploy a Neo4j High Availability cluster. Two
...
applications on top of Neo4j.

== Background ==

A Neo4j HA cluster consists of a set of running Neo4j Enterprise instances. Each instance must be assigned an
integer ID, which serves as its identifier in the cluster.

At startup, a Neo4j instance contacts the other instances specified in the +ha.initial_hosts+ configuration option.
When it establishes a connection to any one of these, it determines the current state of the cluster and ensures that
it is eligible to join the cluster. To be eligible the Neo4j instance must be hosting the same database store as
other members of the cluster (although it is allowed to be in an older state), or be a new deployment without a
database store.
...
.Startup Time
==================
Note that a Neo4j HA instance will wait during startup until it can reach at least half of the instances configured in
+ha.initial_hosts+. In this example, neo4j-01 will not complete its startup until neo4j-02 is also started.
==================

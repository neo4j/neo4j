...
public class GCResistantCache<E extends EntityWithSize> implements Cache<E>, DiagnosticsProvider
{
    ...
    /**
     * Updates the highest set id if the given id is higher than any previously
     * registered id.
     * Helps {@link #clear()} performance wise so that only the used part of the
     * array is cleared.
     * 
    ...
    private void setHighest( long id )
    {
        while ( true )
        {
            ...
            if ( id > highest )
            {
                if ( highestIdSet.compareAndSet( highest, id ) ) break;
            }
            else
                break;
        }
    ...
    private synchronized void purgeFrom( int pos )
    {
        ...
        finally
        {
            ...
            if ( timestamp - lastPurgeLogTimestamp > minLogInterval )
            {
                ...
                logger.logMessage( name + " purge (nr " + purgeCount + ") " + sizeBeforeStr + " -> " + sizeAfterStr
                                   + " (" + diffStr + ") " + missPercentage + " misses, " + colPercentage
    ...
    private void printAccurateStatistics()
    {
        ...
        logger.logMessage( name + " purge (nr " + purgeCount + "): elementCount:" + elementCount + " and sizes actual:"
                           + getSize( actualSize ) + ", perceived:" + getSize( currentSize.get() ) + " (diff:"
                           + getSize( currentSize.get() - actualSize ) + "), registered:" + getSize( registeredSize ),
                true );
    }

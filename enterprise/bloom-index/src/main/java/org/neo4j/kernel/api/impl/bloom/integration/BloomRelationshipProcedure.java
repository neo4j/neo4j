...
public class BloomRelationshipProcedure extends CallableProcedure.BasicProcedure
{
    private static final String PROCEDURE_NAME = "bloomRelationships";
    private static final String[] PROCEDURE_NAMESPACE = {"dbms", "bloom"};
    private static final String OUTPUT_NAME = "relid";
    private BloomIndex bloomIndex;

    BloomRelationshipProcedure( BloomIndex bloomIndex )
    {
        super( procedureSignature( new QualifiedName( PROCEDURE_NAMESPACE, PROCEDURE_NAME ) ).in( "terms", Neo4jTypes.NTList( Neo4jTypes.NTString ) ).out(
                OUTPUT_NAME, Neo4jTypes.NTInteger ).description( "Queries the bloom index for relationships." ).build() );
    ...
    public RawIterator<Object[],ProcedureException> apply( Context ctx, Object[] input ) throws ProcedureException
    {
        ...
        try ( BloomIndexReader relationshipReader = bloomIndex.getRelationshipReader() )
        {
            PrimitiveLongIterator primitiveLongIterator = relationshipReader.query( query );
        ...
        {
            throw new ProcedureException( Status.statusCodeOf( e ), e, "Failed to query bloom index for relationships", input );
        }
    }
}

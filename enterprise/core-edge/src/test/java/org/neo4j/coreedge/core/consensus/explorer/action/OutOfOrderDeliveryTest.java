...
public class OutOfOrderDeliveryTest
{
    ...
    public void shouldReOrder() throws Exception
    {
        ...
        clusterState.queues.get( member( 0 ) ).add( new Election( CURRENT_VERSION, member( 0 ) ) );
        clusterState.queues.get( member( 0 ) ).add( new Heartbeat( CURRENT_VERSION, member( 0 ) ) );

        // when
        ClusterState reOrdered = new OutOfOrderDelivery( member( 0 ) ).advance( clusterState );

        // then
        assertEquals( new Heartbeat( CURRENT_VERSION, member( 0 ) ), reOrdered.queues.get( member( 0 ) ).poll() );
        assertEquals( new Election( CURRENT_VERSION, member( 0 ) ), reOrdered.queues.get( member( 0 ) ).poll() );
    }
}

...
import static org.neo4j.coreedge.discovery.Cluster.buildAddresses;
...
/**
 * At the time of writing this test, certain operations required by Core/Edge
 ...
 * tries to combine Core/Edge with custom IO configurations.
 * Specifically, the functionality that Core/Edge needs that
...
public class RejectCustomIOTest
{
    ...
    @Test
    public void shouldFailToStartWithCustomIOConfigurationInEdgeModeTest() throws Exception
    {
        try
        {
            ...
            Map<String,String> extraParams =
                    stringMap( GraphDatabaseSettings.pagecache_swapper.name(), TEST_PAGESWAPPER_NAME );
            EdgeClusterMember clusterMember =
                    new EdgeClusterMember( storeDir.directory(), 2, discovery, buildAddresses( set( 0, 1, 2 ) ),

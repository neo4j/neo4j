...
public class CheckTxLogsTest
{
    ...
    public void shouldReportNodeInconsistenciesFromSingleLog() throws IOException
    {
        ...
        CaptureInconsistencies handler = new CaptureInconsistencies();
        CheckTxLogs checker = new CheckTxLogs( fsRule.get(), handler, mock( PrintStream.class ) );

        // When
        checker.scan( new File[]{log}, CheckType.NODE );

        // Then
        assertEquals( 1, handler.inconsistencies.size() );

        NodeRecord seenRecord = (NodeRecord) handler.inconsistencies.get( 0 ).seen.record();
    ...
    public void shouldReportNodeInconsistenciesFromDifferentLogs() throws IOException
    {
        ...
        // When
        checker.scan( new File[]{log1, log2, log3}, CheckType.NODE );

        // Then
        assertEquals( 2, handler.inconsistencies.size() );

        NodeRecord seenRecord1 = (NodeRecord) handler.inconsistencies.get( 0 ).seen.record();
        ...
        NodeRecord seenRecord2 = (NodeRecord) handler.inconsistencies.get( 1 ).seen.record();
    ...
    public void shouldReportPropertyInconsistenciesFromSingleLog() throws IOException
    {
        ...
        // When
        checker.scan( new File[]{log}, CheckType.PROPERTY );

        // Then
        assertEquals( 1, handler.inconsistencies.size() );

        PropertyRecord seenRecord = (PropertyRecord) handler.inconsistencies.get( 0 ).seen.record();
    ...
    public void shouldReportPropertyInconsistenciesFromDifferentLogs() throws IOException
    {
        ...
        // When
        checker.scan( new File[]{log1, log2, log3}, CheckType.PROPERTY );

        // Then
        assertEquals( 2, handler.inconsistencies.size() );

        PropertyRecord seenRecord1 = (PropertyRecord) handler.inconsistencies.get( 0 ).seen.record();
        ...
        PropertyRecord seenRecord2 = (PropertyRecord) handler.inconsistencies.get( 1 ).seen.record();
    ...
    private static class CaptureInconsistencies implements CheckTxLogs.InconsistenciesHandler
    {
        ...
        {
            inconsistencies.add( new Inconsistency( seen, current ) );
        }
    }

    private static class Inconsistency
    {
        final LogRecord<?> seen;
        final LogRecord<?> current;

        Inconsistency( LogRecord<?> seen, LogRecord<?> current )
        {
            this.seen = seen;

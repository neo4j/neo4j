...
To connect to a running Neo4j database, use <<read-only-mode>> for local databases and see <<shell-enabling-remote>> for remote databases.
...
Shell is enabled from the main configuration of Neo4j.
...
Then you will get the shell prompt like this:
...
Given that the right neo4j-kernel-<version>.jar is in the same path as your neo4j-shell-<version>.jar file you run it with:
...
And even to another neo4j-shell, e.g. for importing a `dump` of another database or cypher result.
When used with command mode the shell will not output a welcome message.
...
[source, plain]
----
$ neo4j-shell -config conf/neo4j.conf -path mydb
----
...
This is faster than piping to the shell which still handles the input as if it was user input.

For example reading a dump file directly from the command line and executing it against the given database.
For example:
...
Here all three options are written together after a single `-` prefix.
...
Which means traverse depth first for relationships with type `KNOWS` disregarding direction and incoming relationships with type matching `HAS_.\*` and do a `ls <matching node>` for each match.
The node filtering is supplied with the `-f` option, see <<shell-filters>>.
See <<shell-options-enum>> for the traversal order option.
Even relationship types/directions are supplied using the same format as filters.
...
The `schema` command provides information about indexes and constraints together with their current status.
...
The `schema` command supports the following actions:

* `ls` lists indexes and constraints
* `sample` samples indexes
* `await` blocks until index population is complete
* `progress` prints the current status of the index population

The `schema` command also supports the following optional parameters:

* `-l :Label` limit to indexes or constraints on `:Label` nodes
...
* `-r :Relationship:` limit to indexes or constraints for the given relationship
...
* `-p propertyKey` limit to indexes or constraints for the given property
...
Indexes and constraints can be created or removed using Cypher or the Java Core API.
They are updated automatically
whenever the graph is changed.
See <<graphdb-neo4j-schema>> for more information.
...
// Put on hold as the output can't be imported as is.
//include::shell/database-dump.asciidoc[]

[[shell-example-sessions]]

...
public class BatchOperationDocIT extends org.neo4j.doc.server.rest.AbstractRestFunctionalDocTestBase
{
...
//        assertEquals(1, JsonHelper.jsonToList(  rels ).size());
    ...
    public void shouldGetLocationHeadersWhenCreatingThings() throws Exception
    {
        ...
        org.neo4j.doc.server.rest.JaxRsResponse response = RestRequest.req().post(batchUri(), jsonString);
    ...
    {
        org.neo4j.doc.server.rest.JaxRsResponse response = RestRequest.req().post(batchUri(), new PrettyJSON()
            .array()
                .object()
                    .key("method") .value("POST")
                    .key("to")     .value("/node")

...
However, support for legacy indexes remains, because certain features, such as full-text search, are not yet handled by the new indexes.
...
For example link:javadocs/org/neo4j/graphdb/index/IndexHits.html#getSingle%28%29[getSingle()] returns the first and only item from the result iterator, or +null+ if there isn't any hit.

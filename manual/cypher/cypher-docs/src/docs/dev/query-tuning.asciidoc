...
You can force it to use a specific planner by using the `cypher.planner` configuration setting (see <<config_cypher.planner>>), or by prepending your query with `CYPHER planner=cost` or `CYPHER planner=rule`.

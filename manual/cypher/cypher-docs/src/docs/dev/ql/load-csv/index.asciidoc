...
* When not set, file URLs will be resolved as relative to the root of the database server filesystem.
  ...
  _For security reasons you may not want users to be able to load files located anywhere on the database server filesystem and should set `dbms.directories.import` to a safe directory to load files from._

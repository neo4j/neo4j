...
Under Unix/Linux/OSX, the command is named `neo4j-admin import --mode=csv`. Depending on the installation type, the tool
 is either
available globally, or used by executing `./bin/neo4j-admin import --mode=csv` from inside the installation directory.

=== Windows

Under Windows, used by executing `bin\neo4j-admin import --mode=csv` from inside the installation directory.

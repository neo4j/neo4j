[[import]]
Data Import
===========

For high-performance data import, the batch insert facilities described in this chapter are recommended.

Other ways to import data into Neo4j include using Gremlin graph import (see <<rest-api-load-a-sample-graph>>) or using the Geoff notation (see http://nigelsmall.com/geoff ).


:leveloffset: 2

include::{importdir}/neo4j-kernel-docs-jar/dev/batchinsert.asciidoc[]

:leveloffset: 3

include::{importdir}/neo4j-lucene-index-docs-jar/dev/batchinsertindex.asciidoc[]



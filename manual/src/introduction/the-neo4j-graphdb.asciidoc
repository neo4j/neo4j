...
Neo4j is a schema-optional graph database; you can use Neo4j without any schema, but optionally introduce it in order to gain performance or modelling benefits.
...
Performance is gained by introducing indexes, which improve the speed of looking up nodes in the database.

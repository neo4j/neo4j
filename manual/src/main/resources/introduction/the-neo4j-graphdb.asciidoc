...
Apart from properties and relationships, nodes can also be <<graphdb-neo4j-labels,labelled>> with zero or more labels.
...
A label is a named graph construct that is used to group nodes into sets; all nodes labelled with the same label belongs to the same set.
...
In Cypher, you may need to use the backtick (``) syntax to avoid clashes with cypher keywords.

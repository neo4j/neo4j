...
Notably, most queries will contain a 'fields' metadata section in the `SUCCESS` message for the `RUN` statement, which lists the result record field names, and a `result-available-after` section measuring the number of milliseconds it took for the results to be available for consumption.
...
.Response
- `SUCCESS {"fields": ..., "result-available-after"}` if the statement has been accepted for execution

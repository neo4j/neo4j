...
/**
 * This is used rather than a direct (RenderedWebElement) reference, because our
 * UI renders a lot, and such references often become stale. This allows code
 * that encounters StaleElementReferenceException to simply reload the element.
 * 
...
public class ElementReference
{
    ...
    /**
     * Attempts to fetch this element. If no element can be found, fetching the
     * element will be retried until one is found, or 10 seconds have passed.
     * 
     * @return
     */
    ...
    /**
     * Attempts to fetch an element. If no element can be found, fetching will
     * be retried until one is found, or 10 seconds have passed.
     * 
     * @return
     */
    ...
    public void waitUntilNotVisible()
    {
        ...
        try
        {
            while ( System.currentTimeMillis() < end )
            {
                if ( this.getElement().getValueOfCssProperty( "display" ).equals(
                        "none" ) )
                {
                    return;
                }
        ...
        while ( System.currentTimeMillis() < end )
        {
            try
            {
                ...
                if ( ( attr == null && expectedValue == null )
                     || ( attr != null && attr.equals( expectedValue ) ) )
                {
                    return;
                }
            }
            ...
            String currentValue )
    {
        ...
        while ( System.currentTimeMillis() < end )
        {
            ...
            {
                attr = this.getAttribute( attributeName );
                if ( ( attr == null && currentValue != null )
                     || ( attr != null && !attr.equals( currentValue ) ) )
                {
                    return;
                }
            }

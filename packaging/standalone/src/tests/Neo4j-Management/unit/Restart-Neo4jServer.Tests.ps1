...
InModuleScope Neo4j-Management {
  Describe "Restart-Neo4jServer" {
    ...
    Context "Invalid or missing specified neo4j installation" {
      Mock Get-Neo4jServer { return }
      $result = Restart-Neo4jServer -Neo4jServer 'TestDrive:\some-dir-that-doesnt-exist'
    ...
    Context "Invalid or missing server object" {
      ...
      It "throws error for an invalid server object" {
        { Restart-Neo4jServer -Neo4jServer (New-Object -TypeName PSCustomObject) -ErrorAction Stop } | Should Throw
      }
    ...
    Context "Missing service name in configuration files" {
      ...
      It "throws error for missing service name in configuration file" {
        { Restart-Neo4jServer -ErrorAction Stop } | Should Throw
      }
    ...
    Context "Restart service by named parameter" {
      ...
      $result = Restart-Neo4jServer -ServiceName 'SomeOtherServiceName'
    ...
    Context "Stop service and passthru server object" {
      ...
      $result = Restart-Neo4jServer -PassThru

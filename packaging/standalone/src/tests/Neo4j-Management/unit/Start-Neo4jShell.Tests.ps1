...
if ($PSVersionTable.PSVersion -ne '2.0') {
    ...
    Context "Uses values in the configuration file" {
      ...
      Mock Get-Neo4jSetting {
        @(
          New-Object -TypeName PSCustomObject -Property (@{ 'ConfigurationFile'='neo4j.conf'; 'Name'='dbms.shell.enabled'; 'Value'='true' })
          New-Object -TypeName PSCustomObject -Property (@{ 'ConfigurationFile'='neo4j.conf'; 'Name'='dbms.shell.host'; 'Value'='somehost.domain.com' })
          New-Object -TypeName PSCustomObject -Property (@{ 'ConfigurationFile'='neo4j.conf'; 'Name'='dbms.shell.port'; 'Value'='1234' })
        )
      }
      Mock Get-Java { return New-Object -TypeName PSCustomObject -Property (@{'java'='ignoreme'; 'args' = @();}) }
      Mock Start-Process { }

...
test_expect_success "should read port and address from config" "
  set_config 'org.neo4j.server.webserver.port' '1234' neo4j-server.properties &&
  set_config 'org.neo4j.server.webserver.address' 'neo4j.example.com' neo4j-server.properties &&
...
test_expect_success "should write a specific message in HA mode" "
  set_config 'org.neo4j.server.database.mode' 'HA' neo4j-server.properties &&

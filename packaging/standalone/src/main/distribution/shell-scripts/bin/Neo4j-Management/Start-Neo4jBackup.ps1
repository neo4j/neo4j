...
Function Start-Neo4jBackup
{
  ...
  Process
  {
    ...
    {
      if (($_.ConfigurationFile -eq 'neo4j.conf') -and ($_.Name -eq 'dbms.backup.enabled')) { $BackupEnabled = ($_.Value.ToUpper() -eq 'TRUE') }
      if (($_.ConfigurationFile -eq 'neo4j.conf') -and ($_.Name -eq 'dbms.backup.address')) { $BackupHost = ($_.Value) }
    }
